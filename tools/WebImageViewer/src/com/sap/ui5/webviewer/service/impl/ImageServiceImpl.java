package com.sap.ui5.webviewer.service.impl;
...
import com.sap.ui5.webviewer.ResourceProvider;
import com.sap.ui5.webviewer.comparator.FailedImageNameComparator;
import com.sap.ui5.webviewer.comparator.FailedNumberComparator;
import com.sap.ui5.webviewer.comparator.JobNameComparator;
...
public class ImageServiceImpl implements ImageService {
	...
	public Response countResultByJob() {
		...
		for (String controlPath : allControlsPath) {
			...
			LOG.debug("Find image count from Need verify folder [" + needVerifyImagePath + "]");
			int failedImageCount = resource.getSubFileSizeBy(needVerifyImagePath);

			if (!countFailedControllers.containsKey(jobName)) {
				countFailedControllers.put(jobName, new HashMap<String, Integer>());
			}
	...
	public Response findFailedControllersByPath(String path) {
		...
		for (String controlPath : allControlsPath) {
			...
			String expectedImagePath = StringUtils.combinedResourcePath(resourceSeparator, controlPath,
					Constants.EXPECTED_IMAGES_FOLDER);

			// Failed image count from "NeedVerifyImages" folder under current control path.
			int failedImageCount = resource.getSubFileSizeBy(needVerifyImagePath);

			// all image count from "ExpectedImages" folder under current control path.
			int allImagesCount = resource.getSubFileSizeBy(expectedImagePath);
			...
			int passCount = (allImagesCount - failedImageCount) >= 0 ? (allImagesCount - failedImageCount) : 0;
			failedControls.put(Constants.PASS, passCount);

			String resultKey = categoryName + Constants.UNDERLINE_SEPARATOR + moduleName;
			if (!jobDetailResults.containsKey(resultKey)) {
				jobDetailResults.put(resultKey, new ArrayList<Map<String, Object>>());
			}
	...
	public Response updateImages(String controlPath, String... imageNames) {
		...
		Resource resource = ResourceProvider.getResource();
		String resourceSeparator = resource.getResourcePathSeparator();
		String expectedImagePath = StringUtils.combinedResourcePath(resourceSeparator, controlPath, Constants.EXPECTED_IMAGES_FOLDER);
		String needVerifyImagePath = StringUtils.combinedResourcePath(resourceSeparator, controlPath, Constants.NEED_VERIFY_IMAGES_FOLDER);
		String diffImagePath = StringUtils.combinedResourcePath(resourceSeparator, controlPath, Constants.DIFF_IMAGES_FOLDER);
		...
		for (String imageName : imageNames) {
			...
			synchronized (objectLock) {
				isCopySuccessed = resource.copyFile(needVerifyImage, expectedImagePath + imageName);
				if (isCopySuccessed) {
	...
	public Response countResultByMoudle() {
		...
		for (String controlPath : allControlsPath) {
			...
			if (!allControllersMap.containsKey(categoryName)) {
				allControllersMap.put(categoryName, new HashMap<String, Integer>());
			}
	...
	public Response findFailedControlsByModule(String path) {
		...
		Map<String, Integer> controlAllImagesCountMap = new HashMap<String, Integer>();
		List<String> controlsPathForCurrentModule = new ArrayList<String>();
		for (String controlPath : allControlsPath) {
			if (!controlPath.contains(path)) {
				continue;
			}
			controlsPathForCurrentModule.add(controlPath);
			String needVerifyImagePath = StringUtils.combinedResourcePath(resourceSeparator, controlPath, Constants.NEED_VERIFY_IMAGES_FOLDER);
			int failedImageCount = resource.getSubFileSizeBy(needVerifyImagePath);
			String subModuleName = resource.getModuleNameByCurrentPath(controlPath);
			if (!modulesInfoMap.containsKey(subModuleName)) {
				modulesInfoMap.put(subModuleName, new HashMap<String, Integer>());
			}
			...
			count = count + failedImageCount;
			controlsMap.put(controlName, count);

			if (!controlAllImagesCountMap.containsKey(controlName)) {
				String expectedImagePath = StringUtils.combinedResourcePath(resourceSeparator, controlPath, Constants.EXPECTED_IMAGES_FOLDER);
				// all image count from "ExpectedImages" folder under current control path.
				int allImagesCount = resource.getSubFileSizeBy(expectedImagePath);
				controlAllImagesCountMap.put(controlName, allImagesCount);
			}
		}

		List<Map<String, Object>> resultList = new ArrayList<Map<String, Object>>();
		List<String> allTestConditions = resource.findAllTestConditions();
		int testConditionsCount = allTestConditions.size();
		for (String subModuleName : modulesInfoMap.keySet()) {
			...
			for (String controlName : controlsMap.keySet()) {
				...
				int failedImageCount = controlsMap.get(controlName);
				controlMap.put(Constants.CONTROL_NAME, controlName);
				controlMap.put(Constants.FAIL, failedImageCount);
				int allImagesCount = controlAllImagesCountMap.get(controlName) * testConditionsCount;
				int passCount = (allImagesCount - failedImageCount) >= 0 ? (allImagesCount - failedImageCount) : 0;
				controlMap.put(Constants.PASS, passCount);
