...
describe("Generator.js", function () {
	...
	it("cleanupTimezoneNames", function () {
		const oGenerator = new Generator();
		const oTerritories = {
			"002": "~002",
			"019": "~019",
			"142": "~142",
		...
		sinon.mock(oGenerator._oTimezones).expects("deleteUnsupportedTimezoneNames")
			.withExactArgs(oExpectedTimezoneNamesForDelete);

		// code under test
		oGenerator.cleanupTimezoneNames(oResult);
	...
	it("generateTimeZoneNames", function () {
		...
		const aCompleteCLDRTimezoneIDs = ["Africa/Addis_Ababa", "Africa/Algiers", "America/Argentina/Buenos_Aires1",
			"Europe/Amsterdam", "America/North_Dakota/Beulah", "Europe/Dublin"];
		const oResult = {
			"Africa": {
	...
	it("replaceOutdatedCLDRTimezoneNames", function () {
		...
		sinon.mock(oGenerator._oTimezones)
		...
		// code under test
		oGenerator.replaceOutdatedCLDRTimezoneNames(oTimezoneNames);
	...
	it("updateTimezones: remember complete time zone IDs", function () {
		const oGenerator = {
			...
			cleanupTimezoneNames() {},
			replaceOutdatedCLDRTimezoneNames() {}
		};
		...
		oStaticGeneratorMock.expects("getTimezoneIDs").withExactArgs("~timezoneNames").returns("~completeTimezonNames");
		...
		oStaticGeneratorMock.expects("sort").withExactArgs("~timezoneNames").returns("~sortedTimezoneNames");
		oGeneratorMock.expects("checkTimezonesConsistency").withExactArgs("~sCLDRTag", "~sortedTimezoneNames");
	...
	it("updateTimezones: generateTimeZoneNames", function () {
		...
		oStaticGeneratorMock.expects("generateTimeZoneNames")
			.withExactArgs("~timezoneNames", "~_aCompleteCLDRTimezoneIDs");
		oGeneratorMock.expects("replaceOutdatedCLDRTimezoneNames").withExactArgs("~timezoneNames");
		oGeneratorMock.expects("cleanupTimezoneNames").withExactArgs(sinon.match.same(oResult));
		oStaticGeneratorMock.expects("sort").withExactArgs("~timezoneNames").returns("~sortedTimezoneNames");
		oGeneratorMock.expects("checkTimezonesConsistency").withExactArgs("~sCLDRTag", "~sortedTimezoneNames");
	...
	it("updateTimezones: fix appendItems for zh-Hans-* locales", function () {
		...
		oGeneratorMock.expects("cleanupTimezoneNames").withExactArgs(sinon.match.same(oResult));
		oStaticGeneratorMock.expects("sort").withExactArgs("~timezoneNames").returns("~sortedTimezoneNames");
		oGeneratorMock.expects("checkTimezonesConsistency")
	...
	it("start: success", function () {
		...
		// code under test
		return Generator.prototype.start.call(oGenerator).then(function (oGeneratorResolved) {
	...
	it("start: error", async function () {
		...
		sinon.mock(util).expects("setSupplePath").withExactArgs(join("~_sSourceFolder/cldr-core/supplemental"));
		sinon.mock(oGenerator._oTimezones).expects("updateTimezones").withExactArgs().rejects(oError);
