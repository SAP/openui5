...
describe("Generator.js", function () {
	...
	it("constructor", function () {
		...
		assert.ok(oGenerator._oLegacyUnitMapper instanceof LegacyUnitKeyMapping);
	...
	["~sUI5Tag", "zh_TW" /* BCP: 2380141822 */].forEach((sUI5Tag) => {
		it("generateLocaleFile", function () {
			...
			sinon.mock(oGenerator).expects("checkAllDateTimePatterns")
				.withExactArgs(sUI5Tag, sinon.match((oResult0) => oResult0 === oResult));
			// BCP: 2380141822
			oGeneratorMock.expects("removeSymbolBFromTimeFormatsValue")
				.withExactArgs(sinon.match((oResult0) => oResult0 === oResult))
				.exactly(sUI5Tag === "zh_TW" ? 1 : 0);
	...
	it("start: success", function () {
		...
		sinon.mock(oGenerator._oLegacyUnitMapper).expects("importOldUnits").withExactArgs("~_sOutputFolder");
		...
		sinon.mock(oGenerator._oLegacyUnitMapper).expects("writeUnitMappingToLocaleData").withExactArgs()
			.returns("~writeUnitMappingToLocaleData");
		...
		// code under test
		return Generator.prototype.start.call(oGenerator).then((oResult) => {
			assert.strictEqual(oResult, oGenerator);
		});
	});
	...
	//*********************************************************************************************
	// BCP: 2380141822
	it("removeSymbolBFromTimeFormatsValue", function () {
		const oInputResult = {};
		const oExpectedResult = {};
		const oConsoleMock = sinon.mock(console);
		["ca-gregorian", "ca-islamic", "ca-japanese", "ca-persian", "ca-buddhist"].forEach((sCalendar) => {
			oConsoleMock.expects("log").withExactArgs(`WARNING: Replacements in locale 'zh_TW' for`
				+ ` '${sCalendar}.timeFormats', make sure that only the 'B' is replaced by an 'a':\n\t`
				+ `{"foo":"bar"} was replaced by:\n\t`
				+ `{"full":"ah:mm:ss [zzzz]","long":"ah:mm:ss [z]","medium":"ah:mm:ss","short":"ah:mm"}`);
			oInputResult[sCalendar] = {timeFormats: {foo: "bar"}};
			oExpectedResult[sCalendar] = {
				timeFormats: {
					"full": "ah:mm:ss [zzzz]",
					"long": "ah:mm:ss [z]",
					"medium": "ah:mm:ss",
		...
		// code under test
		Generator.removeSymbolBFromTimeFormatsValue(oInputResult);

		assert.deepEqual(oInputResult, oExpectedResult);
	});
});
