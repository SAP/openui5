...
describe("Timezones.js", function () {
	...
	//*********************************************************************************************
	it("loadTimezonesFile: success", async function () {
		const oTimezones = new Timezones();

		sinon.mock(fs).expects("readFile").withExactArgs(sinon.match(/_timezones.js$/), "utf8").resolves("~content");

		// code under test
		await oTimezones.loadTimezonesFile();

		assert.strictEqual(oTimezones.sFileContent, "~content");
	});

	//*********************************************************************************************
	it("loadTimezonesFile: error", async function () {
		const oTimezones = new Timezones();
	...
	//*********************************************************************************************
	it("writeTimezonesFile: success", async function () {
		const oTimezones = new Timezones();
		oTimezones.sFileContent = "~content";

		let sPath;
		const oMock = sinon.mock(fs);
		oMock.expects("writeFile").callsFake(function (sPath0, sContent0) {
			sPath = sPath0;
			assert.ok(/^.*_timezones.js$/.test(sPath0));
			assert.strictEqual(sContent0, "~content");
			return Promise.resolve();
		});
		sinon.mock(console).expects("log").callsFake(function (sText) {
			assert.strictEqual(sText, `DONE, timezones updated: ${sPath}`);
		});

		// code under test
		await oTimezones.writeTimezonesFile();
	});

	//*********************************************************************************************
	it("writeTimezonesFile: error", async function () {
		const oTimezones = new Timezones();
		oTimezones.sFileContent = "~content";
	...
	it("updateTimezones", async function () {
		...
		oMock.expects("loadTimezonesFile").withExactArgs().resolves();
		oMock.expects("processIanaTimezones").withExactArgs().resolves();
		oMock.expects("writeTimezonesFile").withExactArgs().resolves();
		oMock.expects("fnResolveTimezonesReady").withExactArgs();

		// code under test
		await oTimezones.updateTimezones();
	});
