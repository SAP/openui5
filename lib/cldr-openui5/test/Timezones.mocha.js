...
describe("Timezones.js", function () {
	...
	it("processABAPTimezonesArray: success", async function () {
		...
		sinon.mock(console).expects("log")
			.withExactArgs("DONE, the aABAPTimezonesIDs in the _timezones.js file have been updated");
	...
	it("requestIanaTimeZoneRawData", async function () {
		...
		const aFetchResult = [
			...
			// exclude: file name "factory", see SNOW DINC0297150
			{name: "factory", "download_url": "url7"}
		];
		...
		// code under test
		assert.deepEqual(await oTimezones.requestIanaTimeZoneRawData("~version"), ["content0", "content1"]);
	});
	...
	it("loadBCP47Timezones", async function () {
		...
		assert.deepEqual(oTimezones.mAlias2SupportedTz, {"Baz/Bar": "Bar/Foo"});
	});
