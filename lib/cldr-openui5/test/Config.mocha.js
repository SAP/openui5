...
describe("Json2json templates in config.js", function () {
	...
	//*********************************************************************************************
	it("lenient scope number template", function () {
		const oJson2JsonConfig = configs.find((oConfig) => oConfig.id === "characters.json -> lenient-scope-number");
		const fnTemplateFormat = oJson2JsonConfig.template.format;
		assert.deepEqual(oJson2JsonConfig, {
		...
		sinon.mock(console).expects("log").withExactArgs("WARNING: Ignored unknown lenient scope number key: ~ignored");

		// code under test
		const oResult = fnTemplateFormat("~UnusedNode", oValue, "lenient-scope-number");

		assert.notStrictEqual(oResult.value, oValue);
		assert.deepEqual(oResult, {
			key: "lenient-scope-number",
			value: {minusSign: "", commaSign: ",", plusSign: "+＋﬩﹢⁺₊➕"}
		});
	});
});
