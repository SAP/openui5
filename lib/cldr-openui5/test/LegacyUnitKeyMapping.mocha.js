...
describe("LegacyUnitKeyMapping.js", function () {
	...
	it("importOldUnits", async function () {
		...
		oStub.withArgs(join("~path", "de.json")).resolves("{\"units\":{\"short\":\"~deUnits\"}}");
		oStub.withArgs(join("~path", "foo.json")).rejects(/*file not found*/);
		oStub.withArgs(join("~path", "en.json")).resolves("{\"units\":{\"short\":\"~enUnits\"}}");
	...
	it("analyseUnits: skip unknown locales", function () {
		...
		const oStub = sinon.stub(oUnitMapper, "collectChangedUnitKeys");
	...
	it("analyseUnits: add renamed keys to mChangedUnitKeys", function () {
		...
		const oStubFindNewUnitKey = sinon.stub(oUnitMapper, "findNewUnitKey");
		const oStubLogError = sinon.stub(console, "error");
		...
		oStubFindNewUnitKey.withArgs("~unit0", "~newUnits").returns("newKey0");
		oStubFindNewUnitKey.withArgs("~unit1", "~newUnits").returns(undefined);
		...
		assert.strictEqual(oStubLogError.callCount, 0);

		oStubFindNewUnitKey.withArgs("~unit0", "~newUnits").returns(undefined);
		oStubFindNewUnitKey.withArgs("~unit1", "~newUnits").returns("newKey1");
		...
		oStubFindNewUnitKey.withArgs("~unit2", "~newUnits").returns(undefined);
		oStubFindNewUnitKey.withArgs("~unit3", "~newUnits").returns("~unexpectedKey");
		...
		assert.deepEqual(oUnitMapper.mChangedUnitKeys,
			{oldKey0: {key: "newKey0", locale: "de"}, oldKey1: {key: "newKey1", locale: "en"}});
		assert.strictEqual(oStubLogError.callCount, 1);
		assert.deepEqual(oStubLogError.getCall(0).args,
			["ERROR: contradictory unit keys found for ~unit3\n"
	...
	[undefined, {}].forEach(function (vEmpty) {
		it(`writeUnitMappingToLocaleData: no changed keys found: empty value ${JSON.stringify(vEmpty)}`, function () {
			const oUnitMapper = new LegacyUnitKeyMapping();
			const oStub = sinon.stub(console, "log");
			...
			assert.deepEqual(oStub.getCall(0).args, ["INFO: no new legacy unit keys detected"]);
		});
	});
	...
	it("writeUnitMappingToLocaleData: writeFile successful", async function () {
		...
		const oStubReadFile = sinon.stub(fs, "readFile");
		const oStubWriteFile = sinon.stub(fs, "writeFile");

		oUnitMapper.mChangedUnitKeys = "~mChangedUnitKeys";
		sinon.stub(oUnitMapper, "getFinalUnitMapping").returns("~legacyUnitMapping");
		let sFilePath;
		oStubReadFile.withArgs(sinon.match("LocaleData.js")).callsFake(function (sFilePath0) {
		...
		assert.deepEqual(oStubWriteFile.getCall(0).args, [
			sFilePath,
		...
		assert.deepEqual(oStubLog.getCall(0).args, ["DONE, new legacy unit keys added to LocaleData.js"]);
	});
	...
	it("writeUnitMappingToLocaleData: writeFile fails", async function () {
		...
		oUnitMapper.mChangedUnitKeys = "~mChangedUnitKeys";
		sinon.stub(oUnitMapper, "getFinalUnitMapping").returns("~legacyUnitMapping");
		oStubReadFile.withArgs(sinon.match("LocaleData.js")).returns("~fileContent");
		...
		assert.strictEqual(oStubWriteFile.callCount, 1);
		assert.strictEqual(oStubLog.callCount, 0);
	});
