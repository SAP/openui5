...
describe("LegacyUnitKeyMapping.js", function () {
	...
	it("findNewUnitKey", function () {
		...
		// code under test - Old unit not present in new units anymore
		assert.strictEqual(oUnitMapper.findNewUnitKey({displayName: "l"}, oNewUnits), undefined);
		// code under test - Replacement suggestion found
		assert.strictEqual(oUnitMapper.findNewUnitKey({displayName: "t"}, oNewUnits), "key2");
		// code under test - Old unit not present in the currently processed locale
		assert.strictEqual(oUnitMapper.findNewUnitKey(undefined, oNewUnits), undefined);
	});
