...
export default class Generator extends EventEmitter {
	...
	generateLocaleFile(sUI5Tag) {
		...
		// Temporary workaround where the "timeFormats" value of the Taiwanese locale is replaced
		// with a hard coded value to not contain "B" anymore as this pattern symbol is currently not supported
		// BCP: 2380141822
		if (sUI5Tag === "zh_TW") {
			Generator.removeSymbolBFromTimeFormatsValue(oResult);
		}
	...
	/**
	 * Replaces the <code>timeFormats</code> value with a hard coded one.
	 * Only used for the Taiwanese locale as a temporary workaround as this is the only locale to contain the pattern
	 * symbol <code>B</code> in its <code>timeFormats</code> and this symbol is currently not supported.
	 * BCP: 2380141822
	 *
	 * @param {object} oResult The resulting CLDR object for the current locale
	 */
	static removeSymbolBFromTimeFormatsValue(oResult) {
		["ca-gregorian", "ca-islamic", "ca-japanese", "ca-persian", "ca-buddhist"].forEach((sCalendar) => {
			const oReplacementTimeFormats = {
				"full": "ah:mm:ss [zzzz]",
				"long": "ah:mm:ss [z]",
				"medium": "ah:mm:ss",
				"short": "ah:mm"
			};
			console.log(`WARNING: Replacements in locale 'zh_TW' for '${sCalendar}.timeFormats',`
				+ ` make sure that only the 'B' is replaced by an 'a':\n\t`
				+ `${JSON.stringify(oResult[sCalendar]["timeFormats"])} was replaced by:\n\t`
				+ `${JSON.stringify(oReplacementTimeFormats)}`);

			oResult[sCalendar]["timeFormats"] = oReplacementTimeFormats;
		});
	}
