...
export default class Timezones {
	...
	async loadBCP47Timezones() {
		...
		Object.keys(oBCP47Timezones).forEach((sKey) => {
			...
			const [sCLDRTimezone, ...aRemainingAliases] = aAliases;
			if (sCLDRTimezone === "Etc/Unknown") {
				return;
			}
			this.aSupportedTimezones.push(sCLDRTimezone);
			aRemainingAliases.forEach((sAlias) => {
	...
	async processABAPTimezonesArray() {
		...
		console.log("DONE, the aABAPTimezonesIDs in the _timezones.js file have been updated");
	}
	 ...
	 * updates the <code>aABAPTimezoneIDs and aTzTimezoneIDs</code> arrays in the <code>_timezones.js</code>, and the
	 * <code>mTimezoneAliases2ABAPTimezones</code> in the <code>TimezoneUtils.js</code> contents.
