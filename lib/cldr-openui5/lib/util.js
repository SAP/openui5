...
export default {
	...
	getTerritory: function(sTag) {
		var mLikelySubtags = getContent(join(sFilePathSupple, 'likelySubtags.json')).supplemental.likelySubtags,
	...
	getCurrencyDigits: function() {
		var sFilePath = join(sFilePathSupple, "currencyData.json"),
			mFractions = getContent(sFilePath).supplemental.currencyData.fractions,
			sDigits,
			sCurrencyCode,
			res = {};
	...
	getWeekData: function(sTag) {
		var mWeekData = getContent(join(sFilePathSupple, "weekData.json")).supplemental.weekData,
	...
	getTimeData: function(sTag) {
		var mTimeData = getContent(join(sFilePathSupple, "timeData.json")).supplemental.timeData,
			sTerritory = this.getTerritory(sTag),
			res = {};
	...
	getCalendarData: function() {
		var mCalendarData = getContent(join(sFilePathSupple, "calendarData.json")).supplemental.calendarData,
			res = {};
	...
	getCalendarPreference: function(sTag) {
		var mCalenderPref = getContent(join(sFilePathSupple, "calendarPreferenceData.json")).supplemental.calendarPreferenceData,
			sTerritory = this.getTerritory(sTag);
		return mCalenderPref[sTerritory];
	},

	getPluralRules: function(sTag) {
		var mPluralRules = getContent(join(sFilePathSupple, "plurals.json")).supplemental["plurals-type-cardinal"],
	...
	getDayPeriodRules : function (sTag) {
		var sDayPeriodKey,
			mDayPeriodRuleSet = getContent(join(sFilePathSupple, "dayPeriods.json")).supplemental.dayPeriodRuleSet,
			oDayPeriods = mDayPeriodRuleSet[sTag];

		for (sDayPeriodKey in oDayPeriods) {
