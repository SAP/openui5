		...
		template: {
			...
			format: function (node, value, key) {
				Object.keys(value).forEach(function (sKey) {
					...
					// whitespace characters are handled in the format patterns
					// and whitespace characters are neither 'plusSign' nor 'minusSign' nor 'commaSign' nor 'colonSign'
					// e.g. in newer CLDR versions this is fixed (e.g. in '32.0.0') but in '31.0.1' it is still present
					// https://github.com/unicode-cldr/cldr-misc-modern/blob/31.0.1/main/nb/characters.json#L36
					oPattern = oPattern.filter(function (value) {
						return !value.match(/\s/);
					});

					value[sKey] = oPattern.join("");
