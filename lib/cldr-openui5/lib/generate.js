...
var sTerritoriesPath = new URL("./resources/territories.json", import.meta.url);
var mTerritories = JSON.parse(readFileSync(sTerritoriesPath));
 ...
 */
export default function Generator(sSourceFolder, sOutputFolder, bPrettyPrint, sCLDRVersion) {
...
Generator.prototype.start = function() {
	var sSrcFolder = this._sSourceFolder,
		...
		sFolderPathSupple = join(sSrcFolder, "cldr-core", "supplemental"),
		sFileName,
		oData,
		oResult = {},
		oOutputData,
	...
	aUI5Tags.forEach(function(sUI5Tag) {
		...
		configs.forEach(function(mConfig) {
			...
			sFileName = join(sSrcFolder, mConfig.packageName, "main", sPathTag, mConfig.fileName);
			oData = getContent(sFileName).main[sPathTag];
		...
		oResult.currencyFormat["sap-standard"] = sStandardTrailing || transformCurrencyPattern(oResult.currencyFormat.standard, sCurrencySpacingBefore);
		oResult.currencyFormat["sap-accounting"] = sAccountingTrailing || transformCurrencyPattern(oResult.currencyFormat.accounting, sCurrencySpacingBefore);

		// SAP specific format for style "short"
		oCurrencyFormatShort = oResult["currencyFormat-short"];
		var oCurrencyFormatShortTrailingResult = oResult["currencyFormat-sap-short"] = {};
		Object.keys(oCurrencyFormatShort).forEach(function(sKey) {
			oCurrencyFormatShortTrailingResult[sKey] = oCurrencyFormatShortTrailing[sKey] || transformShortCurrencyPattern(oCurrencyFormatShort[sKey], sCurrencySpacingBefore);
		});
		...
		aSubTasks = [
			...
			(function(r) {
				return function(callback) {
					var sPath = resolve(join(sOutputFolder, sUI5Tag + '.json'));
