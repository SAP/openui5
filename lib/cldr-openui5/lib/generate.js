...
Generator.prototype.start = function() {
	...
	aUI5Tags.forEach(function(sUI5Tag) {
		...
		if (oResult.rtl) {
			// If the character order is right-to-left,
			// insert a RLM marker in front of the first 0 to make the number to be displayed
			// in the same direction as the scale
			oCurrencyFormatShort = oResult["currencyFormat-short"];
			Object.keys(oCurrencyFormatShort).forEach(function(sKey) {
				oCurrencyFormatShort[sKey] = oCurrencyFormatShort[sKey].replace("0", "\u200f0");
			});
		}

		// Workround for arabic short number million issue, can be removed, when
		// http://unicode.org/cldr/trac/ticket/9859
		// is resolved
		if (sCLDRTag.indexOf("ar") === 0) {
			oResult["decimalFormat-short"] = oResult["decimalFormat-long"];
		}
