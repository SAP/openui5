...
Generator.prototype.start = function() {
	...
	aUI5Tags.forEach(function(sUI5Tag) {
		...
		that.updateMonthAbbreviations(oResult, sUI5Tag);
 ...
 */
Generator.prototype.updateMonthAbbreviations = function (oResult, sUI5Tag) {
	var oFormerResults;
	try {
		oFormerResults = fileContent.getContent(path.join(this._sOutputFolder, sUI5Tag + ".json"));
	} catch (oError) {
	...
	].forEach(function (aPropertyPathNames) {
 ...
 */
Generator.getPropertyPathValue = function(oSource, aPropertyPathNames) {
	return aPropertyPathNames.reduce(function (oNode, sPropertyName) {
		return oNode && oNode[sPropertyName];
	}, oSource);
};
 ...
 */
Generator.updateAlternatives = function (oFormerCLDRData, oResult, aPropertyPathNames) {
	var aOldAbbreviations = Generator.getPropertyPathValue(oFormerCLDRData, aPropertyPathNames),
		aAbbreviations = Generator.getPropertyPathValue(oResult, aPropertyPathNames);

	aAbbreviations.forEach(function (sAbbreviation, i) {
		...
		if (typeof aOldAbbreviations[i] === "string") {
			...
			var iAt = aOldAbbreviations[i].indexOf(sAbbreviation),
				aAbbreviationsForCurrentMonth = aOldAbbreviations[i].slice();

			if (iAt >= 0) { // remove duplicate
				aAbbreviationsForCurrentMonth.splice(iAt, 1);
			}
			// if there is more than two abbreviation variants insert the latest variant at position 0
			aAbbreviationsForCurrentMonth.unshift(sAbbreviation);
			aAbbreviations[i] = aAbbreviationsForCurrentMonth;
		}
	});
};
