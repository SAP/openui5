		...
		function preProcessSymbols(symbols) {
			...
			symbols.forEach(oSymbol => {
				if (oSymbol.parent) {
					...
					oParent.nodes.push({name: oSymbol.displayName, description: formatters._preProcessLinksInTextBlock(oSymbol.description)});
				}
			});
		...
		oData.symbols.forEach((oSymbol) => {
			...
			if (oSymbol.methods) {
				...
				oSymbol.methods.forEach(oMethod => {
					...
					// Manage References
					if (oMethod.references && Array.isArray(oMethod.references)) {
						oMethod.references = oMethod.references.map(sReference => {
						...
						if (!oMethod.description) {
							...
							oMethod.description += '<br><br>';
						}
						oMethod.description += `References: <ul>${oMethod.references.join("")}</ul>`;
					}
