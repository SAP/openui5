...
module.exports = function transformer(sInputFile, sOutputFile, sLibraryFile) {
	...
	let formatters = {
		...
		handleExternalUrl: function (sTarget, sText) {
			...
			return `<a target="blank" href="${sTarget}">${sText}</a>
			...
			var topicsData = this._oTopicData, //this.getModel('topics').oData,
				...
				oOptions = {
					linkFormatter: function (target, text) {
						...
						if (target.match("://")) {
							return this.handleExternalUrl(target, text);
						}

						if (target.indexOf("module:") === 0) {
							target = target.slice(7); // Remove "module:" prefix
							bModule = true;
						}
						...
						// Restore some characters after encoding as current link processing is relying on them.
						// BCP: 1880609747
						target = target.replace(/%23/g, "#");
						target = target.replace(/%3A/g, ":");
