...
function createAutoDoc(oClassInfo, classComment, node, parser, filename, commentAlreadyProcessed) {
	...
	var newStyle = !!(env.conf.templates && env.conf.templates.ui5 && env.conf.templates.ui5.newStyle),
		includeSettings = !!(env.conf.templates && env.conf.templates.ui5 && env.conf.templates.ui5.includeSettingsInConstructor),
...
if ( env.conf.templates && env.conf.templates.ui5 && env.conf.templates.ui5.noExit ) {
...
exports.handlers = {
	...
	parseBegin : function(e) {
		...
		pathPrefixes = env.opts._.reduce(function(result, fileOrDir) {
			...
			if ( fs.statSync(fileOrDir).isDirectory() ) {
				result.push(ensureEndingSlash(fileOrDir.replace(/\\/g, '/')));
			}
			return result;
		}, []);
		resourceNamePrefixes = (env.conf.templates && env.conf.templates.ui5 && env.conf.templates.ui5.resourceNamePrefixes) || [];
	...
	parseComplete : function(e) {
		...
		if ( env.conf.templates && env.conf.templates.ui5 && env.conf.templates.ui5.saveSymbols ) {
...
exports.astNodeVisitor = {
	...
	visitNode: function(node, e, parser, currentSourceName) {
		...
		} else if ( node.type === Syntax.VariableDeclaration && node.declarations.length == 1 && isExtendCall(node.declarations[0].init) ) {

			// var NewClass = Something.extend(...)

			// className = node.declarations[0].init.arguments[0].value;
			comment = getLeadingCommentNode(node) || getLeadingCommentNode(node.declarations[0]);
			// console.log("ast node with comment " + comment);
