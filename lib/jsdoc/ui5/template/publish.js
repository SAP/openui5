...
function createAPIJSON4Symbol(symbol, omitDefaults) {
	...
	function methodSignature(member, suppressReturnValue) {
		...
		if ( !suppressReturnValue ) {
			...
			var type = (returns && returns.type) || member.type;
			type = listTypes(type);
			//if ( type && type !== 'void' ) {
			...
			symbol.methods.forEach(function(method) {
				if ( method.name === "getMetadata" && method.returnValue ) {
					...
					if ( metadataClass && metadataClass !== method.returnValue.type ) {
						...
						info("  return type of " + symbol.name + (method.static ? "." : "#") + "getMetadata changed to '" + metadataClass + "'");
					}
				}
			});
		}
...
function validateAPIJSON(api) {
	...
	var defined = Object.assign(Object.create(null), builtinTypes, externalSymbols);
	if ( api.symbols ) {
