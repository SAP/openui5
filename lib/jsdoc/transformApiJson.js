			...
			if (oSymbol["ui5-metadata"]) {
				...
				if (oMeta.events) {
					...
					oMeta.events.forEach(oEvent => {
						...
						aParams && Object.keys(aParams).forEach(oParam => {
							let sSince = aParams[oParam].since;
							let oDeprecated = aParams[oParam].deprecated;
							...
							if (typeof oParamInSymbol === 'object' && oParamInSymbol !== null) {
								if (sSince) {
									oParamInSymbol[oParam].since = sSince;
								}

								if (oDeprecated) {
									oParamInSymbol[oParam].deprecated = oDeprecated;
								}
							}
						})
					});
