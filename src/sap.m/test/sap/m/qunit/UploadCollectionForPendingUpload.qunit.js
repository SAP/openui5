...
sap.ui.define("sap.m.qunit.UploadCollectionForPendingUpload", [
	...
	QUnit.test("Set of 'instantUpload' at runtime is not allowed", function(assert) {
		...
		oUploadCollection = new undefined/*UploadCollection*/({
			instantUpload: false
		...
		oUploadCollection = new undefined/*UploadCollection*/("secondContructorWayToCall", {
			instantUpload: false
		}).placeAt("qunit-fixture");
		oCore.applyChanges();
		assert.ok(!oUploadCollection.getInstantUpload(), "Instance was created with false");
	...
	QUnit.test("Test for method _onChange for instantUpload = false", function(assert) {
		...
		assert.deepEqual(oFileUploader, Element.registry.get(oUploadCollection.getItems()[0].getAssociation("fileUploader")), "Association fileUploader should contain the FileUploader instance with which the Change event was fired");
	...
	QUnit.module("PendingUpload: test setters", {
		beforeEach: function() {
			this.createUploadCollection = function(oAddToContructor) {
				...
				this.oUploadCollection = new undefined/*UploadCollection*/(oAddToContructor).placeAt("qunit-fixture");
				oCore.applyChanges();
			};
			this.oUploadCollection = new undefined/*UploadCollection*/("pendingUploads", {}).placeAt("qunit-fixture");
			oCore.applyChanges();
		},
		afterEach: function() {
			this.oUploadCollection.destroy();
	...
	QUnit.test("Focus is not set if file uploader is invisible", function(assert) {
		//Arrange
		var oItem = new undefined/*UploadCollectionItem*/();
	...
	QUnit.test("Event beforeUploadStarts", function(assert) {
		...
		function onBeforeUploadStarts(oEvent) {
			var oHeaderParameter1 = new undefined/*UploadCollectionParameter*/({ name: sSlugName, value: sSlugValueBefore });
			oEvent.getParameters().addHeaderParameter(oHeaderParameter1);
			var oHeaderParameter2 = new undefined/*UploadCollectionParameter*/({
	...
	QUnit.test("Check file list", function(assert) {
		...
		var oDeleteButton = Element.registry.get(sDeleteButtonId);
