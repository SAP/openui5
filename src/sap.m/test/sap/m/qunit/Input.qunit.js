...
sap.ui.define([
	...
	QUnit.test("sTypedInValue should be reset on item press - tabbular suggestions.", function(assert) {
		// Arrange
		var oSuggPopover, oSpy;
		var oData = {
			data: [
	...
	QUnit.test("sTypedInValue should be reset on enter - mobile", function(assert) {
		...
		oInput.setModel(oModel);
		oSuggPopover = oInput._oSuggPopover;
		oSuggPopover._bDoTypeAhead = true;
		...
		oPopupInput = oSuggPopover._oPopupInput;
	...
	QUnit.test("Tabular Suggestions with custom filter and result function", function(assert){
		var oInput = new Input({
			showSuggestion: true,
			width: "100px",
			maxSuggestionWidth: "500px",
	...
	QUnit.test("Tabular Suggestions - 'typahead' should consider visibility of suggestion", function (assert) {
		...
		oInput._oSuggPopover._bDoTypeAhead = true;
		oInput._oSuggPopover._handleTypeAhead();
		this.clock.tick(300);
	...
	QUnit.test("Close button should revert user input on mobile dialog", function (assert) {
		...
		oSuggPopover = oInput._oSuggPopover;
		oSuggPopoverInput = oSuggPopover._oPopupInput;
	...
	QUnit.test("Input with list suggestion - Braces in binded text and key properties do not cause error", function(assert) {
		...
		// Assert
		assert.strictEqual(oInput._getSuggestionsPopover()._oList.getItems()[0].getTitle(), "1 {{}}{", "Braces are escaped in inputs item group header");
		assert.strictEqual(oInput._getSuggestionsPopover()._oList.getItems()[1].getTitle(), "test1 {{}}", "Braces are escaped in inputs item text");
	...
	QUnit.test("Input with table suggestions - Braces in binded text and key properties do not cause error", function(assert) {
		...
		// Assert
		assert.strictEqual(oInput._getSuggestionsPopover()._oList.getItems()[0].getTitle(), "1 {{}}{", "Braces are escaped in inputs item group header");
		assert.strictEqual(oInput._getSuggestionsPopover()._oList.getItems()[1].getCells()[0].getText(), "test1 {{}}", "Braces are escaped in inputs item text");
