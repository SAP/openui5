...
sap.ui.define([
	...
	*/
	...
	QUnit.test("Check if aria-labelledby attribute is set on the focused item", function(assert) {
	...
	*/
	...
	QUnit.test("Value State - Aria-describedby for value state links announcement when state is not Error", function(assert) {
		...
		const sMultipleLinksText = oCore.getLibraryResourceBundle("sap.m").getText(sLinksTextId);
		...
		const sSingleLink = oCore.getLibraryResourceBundle("sap.m").getText(sLinkTextId);
	...
	QUnit.test("Value State - Dynamic announcement of value state Error with formatted text and links", function(assert) {
		...
		const sText = oCore.getLibraryResourceBundle("sap.m").getText(sLinksTextId);
