...
sap.ui.define([
	...
	QUnit.test("_createSugesstionsTable should not be called when getSuggestionColumns is called.", function (assert) {
		// arrange
		var oInput = new Input({
			showSuggestion: true
		});
		var oCreateSuggestionsTableSpy = sinon.spy(oInput, "_createSuggestionsTable");

		oInput.placeAt("content");
		sap.ui.getCore().applyChanges();

		assert.strictEqual(oCreateSuggestionsTableSpy.callCount, 0, "_createSuggestionsTable was not called.");

		// act
		oInput.getSuggestionColumns();
		oInput.getSuggestionColumns();

		// assert
		assert.strictEqual(oCreateSuggestionsTableSpy.callCount, 0, "_createSuggestionsTable was called only once.");

		// clean
		oInput.destroy();
	});

	QUnit.test("getSuggestionRows internally calls getAggregation('suggestionColumns')", function (assert) {
		// arrange
		var oInput = new Input({
			showSuggestion: true,
			suggestionColumns: [
		...
		var oGetterSpy = sinon.spy(oInput, "getAggregation");
		...
		assert.strictEqual(oGetterSpy.callCount, 1, "getAggregation was called once.");
		assert.strictEqual(oGetterSpy.firstCall.args[0], "suggestionColumns", "The correct aggregation was requested");

		// clean
		oInput.destroy();
	});

	QUnit.test("_createSugesstionsTable should be called once when addSuggestionColumn is called.", function (assert) {
		// arrange
		var oInput = new Input({
			showSuggestion: true
		});
		...
		// act
		oInput.addSuggestionColumn(new Column({}));
		oInput.addSuggestionColumn(new Column({}));

		// assert
		assert.strictEqual(oCreateSuggestionsTableSpy.callCount, 1, "_createSuggestionsTable was called only once.");

		// clean
		oInput.destroy();
	});

	QUnit.test("_createSugesstionsTable should be called once insertSuggestionColumn is called.", function (assert) {
		// arrange
		var oInput = new Input({
			showSuggestion: true
		});
		...
		// act
		oInput.insertSuggestionColumn(new Column({}), 0);
		oInput.insertSuggestionColumn(new Column({}), 0);
	...
	QUnit.test("_createSugesstionsTable should not be called when indexOfSuggestionColumn is called.", function (assert) {
		// arrange
		var oInput = new Input({
			showSuggestion: true
		});
		...
		// act
		oInput.indexOfSuggestionColumn(oColumn);
		oInput.indexOfSuggestionColumn(oColumn);
	...
	QUnit.test("indexOfSuggestionColumn internally calls indexOfAggregation('suggestionColumns')", function (assert) {
		// arrange
		var oColumn = new Column({id: "test"});
		var oInput = new Input({
			showSuggestion: true
		});
		var oSpy = sinon.spy(oInput, "indexOfAggregation");
		var iIndex;
		...
		// act
		oSpy.reset();
		iIndex = oInput.indexOfSuggestionColumn(oColumn);

		assert.strictEqual(oSpy.callCount, 0, "indexOfAggregation was not called.");
		assert.strictEqual(iIndex, -1, "-1 was returned as there is still no table created.");

		oInput.addSuggestionColumn(oColumn);
		sap.ui.getCore().applyChanges();


		iIndex = oInput.indexOfSuggestionColumn(oColumn);

		assert.strictEqual(oSpy.callCount, 1, "indexOfAggregation was called once.");
		assert.strictEqual(oSpy.firstCall.args[0], "suggestionColumns", "The correct aggregation was requested.");
		assert.strictEqual(oSpy.firstCall.args[1].getId(), "test", "The correct parameter was passed.");
		assert.strictEqual(iIndex, 0, "0 was returned as there is still no table created.");

		// clean
		oInput.destroy();
	});

	QUnit.test("_createSugesstionsTable should not be called when removeSuggestionColumn is called.", function (assert) {
		// arrange
		var oInput = new Input({
			showSuggestion: true
		});
		...
		// act
		oInput.removeSuggestionColumn(oColumn);
		oInput.removeSuggestionColumn(oColumn);

		// assert
		assert.strictEqual(oCreateSuggestionsTableSpy.callCount, 0, "_createSuggestionsTable was called once.");

		// clean
		oInput.destroy();
	});

	QUnit.test("removeSuggestionColumn internally calls removeAggregation('suggestionColumns')", function (assert) {
		// arrange
		var oColumn = new Column({id: "test"});
		var oInput = new Input({
			showSuggestion: true
		});
		var oSpy = sinon.spy(oInput, "removeAggregation");
		var result;
		...
		// act
		oSpy.reset();
		result = oInput.removeSuggestionColumn(oColumn);

		assert.strictEqual(oSpy.callCount, 0, "removeAggregation was not called.");
		assert.strictEqual(result, null, "null was returned as there is still no table created.");
		...
		assert.strictEqual(oSpy.callCount, 1, "removeAggregation was called once.");
		assert.strictEqual(oSpy.firstCall.args[0], "suggestionColumns", "The correct aggregation was requested.");
		assert.strictEqual(oSpy.firstCall.args[1].getId(), "test", "The correct parameter was passed.");
		assert.strictEqual(result.getId(), "test", "The correct result was returned.");
	...
	QUnit.test("_createSugesstionsTable should not be called when removeAllSuggestionColumns was called.", function (assert) {
		// arrange
		var oInput = new Input({
			showSuggestion: true
		});
		...
		// act
		oInput.removeAllSuggestionColumns();
		oInput.removeAllSuggestionColumns();
	...
	QUnit.test("removeAllSuggestionColumn internally calls removeAllAggregation('suggestionColumns')", function (assert) {
		// arrange
		var oColumn1 = new Column({id: "test1"});
		var oColumn2 = new Column({id: "test2"});
		var oInput = new Input({
			showSuggestion: true
		});
		var oSpy = sinon.spy(oInput, "removeAllAggregation");
		var result;
		...
		// act
		oSpy.reset();
		result = oInput.removeAllSuggestionColumns();

		assert.strictEqual(oSpy.callCount, 0, "removeAllAggregation was not called.");
		assert.ok(Array.isArray(result), "Result of operation is array.");
		assert.strictEqual(result.length, 0, "Empty array was returned as there is still no table created.");

		oInput.addSuggestionColumn(oColumn1);
		oInput.addSuggestionColumn(oColumn2);
		sap.ui.getCore().applyChanges();

		result = oInput.removeAllSuggestionColumns();

		assert.strictEqual(oSpy.callCount, 1, "removeAllAggregation was called once.");
		assert.strictEqual(oSpy.firstCall.args[0], "suggestionColumns", "The correct aggregation was requested.");
		assert.ok(Array.isArray(result), "Result of operation is array.");
		assert.strictEqual(result.length, 2, "The correct result was returned.");
	...
	QUnit.test("_createSugesstionsTable should not be called when destroySuggestionColumns was called.", function (assert) {
		// arrange
		var oInput = new Input({
			showSuggestion: true
		});
		...
		// act
		oInput.destroySuggestionColumns();
		oInput.destroySuggestionColumns();
	...
	QUnit.test("destroySuggestionColumns internally calls destroyAggregation('suggestionColumns')", function (assert) {
		// arrange
		var oColumn = new Column({id: "test"});
		var oInput = new Input({
			showSuggestion: true
		});
		var oSpy = sinon.spy(oInput, "destroyAggregation");
		...
		assert.strictEqual(oSpy.callCount, 0, "destroyAggregation was not called.");
		...
		assert.strictEqual(oSpy.callCount, 1, "destroyAggregation was called once.");
		assert.strictEqual(oSpy.firstCall.args[0], "suggestionColumns", "The correct aggregation was requested.");
	...
	QUnit.test("_createSugesstionsTable should be called once when bindSuggestionColumns was called.", function (assert) {
		// arrange
		var oInput = new Input({
			showSuggestion: true
		});
		...
		// act
		oInput.bindSuggestionColumns(path, template);
		oInput.bindSuggestionColumns(path, template);

		// assert
		assert.strictEqual(oCreateSuggestionsTableSpy.callCount, 1, "_createSuggestionsTable was called once.");

		// clean
		oInput.destroy();
	});

	QUnit.test("_createSugesstionsTable should not be called when unbindSuggestionColumns was called.", function (assert) {
		// arrange
		var oInput = new Input({
			showSuggestion: true
		});
		...
		// act
		oInput.unbindSuggestionColumns(path);
		oInput.unbindSuggestionColumns(path);
	...
	QUnit.test("_createSugesstionsTable should not called when getSuggestionRows is called.", function (assert) {
		// arrange
		var oInput = new Input({
			showSuggestion: true
		});
	...
	QUnit.test("getSuggestionRows internally calls getAggregation('suggestionRows')", function (assert) {
		// arrange
		var oInput = new Input({
			showSuggestion: true,
			suggestionRows: [
		...
		assert.strictEqual(oGetterSpy.callCount, 1, "getAggregation was called once.");
		assert.strictEqual(oGetterSpy.firstCall.args[0], "suggestionRows", "The correct aggregation was requested");

		// clean
		oInput.destroy();
	});

	QUnit.test("_createSugesstionsTable should be called once when addSuggestionRow is called.", function (assert) {
		// arrange
		var oInput = new Input({
			showSuggestion: true
		});
	...
	QUnit.test("_createSugesstionsTable should be called once when insertSuggestionRow is called.", function (assert) {
		// arrange
		var oInput = new Input({
			showSuggestion: true
		});
		...
		// act
		oInput.insertSuggestionRow(new ColumnListItem());
		oInput.insertSuggestionRow(new ColumnListItem());
	...
	QUnit.test("_createSugesstionsTable should not be called when indexOfSuggestionRow is called.", function (assert) {
		// arrange
		var oInput = new Input({
			showSuggestion: true
		});
		var oCreateSuggestionsTableSpy = sinon.spy(oInput, "_createSuggestionsTable");
		var oSuggestionRow = new ColumnListItem();
		...
		// act
		oInput.indexOfSuggestionRow(oSuggestionRow);
		oInput.indexOfSuggestionRow(oSuggestionRow);
	...
	QUnit.test("indexOfSuggestionRow internally calls indexOfAggregation('suggestionRows')", function (assert) {
		// arrange
		var oRow = new ColumnListItem({id: "test"});
		var oInput = new Input({
			showSuggestion: true
		});
		var oSpy = sinon.spy(oInput, "indexOfAggregation");
		...
		// act
		oSpy.reset();
		iIndex = oInput.indexOfSuggestionRow(oRow);

		assert.strictEqual(oSpy.callCount, 0, "indexOfAggregation was not called.");
		assert.strictEqual(iIndex, -1, "-1 was returned as there is still no table created.");

		oInput.addSuggestionRow(oRow);
		sap.ui.getCore().applyChanges();

		iIndex = oInput.indexOfSuggestionRow(oRow);

		assert.strictEqual(oSpy.callCount, 1, "indexOfAggregation was called once.");
		assert.strictEqual(oSpy.firstCall.args[0], "suggestionRows", "The correct aggregation was requested.");
		assert.strictEqual(oSpy.firstCall.args[1].getId(), "test", "The correct parameter was passed.");
		assert.strictEqual(iIndex, 0, "0 was returned as there is still no table created.");
	...
	QUnit.test("_createSugesstionsTable should not be called when removeSuggestionRow is called.", function (assert) {
		// arrange
		var oInput = new Input({
			showSuggestion: true
		});
		...
		// act
		oInput.removeSuggestionRow(oSuggestionRow);
		oInput.removeSuggestionRow(oSuggestionRow);
	...
	QUnit.test("removeSuggestionRow internally calls removeAggregation('suggestionRows')", function (assert) {
		// arrange
		var oRow = new ColumnListItem({id: "test"});
		var oInput = new Input({
			showSuggestion: true
		...
		// act
		oSpy.reset();
		result = oInput.removeSuggestionRow(oRow);
	...
	QUnit.test("_createSugesstionsTable should not be called when removeAllSuggestionRows is called.", function (assert) {
		// arrange
		var oInput = new Input({
			showSuggestion: true
		});
	...
	QUnit.test("removeAllSuggestionRows internally calls removeAllAggregation('suggestionRows')", function (assert) {
		// arrange
		var oRow1 = new ColumnListItem({id: "test1"});
		var oRow2 = new ColumnListItem({id: "test2"});
		var oInput = new Input({
			showSuggestion: true
		});
		var oSpy = sinon.spy(oInput, "removeAllAggregation");
		...
		// act
		oSpy.reset();
		result = oInput.removeAllSuggestionRows();

		assert.strictEqual(oSpy.callCount, 0, "removeAllAggregation was not called.");
		assert.ok(Array.isArray(result), "Result of operation is array.");
		assert.strictEqual(result.length, 0, "Empty array was returned as there is still no table created.");

		oInput.addSuggestionRow(oRow1);
		oInput.addSuggestionRow(oRow2);
		sap.ui.getCore().applyChanges();
	...
	QUnit.test("_createSugesstionsTable should be called once when destroySuggestionRows is called.", function (assert) {
		// arrange
		var oInput = new Input({
			showSuggestion: true
		});
	...
	QUnit.test("destroySuggestionRows internally calls destroyAggregation('suggestionRows')", function (assert) {
		// arrange
		var oRow = new ColumnListItem({id: "test"});
		var oInput = new Input({
			showSuggestion: true
	...
	QUnit.test("_createSugesstionsTable should be called once when bindSuggestionRows was called.", function (assert) {
		// arrange
		var oInput = new Input({
			showSuggestion: true
		});
		...
		// act
		oInput.bindSuggestionRows(path, template);
		oInput.bindSuggestionRows(path, template);
	...
	QUnit.test("_createSugesstionsTable should not be called when unbindSuggestionRows was called.", function (assert) {
		// arrange
		var oInput = new Input({
			showSuggestion: true
		});
		...
		// act
		oInput.unbindSuggestionRows(path);
		oInput.unbindSuggestionRows(path);
	...
	QUnit.test("Table should be destroyed when destroying the SuggestionsPopover", function (assert) {
		// arrange
		var oInput = new Input({
			showSuggestion: true,
			suggestionColumns: [
		...
		var oCreateSuggestionsTableSpy = sinon.spy(oInput, "_createSuggestionsTable");
		var oTable, oTableDestroySpy;

		oInput.placeAt("content");
		sap.ui.getCore().applyChanges();

		oTable = oInput._getSuggestionsPopover().getItemsContainer();
		assert.ok(oTable.isA("sap.m.Table"), "The table was created and is available.");

		oTableDestroySpy = sinon.spy(oTable, "destroy");

		// act
		oInput.exit();

		// assert
		assert.ok(oTableDestroySpy.called, "The table was created and is available.");
	...
	QUnit.test("Should show all the items", function (assert) {
		...
		// Assert
		assert.strictEqual(this.oInput._getSuggestionsPopover().getItemsContainer().getItems().length, 5, "All the items are available");
		assert.strictEqual(fnGetVisisbleItems(this.oInput._getSuggestionsPopover().getItemsContainer().getItems()).length, 5, "Shows all items");
	});
	...
	QUnit.test("Should filter the items", function (assert) {
		...
		// Assert
		assert.strictEqual(this.oInput._getSuggestionsPopover().getItemsContainer().getItems().length, 5, "All the items are available");
		assert.strictEqual(fnGetVisisbleItems(this.oInput._getSuggestionsPopover().getItemsContainer().getItems()).length, 1, "Only the matching items are visible");
	});
