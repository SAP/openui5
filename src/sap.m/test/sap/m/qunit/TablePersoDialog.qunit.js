...
//			only one item in list is available(selected) => both buttons have to be disabled!
	...
	QUnit.test("Arrow buttons and scrolling", function(assert){
		...
		setTimeout( function(){
			...
			assert.equal(spyScrollTo.callCount, 1, "scrollToIndex should be called once when moving down");
			//Now press up to check if the scrolls up as the item is at the top
			for (var i = 0; i < 1; i++) {
				oButtonUp.firePress();
				oCore.applyChanges();
			}
			assert.equal(spyScrollTo.callCount, 1, "scrollToIndex should be called once when moving up");
			oTPC3.destroy();
			oTable3.destroy();
			fnDone();
