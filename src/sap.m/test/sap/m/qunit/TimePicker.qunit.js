...
sap.ui.define([
	...
	QUnit.test("Set displayFormat to 'HH:mm'", function (assert) {
		...
		var sPlaceholderPrefix = sap.ui.getCore().getLibraryResourceBundle("sap.ui.core").getText("date.placeholder").split("{")[0];
	...
	QUnit.test("support2400 - 24:00:00 value works correctly in all scenarios", function(assert) {
		...
		var tpId = "timepicker",
			...
			oGetSlidersStub = this.stub(oTP, "_getSliders", function () { return oTimePickerSliders; });
	...
	QUnit.test("support2400 - _formatValue", function(assert) {
		// Prepare
		var oTP = new TimePicker({
				displayFormat: "HH:mm",
				valueFormat: "HH:mm:ss",
			...
			oIsOpenStub = this.stub(oTP, "isOpen", function () { return false; }),
			sResult;

		sap.ui.getCore().applyChanges();
		oTP._createPicker("HH:mm");
		oTP._getSliders()._getHoursSlider().setSelectedValue("24");

		// Act
		sResult = oTP._formatValue(new Date(2000, 1, 2, 23), true);

		// Assert
		assert.equal(sResult, "23:00:00", "The output value is correct.");
