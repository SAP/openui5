describe("sap.m.infra_SemanticPage", function() {
	...
	var fnClickThenCompare = function (sId, sImageName, sTestMessage, bEnsureOverflowVisible) {
		it(sTestMessage, function () {
			if (browser.testrunner.runtime.browserName != "safari") {
				...
				browser.actions().mouseMove(element(by.id(sId))).click().mouseMove(body,{x:0,y:0}).perform();
	...
	var fnEnsureOverflowVisible = function() {
		...
		overflowBtn.isPresent().then(function(isPresent) {
			if(isPresent) {
				browser.actions().mouseMove(overflowBtn).click().mouseMove(body,{x:0,y:0}).perform();
