...
sap.ui.define(['jquery.sap.global', 'sap/ui/core/Control', './TimePickerSliderRenderer'],
	function(jQuery, Control, TimePickerSliderRenderer) {
		"use strict";
		 ...
		 */
		var TimePickerSlider = Control.extend("sap.m.TimePickerSlider", /** @lends sap.m.TimePickerSlider.prototype */ { metadata: {
			library : "sap.m",
			properties: {
				/**
				 * This property corresponds to the currently selected value of the slider.
				 */
				selectedValue: {name: "selectedValue", type: "string", defaultValue: null},
				/**
				 * This property corresponds to the ability of the slider to implement its values cycling.
				 */
				cycle: {name: "cycle", type: "boolean", defaultValue: true},
				/**
				 * This property defines the descriptive text for the slider.
				 */
				label: {name: "label", type: "string", defaultValue: null},
				/**
				 * This property defines whether the slider is expanded.
				 */
				isExpanded: {name: "isExpanded", type: "boolean", defaultValue: false}
			},
			aggregations: {
				/**
		...
		TimePickerSlider.prototype.fireTap = function(oEvent) {
			...
			if (!this.getIsExpanded()) {
				...
				if (oScrElement.tagName === "LI") {
					this.setSelectedValue(jQuery(oScrElement).text());
		...
		TimePickerSlider.prototype.setSelectedValue = function(sValue) {
			var iIndexOfValue = findIndexInArray(this.getItems(), function(oElement) {
				return oElement.getText() === sValue;
			});
			if (iIndexOfValue === -1) {
				return;
			}
...
//			this.setSelectedValue(this.getSelectedValue());
		...
		TimePickerSlider.prototype.onsappageup = function(oEvent) {
			if (this.getIsExpanded()) {
				...
				this.setSelectedValue(iFirstItem.getText());
			}
		};
		...
		TimePickerSlider.prototype.onsappagedown = function(oEvent) {
			if (this.getIsExpanded()) {
				...
				this.setSelectedValue(iLastItem.getText());
			}
		};
		 ...
		 * The method is called only when cycle property is set to true.
		 * @private
		...
		TimePickerSlider.prototype._getContentRepeat = function() {
			...
			//we target to get at least 50 items in the list,
			...
			if (!this._contentRepeat) {
				if (this.getCycle()) {
					this._contentRepeat = Math.ceil(50 / this.getItems().length);
		...
		TimePickerSlider.prototype._animateScroll = function(iSpeed) {
			var iPreviousScrollTop = this._getSliderContainerDomRef().scrollTop(),
				that = this,
				frameFrequencyMs = 25, //milliseconds - 40 frames per second
		...
		TimePickerSlider.prototype._scrollerSnapped = function(iCurrentItem) {
			...
			sNewValue = this.getItems()[iSelectedItemIndex].getText();
		...
		TimePickerSlider.prototype._updateScroll = function() {
			var sSelectedValue = this.getSelectedValue();
			if (sSelectedValue !== this.getItems()[0].getText()
		...
		TimePickerSlider.prototype._addSelectionStyle = function() {
			...
			//WAI-ARIA region
			document.getElementById(this.getId() + "-valDescription").setAttribute("aria-label", $aItems.eq(this._iSelectedItemIndex).text());
		};
		...
		TimePickerSlider.prototype._offsetValue = function(iIndexOffset) {
			...
			this._getSliderContainerDomRef().animate({ scrollTop: iSnapScrollTop}, 200, 'linear', function() {
