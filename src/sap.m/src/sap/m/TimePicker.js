	...
	function(jQuery, InputBase, ResponsivePopover, EnabledPropagator, IconPool, DateModel, TimePickerSliders) {
		...
		var TimePicker = InputBase.extend("sap.m.TimePicker", /** @lends sap.m.TimePicker.prototype */ {
			metadata : {
				...
				properties : {
					/**
					 * Displays date value in the given format in text field.
					 * The default value is taken from the locale settings on init(). The locale taken
					 * corresponds to the 'medium' time format.
					 * (https://openui5.hana.ondemand.com/docs/api/symbols/sap.ui.core.LocaleData.html#getTimePattern)
					 * If data-binding
					 * is used on value property with type sap.ui.model.type.Date then this property is ignored.
					 */
					displayFormat : {type : "string", group : "Appearance", defaultValue : null},

					/**
					 * The provided value property (@see https://openui5.hana.ondemand.com/docs/api/symbols/sap.m.Input.html) should match with valueFormat to be able to parse the date.
					 * The default format is taken from the locale settings. You can set and get value in this format.
					 * If you use data-binding on value property with type sap.ui.model.type.Date then this
					 * property is ignored.
					 */
					 ...
					 * Makes possible setting the 'value' property as a string in language different than english.
					 ...
					 * localeId defaults to en-US.
					 */
					localeId: {type : "string", group: "Data", defaultValue : "en-US"},

					/**
					 * As a JavaScript Date Object, this property is used to assign a new value, which is independent from the valueFormat. This property stores the last valid date, either
					 * typed in by the user or programmatically set.
					 */
					dateValue : {type : "object", group : "Data", defaultValue : null},

					/**
					 * This property displays the text of the general picker label and is read by screen readers. It is visible only on phone.
					 */
		...
		EnabledPropagator.apply(TimePicker.prototype, [true]);
		...
		TimePicker.prototype.onclick = function(oEvent) {
			...
			if (!bPickerOpened && (bIconClicked || sap.ui.Device.browser.mobile)) {
				this._openPicker();
		...
		TimePicker.prototype.onAfterOpen = function() {
			this._bPickerOpening = false;
			this._getSliders().updateSlidersValues();
			this._getSliders()._initFocus();
