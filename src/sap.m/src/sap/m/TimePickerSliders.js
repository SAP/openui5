...
sap.ui.define(['jquery.sap.global', 'sap/ui/core/Control', './TimePickerSlidersRenderer', './TimePickerSlider'],
	function (jQuery, Control, SlidersRenderer, TimePickerSlider) {
		...
		TimePickerSliders.prototype.getTimeValues = function () {
			...
			if (sAmpm === this._sAM && iHours === 12) {
				iHours = 0;
			} else if (sAmpm === this._sPM && iHours !== 12) {
				iHours += 12;
			}
		...
		TimePickerSliders.prototype.setTimeValues = function (oDate) {
			...
			if (oListAmPm) {
				sAmpm = iHours >= 12 ? this._sPM : this._sAM;
		 ...
		 */
		TimePickerSliders.prototype._generatePickerListValues = function (iFrom, iTo) {
			var aValues = [];
			for (var iIndex = iFrom; iIndex <= iTo; iIndex++) {
				aValues.push({ key: iIndex.toString(), text: iIndex.toString()});
			}
		...
		TimePickerSliders.prototype._setupLists = function (sFormat) {
			...
			for (var i = 0; i < aFormat.length; i++) {
				if (aUsedChars.indexOf(aFormat[i]) !== -1) {
					continue;
				}
				aUsedChars.push(aFormat[i]);

				switch (aFormat[i]) {
					case 'H':
						this.addAggregation("_columns", new TimePickerSlider(this.getId() + "-listHours", {
							items: this._generatePickerListValues(0, 23),
							expanded: jQuery.proxy(onSliderExpanded, this),
							label: sLabelHours
						}));
						break;
					case 'h':
						//second time, same id - not impossible of a format?
						this.addAggregation("_columns", new TimePickerSlider(this.getId() + "-listHours", {
							items: this._generatePickerListValues(1, 12),
							expanded: jQuery.proxy(onSliderExpanded, this),
							label: sLabelHours
						}));
						break;
					case 'm':
						this.addAggregation("_columns", new TimePickerSlider(this.getId() + "-listMins", {
							items: this._generatePickerListValues(0, 59),
							expanded: jQuery.proxy(onSliderExpanded, this),
							label: sLabelMinutes
						}));
						break;
			...
			function onSliderExpanded(oEvent) {
				var aSliders = this.getAggregation("_columns"),
					oExpandedSlider = oEvent.getParameter("ctrl");

				for (var i = 0; i < aSliders.length; i++) {
					if (aSliders[i] !== oExpandedSlider && aSliders[i].getIsExpanded()) {
