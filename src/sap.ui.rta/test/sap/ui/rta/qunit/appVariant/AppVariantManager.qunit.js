...
function(AppVariantManager, DescriptorVariantFactory, FakeLrepConnector) {
	...
	QUnit.test("When saveAppVariantDescriptorAndDirtyChangesToLREP() method is called and server response is successful", function (assert) {
		...
		return DescriptorVariantFactory.createNew({
			...
			return this.oAppVariantManager.saveDescriptorAndFlexChangesToLREP(oDescriptor, oRootControl).then(function() {
				assert.ok(true, "then the descriptor changes and dirty UI changes have been saved to LREP");
			});
		}.bind(this));
	});

	QUnit.test("When saveAppVariantDescriptorAndDirtyChangesToLREP() method is called and server response is not successful", function (assert) {
		...
		assert.throws(function(){
			this.oAppVariantManager.saveDescriptorAndFlexChangesToLREP(oDescriptor, oRootControl);
		});
	});
});