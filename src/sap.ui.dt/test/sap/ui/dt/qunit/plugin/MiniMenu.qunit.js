...
sap.ui.require([
	...
	QUnit.test("Calling the _popupClosed function", function (assert) {
		...
		oMiniMenu.bOpenNew = false;
		oMiniMenu._popupClosed();
		assert.ok(!oMiniMenu.bOpen, "MiniMenu should be closed");
	...
	QUnit.test("Calling the _popupClosed function in expanded mode", function (assert) {
		...
		assert.ok(oMiniMenu.bOpen, "MiniMenu should be opened");
		oMiniMenu._popupClosed();
		assert.ok(!oMiniMenu.bOpen, "MiniMenu should be closed");
		oMiniMenu = null;
	});
	...
	QUnit.test("Testing onHover function", function (assert) {
		...
		assert.ok(!oMiniMenu.bOpen, "MiniMenu should not be opened");

		this.clock.tick(this.oMiniMenuPlugin.iMenuHoverOpeningDelay);

		assert.ok(oMiniMenu.bOpen, "MiniMenu should be open");
	...
	QUnit.test("Testing onHover with onHoverExit function", function (assert) {
		...
		assert.ok(!oMiniMenu.bOpen, "MiniMenu should not be open");
		oMiniMenu = null;
	...
	QUnit.test("Testing onTouch function", function (assert) {
		...
		assert.ok(!oMiniMenu.getPopover().isOpen(), "MiniMenu should not be opened");
	...
	QUnit.test("setting value of maxButtonsDisplayed to an illegal value", function (assert) {
		...
		assert.throws(function () {
			this.oMiniMenu.setMaxButtonsDisplayed(1);
	...
	QUnit.test("Inserting a button", function (assert) {
		assert.strictEqual(this.oMiniMenu.insertButton(new sap.m.Button({
			text: "abc"
		}), 1), this.oMiniMenu, "Should return the MiniMenu");
	...
	QUnit.test("Calling _setFocusOnNextButton", function (assert) {
		...
		var oTestButton1 = new sap.m.Button({}).placeAt("qunit-fixture");
		var oTestButton2 = new sap.m.Button({}).placeAt("qunit-fixture");
		var oTestButton3 = new sap.m.Button({}).placeAt("qunit-fixture");

		sap.ui.getCore().applyChanges();
		oTestButton1.focus();

		this.oMiniMenu._setFocusOnNextButton([oTestButton1, oTestButton2, oTestButton3], 0);
		assert.strictEqual(document.activeElement.id, oTestButton2.getId(), "Focus should be at second button");
		this.oMiniMenu._setFocusOnNextButton([oTestButton1, oTestButton2, oTestButton3], 2);
	...
	QUnit.test("Calling _setFocusOnPreviousButton", function (assert) {
		...
		this.oMiniMenu._setFocusOnPreviousButton([oTestButton1, oTestButton2, oTestButton3], 1);
		assert.strictEqual(document.activeElement.id, oTestButton1.getId(), "Focus should be at second button");
		this.oMiniMenu._setFocusOnPreviousButton([oTestButton1, oTestButton2, oTestButton3], 0);
	...
	QUnit.test("Calling _changeFocusOnButtons", function (assert) {
		...
		var oTestBtn = new sap.m.Button({}).placeAt("qunit-fixture");

		sap.ui.getCore().applyChanges();

		var nextSpy = sinon.spy(this.oMiniMenu, "_setFocusOnNextButton");
		var prevSpy = sinon.spy(this.oMiniMenu, "_setFocusOnPreviousButton");

		this.oMiniMenu.show(oTestBtn, false);

		var sId = this.oMiniMenu.getButtons()[0].getId();

		this.oMiniMenu._changeFocusOnButtons(sId);
		...
		this.oMiniMenu._changeFocusOnButtons(sId, true);
	...
	QUnit.test("calling _placeMiniMenu", function (assert) {
		...
		this.oMiniMenu._oContextMenuPosition = {
			x: 314,
			y: 42
		};

		this.oMiniMenu.addButton({
			text: "button",
			handler: function () {
				return undefined;
			},
		...
		var oFakeDiv = this.oMiniMenu._placeMiniMenu(this.oButton2Overlay, true, true);
	...
	QUnit.test("calling _setButtonsForMiniMenu with 3 disabled Buttons", function (assert) {
		...
		this.oMiniMenu._setButtonsForMiniMenu(aButtons, new sap.m.Button({
			id: "btn0_"
		}));
