...
sap.ui.define([
	...
	QUnit.module("Given that an Overlay is created for a control", {
		...
		QUnit.test("when getFirstDraggableParentOverlay is called on the button and layout1 is draggable", function(assert) {
			this.oLayoutOverlay1.getDomRef().draggable = true;
			assert.strictEqual(
				OverlayUtil.getFirstDraggableParentOverlay(this.oButtonOverlay01),
				this.oLayoutOverlay1,
				"then the draggable layout1 is found"
			);
		});
	});
