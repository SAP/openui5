...
sap.ui.define([
	...
	ElementUtil.getElementInstance = function(vElement) {
		if (typeof vElement === "string") {
			var oElement = Element.registry.get(vElement);
	...
	ElementUtil.getClosestElementForNode = function(oNode) {
		...
		return oClosestElement ? Element.registry.get(oClosestElement.getAttribute("data-sap-ui")) : undefined;
	};
	...
	ElementUtil.extractTemplateId = function(mBoundControl) {
		if (isPlainObject(mBoundControl) && mBoundControl.templateId) {
			if (mBoundControl.stack.length > 1) {
				var oResultControl;
				var oAggregatedControl = Element.registry.get(mBoundControl.templateId);
				var sAggregation;
				var iIndex;
