sap.ui.define([
	...
	return function (opaTestOrSkip) {
		...
		// ----------------------------------------------------------------
		opaTestOrSkip("Open and confirm the filter dialog to check if the values have been saved", function (Given, When, Then) {
			//Reopen the dialog
			//as the Table overlay is still marked as selected, we need to click it twice..
			When.iClickOnOverlayForControl("sap.ui.mdc.Table");
			When.iClickOnOverlayForControl("sap.ui.mdc.Table");
		...
		// ----------------------------------------------------------------
		opaTestOrSkip("When I select columns and cancel, the changes should discard", function (Given, When, Then) {
		...
		// ----------------------------------------------------------------
		opaTestOrSkip("check 'order' of columns", function (Given, When, Then) {
			//Reorder table items
			Then.iShouldSeeP13nItem("regionOfOrigin_code", 0);
			Then.iShouldSeeP13nItem("Breakout Year", 1);
			Then.iShouldSeeP13nItem("Country", 2);
		...
		opaTestOrSkip("check column items", function (Given, When, Then) {
			//Select table items
