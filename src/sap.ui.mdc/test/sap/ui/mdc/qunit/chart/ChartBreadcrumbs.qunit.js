...
sap.ui.define([
	...
	function (
		...
		QUnit.test("DrillBreadcrumbs _createCrumb", function (assert) {
			...
			const oPressSpy = this.spy();

			oCrumb.attachPress(oPressSpy);
			oCrumb.firePress();
			assert.ok(oPressSpy.calledOnce, "Press event was triggered");
		});
		...
		QUnit.test("DrillBreadcrumbs updateDrillBreadcrumbs", function (assert) {
			...
			oDrillableItems = [{ text: "label1", key: "name1" }, { text: "label2", key: "name2" }];
			const getLinksStub = sinon.stub(this.oMDCDrillBreadcrumbs, "getLinks").returns([this.oMDCDrillBreadcrumbs._createLink("name2", "label12")]);
