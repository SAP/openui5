...
//		assert.ok(oWrapper._getSingleSelection(), "_getSingleSelection");
	...
	QUnit.test("getTextForKey with unbound table", function(assert) {

		// TODO: check if this case is really needed, was and ust case of the very beginning
		sKeyPath = undefined;
		oTable.unbindItems();
		oTable.addItem(new ColumnListItem({
			type: "Active",
			cells: [new Text({text: "X"}),
			        new Text({text: "XX"}),
			        new Text({text: "XXX"})]
		}));
		oTable.addItem(new ColumnListItem({
			type: "Active",
			cells: [new Text({text: "Y"}),
			        new Text({text: "YY"}),
			        new Text({text: "YYY"})]
		}));
		oTable.addItem(new ColumnListItem({
		...
		var oResult = oWrapper.getTextForKey("X", undefined, undefined, true);
		assert.ok(typeof oResult === "object", "Object returned");
		assert.equal(oResult.description, "XX", "Text for key");
		assert.equal(oResult.key, "X", "key");
		assert.notOk(oResult.inParameters, "no in-parameters returned");
		assert.notOk(oResult.outParameters, "no out-parameters returned");
		...
		var oException;
		try {
			oWrapper.getTextForKey("Y", undefined, undefined, true);
		} catch (oError) {
			oException = oError;
		}
		assert.ok(oException instanceof FormatException, "FormatException fired");
		...
		try {
			oWrapper.getTextForKey("Z", undefined, undefined, true);
		} catch (oError) {
			oException = oError;
		}
		assert.ok(oException instanceof FormatException, "FormatException fired");
