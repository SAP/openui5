...
sap.ui.define([
...
], function (Opa5, KeyCodes, OpaPage, waitForFilterField) {
	'use strict';
	...
	return function (opaTestOrSkip) {
		...
		opaTestOrSkip("Typing raises fitting suggestions", function (Given, When, Then) {
		...
		opaTestOrSkip("Select and Confirm", function (Given, When, Then) {
		...
		opaTestOrSkip("Escape key hides suggestions", function (Given, When, Then) {
			When.onTheOPAPage.iEnterTextOnTheFilterField({label: "TestField"}, "aust", {keepFocus: true, clearTextFirst: true});
		...
		opaTestOrSkip("Reopen F4, change ColSearch and select from SingleMaster table", function (Given, When, Then) {
			When.onTheOPAPage.iOpenTheValueHelpForFilterField({label: "TestField"});
			Then.onTheOPAPage.iShouldSeeTheValueHelpDialog();
		...
		opaTestOrSkip("MultiSelect and Confirm", function (Given, When, Then) {
		...
		opaTestOrSkip("De-select conditions by removing tokens", function (Given, When, Then) {
			When.onTheOPAPage.iOpenTheValueHelpForFilterField({label: "TestField"});
		...
		opaTestOrSkip("De-select conditions by removing all tokens at once", function (Given, When, Then) {
			When.onTheOPAPage.iOpenTheValueHelpForFilterField({label: "TestField"});
			When.onTheOPAPage.iToggleTheValueHelpListItem("Twain, Mark");
		...
		opaTestOrSkip("Range-Selection across multiple pages", function (Given, When, Then) {
			When.onTheOPAPage.iOpenTheValueHelpForFilterField({label: "TestField"});
		...
		opaTestOrSkip("Navigation between tabs", function (Given, When, Then) {
			When.onTheOPAPage.iOpenTheValueHelpForFilterField({label: "TestField"});
		...
		Object.keys(oFilterBarConfigs).forEach(function (sModuleName) {
			...
			opaTestOrSkip("Initial conditions", function (Given, When, Then) {
			...
			opaTestOrSkip("Reverting to initial conditions after dialog search", function (Given, When, Then) {
				When.onTheOPAPage.iEnterTextOnTheFilterField({label: "TestField"}, "doug", {keepFocus: true, clearTextFirst: true});
				When.onTheOPAPage.iOpenTheValueHelpForFilterField({label: "TestField"});
		...
		opaTestOrSkip("Considers filterconditions", function (Given, When, Then) {
