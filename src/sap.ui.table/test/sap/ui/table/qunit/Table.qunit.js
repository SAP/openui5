...
sap.ui.require([
	...
	QUnit.test("BusyIndicator handling", function(assert) {
		...
		function localTest(bBusyIndicatorVisibleBefore, bBusyIndicatorVisibleAfter, bDataRequestedListener, bPendingRequest) {
			// Prepare for next test iteration.
			oTable.setBusy(bBusyIndicatorVisibleBefore);
			sap.ui.getCore().applyChanges();
			onBusyStateChangedEventHandler.reset();

			assert.strictEqual(oTable.getBusy(), bBusyIndicatorVisibleBefore,
				"Initial: The busy state of the table is: " + bBusyIndicatorVisibleBefore);
			assert.strictEqual(oTable.getDomRef(sBusyIndicatorParent).querySelector("." + sBusyIndicatorClass) != null, bBusyIndicatorVisibleBefore,
				"Initial: The busy indicator element exists in the DOM: " + bBusyIndicatorVisibleBefore);

			// Perform next test iteration.
			oTable._bPendingRequest = !bDataRequestedListener;
			if (bDataRequestedListener) {
				oTable._onBindingDataRequestedListener.call(oTable, oEvent);
			} else {
				oTable._onBindingDataReceivedListener.call(oTable, oEvent);
				clock.tick(1);
			}
			sap.ui.getCore().applyChanges();

			assert.strictEqual(oTable.getBusy(), bBusyIndicatorVisibleAfter, "Test: The busy state of the table is: " + bBusyIndicatorVisibleAfter);

			if (bBusyIndicatorVisibleBefore === bBusyIndicatorVisibleAfter) {
				assert.ok(onBusyStateChangedEventHandler.callCount === 0, "Test: BusyStateChanged event has not been fired");
			} else {
				assert.ok(onBusyStateChangedEventHandler.callCount === 1, "Test: BusyStateChanged event has been fired");

				if (onBusyStateChangedEventHandler.callCount === 1) {
					assert.deepEqual(onBusyStateChangedEventHandler.parameters, {busy: bBusyIndicatorVisibleAfter, id: oTable.getId()},
						"Test: BusyStateChanged event has been fired with the correct arguments");
				}
			}

			assert.strictEqual(oTable.getDomRef(sBusyIndicatorParent).querySelector("." + sBusyIndicatorClass) != null, bBusyIndicatorVisibleAfter,
				"Test: The busy indicator element exists in the DOM: " + bBusyIndicatorVisibleAfter);

			assert.strictEqual(oTable._bPendingRequest, bPendingRequest, "Test: The pending request flag is correctly set");
		}
		...
		localTest(false, false, true, true);
		localTest(true, true, true, true);

		// No binding: No busy state change.
		oTable.setEnableBusyIndicator(true);
		this.stub(oTable, "getBinding").withArgs("rows").returns(undefined);
		localTest(false, false, true, false);
		localTest(true, true, true, false);
		...
		// Data requested: Set busy state to true.
		oEvent.getParameter = function() {
			return false;
		};
		localTest(false, true, true, true);
		localTest(true, true, true, true);
		...
		// Busy indicator handling disabled: No busy state change.
		oTable.setEnableBusyIndicator(false);
		localTest(false, false, false, false);
		localTest(true, true, false, false);

		// No binding: No busy state change.
		oTable.setEnableBusyIndicator(true);
		this.stub(oTable, "getBinding").withArgs("rows").returns(undefined);
		localTest(false, false, false, true);
		localTest(true, true, false, true);
		...
		// Data received: Set busy state to false.
		oEvent.getParameter = function() {
			return false;
		};
		localTest(false, false, false, false);
		localTest(true, false, false, false);
