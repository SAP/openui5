...
function(
	...
	QUnit.module("Given an instance of VariantModel", {
		...
		QUnit.test("when calling '_removeDirtyChanges'", function(assert) {
			...
			sandbox.stub(this.oFlexController._oChangePersistence, "getDirtyChanges").returns(aDirtyChanges);
		...
		QUnit.test("when calling 'manageVariants' in Adaptation mode once with changes and then without changes", function(assert) {
			...
			this.oModel._setModelPropertiesForControl(sVariantManagementReference, true, oVariantManagement);
	...
	QUnit.module("Given an empty VariantModel and a VariantManagement control", {
		...
		QUnit.test("when variant management controls are initialized with with 'updateVariantInURL' property set and default (false)", function(assert) {
			...
			assert.deepEqual(this.oAttachHashHandlersStub.getCall(0).args, [this.oModel.oComponent.getLocalId(this.oVariantManagement.getId()), false], "then VariantUtil.attachHashHandlers was called once for a control to update URL");
