...
function(
	...
	jQuery.sap.registerModulePath("sap.ui.fl.qunit.integration.async", "./");
	...
	QUnit.test("applies the change after the recreation of the changed control", function(assert) {
		...
		var oXmlPrepossessSpy = this.spy(XmlPreprocessorImpl, "process");
		var oAddGroupChangeHandlerSpy = this.spy(AddSimpleFormGroup, "applyChange");
	...
	if (!CacheManager._isSupportedEnvironment()){
		...
		QUnit.test("a group is added and passed to the core/CacheManager", function(assert) {
			...
			var oCacheManagerSpy = this.spy(CacheManager, "set");
			var oAddGroupChangeHandlerSpy = this.spy(AddSimpleFormGroup, "applyChange");
