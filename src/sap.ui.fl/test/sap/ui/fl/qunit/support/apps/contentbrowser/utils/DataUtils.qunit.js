...
sap.ui.define([
	...
	QUnit.test("when formatting data of other files type", function (assert) {
		var oData = {};
		var oStubbedJsonParse = this.stub(JSON, "parse");
		var oStubbedJsonStringify = this.stub(JSON, "stringify");
	...
	QUnit.test("when formatting invalid data", function (assert) {
		...
		var oStubbedDisplayError = this.stub(ErrorUtils, "displayError");
