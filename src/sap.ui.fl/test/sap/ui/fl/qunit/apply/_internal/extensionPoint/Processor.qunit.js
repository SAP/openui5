sap.ui.define([
	...
	function check(bSync, assert) {
		...
		var iPoll = setInterval(function() {
			...
			var aTableItems1 = oView.byId("sync").byId('fragmentWithExtensionPoint3--customTable').getItems();
			var aTableItems2 = oView.byId("async").byId('fragmentWithExtensionPoint3--customTable').getItems();
			if (
				aPanelContent1.length === 27
				&& aPanelContent2.length === 27
				&& aTableItems1.length === 2
				&& aTableItems2.length === 2
			) {
				fnAssert();
				clearInterval(iPoll);
			}
