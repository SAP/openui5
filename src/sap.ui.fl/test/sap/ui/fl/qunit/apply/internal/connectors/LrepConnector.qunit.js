...
sap.ui.define([
	...
	QUnit.module("Connector", {
		...
		QUnit.test("given no static changes-bundle.json placed for 'reference' resource roots and a mock server, when loading flex data is triggered and an empty response is returned", function (assert) {
			...
			return LrepConnector.loadFlexData({urls: {LrepConnector: "/sap/bc/lrep"}, reference: "reference", appVersion: "1.0.0"}).then(function (oServer, oResult) {
		...
		QUnit.test("given a mock server, when loading flex data is triggered with a cacheKey", function (assert) {
			...
			return LrepConnector.loadFlexData({urls: {LrepConnector: "/sap/bc/lrep"}, reference: "reference", appVersion: "1.0.0", cacheKey: sCacheKey}).then(function (oServer) {
		...
		QUnit.test("given a mock server, when loading flex data and returning the flag to also load modules", function (assert) {
			...
			return LrepConnector.loadFlexData({urls: {LrepConnector: "/sap/bc/lrep"}, reference: "reference", appVersion: "1.0.0", cacheKey: sCacheKey}).then(function (oServer, oResult) {
