...
sap.ui.define([
	...
	function(assert, Log, LanguageFallback, Localization, formatMessage, Properties, merge) {
		"use strict";
		...
		function ResourceBundle(sUrl, sLocale, bIncludeInfo, bAsync, aSupportedLocales, sFallbackLocale, bSkipFallbackLocaleAndRaw){
			...
			if (bAsync) {
				return loadNextPropertiesAsync(this).then(() => this);
			}
		...
		ResourceBundle.prototype._formatValue = function(sValue, sKey, aArgs){
			if (typeof sValue === "string") {
				...
				if (aArgs !== undefined && !Array.isArray(aArgs)){
					throw new Error("sap/base/i18n/ResourceBundle: value for parameter 'aArgs' is not of type array");
				}
		...
		ResourceBundle.create = function(mParams) {
			if (mParams?.async !== true) {
				throw new Error(
					"sap/base/i18n/ResourceBundle.create: As of version 1.135, synchronous loading is deprecated. The 'async' parameter must have the value 'true'"
				);
			}
