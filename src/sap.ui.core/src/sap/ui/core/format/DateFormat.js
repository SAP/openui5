...
sap.ui.define([
	...
	var oParseHelper = {
		...
		findEntry: function (sValue, aList, sLocale) {
			...
			aList.forEach(function (vEntry, j) {
				(Array.isArray(vEntry) ? vEntry : [vEntry]).forEach(function (sEntry) {
					if (sEntry.length > iMatchedLength && that.startsWithIgnoreCase(sValue, sEntry, sLocale)) {
						iFoundIndex = j;
	...
	DateFormat.prototype.oSymbols = {
		...
		"M": DateFormat._createPatternSymbol({
			...
			format: function(oField, oDate, bUTC, oFormat) {
				...
				if (oField.digits === 3) {
					var vName = oFormat.aMonthsAbbrev[iMonth]; // vName may be an array if there are alternatives
		...
		"L": DateFormat._createPatternSymbol({
			...
			format: function(oField, oDate, bUTC, oFormat) {
				var iMonth = oDate.getUTCMonth();
				if (oField.digits === 3) {
					var vName = oFormat.aMonthsAbbrevSt[iMonth]; // vName may be an array if there are alternatives
