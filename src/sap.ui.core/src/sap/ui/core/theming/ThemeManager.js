...
sap.ui.define([
	...
	ThemeManager.prototype.includeLibraryTheme = function(sLibName, sVariant, vQueryOrLibInfo) {
		...
		*/
		...
		if ((sLibName != "sap.ui.legacy") && (sLibName != "sap.ui.classic")) {
			...
			var bCssVariables = /^(true|x|additional)$/i.test(Configuration.getValue('xx-cssVariables'));
	...
	function updateThemeUrl(oLink, sThemeName, bSuppressFOUC) {
		...
		sHref = new URL(oThemeManager._getThemePath(sLibName, sThemeName) + sLibFileName + sQuery, document.baseURI).toString();
		if ( sHref != oLink.href ) {
