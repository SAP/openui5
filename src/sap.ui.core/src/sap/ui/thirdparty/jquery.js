			...
			if ( ( event.isTrigger & 1 ) && this[ type ] ) {
				...
				if ( !saved.length ) {
					...
					if ( saved !== result ) {
						...
						// During calling this[ type ]() (13 lines before) the target DOM element may
						// be replaced with new DOM element (due to a sync UI5 control rerendering).
						// Therefore the result variable is undefined because all data which is saved
						// on the DOM element is deleted.
						// A check whether result is undefined is needed before accessing its value
						// property
						//
						// There's already a bug reported opened in jQuery github. Remove this patch
						// once the bug is fixed by jQuery.
