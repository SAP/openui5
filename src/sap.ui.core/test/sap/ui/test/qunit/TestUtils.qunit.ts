...
[{
    ...
    QUnit.test("useFakeServer: " + sTitle + " (direct)", function (assert) {
        var mHeaders = oFixture.method === "MERGE" ? { "DataServiceVersion": "2.0" } : { "OData-Version": "4.0" };
        Object.keys(oFixture.requestHeaders || {}).forEach(function (sKey) {
            mHeaders[sKey] = oFixture.requestHeaders[sKey];
        });
        TestUtils.useFakeServer(this._oSandbox, "sap/ui/test/qunit/data", mServerFixture, aRegExpFixture);
        this.oLogMock.expects("info").withExactArgs(oFixture.expectedLogMessage || oFixture.method + " " + oFixture.url, "{\"If-Match\":undefined}", "sap.ui.test.TestUtils");
        TestUtils.resetRequestCount();
        return request(oFixture.method, oFixture.url, mHeaders, oFixture.requestBody).then(function (oXHR) {
            assert.strictEqual(oXHR.status, oFixture.status, "status");
            assert.strictEqual(oXHR.responseText, oFixture.responseBody || "", "body");
            assert.strictEqual(oXHR.getAllResponseHeaders(), headerString(oFixture.responseHeaders), "headers");
            assert.strictEqual(TestUtils.getRequestCount(), 1);
        });
    });
    QUnit.test("useFakeServer: " + sTitle + " (batch)", function (assert) {
        var mBatchHeaders = {}, mInitialHeaders = {}, sUrl = oFixture.url.replace("/Foo/", "");
        ...
        this.oLogMock.expects("info").withExactArgs(oFixture.expectedLogMessage || oFixture.method + " " + oFixture.url, "{\"If-Match\":undefined}", "sap.ui.test.TestUtils");
        TestUtils.resetRequestCount();
        return request("POST", "/Foo/$batch", mInitialHeaders, "--batch_id-0123456789012-345\r\n" + "Content-Type: application/http\r\n" + "Content-Transfer-Encoding: binary\r\n" + "\r\n" + oFixture.method + " " + sUrl + " HTTP/1.1\r\n" + headerString(oFixture.requestHeaders) + "\r\n" + (oFixture.requestBody || "") + "\r\n" + "--batch_id-0123456789012-345--\r\n" + "epilogue").then(function (oXHR) {
            assert.strictEqual(oXHR.status, 200, "status");
            assert.strictEqual(oXHR.responseText, "--batch_id-0123456789012-345\r\n" + "Content-Type: application/http\r\n" + "Content-Transfer-Encoding: binary\r\n" + "\r\n" + "HTTP/1.1 " + oFixture.status + " \r\n" + headerString(oFixture.responseHeaders) + "\r\n" + (oFixture.responseBody || "") + "\r\n" + "--batch_id-0123456789012-345--\r\n", "body");
            assert.strictEqual(oXHR.getAllResponseHeaders(), headerString(mBatchHeaders), "batch headers");
            assert.strictEqual(TestUtils.getRequestCount(), 1);
        });
    });
});
QUnit.test("useFakeServer: multiple RegExp matches", function (assert) {
    ...
    this.oLogMock.expects("warning").withExactArgs("Multiple matches found for GET /Foo/regexp/bar", undefined, "sap.ui.test.TestUtils");
...
QUnit.test("useFakeServer: HEAD /Foo/any (direct)", function (assert) {
    ...
    this.oLogMock.expects("info").withExactArgs("HEAD /Foo/any", "{\"If-Match\":undefined}", "sap.ui.test.TestUtils");
    return request("HEAD", "/Foo/any", { "OData-Version": "4.0" }).then(function (oXHR) {
        assert.strictEqual(oXHR.status, 200, "status");
        assert.strictEqual(oXHR.responseText, "", "body");
        assert.strictEqual(oXHR.getAllResponseHeaders(), headerString({ "OData-Version": "4.0" }), "headers");
    });
});
QUnit.test("useFakeServer: change set - success", function (assert) {
    TestUtils.useFakeServer(this._oSandbox, "sap/ui/test/qunit/data", mServerFixture);
    this.oLogMock.expects("info").withExactArgs("PATCH /Foo/any", "{\"If-Match\":undefined}", "sap.ui.test.TestUtils");
    this.oLogMock.expects("info").withExactArgs("PATCH /Foo/bar", "{\"If-Match\":undefined}", "sap.ui.test.TestUtils");
    this.oLogMock.expects("info").withExactArgs("GET /Foo/bar", "{\"If-Match\":undefined}", "sap.ui.test.TestUtils");
    TestUtils.resetRequestCount();
...
QUnit.test("useFakeServer: change set - failure", function (assert) {
    TestUtils.useFakeServer(this._oSandbox, "sap/ui/test/qunit/data", mServerFixture);
    this.oLogMock.expects("info").withExactArgs("POST /Foo/any", "{\"If-Match\":undefined}", "sap.ui.test.TestUtils");
    this.oLogMock.expects("info").withExactArgs("POST /Foo/baz, alternative (ifMatch) #0", "{\"If-Match\":undefined}", "sap.ui.test.TestUtils");
    this.oLogMock.expects("info").withExactArgs("GET /Foo/bar", "{\"If-Match\":undefined}", "sap.ui.test.TestUtils");
    TestUtils.resetRequestCount();
    return request("POST", "/Foo/$batch", { "OData-Version": "4.0" }, [
        "--batch_id-1538663822135-19",
...
[{
    ...
    var oOriginalResponseHeaders = jQuery.extend({}, oFixture.responseHeaders), mUrls = {
        "/Foo/bar": {
    ...
    QUnit.test("TestUtils: GET, " + i, function (assert) {
        ...
        return jQuery.ajax("/Foo/bar", {
            ...
            assert.strictEqual(jqXHR.getResponseHeader("OData-Version"), oFixture.expectedODataVersion);
            assert.strictEqual(jqXHR.getResponseHeader("DataServiceVersion"), oFixture.expectedDataServiceVersion);
    ...
    QUnit.test("TestUtils: $batch with GET, " + i, function (assert) {
        ...
        return jQuery.ajax("/$batch", {
            ...
            assert.strictEqual(jqXHR.getResponseHeader("OData-Version"), oFixture.requestHeaders["OData-Version"] || null);
            assert.strictEqual(jqXHR.getResponseHeader("DataServiceVersion"), oFixture.requestHeaders["DataServiceVersion"] || null);
            assert.deepEqual(oFixture.responseHeaders, oOriginalResponseHeaders);
            sResponseHeaders = (vData.split("\r\n\r\n"))[1];
            checkHeader(assert, sResponseHeaders, "OData-Version", oFixture.expectedODataVersion);
            checkHeader(assert, sResponseHeaders, "DataServiceVersion", oFixture.expectedDataServiceVersion);
        });
    });
});
["DELETE", "PATCH", "POST"].forEach(function (sMethod) {
    [{
        ...
        QUnit.test("TestUtils: " + sTitle, function (assert) {
            ...
            this.oLogMock.expects("info").withExactArgs(sMethod + " /Foo/bar", "{\"If-Match\":undefined}", "sap.ui.test.TestUtils");
            return jQuery.ajax("/Foo/bar", {
...
QUnit.test("useFakeServer: change set - failure - continue on error (V2)", function (assert) {
    ...
    this.oLogMock.expects("info").withExactArgs("POST /Foo/Any", "{\"If-Match\":undefined}", "sap.ui.test.TestUtils");
    this.oLogMock.expects("info").withExactArgs("GET /Foo/Any?$skip=0&$top=4", "{\"If-Match\":undefined}", "sap.ui.test.TestUtils");
    TestUtils.resetRequestCount();
