...
function handleGetUserRequests(oXhr, bCount) {
    var iCount, sCount = "", aExpand, sExpand, iIndex, sKey, oResponse, sResponseBody, aResult, aSelect, sSelect, aSubSelects;
    ...
    if (sKey) {
        ...
        if (iIndex > -1) {
            sResponseBody = "{\"@odata.context\": \"" + getBaseUrl(oXhr.url) + "$metadata#People(Age,FirstName,LastName,UserName)/$entity\"," + JSON.stringify(oResponse).slice(1);
        ...
        sResponseBody = "{\"@odata.context\": \"" + getBaseUrl(oXhr.url) + "$metadata#People(Age,FirstName,LastName,UserName)\"," + sCount + JSON.stringify(oResponse).slice(1);
        return getSuccessResponse(sResponseBody);
    }
}
