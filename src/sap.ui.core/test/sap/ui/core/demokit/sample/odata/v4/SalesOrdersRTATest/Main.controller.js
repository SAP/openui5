...
sap.ui.define([
		...
		'sap/ui/model/Sorter'
], function (Column, Dialog, Label, MessageBox, MessageToast, Text, DateFormat, Item, Controller,
		ValueState, Filter, FilterOperator, JSONModel, Sorter) {
	"use strict";
...
//	}
	...
	return Controller.extend("sap.ui.core.sample.odata.v4.SalesOrdersRTATest.Main", {
		...
		onAdaptSODetails : function () {
			this.adaptControl(this.byId("SODetails"));
		},
...
//TODO enable as soon as resume is implemented
//			oView.byId("NewBuyerID").getBinding("suggestionItems").resume();
		...
		onDataEvents : function (oEvent) {
			...
			jQuery.sap.log.info(oEvent.getId() + " event processed for path " + oSource.getPath(),
				oSource, "sap.ui.core.sample.odata.v4.SalesOrdersRTATest.Main.controller");
			...
			if (oEvent.getId() === "dataReceived") {
				if (oSource.getPath() === "/SalesOrderList") {
					...
					jQuery.sap.log.info("Current SalesOrderIDs: " + aSalesOrderIDs.join(", "),
						null, "sap.ui.core.sample.odata.v4.SalesOrdersRTATest.Main.controller");
