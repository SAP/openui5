            ...
            postponeDeliveryDateByOneDay: function () {
                this.waitFor({
                    ...
                    success: function (oControl) {
                        var oBinding = oControl.getBinding("value"), oDateTimeOffset = new DateTimeOffset(), sOldValue = oControl.getValue(), oDate = oDateTimeOffset.formatValue(oBinding.getValue(), "object"), sNewValue;
                        oDate.setDate(oDate.getDate() + 1);
                        oControl.setValue(oDateTimeOffset.formatValue(oDate, "string"));
                        sNewValue = oControl.getValue();
                        Opa5.assert.notStrictEqual(sNewValue, sOldValue, "Delivery Date postponed by one day from " + sOldValue + " to " + sNewValue);
                    },
                    viewName: sViewName
                });
            },
                ...
                this.waitFor({
                    ...
                    success: function (aControls) {
                        aControls.forEach(function (oControl, i) {
                            ...
                            sProperty = oControl.getValue ? oControl.getValue() : oControl.getText();
                            Opa5.assert.ok(sProperty, "Control: " + oControl.getId() + " has content: " + sProperty);
                        });
                    },
                    viewName: sViewName
                });
