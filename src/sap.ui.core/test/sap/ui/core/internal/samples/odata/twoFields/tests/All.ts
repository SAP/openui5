...
opaTest("1) Entry of amount (step 1) and currency (step 2) " + "in previously empty input fields (success case)", function (Given, When, Then) {
...
opaSkip("2) Entry of amount (step 1) and currency (step 2) " + "in previously empty input fields (error case)", function (Given, When, Then) {
    Given.iStartMyUIComponent(oComponentOptions);
    When.onMainPage.initializeCurrency();
    Then.onMainPage.checkUnit("");
    Then.onMainPage.checkValue("");
    ...
    Then.onMainPage.checkValue("100.1", "100.1", ValueState.Error, "Currency.WithoutDecimals");
    Given.iTeardownMyUIComponent();
});
opaSkip("3) Currency is already available, the user enters an invalid amount.", function (Given, When, Then) {
    Given.iStartMyUIComponent(oComponentOptions);
...
opaTest("4) Currency is already available, the user enters a valid amount.", function (Given, When, Then) {
    Given.iStartMyUIComponent(oComponentOptions);
    When.onMainPage.initializeCurrency(undefined, { content: "JPY" });
    Then.onMainPage.checkUnit("JPY");
    Then.onMainPage.checkValue("");
...
["editable", "enabled"].forEach(function (sProperty) {
    opaSkip("5) Currency is available, the field is not " + sProperty + ". The user enters an invalid amount.", function (Given, When, Then) {
...
["editable", "enabled"].forEach(function (sProperty) {
    opaSkip("6) Amount is in a not " + sProperty + " field, currency is changed so that the amount has too many decimal places", function (Given, When, Then) {
...
opaTest("7) Entry of amount (step 1) and new currency (step 2) with " + "previously available currency (success case)", function (Given, When, Then) {
    Given.iStartMyUIComponent(oComponentOptions);
...
opaSkip("8) Entry of amount (step 1) and new currency (step 2) with " + "previously available currency (error case)", function (Given, When, Then) {
    Given.iStartMyUIComponent(oComponentOptions);
    When.onMainPage.initializeCurrency(undefined, { content: "EUR" });
    Then.onMainPage.checkUnit("EUR");
    Then.onMainPage.checkValue("");
...
opaTest("9) The backend sends an invalid quantity: No error is shown", function (Given, When, Then) {
    Given.iStartMyUIComponent(oComponentOptions);
...
opaTest("10) Entry of an invalid unit", function (Given, When, Then) {
    Given.iStartMyUIComponent(oComponentOptions);
...
opaSkip("11) Invalid currency amount can be fixed by changing to a valid matching currency", function (Given, When, Then) {
    Given.iStartMyUIComponent(oComponentOptions);
...
opaTest("12) Invalid value at one part only shown at that part", function (Given, When, Then) {
    Given.iStartMyUIComponent(oComponentOptions);
    When.onMainPage.initializeCurrency(undefined, { content: "EUR" });
    Then.onMainPage.checkUnit("EUR");
    Then.onMainPage.checkValue("");
...
opaSkip("13) Changing a non parsable value to a value that leads to a ValidateException on both" + " parts leads to both parts highlighted", function (Given, When, Then) {
    Given.iStartMyUIComponent(oComponentOptions);
...
opaSkip("14) Changing to a different valid value after entering a non-parsable unit must keep" + " the illegal unit. Correction to the unit writes both value and unit to the model." + " The same applies when changing roles of value and unit.", function (Given, When, Then) {
    Given.iStartMyUIComponent(oComponentOptions);
