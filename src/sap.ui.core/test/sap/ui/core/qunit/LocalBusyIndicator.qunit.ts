import LocalBusyIndicatorSupport from "sap/ui/core/LocalBusyIndicatorSupport";
...
import CommonsSlider from "sap/ui/commons/Slider";
...
QUnit.test("Ensuring DelayedCall Only Used Once", function (assert) {
    var done = assert.async();
    assert.expect(2);
    var iFirstDelayedCallId, iSecondDelayedCallId;
...
QUnit.module("setBusy with rendering delegate", {
    beforeEach: function () {
        ...
        this.testClickEventOn = function (oControl, assert) {
            var oDomRef = oControl.getDomRef(), fnEventListener = sinon.spy();
            if (oDomRef) {
...
QUnit.test("LocalBusyIndicatorSupport", function (assert) {
    ...
    assert.equal(Control.prototype.setDelay, Control.prototype.setBusyIndicatorDelay, "Control#setDelay should be available and a reference to #setBusyIndicatorDelay after applying legacy support");
