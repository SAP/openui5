...
QUnit.test("constructor - wrong args", function (assert) {
    ...
    }, new Error(sLegacyAnyAll), "'Any' is not accepted with legacy syntax");
    assert.throws(function () {
        return new Filter("test", FilterOperator.All, "notSupported", new Filter());
    }, new Error(sLegacyAnyAll), "'All' is not accepted with legacy syntax");
    assert.throws(function () {
        return new Filter("test", FilterOperator.Any);
    }, new Error(sLegacyAnyAll), "'Any' is not accepted with legacy syntax and missing 3rd and 4th argument");
    assert.throws(function () {
        return new Filter({
            path: "foo",
            operator: FilterOperator.Any,
    ...
    }, new Error(sOneMissing), "Error thrown if condition is Missing in 'Any'.");
    assert.throws(function () {
        return new Filter({
            path: "foo",
    ...
    }, new Error(sConditionError), "Error thrown if condition is Missing in 'All'.");
    assert.throws(function () {
        return new Filter({
            path: "foo",
            operator: FilterOperator.All
        });
    }, new Error(sVariableError), "Error thrown if no args are given with 'All' operator");
});
