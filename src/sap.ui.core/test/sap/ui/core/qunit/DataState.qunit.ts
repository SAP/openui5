...
QUnit.test("constructor", function (assert) {
    var oDataState, oDataStateMock = this.mock(DataState);
...
[
    ...
    QUnit.test("getMessagesForProperties, " + i, function (assert) {
        var aConcatCalls, oConcatSpy = this.spy(Array.prototype, "concat"), aControlMessages = oFixture.controlMessages, aMessages, aModelMessages = oFixture.modelMessages, mProperties = {
        ...
        aConcatCalls = oConcatSpy.getCalls().filter(function (oCall) {
            return oCall.calledWithExactly(aModelMessages || [], aControlMessages || []) && deepEqual(oCall.thisValue, []);
        });
...
QUnit.test("getMessagesForProperties, no messages", function (assert) {
    var oConcatSpy = this.spy(Array.prototype, "concat"), mProperties = {
...
QUnit.test("getMessages", function (assert) {
    ...
    this.mock(DataState).expects("getMessagesForProperties").withExactArgs("mChangedProperties").returns("aMessages");
...
QUnit.test("_getOldMessages", function (assert) {
    ...
    this.mock(DataState).expects("getMessagesForProperties").withExactArgs("mProperties").returns("aMessages");
