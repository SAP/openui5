...
import jQuery from "jquery.sap.xml";
var ViewType = coreLibrary.mvc.ViewType;
var sRootView = "<mvc:View height=\"100%\" xmlns:mvc=\"sap.ui.core.mvc\" xmlns=\"sap.m\" id=\"root\">" + "</mvc:View>";
var sView = "<mvc:View height=\"100%\" xmlns:mvc=\"sap.ui.core.mvc\" xmlns:core=\"sap.ui.core\" xmlns=\"sap.m\" id=\"view\" " + "xmlns:dt=\"sap.ui.dt\" displayBlock=\"true\" unknownProperty=\"true\">" + "<Panel id=\"panel\">" + "<content>" + "<Button text=\"Button\" id=\"button\"></Button>" + "<Button text=\"Button With Designtime Data\" id=\"buttonWithDTData\" dt:test=\"testvalue\"></Button>" + "<Button text=\"Button using core:require\" id=\"buttonRequire\" core:require=\"{Link:'sap/m/Link'}\"></Button>" + "<Button text=\"Button using Designtime Data and core:require\" id=\"buttonWithDTDataAndRequire\" dt:test=\"testvalue2\" core:require=\"{Link:'sap/m/Link'}\"></Button>" + "<Button text=\"StashedButton\" id=\"stashedButton\" stashed=\"true\"></Button>" + "<Button text=\"Wrong Type value\" id=\"brokenButton\" type=\"somethingInvalid\"></Button>" + "<core:ExtensionPoint name=\"extension\">" + "<Button text=\"ExtensionButton\" id=\"extensionButton\"></Button>" + "</core:ExtensionPoint>" + "</content>" + "</Panel>" + "</mvc:View>";
...
QUnit.test("do not collect known namespaces as custom settings", function (assert) {
    ...
    return this.oView.loaded().then(function () {
        XMLTemplateProcessor.enrichTemplateIds(this.xml.documentElement, this.oView);
        this.xml = jQuery.sap.parseXML(oXMLSerializer.serializeToString(this.xml.documentElement));
        XMLTemplateProcessor.parseTemplate(this.xml.documentElement, this.oView);
        assert.equal(this.oView.data("sap-ui-custom-settings"), null, "no custom setting should have been collected (view)");
        assert.equal(this.oView.byId("panel").data("sap-ui-custom-settings"), null, "no custom setting should have been collected (panel)");
        assert.equal(this.oView.byId("button").data("sap-ui-custom-settings"), null, "no custom setting should have been collected (button)");
        assert.equal(this.oView.byId("buttonRequire").data("sap-ui-custom-settings"), null, "no custom setting should have been collected (button with core:require)");
        assert.deepEqual(this.oView.byId("buttonWithDTDataAndRequire").data("sap-ui-custom-settings"), {
            "sap.ui.dt": {
                "test": "testvalue2"
            }
...
QUnit.test("Adding and cloning of sap-ui-custom-settings from xml namespaced attributes", function (assert) {
    return this.oView.loaded().then(function () {
        var oButton = this.oView.byId("buttonWithDTData"), mCustomSettings = oButton.data("sap-ui-custom-settings");
        ...
        var oClone = oButton.clone(), mCustomSettingsClone = oClone.data("sap-ui-custom-settings");
