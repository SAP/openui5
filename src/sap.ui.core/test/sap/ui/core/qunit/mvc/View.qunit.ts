...
QUnit.test("runPreprocessor w/ valid preprocessor", function (assert) {
    assert.expect(1);
    var oSource = {}, bCalled = false, oView = new View({
        preprocessors: {
            xml: [{
                    preprocessor: {
...
QUnit.test("runPreprocessor w/ config", function (assert) {
    ...
    }, oResult = { foo: true }, oSource = { bar: true }, oViewInfo;
    return new View({
        preprocessors: oPreprocessors,
        viewName: "foo",
        async: true
...
QUnit.test("runPreprocessor w/ config and settings", function (assert) {
    ...
    oView.loaded().then(function (oView) {
        var oSpy;
        oPreprocessors.foo.preprocessor.then(function (oPreprocessorImpl) {
...
QUnit.test("register global preprocessor", function (assert) {
    assert.expect(2);
    assert.deepEqual(View._mPreprocessors, {
        "XML": {
            "xml": [
                {
                    "_onDemand": true,
    ...
    assert.deepEqual(View._mPreprocessors["test"]["controls"], [{ _onDemand: false, preprocessor: this.oPreprocessor, _syncSupport: true, _settings: this.mSettings }], "preprocessor stored at view");
});
...
QUnit.test("sap.ui.core.mvc.View#getPreprocessorInfo", function (assert) {
    var oView = new View("dummy", {}), oPreprocessorInfo = {
...
QUnit.test("Retrieve promise for view generally", function (assert) {
    assert.expect(2);
    var done = assert.async();
    var oView = new View({}), oPromise = oView.loaded();
