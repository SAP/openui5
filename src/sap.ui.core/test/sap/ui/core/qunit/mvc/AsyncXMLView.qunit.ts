import Cache from "sap/ui/core/cache/CacheManager";
import Component from "sap/ui/core/Component";
import View from "sap/ui/core/mvc/View";
import XMLView from "sap/ui/core/mvc/XMLView";
import TestPreprocessor from "./testdata/TestPreprocessor";
import asyncTestsuite from "./AnyViewAsync.qunit";
import Log from "sap/base/Log";
import LoaderExtensions from "sap/base/util/LoaderExtensions";
import jQuery from "jquery.sap.script";
var oConfig = {
    type: "XML",
...
QUnit.test("Error in template - no default aggregation defined", function (assert) {
    var sXml = [
        ...
        "\t</m:Button>",
        "</mvc:View>"
    ].join(""), sError = "Cannot add direct child without default aggregation defined for control sap.m.Button";
...
if (Cache._isSupportedEnvironment()) {
    Cache.reset().then(function () {
        var sLocation = window.location.host + window.location.pathname, sBuildTimeStamp = "12345";
        ...
        function getKeyParts(aKeys, sManifest, aUsedTerminologies) {
            ...
            var sLanguageTag = sap.ui.getCore().getConfiguration().getLanguageTag(), sHashCode = jQuery.sap.hashCode(sManifest || "");
        ...
        QUnit.test("no cache key - async part", function (assert) {
            var error = new Error("Provided cache keys may not be empty or undefined."), oSpy = this.oSpy, oLogSpy = sinon.spy(Log, "error");
            assert.expect(3);
        ...
        QUnit.test("cache key error", function (assert) {
            var error = new Error("Some Error"), oView = viewFactory({ keys: [Promise.reject(error)] });
            assert.expect(1);
        ...
        QUnit.test("Preprocessor integration", function (assert) {
            var sKey = "key", oSpy = this.oSpy, oViewInfo = {
    ...
    QUnit.test("should not call the cache", function (assert) {
        var sKey = "key", oGetSpy = sinon.spy(Cache, "get"), oSetSpy = sinon.spy(Cache, "set");
        assert.expect(2);
        return viewFactory({
            keys: [sKey]
        }).loaded().then(function (oView1) {
