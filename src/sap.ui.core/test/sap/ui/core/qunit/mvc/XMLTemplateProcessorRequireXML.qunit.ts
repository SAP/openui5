...
QUnit.test("Throw error if core:require is defined with non-object", function (assert) {
    var sView = "<mvc:View xmlns=\"sap.m\" xmlns:mvc=\"sap.ui.core.mvc\" xmlns:core=\"sap.ui.core\">" + "<Panel id=\"panel\" core:require=\"true\">" + "</Panel>" + "</mvc:View>";
...
QUnit.test("Throw error if core:require contains invalid identifier", function (assert) {
    var aViews = [
        "<mvc:View xmlns=\"sap.m\" xmlns:mvc=\"sap.ui.core.mvc\" xmlns:core=\"sap.ui.core\">" + "<Panel id=\"panel\" core:require=\"{'nested.name': 'some/nested/path'}\">" + "</Panel>" + "</mvc:View>",
        "<mvc:View xmlns=\"sap.m\" xmlns:mvc=\"sap.ui.core.mvc\" xmlns:core=\"sap.ui.core\">" + "<Panel id=\"panel\" core:require=\"{'nested/name': 'some/nested/path'}\">" + "</Panel>" + "</mvc:View>",
        "<mvc:View xmlns=\"sap.m\" xmlns:mvc=\"sap.ui.core.mvc\" xmlns:core=\"sap.ui.core\">" + "<Panel id=\"panel\" core:require=\"{'nested;name': 'some/nested/path'}\">" + "</Panel>" + "</mvc:View>"
    ];
...
QUnit.test("Throw error if core:require contains non-string value", function (assert) {
    var sView = "<mvc:View xmlns=\"sap.m\" xmlns:mvc=\"sap.ui.core.mvc\" xmlns:core=\"sap.ui.core\">" + "<Panel id=\"panel\" core:require=\"{'module': {path: 'some/good/path'}}\">" + "</Panel>" + "</mvc:View>";
    return XMLView.create({
        definition: sView
    }).catch(function (oError) {
...
[{
        ...
        runAssertions: function (oView, mSpies, assert, bAsync) {
            ...
            assert.strictEqual(oFunctionControl.getHandler().toString(), Helper.groupA.lowerCase.bind(Helper.groupA).toString(), "The function property is resolved correctly");
    ...
    QUnit.test(oConfig.testDescription + " - async", function (assert) {
        var that = this, bAsync = true, mSpies;
        if (oConfig.settings.async.spies) {
            mSpies = createSpies(oConfig.settings.async.spies, this);
        }
        return oConfig.settings.async.create(oConfig.viewName, oConfig.settings.async.additionalViewSettings).then(function (oView) {
            return oConfig.runAssertions.call(that, oView, mSpies, assert, bAsync);
        });
    });
    QUnit.test(oConfig.testDescription + " - sync", function (assert) {
        var that = this, bAsync = false, mSpies;
        if (oConfig.settings.sync.spies) {
            mSpies = createSpies(oConfig.settings.sync.spies, this);
        }
        return oConfig.settings.sync.create(oConfig.viewName, oConfig.settings.sync.additionalViewSettings).then(function (oView) {
            return oConfig.runAssertions.call(that, oView, mSpies, assert, bAsync);
        });
    });
});
QUnit.test("core:require no async require for loaded modules", function (assert) {
    ...
    var oRequireSpy = this.spy(sap.ui, "require"), sXML = "<mvc:View xmlns=\"sap.m\" xmlns:mvc=\"sap.ui.core.mvc\" xmlns:core=\"sap.ui.core\">" + "<Panel id=\"panel\" core:require=\"{" + "'mod1': 'sap/ui/model/FilterOperator'," + "'mod2': 'test/never-seen-before'," + "'mod3': 'sap/ui/core/Core'" + "}\" />" + "</mvc:View>";
    return XMLView.create({
        definition: sXML
