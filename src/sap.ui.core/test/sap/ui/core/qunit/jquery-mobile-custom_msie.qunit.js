...
sap.ui.define(["sap/ui/Device", "sap/base/util/deepClone"], function (Device, deepClone) {
    ...
    QUnit.module("Test Touch Support for MS Internet Explorer");

    QUnit.test("Should detect the touch capibilities correctly on touch devices", function (assert) {
        // Preparation
        var done = assert.async();

        // MS Internet Explorer fires no 'touchend' event
        delete document.ontouchend;

        // Stub browser to msie, deepClone is necessary because Core.boot needs all browser values registered correctly
        ...
        sap.ui.require(["sap/ui/core/Core"], function (core) {
            sap.ui.getCore().attachInit(function () {
                ...
                assert.notOk(jQuery.mobile.support.touch, "The 'ontouchend' event is not available on MS Internet Explorer");
