...
import qUnitTestHarness from "sap/ui/test/gherkin/qUnitTestHarness";
import opa5TestHarness from "sap/ui/test/gherkin/opa5TestHarness";
import simpleGherkinParser from "sap/ui/test/gherkin/simpleGherkinParser";
import StepDefinitions from "sap/ui/test/gherkin/StepDefinitions";
import opaTest from "sap/ui/test/opaQunit";
import Opa5 from "sap/ui/test/Opa5";
import require from "require";
export class testHarnessTests {
    static beforeEach(fnMockSetup: any, fnMockTeardown: any, oTestHarness: any) {
        ...
        opaTest("Deep testing of duplicate step definition behaviour", function () {
            ...
            oOpa5.waitFor({
                ...
                success: function () {
                    ...
                    var oDupe = oFrame$(".test-message").filter(":contains(\"StepDefinitions.register: Duplicate step definition '/^duplicate regex$/i'\")");
        ...
        opaTest("Deep testing of ambiguous step definition behaviour", function () {
            ...
            oOpa5.waitFor({
                ...
                success: function () {
                    ...
                    Opa5.assert.strictEqual(iFailedTests, 1, "Verified failed tests");
                    Opa5.assert.strictEqual(iTotalTests, 1, "Verified total tests");
                    var oAmbiguous = oFrame$(".test-message").filter(":contains(\"Ambiguous step definition error: 2 step definitions '/^I should be served a coffee$/i' and '/^I should be served a .*$/i' match the feature file step 'I should be served a coffee'\")");
        ...
        opaTest("Deep testing of test harness behaviour", function () {
            ...
            oOpa5.waitFor({
                ...
                success: function () {
                    ...
                    var oWip = oFrame$(".test-name").filter(":contains(\"(WIP) Scenario: Don't fail a test with no assertions, if it's @wip\")");
                    ...
                    Opa5.assert.strictEqual(oActualError.text(), "Expected at least one assertion, but none were run - call expect(0) to accept zero assertions.", "Verified that scenario with no assertions triggers expect(0) error message");
                    ...
                    var oWipFeature = oFrame$(".module-name").filter(":contains(\"(WIP) Feature: An @wip feature automatically sets all its scenarios as @wip too\")");
                    Opa5.assert.strictEqual(oWipFeature.length, 1, "Verified that @wip feature does not run its scenarios\"");
                    var sNotFoundText = oFrame$(".test-message").filter(":contains(\"this test step does not exist and should fail the build\")").parent().parent().find(".test-message").last().text();
                    Opa5.assert.ok(sNotFoundText.indexOf("expect(0)") === -1, "Verified that a (NOT FOUND) test step doesn't trigger an expect(0) error\"");
                    fnScenarioOutlineVerification(oFrame$);
                    var oNotFoundNotSkipped = oFrame$(".test-message").filter(":contains(\"(NOT FOUND) this step has no step definition defined\")");
                    Opa5.assert.strictEqual(oNotFoundNotSkipped.length, 1, "Verified found oNotFoundNotSkipped text");
                    var oAtWipSkipped = oFrame$(".module-name").filter(":contains(\"Feature: a feature whose scenarios are all @wip will be skipped\")");
