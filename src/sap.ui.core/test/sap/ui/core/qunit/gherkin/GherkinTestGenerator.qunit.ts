...
QUnit.test("@wip tag on feature prevents whole feature from being run", function (assert) {
    ...
    assert.strictEqual(featureTest.testScenarios[0].name, "(WIP) Scenario Outline: Coffee changes peoples moods if you are ready", "@wip tag on feature 05");
    ...
    assert.strictEqual(featureTest.testScenarios[1].name, "(WIP) Scenario: Buy expensive coffee once barista is ready", "@wip tag on feature 08");
...
QUnit.test("@wip tag on scenario prevents only that scenario from running", function (assert) {
    ...
    assert.strictEqual(featureTest.testScenarios[0].name, "Scenario: Buy expensive coffee when the barista is alive", "@wip tag on scenario 04");
    ...
    assert.strictEqual(featureTest.testScenarios[0].testSteps[0].text, "that quantum phenomena exist at the macroscopic level", "@wip tag on scenario 07");
    ...
    assert.strictEqual(featureTest.testScenarios[1].name, "(WIP) Scenario: Buy expensive coffee when the barista is dead", "@wip tag on scenario 08");
...
QUnit.test("Parameters and tables are parsed and passed to steps in test suite", function (assert) {
    ...
    assert.strictEqual(featureTest.testScenarios[0].testSteps[0].text, "these awesome Users:", "Parameters and tables are parsed and passed 03");
    assert.strictEqual(featureTest.testScenarios[0].testSteps[0].parameters.length, 2, "Parameters and tables are parsed and passed 04");
    assert.strictEqual(featureTest.testScenarios[0].testSteps[0].parameters[0], "awesome", "Parameters and tables are parsed and passed 05");
    ...
    assert.strictEqual(featureTest.testScenarios[0].testSteps[1].text, "when I give them the coffee price list:", "Parameters and tables are parsed and passed 06");
    assert.strictEqual(featureTest.testScenarios[0].testSteps[1].parameters.length, 1, "Parameters and tables are parsed and passed 07");
    ...
    assert.strictEqual(featureTest.testScenarios[0].testSteps[2].text, "they should be happy", "Parameters and tables are parsed and passed 08");
    assert.strictEqual(featureTest.testScenarios[0].testSteps[2].parameters.length, 2, "Parameters and tables are parsed and passed 09");
...
QUnit.test("Given missing step definition then that step and all remaining steps are skipped", function (assert) {
    ...
    assert.strictEqual(featureTest.testScenarios[0].testSteps[2].text, "(NOT FOUND) I encounter a person whose feet are very dirty", "Test 009");
    ...
    assert.strictEqual(featureTest.testScenarios[0].testSteps[3].text, "(SKIPPED) they are classified as a 'Category 3 Unwashed Mass'", "Test 012");
...
QUnit.test("Given feature with some skipped scenarios, calling 'tearDown' must execute 'closeApplication' only for " + "non-skipped scenarios", function (assert) {
    assert.expect(4);
...
QUnit.test("Test alternate test step generator", function (assert) {
    ...
    assert.strictEqual(featureTest.testScenarios[0].testSteps[2].text, "(NOT FOUND) the generator will not match this one", "Test 041");
...
QUnit.test("Test invalid input to constructor", function (assert) {
    ...
    assert.throws(function () {
        return new GherkinTestGenerator(this.parser.parse("Feature: Serve coffee"), StepDefinitions, "not a function");
    }, function (error) {
