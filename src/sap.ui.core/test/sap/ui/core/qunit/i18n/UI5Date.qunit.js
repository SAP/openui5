...
[{ // America/Adak: UTC-10 (Standard), UTC-9 (DST); 02:00 am to 03:00 am resp. 02:00 am to 01:00 am
	...
	var sTimezone = oFixture.sTimezone;

	QUnit.test("Daylight saving time checks: " + sTimezone, function (assert) {
		var sAandBTime, iATimestamp, iBTimestamp, iTimeshift,
			oDate = new UI5Date(oFixture.toDST.aDate, sTimezone);
		...
		sAandBTime = oFixture.toStandard.sAandBTime;
		iTimeshift = oFixture.toStandard.iTimeshift || 3600000;
		iBTimestamp = oDate.getTime() - iTimeshift;
		// b-time in standard time zone
		assert.strictEqual(toLocalDateString(new UI5Date([iBTimestamp], sTimezone)), sAandBTime, "b-time");
		iATimestamp = oDate.getTime() - (2 * iTimeshift);
