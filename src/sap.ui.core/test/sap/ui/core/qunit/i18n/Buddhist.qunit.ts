import Buddhist from "sap/ui/core/date/Buddhist";
var aTestData = [
...
QUnit.test("with no arguments", function (assert) {
    ...
    var oBuddhistDate = new Buddhist();
    var now = new Date();
    verifyDate(assert, "Constructor with no parameters must always return the Buddhist date corresponding to the current " + "Gregorian one.", oBuddhistDate, 2513, 0, 1, now.getDay(), now.getHours(), now.getMinutes(), now.getSeconds(), now.getMilliseconds());
    clock.restore();
});
QUnit.test("with value parameter (timestamp)", function (assert) {
    ...
    var now = new Date(0);
    verifyDate(assert, "Constructor with value(timestamp)=0 must represents BuddhistDate corresponding to the date of 1st January 1970 Gregorian/(1389/10/22 Buddhist)", oBuddhistDate, 2513, 0, 1, now.getDay(), now.getHours(), now.getMinutes(), now.getSeconds(), now.getMilliseconds());
    var iOneDay = 24 * 60 * 60 * 1000;
    oBuddhistDate = new Buddhist(iOneDay);
    var oGregorianDate = new Date(iOneDay);
    verifyDate(assert, "Constructor with value(timestamp)= 'one day after 01.01.1970' must represents BuddhistDate corresponding to the date of 2nd January 1970 Gregorian/(1389/10/23 Buddhist)", oBuddhistDate, 2513, 0, 2, oGregorianDate.getDay(), oGregorianDate.getHours(), oGregorianDate.getMinutes(), oGregorianDate.getSeconds(), oGregorianDate.getMilliseconds());
    oGregorianDate = new Date(-iOneDay);
    oBuddhistDate = new Buddhist(-iOneDay);
    verifyDate(assert, "Constructor with value(timestamp)= 'one day before 01.01.1970' must represents BuddhistDate corresponding to the date of 31st December 1970 Gregorian/(1389/10/21 Buddhist)", oBuddhistDate, 2512, 11, 31, oGregorianDate.getDay(), oGregorianDate.getHours(), oGregorianDate.getMinutes(), oGregorianDate.getSeconds(), oGregorianDate.getMilliseconds());
});
...
QUnit.test("Overflow/underflow of date values", function (assert) {
    var oTestDate = aTestData[0], oBuddhistDate;
    oBuddhistDate = createBuddhistDateFromTestEntry(oTestDate);
    oBuddhistDate.setMonth(12);
...
QUnit.test("Set/Get Full Year", function (assert) {
    var oTestDate = aTestData[0], oBuddhistDate = createBuddhistDateFromTestEntry(oTestDate);
...
function verifyDate(assert, sMessage, oDate, year, month, day, bUTC) {
    ...
    var sReal = formatDate(String(bUTC ? oDate.getUTCFullYear() : oDate.getFullYear()), String(bUTC ? (oDate.getUTCMonth() + 1) : (oDate.getMonth() + 1)), String(bUTC ? oDate.getUTCDate() : oDate.getDate()));
