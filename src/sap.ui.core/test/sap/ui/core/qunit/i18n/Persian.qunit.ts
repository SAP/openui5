import Persian from "sap/ui/core/date/Persian";
var aTestData = [
...
QUnit.test("with no arguments", function (assert) {
    ...
    var oPersianDate = new Persian();
    var now = new Date();
    verifyDate(assert, "Constructor with no parameters must always return the Persian date corresponding to the current " + "Gregorian one.", oPersianDate, 1348, 9, 11, now.getDay(), now.getHours(), now.getMinutes(), now.getSeconds(), now.getMilliseconds());
    clock.restore();
});
QUnit.test("with value parameter (timestamp)", function (assert) {
    ...
    var now = new Date(0);
    verifyDate(assert, "Constructor with value(timestamp)=0 must represents PersianDate corresponding to the date of 1st January 1970 Gregorian/(11.10.1348 Persian)", oPersianDate, 1348, 9, 11, now.getDay(), now.getHours(), now.getMinutes(), now.getSeconds(), now.getMilliseconds());
    var iOneDay = 24 * 60 * 60 * 1000;
    oPersianDate = new Persian(iOneDay);
    var oGregorianDate = new Date(iOneDay);
    verifyDate(assert, "Constructor with value(timestamp)= 'one day after 01.01.1970' must represents PersianDate corresponding to the date of 2nd January 1970 Gregorian/(10.10.1348 Persian)", oPersianDate, 1348, 9, 12, oGregorianDate.getDay(), oGregorianDate.getHours(), oGregorianDate.getMinutes(), oGregorianDate.getSeconds(), oGregorianDate.getMilliseconds());
    oGregorianDate = new Date(-iOneDay);
    oPersianDate = new Persian(-iOneDay);
    verifyDate(assert, "Constructor with value(timestamp)= 'one day before 01.01.1970' must represents PersianDate corresponding to the date of 31st December 1969 Gregorian/(12.10.1348 Persian)", oPersianDate, 1348, 9, 10, oGregorianDate.getDay(), oGregorianDate.getHours(), oGregorianDate.getMinutes(), oGregorianDate.getSeconds(), oGregorianDate.getMilliseconds());
});
...
QUnit.test("Overflow/underflow of month", function (assert) {
    var oTestDate = aTestData[1], oPersianDate;
    oPersianDate = createPersianDateFromTestEntry(oTestDate);
    oPersianDate.setMonth(12);
...
QUnit.test("Set/Get Full Year", function (assert) {
    var oTestDate = aTestData[1], oPersianDate = createPersianDateFromTestEntry(oTestDate);
...
QUnit.test(".Now()", function (assert) {
    ...
    clock.restore();
    clock = sinon.useFakeTimers(7000);
...
function verifyDate(assert, sMessage, oDate, year, month, day, bUTC) {
    ...
    var sReal = formatDateTime(String(bUTC ? oDate.getUTCFullYear() : oDate.getFullYear()), String(bUTC ? (oDate.getUTCMonth() + 1) : (oDate.getMonth() + 1)), String(bUTC ? oDate.getUTCDate() : oDate.getDate()));
