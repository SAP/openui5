...
QUnit.test("with no arguments", function (assert) {
    ...
    var oJapaneseDate = new Japanese();
    var now = new Date();
    verifyDate(assert, "Constructor with no parameters must always return the Japanese date corresponding to the current " + "Gregorian one.", oJapaneseDate, 234, 45, 0, 1, now.getDay(), now.getHours(), now.getMinutes(), now.getSeconds(), now.getMilliseconds());
    clock.restore();
});
QUnit.test("with value parameter (timestamp)", function (assert) {
    ...
    oJapaneseDate = new Japanese(0);
    var now = new Date(0);
    verifyDate(assert, "Constructor with value(timestamp)=0 must represents JapaneseDate corresponding to the date of 1st January 1970 Gregorian/(1389/10/22 Japanese)", oJapaneseDate, 234, 45, 0, 1, now.getDay(), now.getHours(), now.getMinutes(), now.getSeconds(), now.getMilliseconds());
    var iOneDay = 24 * 60 * 60 * 1000;
    oJapaneseDate = new Japanese(iOneDay);
    var oGregorianDate = new Date(iOneDay);
    verifyDate(assert, "Constructor with value(timestamp)= 'one day after 01.01.1970' must represents JapaneseDate corresponding to the date of 2nd January 1970 Gregorian/(1389/10/23 Japanese)", oJapaneseDate, 234, 45, 0, 2, oGregorianDate.getDay(), oGregorianDate.getHours(), oGregorianDate.getMinutes(), oGregorianDate.getSeconds(), oGregorianDate.getMilliseconds());
    oGregorianDate = new Date(-iOneDay);
    oJapaneseDate = new Japanese(-iOneDay);
    verifyDate(assert, "Constructor with value(timestamp)= 'one day before 01.01.1970' must represents JapaneseDate corresponding to the date of 31st December 1970 Gregorian/(1389/10/21 Japanese)", oJapaneseDate, 234, 44, 11, 31, oGregorianDate.getDay(), oGregorianDate.getHours(), oGregorianDate.getMinutes(), oGregorianDate.getSeconds(), oGregorianDate.getMilliseconds());
});
...
QUnit.test("with year without era, when year is in current era and less than 100", function (assert) {
    var oJapaneseDate = null, iCurrentEra = UniversalDate.getCurrentEra(CalendarType.Japanese), oJapanese;
    aTestData.forEach(function (oTestDate) {
...
QUnit.test("Overflow/underflow of date values", function (assert) {
    var oTestDate = aTestData[1], oJapaneseDate;
    oJapaneseDate = createJapaneseDateFromTestEntry(oTestDate);
    oJapaneseDate.setMonth(12);
...
QUnit.test("Set/Get Era", function (assert) {
    var oTestDate = aTestData[0], oJapaneseDate = createJapaneseDateFromTestEntry(oTestDate);
...
function verifyDate(assert, sMessage, oDate, era, year, month, day, bUTC) {
    ...
    var sReal = formatDate(String(bUTC ? oDate.getUTCEra() : oDate.getEra()), String(bUTC ? oDate.getUTCFullYear() : oDate.getFullYear()), String(bUTC ? (oDate.getUTCMonth() + 1) : (oDate.getMonth() + 1)), String(bUTC ? oDate.getUTCDate() : oDate.getDate()));
    assert.equal(sReal, sExpected, sMessage);
}
function formatDate(era, year, month, day) {
    return String(era).padStart(3, "0") + "/" + String(year).padStart(2, "0") + "/" + String(month).padStart(2, "0") + "/" + String(day).padStart(2, "0");
}
...
function verifyJSDate(assert, sMessage, oDate, year, month, day, bUTC) {
    ...
    var sReal = formatJSDate(String(bUTC ? oDate.getUTCFullYear() : oDate.getFullYear()), String(bUTC ? (oDate.getUTCMonth() + 1) : (oDate.getMonth() + 1)), String(bUTC ? oDate.getUTCDate() : oDate.getDate()));
    assert.equal(sReal, sExpected, sMessage);
}
