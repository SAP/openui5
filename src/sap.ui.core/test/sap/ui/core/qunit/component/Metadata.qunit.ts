...
function defineGenericTests() {
    ...
    QUnit.test("ResourceRoots", function (assert) {
        if (this.iExpectedMetadataVersion === 1) {
            ...
            assert.ok(new URI(getModulePath(this.oMetadata.getComponentName(), "/anypath")).equals(new URI(getModulePath("x.y.z"))), "ResourceRoot 'x.y.z' registered (" + getModulePath("x.y.z") + ")");
            assert.ok(new URI(getModulePath(this.oMetadata.getComponentName(), "/../../foo/bar")).equals(new URI(getModulePath("foo.bar"))), "ResourceRoot 'foo.bar' registered (" + getModulePath("foo.bar") + ")");
            assert.ok(!new URI("http://absolute/uri").equals(new URI(getModulePath("absolute"))), "ResourceRoot 'absolute' not registered (" + getModulePath("absolute") + ")");
            assert.ok(!new URI("/server/absolute/uri").equals(new URI(getModulePath("server.absolute"))), "ResourceRoot 'server.absolute' not registered (" + getModulePath("server.absolute") + ")");
        }
    });
...
QUnit.test("ResourceRoots", function (assert) {
    assert.ok(new URI(getModulePath(this.oMetadata.getParent().getComponentName(), "/anypath")).equals(new URI(getModulePath("x.y.z"))), "ResourceRoot 'x.y.z' registered (" + getModulePath("x.y.z") + ")");
    assert.ok(new URI(getModulePath(this.oMetadata.getComponentName(), "/../../foo/bar")).equals(new URI(getModulePath("foo.bar"))), "ResourceRoot 'foo.bar' registered (" + getModulePath("foo.bar") + ")");
    assert.ok(!new URI("http://absolute/uri").equals(new URI(getModulePath("absolute"))), "ResourceRoot 'absolute' not registered (" + getModulePath("absolute") + ")");
    assert.ok(!new URI("/server/absolute/uri").equals(new URI(getModulePath("server.absolute"))), "ResourceRoot 'server.absolute' not registered (" + getModulePath("server.absolute") + ")");
});
...
QUnit.module("Component Metadata: Resolve URI", {
    beforeEach: function (assert) {
        this.assertResolvedUri = function (sInput, sBase, sExpected) {
            var oBaseTag = document.querySelector("base"), sOldHRef = oBaseTag.href;
            oBaseTag.href = "./";
...
function runManifestLoadingTests(sDescription, fnCreateConfig) {
    ...
    QUnit.test("Async loading of manifests with component.json", function (assert) {
        return sap.ui.component(fnCreateConfig("./testdata/inherit/manifest.json", "sap.ui.test.inherit")).then(function (oComponent) {
            ...
            assert.equal(oComponent.getManifest().name, "sap.ui.test.inherit.Component", "Check name of the the main component");
            assert.equal(oComponent.getMetadata().getParent().getManifest().name, "sap.ui.test.inherit.parent.Component", "Check name of the inherited parent component");
    ...
    QUnit.test("Async loading of manifests", function (assert) {
        return sap.ui.component(fnCreateConfig("./testdata/inheritAsync/manifest.json", "sap.ui.test.inheritAsync")).then(function (oComponent) {
            assert.ok(oComponent instanceof Component, "Component has been created.");
            assert.equal(oComponent.getManifest().name, "sap.ui.test.inheritAsync.Component", "Check name of the the main component");
            assert.equal(oComponent.getMetadata().getParent().getManifest().name, "sap.ui.test.inheritAsync.parentB.Component", "Check name of the inherited parent component B");
            assert.equal(oComponent.getMetadata().getParent().getParent().getManifest().name, "sap.ui.test.inheritAsync.parentA.Component", "Check name of the inherited parent component A");
            destroyComponent(oComponent);
        });
    });
    QUnit.test("Async loading of manifests with missing manifest of parent metadata", function (assert) {
        return sap.ui.component(fnCreateConfig("./testdata/inheritAsyncError/manifest.json", "sap.ui.test.inheritAsyncError")).then(function (oComponent) {
            ...
            var result = aLogEntries.filter(function (oEntry) {
                return oEntry.message.indexOf("Failed to load component manifest from \"test-resources/sap/ui/core/qunit/component/testdata/inheritAsyncError/parentFAIL/manifest.json\"") === 0;
            });
            assert.equal(result.length, 1, "Error: 'Failed to laod component manifest from...' was logged.");
            assert.equal(oComponent.getManifest().name, "sap.ui.test.inheritAsyncError.Component", "Check name of the the main component");
            assert.equal(oComponent.getMetadata().getParent().getManifest().name, "sap.ui.test.inheritAsyncError.parentB.Component", "Check name of the inherited parent component B");
            assert.equal(oComponent.getMetadata().getParent().getParent().getManifest().name, "sap.ui.test.inheritAsyncError.parentFAIL.Component", "Check name of the inherited parent component A");
            destroyComponent(oComponent);
        });
    });
}
...
QUnit.test("Async loading of manifests", function (assert) {
    ...
    return sap.ui.component({
        ...
        assert.ok(oComponent instanceof Component, "Component has been created.");
        assert.equal(oComponent.getManifest().name, "sap.ui.test.inheritAsync.Component", "Check name of the the main component");
        assert.equal(oComponent.getManifestEntry("/sap.ui5/config/any/entry1"), "test32352", "Instance specific manifest entry should be available through component instance");
        assert.equal(sap.ui.test.inheritAsync.Component.getMetadata().getManifestEntry("/sap.ui5/config/any/entry1"), "test", "Instance specific manifest should not be reused for static manifest");
        assert.equal(oComponent.getMetadata().getParent().getManifest().name, "sap.ui.test.inheritAsync.parentB.Component", "Check name of the inherited parent component B");
        assert.equal(oComponent.getMetadata().getParent().getParent().getManifest().name, "sap.ui.test.inheritAsync.parentA.Component", "Check name of the inherited parent component A");
        destroyComponent(oComponent);
    });
