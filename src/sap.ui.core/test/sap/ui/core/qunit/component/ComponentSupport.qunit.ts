...
QUnit.test("Finder, Parser and Default Settings", function (assert) {
    ...
    for (var i = 0, l = aElements.length; i < l; i++) {
        ...
        mComponentSettings.componentCreated = "componentCreated";
...
QUnit.test("Parser with unknown property/event", function (assert) {
    ...
    assert.ok(Log.warning.calledWithMatch("Property or event \"unkown\" will be ignored as it does not exist in sap.ui.core.ComponentContainer"), "should log a warning with the expected message");
});
