...
QUnit.test("sync/async", function (assert) {
    var bSyncOngoing = false, done = assert.async();
    jQuery.ajax({
...
QUnit.test("async readyState without listener", function (assert) {
    var asyncXHR = new XMLHttpRequest(), done = assert.async();
    asyncXHR.open("GET", baseURL + "small.txt", true);
    asyncXHR.send();
...
QUnit.test("setTimeout/setInterval with strings", function (assert) {
    var iInterval, done = assert.async();
...
QUnit.test("sync/Promise/setTimeout", function (assert) {
    var bSyncOngoing = false, bTimeoutTriggered = false, bIntervalTriggered = false, bPromiseTriggered = false, vInterval, done = assert.async(), oResolved = Promise.resolve(), oRejected = Promise.reject();
...
QUnit.test("sync/Promise reject/resolve", function (assert) {
    assert.expect(4);
    var done = assert.async(), oError = new Error(), oResolved = Promise.resolve();
    oResolved.then(function () {
        jQuery.ajax({
            url: baseURL + "small.txt",
            async: false,
