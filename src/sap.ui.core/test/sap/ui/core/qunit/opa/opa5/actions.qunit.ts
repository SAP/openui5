...
QUnit.test("Should execute an action", function (assert) {
    var oOpa5 = new Opa5(), fnActionSpy = this.spy(), done = assert.async();
    oOpa5.waitFor({
        id: "foo",
        actions: fnActionSpy
    });
...
QUnit.test("Should execute success after an action", function (assert) {
    var oOpa5 = new Opa5(), fnCheckStub = this.stub().returns(true), fnActionSpy = this.spy(), fnSuccessSpy = this.spy(), done = assert.async();
    oOpa5.waitFor({
        id: "foo",
        check: fnCheckStub,
        actions: fnActionSpy,
...
QUnit.test("Should not execute an action before the check returns true", function (assert) {
    var oOpa5 = new Opa5(), bCheckReturnValue = false, fnActionSpy = this.spy(), done = assert.async();
    oOpa5.waitFor({
        id: "foo",
        check: function () {
...
opaTest("Should execute success of a waitFor added by an action before the success of waitFor", function (oOpa, when) {
    var fnSuccessSpy = sinon.spy(), fnInnerSuccessSpy = sinon.spy(), fnActionSpy = sinon.spy(function () {
        when.waitFor({
...
opaTest("Should execute all success functions if waitFors added by actions before the waitFors success", function (oOpa) {
    var fnSuccessSpy = sinon.spy(), fnAction1WaitFor1 = sinon.spy(), fnAction1WaitFor2 = sinon.spy(), fnAction2WaitFor1 = sinon.spy(), fnAction2WaitFor2 = sinon.spy(), fnActionSpy = sinon.spy(function () {
        oOpa.waitFor({
    ...
    oOpa.waitFor({
        success: function () {
            sinon.assert.callOrder(fnActionSpy, fnSecondActionSpy, fnAction1WaitFor1, fnAction1WaitFor2, fnAction2WaitFor1, fnAction2WaitFor2, fnSuccessSpy);
            sinon.assert.calledOnce(fnSuccessSpy);
...
QUnit.test("Should not execute any subsequent successes as soon as an exception gets thrown in an inner actions", function (assert) {
    var oOpa = new Opa5(), fnSuccessSpy = sinon.spy(), fnDone = assert.async();
    oOpa.waitFor({
        actions: function () {
            oOpa.waitFor({
                actions: function () {
...
opaTest("Should execute all success functions if waitFors added by actions before the waitFors success", function (oOpa) {
    var oButton = this.oButton, oButton2 = this.oButton2, fnSuccessSpy = sinon.spy(), fnAction1 = sinon.spy(), fnAction2 = sinon.spy(), fnActionSpy = sinon.spy(function (oButton) {
        oOpa.waitFor({
            id: oButton.getId(),
            success: fnAction1
        });
    ...
    oOpa.waitFor({
        success: function () {
            sinon.assert.callOrder(fnActionSpy, fnSecondActionSpy, fnAction1, fnAction1, fnAction2, fnAction2, fnSuccessSpy);
            ...
            sinon.assert.calledWithMatch(fnSuccessSpy, function (aButtons) {
                return aButtons.indexOf(oButton) !== -1 && aButtons.indexOf(oButton2) !== -1 && aButtons.length === 2;
            });
        }
    });
});
opaTest("Should be able to double nest actions", function (oOpa) {
    var oButton1 = this.oButton, oButton2 = this.oButton2, fnInnerAction = sinon.spy(), fnInnerSuccess = sinon.spy(), fnNestedSuccess = sinon.spy(), fnNestedAction = sinon.spy(function () {
        oOpa.waitFor({
            id: oButton1.getId(),
    ...
    oOpa.waitFor({
        ...
        success: function () {
            ...
            sinon.assert.callOrder(fnAction, fnNestedAction, fnInnerAction, fnInnerSuccess, fnNestedSuccess);
        }
    });
});
opaTest("Should be able to add 2 waitFors in an action", function (oOpa) {
    var oButton1 = this.oButton, oButton2 = this.oButton2, fnAddedAction1 = sinon.spy(), fnAddedAction2 = sinon.spy();
    oOpa.waitFor({
        id: oButton1.getId(),
        actions: function () {
            oOpa.waitFor({
