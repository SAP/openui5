...
QUnit.test("Should generate selector for control with stable ID", function (assert) {
    ...
    _ControlSelectorGenerator._generate({ control: this.oStableIDButton }).then(function (mSelector) {
...
QUnit.test("Should return undefined for control with generated ID", function (assert) {
    var fnDone = assert.async();
    _ControlSelectorGenerator._generate({ control: this.oGeneratedIDButton, shallow: true }).catch(function (oError) {
