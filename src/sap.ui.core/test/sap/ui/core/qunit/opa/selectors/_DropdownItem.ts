import _DropdownItem from "sap/ui/test/selectors/_DropdownItem";
...
QUnit.test("Should generate selector for item inside dropdown", function (assert) {
    ...
    this.oSelect.open();
    setTimeout(function () {
        _ControlSelectorGenerator._generate({ control: this.oSelect.getItems()[0], includeAll: true }).then(function (aSelectors) {
