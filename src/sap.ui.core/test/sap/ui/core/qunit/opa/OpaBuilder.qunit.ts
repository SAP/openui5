...
function _generateOpaBuilderSpies(aMethodNames) {
    return aMethodNames.reduce(function (mSpies, sMethodName) {
...
function _executeAndValidateBuilderMethod(assert, oOpaBuilder, sMethodName, vParameter, vTargetOption, bResultsInArray) {
    ...
    if (typeof vTargetOption === "string") {
        if (bResultsInArray) {
            assert.deepEqual(oOpaBuilder.build()[vTargetOption], Array.isArray(vParameter) ? vParameter : [vParameter], "option '" + vTargetOption + "' has correct array values");
        }
        else {
            assert.strictEqual(oOpaBuilder.build()[vTargetOption], vParameter, "option '" + vTargetOption + "' is '" + vParameter + "'");
        }
    }
    else {
...
QUnit.test("Should set default options", function (assert) {
    var mNewDefaultOptions = { visible: false }, mSetDefaultOptions = OpaBuilder.defaultOptions({ visible: false });
...
QUnit.test("Should create a new OpaBuilder instance with given options", function (assert) {
    var mSpies = _generateOpaBuilderSpies.call(this, ["hasId", "hasType", "isDialogElement", "has", "do", "options"]), oOpaInstance = new Opa5(), fnMyMatcher = OpaBuilder.Matchers.properties(), fnMyAction = OpaBuilder.Actions.press(), oMyOptions = { errorMessage: "my error" }, oOpaBuilder = OpaBuilder.create(oOpaInstance, "my.Id", "my.Type", true, fnMyMatcher, fnMyAction, oMyOptions);
    assert.ok(oOpaBuilder instanceof OpaBuilder);
...
QUnit.test("Should set 'matchers' when using 'has'", function (assert) {
    _executeAndValidateBuilderMethod(assert, OpaBuilder.create(), "has", function () {
        return true;
    }, "matchers", true);
});
QUnit.test("Should set 'check' and return builder instance", function (assert) {
    _executeAndValidateBuilderMethod(assert, OpaBuilder.create(), "check", function () {
        return true;
    }, "check");
});
...
QUnit.test("Should set 'success' and 'errorMessage' when setting 'description'", function (assert) {
    var oOpaBuilder = new OpaBuilder(), sMyDescription = "my description";
...
QUnit.test("Should create properties matcher when using 'hasProperties'", function (assert) {
    var oSpy = this.spy(OpaBuilder.Matchers, "properties"), oOpaBuilder = new OpaBuilder();
...
QUnit.test("Should create aggregation matcher when using 'hasAggregation'", function (assert) {
    var oSpy = this.spy(OpaBuilder.Matchers, "aggregationMatcher"), fnMatcher = OpaBuilder.Matchers.TRUE, oOpaBuilder = new OpaBuilder();
...
QUnit.test("Should create aggregation matcher that checks properties when using 'hasAggregationProperties'", function (assert) {
    var oSpy = this.spy(OpaBuilder.prototype, "hasAggregation"), oMatcherSpy = this.spy(OpaBuilder.Matchers, "properties"), oOpaBuilder = new OpaBuilder();
...
QUnit.test("Should create aggregation length matcher when using 'aggregationLength'", function (assert) {
    var oSpy = this.spy(OpaBuilder.prototype, "has"), oMatcherSpy = this.spy(OpaBuilder.Matchers, "aggregationLength"), oOpaBuilder = new OpaBuilder();
...
QUnit.test("Should create child matcher when using 'hasChildren'", function (assert) {
    var oSpy = this.spy(OpaBuilder.Matchers, "childrenMatcher"), fnMatcher = OpaBuilder.Matchers.TRUE, oOpaBuilder = new OpaBuilder();
...
QUnit.test("Should create check function when using 'checkNumberOfMatches'", function (assert) {
    var oSpy = this.spy(OpaBuilder.prototype, "check"), oOpaBuilder = new OpaBuilder();
...
QUnit.test("Should create 'press' action when using 'doPress'", function (assert) {
    var oSpy = this.spy(OpaBuilder.prototype, "do"), oActionSpy = this.spy(OpaBuilder.Actions, "press"), oOpaBuilder = new OpaBuilder();
...
QUnit.test("Should create 'enterText' action when using 'doEnterText'", function (assert) {
    var oSpy = this.spy(OpaBuilder.prototype, "do"), oActionSpy = this.spy(OpaBuilder.Actions, "enterText"), oOpaBuilder = new OpaBuilder();
...
QUnit.test("Should chain 'check' functions (with respect to previous result) on consecutive use of 'check' if not replaced", function (assert) {
    var oOpaBuilder = new OpaBuilder(), oFirstCheck = this.spy(function (oObject) {
        return oObject;
...
QUnit.test("Should chain 'success' functions (independent of previous result) on consecutive use of 'success' if not replaced", function (assert) {
    var oOpaBuilder = new OpaBuilder(), oFirstCheck = this.spy(function (oObject) {
        return oObject;
    }), oSecondCheck = this.spy(function (oObject) {
        return oObject;
...
QUnit.test("Should execute functions conditionally when using 'doConditional'", function (assert) {
    var oOpaBuilder = new OpaBuilder(), fnMatcher = function (oObject) {
        return oObject;
    }, fnSuccessAction = function (oObject) {
        return oObject;
    }, fnElseAction = function (oObject) {
        return oObject;
    }, oActionSpy = this.spy(OpaBuilder.Actions, "conditional"), oDoSpy = this.spy(OpaBuilder.prototype, "do");
...
QUnit.test("Should execute functions on aggregation elements when using 'doOnAggregation'", function (assert) {
    var oOpaBuilder = new OpaBuilder(), oMatcherSpy = this.spy(function (oObject) {
        return !!oObject;
...
QUnit.test("Should execute functions on child elements when using 'doOnChildren'", function (assert) {
    var oOpaBuilder = new OpaBuilder(), oDummyControl1 = { match: false }, oDummyControl2 = { match: true }, oDummyParent = { dummyParent: true }, oAncestorSpy = this.spy(OpaBuilder.Matchers.TRUE), oAncestorStub = sinonUtils.createStub(OpaBuilder.Matchers, "ancestor", oAncestorSpy), oMatcherSpy = this.spy(function (oObject) {
    ...
    }), oPluginStub = sinonUtils.createStub(Opa5, "getPlugin", oGetPluginSpy), oWaitForSpy = this.spy(), oWaitForStub = sinonUtils.createStub(Opa5.prototype, "waitFor", oWaitForSpy);
...
QUnit.test("Should call 'Opa5.waitFor' with the defined options when using 'execute'", function (assert) {
    var oOpaBuilder = new OpaBuilder(), oWaitForSpy = this.spy(), oWaitForStub = sinonUtils.createStub(Opa5.prototype, "waitFor", oWaitForSpy), oOptions = oOpaBuilder.hasId("myId").hasType("sap.m.Button").doPress().description("some error message").build();
    oOpaBuilder.execute();
...
QUnit.test("Should create OpaBuilder.Matchers.conditional matcher when using 'hasConditional'", function (assert) {
    var oOpaBuilder = new OpaBuilder(), fnCondition = function (oObject) {
        return oObject;
    }, fnSuccessMatcher = function (oObject) {
        return oObject;
    }, fnElseMatcher = function (oObject) {
        return oObject;
    }, oMatcherSpy = this.spy(OpaBuilder.Matchers, "conditional"), oHasSpy = this.spy(OpaBuilder.prototype, "has");
...
QUnit.test("Should create OpaBuilder.Matchers.some matcher when using 'hasSome'", function (assert) {
    var oOpaBuilder = new OpaBuilder(), fnMatcher1 = function (oObject) {
        return oObject;
    }, fnMatcher2 = function (oObject) {
        return oObject;
    }, fnMatcher3 = function (oObject) {
        return oObject;
    }, oMatcherSpy = this.spy(OpaBuilder.Matchers, "some"), oHasSpy = this.spy(OpaBuilder.prototype, "has");
...
QUnit.test("Should create OpaBuilder.Matchers.i18n matcher when using 'hasI18NText'", function (assert) {
    var oOpaBuilder = new OpaBuilder(), sPropertyName = "Hello", sModelTokenPath = "World", aParameters = ["!"], oMatcherSpy = this.spy(OpaBuilder.Matchers, "i18n"), oHasSpy = this.spy(OpaBuilder.prototype, "has");
...
QUnit.test("Should set/get the Opa5 instance on 'setOpaInstance' and 'getOpaInstance'", function (assert) {
    var oOpaBuilder = new OpaBuilder(), oOpa5Instance = new Opa5();
...
QUnit.test("'children' return a matcher that returns an array of children filtered by given matchers", function (assert) {
    var fnChildren, oDummyControl1 = { match: false }, oDummyControl2 = { match: true }, oDummyParent = { dummyParent: true }, oAncestorSpy = this.spy(OpaBuilder.Matchers.TRUE), oAncestorStub = sinonUtils.createStub(OpaBuilder.Matchers, "ancestor", oAncestorSpy), oMatcherSpy = this.spy(function (oObject) {
        return oObject.match;
    }), oActionSpy = this.spy(function (oObject) {
        return oObject;
    }), oGetMatchingControlsSpy = this.spy(function (oOptions) {
...
QUnit.test("'childrenMatcher' return a matcher that returns whether 'children' has at least one match", function (assert) {
    var fnChildMatcher, oDummyControl1 = { match: false }, oDummyControl2 = { match: true }, oDummyParent = { dummyParent: true }, oAncestorSpy = this.spy(OpaBuilder.Matchers.TRUE), oAncestorStub = sinonUtils.createStub(OpaBuilder.Matchers, "ancestor", oAncestorSpy), oMatcherSpy = this.spy(function (oObject) {
        return oObject.match;
    }), oActionSpy = this.spy(function (oObject) {
        return oObject;
    }), oGetMatchingControlsSpy = this.spy(function (oOptions) {
...
QUnit.test("'aggregation' should return a matcher that returns an array of aggregation items filtered by given matchers", function (assert) {
    ...
    assert.deepEqual(fnAggregationMatcher({
        getItems: function () {
            return ["test", 1];
        }
    }), []);
...
QUnit.test("'aggregationMatcher' should return a matcher that apply given matcher to given aggregation items", function (assert) {
    ...
    assert.strictEqual(fnAggregationMatcher({
        getItems: function () {
            return ["test", 1];
        }
    }), false);
...
QUnit.test("'aggregationAtIndex' should return a matcher that returns aggregation item at given index or undefined if not applicable", function (assert) {
    var fnAggregationAtIndex = OpaBuilder.Matchers.aggregationAtIndex("items", 2);
    assert.strictEqual(fnAggregationAtIndex({
        getItems: function () {
            return ["test", 1];
        }
    }), undefined);
...
QUnit.test("'bindingProperties' should have its model argument optional", function (assert) {
    var fnBindingProperties = OpaBuilder.Matchers.bindingProperties({
        text: "fitting text",
        number: 42
    }), bCalledBindingContextWOModel = false, bCalleModelWOModel = false, oMockControl = {
...
QUnit.test("'bindingProperties' should return a matcher that checks whether bound item has given properties", function (assert) {
    var sModelName = "myModelName", fnBindingProperties = OpaBuilder.Matchers.bindingProperties(sModelName, { text: "fitting text", number: 42 }), oMockControl = {
        getBindingContext: function (sName) {
            if (sName === sModelName) {
                return oContext;
            }
...
QUnit.test("'resourceBundle' should return a matcher that validates the given property against a token text of a library message bundle", function (assert) {
    var fnResourceBundle = OpaBuilder.Matchers.resourceBundle("text", "my.test.lib", "I_AM_A_TOKEN", "first", "second"), fnBundleStub = sinonUtils.createStub(sap.ui.getCore(), "getLibraryResourceBundle", function () {
        return {
...
QUnit.test("'customData' should return a matcher that checks whether bound item has custom data with proper values", function (assert) {
    var fnCustomDataMatcher = OpaBuilder.Matchers.customData({ text: "fitting text", number: 42 }), oMockControl = {
        data: function (sKey) {
...
QUnit.test("'focused' should return a matcher that checks for focused elements", function (assert) {
    var fnFocusedMatcher = OpaBuilder.Matchers.focused(), bHasFocusViaDom = false, bHasFocusViaSapMFocusClass = false, bHasChildWithFocus = false, oMockControl = {
...
QUnit.test("'executor' should return a function that executes given actions on given controls", function (assert) {
    var fnAction1 = this.spy(function (oObject) {
        ...
        return oObject;
    }), oDummyObject1 = { dummy: true }, oDummyObject2 = { anotherDummy: true }, oExecutor;
