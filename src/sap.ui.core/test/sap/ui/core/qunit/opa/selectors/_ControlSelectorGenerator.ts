...
import _ControlSelectorValidator from "sap/ui/test/selectors/_ControlSelectorValidator";
...
QUnit.test("Should continue if ancestor selector generation throws error", function (assert) {
    ...
    _ControlSelectorGenerator._generate({ control: this.oText }).then(function (mSelector) {
...
QUnit.test("Should generate selectors in correct order", function (assert) {
    var fnDone = assert.async();
    _ControlSelectorGenerator._generate({ control: this.oText, includeAll: true }).then(function () {
        var i = 0;
...
QUnit.test("Should generate unique selector for ancestor", function (assert) {
    var fnDone = assert.async();
    _ControlSelectorGenerator._generateUniqueAncestorSelector(this.oButtonHeader1).then(function (mResult) {
...
QUnit.test("Should not generate unique selector for ancestor when depth exceeded", function (assert) {
    var fnDone = assert.async();
    _ControlSelectorGenerator.setParams({ maxDepth: 1 });
    _ControlSelectorGenerator._generateUniqueAncestorSelector(this.oButtonHeader1).catch(function (oError) {
...
QUnit.test("Should generate selector with ancestors - hierarchical", function (assert) {
    var fnDone = assert.async();
    _ControlSelectorGenerator._generateHierarchicalUp({ control: this.oButtonContent }).then(function (mResult) {
...
QUnit.test("Should not generate selector when unique selector in subtree is not found - hierarchical", function (assert) {
    var fnDone = assert.async();
    _ControlSelectorGenerator._generateHierarchicalUp({ control: this.oButtonHeader1 }).catch(function (oError) {
...
QUnit.test("Should generate unique selector for desendant", function (assert) {
    var fnDone = assert.async();
    _ControlSelectorGenerator._generateUniqueDescendantSelector(this.oList.getItems()[0]).then(function (mResult) {
...
QUnit.test("Should generate selector with desendant - hierarchical", function (assert) {
    var fnDone = assert.async();
    _ControlSelectorGenerator._generateHierarchicalDown({ control: this.oList.getItems()[0] }).then(function (mResult) {
...
QUnit.test("Should not generate selector for desendant with no unique 'shallow' selector", function (assert) {
    var fnDone = assert.async();
    _ControlSelectorGenerator.setParams({ maxWidth: 1, maxDepth: 2 });
    _ControlSelectorGenerator._generateUniqueDescendantSelector(this.oList.getItems()[0]).catch(function (oError) {
...
QUnit.test("Should not generate selector with desendant when none has a unique selector - hierarchical", function (assert) {
    var fnDone = assert.async();
    _ControlSelectorGenerator.setParams({ maxWidth: 1, maxDepth: 2 });
    _ControlSelectorGenerator._generateHierarchicalDown({ control: this.oList.getItems()[0] }).catch(function (oError) {
        assert.ok(oError.message.match(/Could not generate unique selector for descendant/), "Should not find unique descendant");
    }).finally(fnDone);
});
...
QUnit.test("Should generate selector with sibling", function (assert) {
    var fnDone = assert.async();
    _ControlSelectorGenerator._generateWithSibling({ control: this.oText }).then(function (mResult) {
...
QUnit.test("Should get ancestor selector when required", function (assert) {
    var fnDone = assert.async();
    _ControlSelectorGenerator._getAncestorSelector(this.oGeneratorWithAncestor, { control: this.oMockControl }).then(function (mResult) {
...
QUnit.test("Should not get ancestor selector when not required", function (assert) {
    var fnDone = assert.async();
    _ControlSelectorGenerator._getAncestorSelector(this.oSimpleGenerator, { control: this.oMockControl }).then(function (mResult) {
...
QUnit.test("Should get validation root selector when required", function (assert) {
    var fnDone = assert.async();
    _ControlSelectorGenerator._getValidationRootSelector(this.oGeneratorWithValidationRoot, { control: this.oMockControl }).then(function (mResult) {
        assert.strictEqual(mResult, this.mRelativeSelector, "Should generate validation root selector when required");
    }.bind(this)).finally(fnDone);
});
QUnit.test("Should not get ancestor selector when not required", function (assert) {
    var fnDone = assert.async();
    _ControlSelectorGenerator._getValidationRootSelector(this.oSimpleGenerator, { control: this.oMockControl }).then(function (mResult) {
