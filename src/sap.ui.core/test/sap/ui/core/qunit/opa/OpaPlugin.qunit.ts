...
QUnit.test("Should retrieve multiple controls by global id's", function (assert) {
    var oButton = new Button(), oButton2 = new Button();
...
QUnit.test("Should log an error if the controlType does not match and a string id is given", function (assert) {
    ...
    sinon.assert.calledWithExactly(this.fnLogErrorSpy, "A control with global ID 'my_id3' is found but does not have required controlType " + "'sap.m.Button'. Found control is 'Element sap.m.CheckBox#my_id3' but null is returned instead");
});
QUnit.test("Should log an error if the controlType does not match and a string id is given", function (assert) {
    var oCheckBox = this.oPlugin.getMatchingControls({
        id: "my_id3",
...
QUnit.test("Should match any controls of a view when ID is given and the control type is not correct", function (assert) {
    ...
    assert.ok(this.oPlugin.getControlConstructor("sap.m.Input"), "Control constructor is found");
...
QUnit.test("Should not call the autoWaiter or interactable if visible is false", function (assert) {
    var fnInteractableSpy = this.spy(Interactable.prototype, "isMatching"), fnWaitSpy = this.spy(_autoWaiter, "hasToWait");
...
QUnit.test("Should filter invisible views with a duplicate name", function (assert) {
    this.oDuplicateView.$().css("visibility", "hidden");
