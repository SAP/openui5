...
[{ component: "sap.ui.test" }, { component: "sap.ui.test.myComponent", match: "myComp" }].forEach(function (mTestData) {
    ...
    QUnit.test("Should guard against memory leaking", function (assert) {
        ...
        if (mTestData.match) {
            ...
            assert.strictEqual(aLogs[0], "Opa has received 500 logs without a consumer - " + "maybe you loaded Opa.js inside of an IFrame? " + "The logs are now cleared to prevent memory leaking -  sap.ui.test._LogCollector");
        }
    });
