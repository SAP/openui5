...
], function (Device, Drag, Drop, Press, Opa5, opaTest) {
	"use strict";

	if (Device.browser.msie && Device.browser.version < 12) {
		QUnit.test("Should not execute drag and drop tests in IE11", function (assert) {
			assert.ok(true, "Should not execute drag and drop tests in IE11");
		});
	} else {
		Opa5.extendConfig({
			autoWait: true
