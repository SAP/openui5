...
QUnit.test("Should throw an exception when start is called twice", function (assert) {
    ...
    document.getElementsByClassName("opaFrame")[0].onload = function () {
        assert.throws(function () {
            ...
            return oError instanceof Error && oError.message === "sap.ui.test.launchers.iFrameLauncher: " + "Launch was called twice without teardown. Only one iFrame may be loaded at a time.";
        });
        iFrameLauncher.teardown();
        fnDone();
    };
...
QUnit.test("Should throw an exception when teardown is called before launch", function (assert) {
    assert.throws(function () {
        ...
        return oError instanceof Error && oError.message === "sap.ui.test.launchers.iFrameLauncher: " + "Teardown was called before launch. No iFrame was loaded.";
    });
});