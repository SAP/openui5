import _TableRowItem from "sap/ui/test/selectors/_TableRowItem";
...
function getViewContent() {
    return "<mvc:View xmlns:core=\"sap.ui.core\" xmlns:mvc=\"sap.ui.core.mvc\" xmlns=\"sap.m\" controllerName=\"myController\" viewName=\"myView\">" + "<App id=\"myApp\">" + "<Page id=\"page1\">" + "<Table id=\"myTable\" items=\"{/items}\" width=\"300px\">" + "<columns>" + "<Column><Text text=\"Name\"/></Column>" + "<Column><Text text=\"Button\"/></Column>" + "</columns>" + "<items>" + "<ColumnListItem>" + "<cells>" + "<ObjectIdentifier id=\"objectId" + iTest + "\" title=\"{id}\" text=\"{name}\"/>" + "<Button id=\"press" + iTest + "\" text=\"Press\"></Button>" + "</cells>" + "</ColumnListItem>" + "</items>" + "</Table>" + "</Page>" + "</App>" + "</mvc:View>";
}
...
QUnit.test("Should select a control in table row", function (assert) {
    ...
    _ControlSelectorGenerator._generate({ control: oControl._getTextControl(), includeAll: true }).then(function (aSelectors) {
