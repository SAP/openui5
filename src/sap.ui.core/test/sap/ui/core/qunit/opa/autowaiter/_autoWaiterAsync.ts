...
import _autoWaiterAsync from "sap/ui/test/autowaiter/_autoWaiterAsync";
...
QUnit.test("Should not start a second wait", function (assert) {
    ...
    assert.ok(fnCallbackSecondSpy.calledWithMatch(/waitAsync is already running/), "Should invoke the callback with an error message on second start");
    assert.ok($.isEmptyObject(this.clock.timers), "Should stop polling when autoWaiter conditions are met");
});
QUnit.test("Should log autoWaiter pending work on timeout", function (assert) {
    ...
    assert.ok(this.fnCallbackSpy.calledWithMatch(/there is still pending asynchronous work/), "Should invoke the callback with the pending work log");
