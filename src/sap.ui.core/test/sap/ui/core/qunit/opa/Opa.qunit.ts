...
QUnit.test("Should wait for something", function (assert) {
    var bFirstCheck = false, oFirstSuccess = this.spy(), bSecondCheck = false, oSecondSuccess = this.spy(), oDoneSpy = this.spy();
    var oOpa = new Opa();
    oOpa.waitFor({
        check: function () {
            return bFirstCheck;
...
QUnit.test("Should wait for additional waitFor's declared in the success handler before continuing the queue", function (assert) {
    ...
    var oSuccess11 = this.spy(), oSuccess121 = this.spy(), fnSuccess12 = function () {
        return oOpa.waitFor({
            success: oSuccess121
        });
    }, fnSuccessSpy12 = this.spy(fnSuccess12), oSuccess13 = this.spy(), fnSuccess1 = function () {
        oOpa.waitFor({
            success: oSuccess11
        });
        oOpa.waitFor({
    ...
    }, fnSuccessSpy1 = this.spy(fnSuccess1), oSuccess2 = this.spy(), oDoneSpy = this.spy();
    oOpa.waitFor({
...
QUnit.test("Should modify the polling interval", function (assert) {
    var bFirstCheck = false, bSecondCheck = false, oFirstCheckSpy = this.spy(function () {
        return bFirstCheck;
...
QUnit.test("Should slow down the execution of OPA", function (assert) {
    var oOpa = new Opa(), fnFirstSuccessSpy = sinon.spy(), fnSecondSuccessSpy = sinon.spy(), fnDone = assert.async();
    Opa.extendConfig({
        executionDelay: 5000
    });
    oOpa.waitFor({
...
QUnit.test("Should reject the emptyQueue promise if there is an error in the check or success", function (assert) {
    var oError = new Error("OpaError"), fnFaultyFunction = function () {
        throw oError;
    }, oOpa = new Opa(), oOpa1 = new Opa(), fnSpy = this.spy();
...
QUnit.test("Should empty the queue if the promise is rejected", function (assert) {
    var oError = new Error("OpaError"), fnFaultyFunction = function () {
        throw oError;
    }, oOpa = new Opa(), fnCheck = this.spy(function () {
        return false;
    }), fnFail = this.spy();
    oOpa.waitFor({
...
QUnit.test("Should be able to handle a waitFor error and reject the empty queue", function (assert) {
    var oOpa = new Opa(), fnSuccess = this.spy(), fnEmptyQueueFail = this.spy(), fnWaitForFail = this.spy(), oFailingWaitFor = oOpa.waitFor({
        success: function () {
...
QUnit.test("Should stop the queue inside of success", function (assert) {
    var oOpa = new Opa(), fnDone = assert.async(), fnShouldNotBeCalled = sinon.spy();
    oOpa.waitFor({
        success: function () {
            Opa.stopQueue();
        }
...
QUnit.test("Should stop a Queue while polling", function (assert) {
    var oOpa = new Opa(), fnDone = assert.async(), fnSuccess = sinon.spy(), fnCheck = sinon.spy(function () {
        Opa.stopQueue();
        return true;
    });
    oOpa.waitFor({
...
QUnit.test("Should show info about last executed check on QUnit timeout", function callingFunction(assert) {
    var oOpa = new Opa(), fnCheckStub = sinon.stub(), fnDone = assert.async();
...
QUnit.test("Should time out if check is returning false", function (assert) {
    var oErrorSpy = sinon.spy(), oWaitForDoneSpy = sinon.spy(), oSuccessSpy = sinon.spy(), oDoneSpy = sinon.spy(), fnDone = assert.async();
    var oOpa = new Opa();
    oOpa.waitFor({
        check: function () {
            return false;
...
["error", "success", "check"].forEach(function (sParameter) {
    QUnit.test("Should throw an error if the " + sParameter + " parameter is not a function", function (assert) {
        ...
        }, new Error("sap.ui.test.Opa#waitFor - the '" + sParameter + "' parameter needs to be a function but " + "'foo' was passed"), "threw the error");
    });
});
["timeout", "pollingInterval", "_stackDropCount"].forEach(function (sParameter) {
    QUnit.test("Should throw an error if the " + sParameter + " parameter is not numeric", function (assert) {
        assert.throws(function () {
            ...
            oWaitFor[sParameter] = "foo";
            new Opa().waitFor(oWaitFor);
        }, new Error("sap.ui.test.Opa#waitFor - the '" + sParameter + "' parameter needs to be numeric but " + "'foo' was passed"), "threw the error");
    });
});
QUnit.test("Should throw an error if the errorMessage parameter is not a string", function (assert) {
    assert.throws(function () {
        ...
        new Opa().waitFor(oWaitFor);
    }, new Error("sap.ui.test.Opa#waitFor - the 'errorMessage' parameter needs to be a string but " + "'[object Object]' was passed"), "threw the error");
});
QUnit.module("waitFor counts");
QUnit.test("Should calculate how many waitFors have been added in a timeframe", function (assert) {
    var fnTimeoutDone = assert.async(), oCounter = Opa._getWaitForCounter();
