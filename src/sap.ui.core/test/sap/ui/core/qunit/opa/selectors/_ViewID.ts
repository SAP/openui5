...
QUnit.test("Should generate selector for control in a view with stable ID", function (assert) {
    ...
    _ControlSelectorGenerator._generate({ control: $("#myView form input").control()[0], includeAll: true }).then(function (aSelector) {
...
QUnit.test("Should generate selector for control with stable ID in any view", function (assert) {
    ...
    _ControlSelectorGenerator._generate({ control: $view.find("form input").control()[0], includeAll: true }).then(function (aSelector) {
...
QUnit.test("Should not generate selector for control with generated ID", function (assert) {
    var fnDone = assert.async();
    _ControlSelectorGenerator._generate({ control: $("#myView form input").control()[1], includeAll: true, shallow: true }).then(function (aSelector) {
...
QUnit.test("Should not generate selector for control with no view", function (assert) {
    var fnDone = assert.async();
    _ControlSelectorGenerator._generate({ control: this.oButton, includeAll: true, shallow: true }).then(function (aSelector) {
        assert.ok(!hasViewIdSelector(aSelector), "Should not generate selector");
    }).finally(fnDone);
});
function createViewContent(sViewName) {
    return "<mvc:View xmlns:core=\"sap.ui.core\" xmlns:mvc=\"sap.ui.core.mvc\" xmlns=\"sap.m\" " + "controllerName=\"myController\" viewName=\"" + sViewName + "\">" + "<App id=\"myApp\">" + "<Page id=\"page1\">" + "<SearchField id=\"mySearch\" placeholder=\"Test\"></SearchField>" + "<SearchField placeholder=\"Placeholder\"></SearchField>" + "</Page>" + "</App>" + "</mvc:View>";
}
