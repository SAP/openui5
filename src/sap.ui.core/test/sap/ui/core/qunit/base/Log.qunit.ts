...
QUnit.test("Log: logSupportInfo", function (assert) {
    ...
    assert.ok(onLogEntry.calledWith(sinon.match({ message: "message", details: "", component: "" })), "when support info is suppressed, fnsupport info must not be logged as 'details'");
    onLogEntry.resetHistory();
    Log.error("message", "details", fnSupportInfo);
    assert.ok(onLogEntry.calledWith(sinon.match({ message: "message", details: "details", component: "" })), "when support info is suppressed, fnsupport info must not be logged as 'component'");
    onLogEntry.resetHistory();
    Log.error("message", "details", "component", fnSupportInfo);
    assert.ok(onLogEntry.calledWith(sinon.match({ message: "message", details: "details", component: "component" })), "when support info is suppressed, fnsupport info must not be logged");
    Log.removeLogListener(oListener);
});
QUnit.test("Log: Listener", function (assert) {
    var fnListener = sinon.spy(), fnAttachListener = sinon.spy(), fnDiscardLogEntriesListener = sinon.spy(), fnDetachListener = sinon.spy(), oListener = {
