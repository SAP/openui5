...
QUnit.test("number test", function (assert) {
    ...
    assert.equal(deepEqual(255, 255), true, "0xff, 255");
    ...
    assert.equal(deepEqual(0 / 0, 0 / 0), true, "0 divided by very small number evaluates to the same NaN");
    assert.equal(deepEqual(1 / 0, 1 / 0), true, "1 divided by very small number evaluates to the same Infinity");
    ...
    assert.equal(deepEqual(255, 255), true, "Hexadecimal number is equal to itself");
});
