...
QUnit.test("Deprecated Access to get(...,true)", function (assert) {
    ...
    try {
        ...
        assert.ok(Log.warning.calledWith(sinon.match(/Deprecated/).and(sinon.match(/must not be used, use getAll/))), "A warning should be logged");
    }
    finally {
        Log.warning.restore();
    }
...
QUnit.test("Deprecated Access to mParams", function (assert) {
    ...
    try {
        ...
        assert.ok(Log.warning.calledWith(sinon.match(/Deprecated/).and(sinon.match(/mParams must not be accessed/))), "A warning should be logged");
    }
    finally {
        Log.warning.restore();
    }
