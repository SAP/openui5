...
QUnit.test("jQuery.sap.now", function (assert) {
    var iDateNow = Date.now(), vTimestamp = jQuery.sap.now();
    assert.expect(2);
...
QUnit.test("jQuery.sap.assert", function (assert) {
    var assertSpy = this.spy(), sWindowName = (typeof window === "undefined" || window.top == window) ? "" : "[" + window.location.pathname.split("/").slice(-1)[0] + "] ", sMessage = "foo", sRefMessage = sWindowName + sMessage, messageSpy = this.spy(function () {
        return sMessage;
    }), consoleAssert;
    if (console.assert) {
...
QUnit.test("jQuery.sap.getter", function (assert) {
    var oValue = {}, fnGetter = jQuery.sap.getter(oValue);
