...
QUnit.test("jQuery.fn.bind()/unbind()/delegate()/undelegate()", function (assert) {
    var $Div = jQuery("<div></div>"), $Body = jQuery("body");
    $Body.append($Div);
...
QUnit.test("jQuery - automatic <tbody> creation when adding <tr> elements to a <table> element", function (assert) {
    ...
    this.checkWarnMessage("Trying to add a <tr> element to a <table> without a <tbody>. " + "At this point, jQuery version 2 would have inserted a <tbody> element for you. " + "Since jQuery version 3, jQuery does not automatically create a <tbody> element anymore. " + "Please add the <tbody> on your own, if your code or CSS expects it.");
});
