import StaticBinding from "sap/ui/model/StaticBinding";
import CompositeBinding from "sap/ui/model/CompositeBinding";
import TypeFloat from "sap/ui/model/type/Float";
import JSONModel from "sap/ui/model/json/JSONModel";
import ManagedObject from "sap/ui/base/ManagedObject";
var sDefaultLanguage = sap.ui.getCore().getConfiguration().getLanguage(), MyObject = ManagedObject.extend("MyObject", {
    metadata: {
        properties: {
            value: { type: "string" },
...
QUnit.test("In CompositeBinding: Getter", function (assert) {
    ...
    assert.deepEqual(oBinding.getValue(), ["test", 123], "composite returns array of static values");
...
QUnit.test("Model independence", function (assert) {
    var iFormatterCount = 0, oBinding, object = new MyObject({
        value: {
            value: "test",
