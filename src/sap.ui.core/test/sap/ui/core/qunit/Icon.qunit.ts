...
if (!Device.support.touch) {
    QUnit.test("Hover color", function (assert) {
        ...
        var sHoverColor = $dummy.css("color"), sHoverBackgroundColor = $dummy.css("background-color");
    ...
    QUnit.test("Icon without Hover color", function (assert) {
        var sColor = "#666666", sBackgroundColor = "#CCCCCC", oIcon = new Icon({
            color: sColor,
            backgroundColor: sBackgroundColor
        }), $dummy = jQuery.sap.byId("dummy");
        $dummy.css("color", sColor);
        $dummy.css("background-color", sBackgroundColor);
        var sTestColor = $dummy.css("color"), sTestBackgroundColor = $dummy.css("background-color");
        oIcon.placeAt("uiAreaA");
...
QUnit.test("Set invalid color", function (assert) {
    var sInvalidColor = "Invalid color", oErrorLogSpy = sinon.spy(Log, "error");
    var oIcon = new Icon({
        src: _IconRegistry.getIconURI("manager"),
        color: sInvalidColor
    });
    sinon.assert.calledWithExactly(oErrorLogSpy, "\"" + sInvalidColor + "\" is not of type sap.ui.core.CSSColor " + "nor of type sap.ui.core.IconColor.");
...
QUnit.test("set src should not change aria-label when 'labelledby' is set", function (assert) {
    var sLabelledById = "foo", oIcon = new Icon({
        src: _IconRegistry.getIconURI("delete"),
...
QUnit.test("src with icon name only should use default icon collection", function (assert) {
    ...
    oIcon.placeAt("uiAreaA");
    sap.ui.getCore().applyChanges();
    assert.equal(oIcon.$().attr("data-sap-ui-icon-content"), IconPool.getIconInfo("sap-icon://delete").content, "Content attribute should be set correctly");
    oIcon.destroy();
});
...
QUnit.test("When ariaLabelledBy IS set", function (assert) {
    var sId = "non-existing-id", $Icon, $InvisibleText, sText;
...
QUnit.test("Refresh icon content after font was loaded lazily", function (assert) {
    ...
    return _IconRegistry.getIconInfo("sap-icon://tnt/technicalsystem", "async").then(function () {
        sap.ui.getCore().applyChanges();
        assert.strictEqual(this.oIcon.$().attr("data-sap-ui-icon-content"), String.fromCharCode(57344), "Icon content has been set properly after the font is loaded");
...
QUnit.test("The same icon is displayed immediately after font was loaded lazily", function (assert) {
    this.createIcon({ src: "sap-icon://tnt/technicalsystem" });
    assert.strictEqual(this.oIcon.$().attr("data-sap-ui-icon-content"), String.fromCharCode(57344), "The icon content attribute is not set yet while the font is loading");
    assert.strictEqual(this.oIcon.$().css("font-family").replace(/"|'/g, ""), "SAP-icons-TNT", "Icon font family has been set properly after the font is loaded");
});
QUnit.test("A different icon is displayed immediately after font was loaded lazily", function (assert) {
    ...
    assert.strictEqual(this.oIcon.$().attr("data-sap-ui-icon-content"), String.fromCharCode(57359), "The icon content attribute is not set yet while the font is loading");
    assert.strictEqual(this.oIcon.$().css("font-family").replace(/"|'/g, ""), "SAP-icons-TNT", "Icon font family has been set properly after the font is loaded");
});