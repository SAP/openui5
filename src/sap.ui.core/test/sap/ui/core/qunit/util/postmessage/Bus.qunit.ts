import PostMessageBus from "sap/ui/core/postmessage/Bus";
...
QUnit.module("Validation", {
    beforeEach: function () {
        ...
        }.bind(this)).then(createIframe.bind(this, sap.ui.require.toUrl("static/iframe.html"))).then(function (oIframe) {
    ...
    QUnit.test("target parameter", function (assert) {
        ...
        assert.strictEqual(this.oPostMessageBus.publish({
...
QUnit.module("Sending message", {
    ...
    beforeEach: function () {
        return Promise.all([
            getPostMessageBus(window),
            getPostMessageBus(this.oIframeWindow)
...
QUnit.module("Receiving message", {
    ...
    QUnit.test("when a CONNECT message has no service name or it's not a string", function (assert) {
        var fnDone = assert.async();
        var oReceiverStub = sandbox.stub().onFirstCall().callsFake(function () {
            assert.ok(!includes(this.oPostMessageBus.getDeclinedOrigins(), this.oIframeWindow.location.origin));
        }.bind(this)).onSecondCall().callsFake(function () {
            assert.ok(!includes(this.oPostMessageBus.getDeclinedOrigins(), this.oIframeWindow.location.origin));
        }.bind(this)).onThirdCall().callsFake(function () {
            assert.ok(!includes(this.oPostMessageBus.getDeclinedOrigins(), this.oIframeWindow.location.origin));
            fnDone();
        }.bind(this));
        ...
        this.oPostMessageBus.subscribe("fakeChannel", "fakeEvent", function (oEvent) {
            switch (fnDone.callCount) {
                ...
                case 1:
                    ...
                    assert.ok(oSpy.withArgs(sinon.match(function (sMessage, sDetail) {
                        return (typeof sMessage === "string" && sMessage.includes("some error happens") || typeof sDetail === "string" && sDetail.includes("some error happens"));
                    })));
                    fnDone();
                    break;
            }
        ...
        this.oPostMessageBus.subscribe("fakeChannel", "fakeEvent", function (oEvent) {
            switch (fnDone.callCount) {
                ...
                case 1:
                    ...
                    assert.ok(oSpy.calledOnce);
                    assert.ok(oSpy.withArgs(sinon.match(function (sMessage, sDetail) {
                        return (typeof sMessage === "string" && sMessage.includes("Some unexpected error happened during post message processing") || typeof sDetail === "string" && sDetail.includes("Some unexpected error happened during post message processing"));
                    })));
                    fnDone();
                    break;
            }
