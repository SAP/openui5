import LibraryInfo from "sap/ui/core/util/LibraryInfo";
import Log from "sap/base/Log";
import URI from "sap/ui/thirdparty/URI";
var ResourcesUtil = {
    getResourceOriginPath: function (sPath) {
        var oConfig, sOrigin, oUri = URI(sPath);
...
QUnit.assert.sameURL = function (actual, expected, message) {
    this.equal(new URI(actual).normalize().toString(), new URI(expected).normalize().toString(), message);
};
