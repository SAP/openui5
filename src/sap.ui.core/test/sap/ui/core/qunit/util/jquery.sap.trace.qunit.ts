import jQuery from "jquery.sap.trace";
...
QUnit.test("Busy indicator without delay", function (assert) {
    ...
    assert.ok(iBusyDurationRequest + 1 >= oMeasurement.busyDuration && iBusyDurationRequest - 1 <= oMeasurement.busyDuration, "Header is filled; FESR entry: " + iBusyDurationRequest + "; Measurement: " + oMeasurement.busyDuration);
    spy.restore();
});
