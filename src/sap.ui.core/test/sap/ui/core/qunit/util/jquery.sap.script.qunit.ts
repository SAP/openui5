...
QUnit.test("number test", function (assert) {
    ...
    assert.equal(jQuery.sap.equal(255, 255), true, "0xff, 255");
...
function testEach(assert, obj) {
    ...
    function callback(i, v) {
        assert.ok(v == null || this == v, "context in the callback should be equal to the current item");
        keys.push(i);
        values.push(v);
    }
...
QUnit.test("arrayDiff", function (assert) {
    ...
    var aDiffResult = jQuery.sap.arrayDiff(aContexts1, aContexts2, function (oOldContext, oNewContext) {
        return jQuery.sap.equal(oOldContext && oLastContextData && oLastContextData[oOldContext], oNewContext && oContextData && oContextData[oNewContext]);
    });
...
QUnit.test("random arrayDiffs", function (assert) {
    for (var t = 0; t < 100; t++) {
        var listA = [], listB = [], listACount = Math.floor(Math.random() * 101), listBCount = Math.floor(Math.random() * 101), aDiff;
...
QUnit.test("arrays with undefined values", function (assert) {
    var a1 = [1, 2, 3, undefined], a2 = [1, undefined], aDiff = jQuery.sap.arraySymbolDiff(a1, a2), aResult = [
        { index: 1, type: "delete" },
        { index: 1, type: "delete" }
    ];
...
QUnit.test("valid expressions", function (assert) {
    [
        ["{}", {}],
        ["{test:'123'}", { test: "123" }],
        ["{test:456.00}", { test: 456 }],
        ["{test:-456}", { test: -456 }],
        ["{test:-456e9}", { test: -456000000000 }],
        ["{test:77E-4}", { test: 0.0077 }],
        ["{test:\"123\"}", { test: "123" }],
        ["{23:'test'}", { 23: "test" }],
        ["{'23':'test'}", { "23": "test" }],
...
QUnit.test("tokenizer with enhancements getCh, getIndex, init, setIndex", function (assert) {
    var oTokenizer = jQuery.sap._createJSTokenizer(), oTokenizer2 = jQuery.sap._createJSTokenizer();
    oTokenizer.init("{='foo'}");
