import fakeService from "./data/JSONModelFakeService";
...
QUnit.test("test model setData", function (assert) {
    var obj1 = { a: true, b: false, c: false }, obj2 = { c: true, d: false }, objMerged = { a: true, b: false, c: true, d: false }, arr1 = ["a", "b", "c"], arr2 = ["d", "e"], arrMerged = ["d", "e", "c"];
...
QUnit.test("test model observation", function (assert) {
    ...
    var oModel = new JSONModel(oData, true), oString = oModel.bindProperty("/string"), oNumber = oModel.bindProperty("/number"), oBool = oModel.bindProperty("/bool"), oDate = oModel.bindProperty("/date"), oObj = oModel.bindProperty("/obj"), oObjName = oModel.bindProperty("/obj/name1"), oArr = oModel.bindProperty("/arr"), oArrName = oModel.bindProperty("/arr/0/name"), bString = false, bNumber = false, bBool = false, bDate = false, bObj = false, bObjName = false, bArr = false, bArrName = false, iChangeCount = 0;
...
QUnit.test("test model setProperty on root", function (assert) {
    var oModel = new JSONModel({ a: 1 }), oTest = { b: 2 };
...
QUnit.test("test JSONListBinding context calculation", function (assert) {
    var oModel = new JSONModel(), oArrayData = [1, 2, 3], aContexts;
