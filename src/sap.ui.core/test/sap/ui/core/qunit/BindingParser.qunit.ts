...
import jQuery0 from "jquery.sap.strings";
...
}, parse = ManagedObject.bindingParser, TestControl = ManagedObject.extend("TestControl", {
    metadata: {
        properties: {
            any: "any",
            text: "string"
...
function checkTextFragments(assert, fnFormatter) {
    assert.strictEqual(fnFormatter.textFragments, "sap.ui.base.BindingParser: composeFormatters", "'textFragments' (mis)used as debug information");
}
QUnit.module("sap.ui.base.BindingParser", {
    beforeEach: function (assert) {
        ...
        this.assertBoundTo = function (fnActual, fnExpected, fnExpectedThis, sMessage) {
            assert.ok(typeof fnActual === "function" && typeof fnExpected === "function" && typeof fnExpected.reset === "function", "(assertion precondition) actual must be a function, expected must be a spy");
            fnExpected.reset();
            fnActual.call({}, "some", 42);
...
QUnit.test("Single Binding with required type", function (assert) {
    var o = parse("{path:'something', type: 'myType'}", null, false, false, true, false, { myType: String });
    assert.strictEqual(typeof o, "object", "parse should return an object");
    assert.strictEqual(o.parts, undefined, "binding info should not be a composite binding info");
    assert.strictEqual(o.path, "something", "path should be as specified");
...
QUnit.test("Filter Operator 'Any' with a valid nested filter (value2)", function (assert) {
    var o = parse("{path:'something', filters: { path: 'someField', operator: 'Any', variable: 'd', condition: { " + "and: true, filters: [" + "{path:'d/someFilterPath', operator:'EQ', value1:'someCompareValue'}," + "{path:'s/someFilterPath2', operator:'BT', value1: 'someCompareValue', value2: 'someOtherCompareValue'}]}}}");
...
QUnit.test("Filter Operator 'All' with a valid nested filter (value2)", function (assert) {
    var o = parse("{path:'something', filters: { path: 'someField', operator: 'All', variable: 'd', condition: { " + "and: true, filters: [" + "{path:'d/someFilterPath', operator:'EQ', value1:'someCompareValue'}," + "{path:'s/someFilterPath2', operator:'BT', value1: 'someCompareValue', value2: 'someOtherCompareValue'}]}}}");
    assert.strictEqual(typeof o, "object", "parse should return a binding info object");
    assert.ok(o.filters instanceof Filter, "binding info should contain a single filter");
...
QUnit.test("Filter Operator 'Any' with multiple nested filters (negative test)", function (assert) {
    assert.throws(function () {
        parse("{path:'something', filters: { path: 'someField', operator: 'Any', variable: 'd', condition: [" + "{path:'d/someFilterPath', operator:'EQ', value1:'someCompareValue'}," + "{path:'s/someFilterPath2', operator:'BT', value1: 'someCompareValue', value2: 'someOtherCompareValue'}]}}");
...
QUnit.test("Single Binding with deeply nested filters", function (assert) {
    var o = parse("{path:'something', filters: { and:false, filters: [" + "{path:'someFilterPath1', operator:'EQ', value1:'someCompareValue1'}," + "{and: true, filters: [" + "{path:'someFilterPath1', operator:'EQ', value1: 'someCompareValue2'}," + "{path:'someFilterPath2', operator:'EQ', value1: 'someCompareValue3'}" + "]}" + "]}}");
    assert.strictEqual(typeof o, "object", "parse should return an object");
    assert.ok(o.filters instanceof Filter, "parse should return the specified filter");
    assert.ok("bAnd" in o.filters, "filter should have a property 'bAnd'");
    assert.notStrictEqual(o.filters.bAnd, false, "filter should have bAnd == false (=or)");
...
[{
    ...
    QUnit.test("Expression binding: " + oFixture.expression, function (assert) {
        var oBindingInfo, oContext = {}, oParseResult = {
            result: {
                formatter: function () { },
        ...
        this.mock(ExpressionParser).expects("parse").withExactArgs(sinon.match.func, oFixture.expression, 2, null, sinon.match.same(oContext)).returns(oParseResult);
...
QUnit.test("Expression binding: propagate ExpressionParser exception", function (assert) {
    var oError = new Error("error message"), sInput = "{=invalid}";
    this.mock(ExpressionParser).expects("parse").withExactArgs(sinon.match.func, sInput, 2, null, null).throws(oError);
    assert.throws(function () {
        parse(sInput);
    }, oError);
});
QUnit.test("Expression binding: expression binding must end with }", function (assert) {
    var sInput = "{='foo',}", sMsg = "Expected '}' and instead saw ',' in expression binding {='foo',} at position 7";
    this.mock(ExpressionParser).expects("parse").withExactArgs(sinon.match.func, sInput, 2, null, null).returns({ at: sInput.length - 2, result: {} });
    assert.throws(function () {
        parse(sInput);
...
QUnit.test("mergeParts w/o root formatter", function (assert) {
    ...
    }, oModel = new JSONModel({ bar: "world", foo: "hello" }), oControl = new TestControl({
        models: oModel
    });
...
QUnit.test("mergeParts w/o any formatter", function (assert) {
    ...
    BindingParser.mergeParts(oBindingInfo);
    assert.deepEqual(oBindingInfo.parts, [{ path: "/foo" }, { path: "/bar" }, { path: "/bar" }, { path: "/foo" }]);
    oControl.bindProperty("text", oBindingInfo);
...
QUnit.test("mergeParts w/ root formatter JSON.stringify", function (assert) {
    ...
    }, oModel = new JSONModel({ bar: "world", foo: "hello", forty2: 42 }), oControl = new TestControl({
        models: oModel
    });
    BindingParser.mergeParts(oBindingInfo);
    oControl.bindProperty("text", oBindingInfo);
...
QUnit.test("mergeParts fails for unsupported properties, error is logged", function (assert) {
    var sBinding = "{:= ${parts:[{path:'/foo'},{path:'/bar'}],type:'sap.ui.model.type.Currency'} }", oBindingInfo;
    this.oLogMock.expects("error").withExactArgs("Cannot merge parts: Unsupported property: type", sBinding, "sap.ui.base.BindingParser");
    oBindingInfo = parse(sBinding);
    assert.strictEqual(oBindingInfo.parts.length, 1, "# embedded bindings in composite binding");
...
QUnit.test("mergeParts with constants and part with empty path", function (assert) {
    var oBindingInfo = {
        ...
        undefined,
        []
    ], oModel = new JSONModel({ bar: "world", baz: "moon", foo: "hello" }), oControl = new TestControl({
        models: oModel
    });
    BindingParser.mergeParts(oBindingInfo);
    assert.deepEqual(oBindingInfo.parts, [{ path: "/foo" }, { path: "/foo" }, { path: "/bar" }, { path: "" }]);
    oControl.bindProperty("text", oBindingInfo);
...
QUnit.test("mergeParts with single constant", function (assert) {
    ["", false, 0, null, undefined, []].forEach(function (vConstant) {
        var oBindingInfo = { parts: [vConstant] }, oControl = new TestControl();
        BindingParser.mergeParts(oBindingInfo);
        assert.deepEqual(oBindingInfo.parts, []);
        checkTextFragments(assert, oBindingInfo.formatter);
...
QUnit.test("functionsNotFound", function (assert) {
    var sBinding = "{path:'/foo',formatter:'foo'} {path:'/bar',formatter:'.bar'}", oBindingInfo;
    ...
    parse(sBinding, null, true, false);
...
QUnit.test("Expression binding: usage in composite binding", function (assert) {
    var oBindingInfo, oControl = new TestControl(), oModel = new JSONModel({ p1: 0, p2: "foo", p3: "bar", p4: 1 });
    ...
    assert.strictEqual(oBindingInfo.parts.length, 4, "# embedded bindings in composite binding");
    ...
    oControl.bindProperty("text", oBindingInfo);
    assert.strictEqual(oControl.getText(), "prefix 10 foo", "composite binding with simple binding syntax: model>path");
    oBindingInfo = parse("prefix {= ${path:'/p1'} + 10} {path:'/p2'}");
    oControl.bindProperty("text", oBindingInfo);
    assert.strictEqual(oControl.getText(), "prefix 10 foo", "composite binding with complex binding syntax: path");
    oBindingInfo = parse("prefix {= ${model:'model',path:'/p1'} + 10} {path:'/p2'}");
    oControl.bindProperty("text", oBindingInfo);
    assert.strictEqual(oControl.getText(), "prefix 10 foo", "composite binding with complex binding syntax: model");
    oBindingInfo = parse("prefix {= 10} {/p2}");
    oControl.bindProperty("text", oBindingInfo);
    assert.strictEqual(oControl.getText(), "prefix 10 foo", "prefix 10 foo");
    oBindingInfo = parse("prefix {parts:[{path:'/p1'},{path:'/p2'},{path:'/p3'}]," + "formatter:'Global.joiningFormatter'}");
    oControl.bindProperty("text", oBindingInfo);
...
QUnit.test("Expression binding: use global context", function (assert) {
    var oBindingInfo, oControl = new TestControl(), oModel = new JSONModel({ value: 99, p1: "foo", p2: "bar" });
    oControl.setModel(oModel);
    oBindingInfo = parse("{= module1.check(${/value}) ? ${/p1} : ${/p2}}", null, false, false, false, false, oGlobalContext);
    oControl.bindProperty("text", oBindingInfo);
    assert.strictEqual(oControl.getText(), "bar", "bar");
    oControl.destroy();
});
QUnit.test("parseExpression: uses Expression.parse", function (assert) {
    var sInput = "foo", iStart = 0, oResult = {};
    this.mock(ExpressionParser).expects("parse").withExactArgs(sinon.match.func, sInput, iStart, undefined).returns(oResult);
...
QUnit.test("parseExpression: oEnv, mGlobals", function (assert) {
    var oEnv = {}, mGlobals = {}, sInput = "foo", iStart = 0, oResult = {};
    this.mock(ExpressionParser).expects("parse").withExactArgs(sinon.match.func, sInput, iStart, sinon.match.same(mGlobals)).returns(oResult);
...
QUnit.test("parseExpression: resolving", function (assert) {
    var sExpression = "[${/blue}?'blue':'red']", oInvisibleText = new InvisibleText(), oResult = BindingParser.parseExpression(sExpression, 1), oModel = new JSONModel({ blue: false });
...
QUnit.test("Expression binding: one time binding", function (assert) {
    var oModel = new JSONModel({ blue: false }), oInvisibleText = new InvisibleText({ models: oModel });
...
QUnit.test("Local functions are bound to context", function (assert) {
    var sBinding = "{path : '/', formatter : '.foo'}", oBindingInfo, oScope = {
        foo: function () {
            assert.strictEqual(this, oBindingInfo, "no Function#bind() used");
        }
    };
    oBindingInfo = parse(sBinding, oScope, false, false, true);
    oBindingInfo.formatter();
});
QUnit.test("Scope access w/o dot", function (assert) {
    var sBinding1 = "{path : '/', formatter : 'foo'}", sBinding2 = "{path : '/', formatter : 'Global.formatter'}", oBindingInfo, oScope = {
        foo: function () { }
    };
    oBindingInfo = parse(sBinding1, oScope, false, true, true);
    assert.strictEqual(oBindingInfo.formatter, undefined);
    assert.deepEqual(oBindingInfo.functionsNotFound, ["foo"]);
    oBindingInfo = parse(sBinding1, oScope, false, false, true, true);
    assert.strictEqual(oBindingInfo.formatter, oScope.foo);
    oBindingInfo = parse(sBinding2, oScope, false, false, false, true);
...
QUnit.test("Scope access w/o dot by given static context", function (assert) {
    var sBinding1 = "{path : '/', formatter : 'foo'}", sBinding2 = "{path : '/', formatter : 'Global.formatter'}", sBinding3 = "{path : '/', formatter : 'module1.formatter'}", sBinding4 = "{path : '/', formatter : 'module1.fn'}", sBinding5 = "{path : '/', formatter : 'module1.ns.fn'}", sBinding6 = "{path : '/', formatter : 'Global.ns.global'}", sBinding7 = "{path : '/', formatter : 'formatter'}", oBindingInfo, oScope = {
        foo: function () { }
    };
    oBindingInfo = parse(sBinding1, oScope, false, true, false, false, oGlobalContext);
    assert.strictEqual(oBindingInfo.formatter, undefined);
    assert.deepEqual(oBindingInfo.functionsNotFound, ["foo"]);
    oBindingInfo = parse(sBinding1, oScope, false, true, true, true, oGlobalContext);
    assert.strictEqual(oBindingInfo.formatter, oScope.foo);
    assert.deepEqual(oBindingInfo.functionsNotFound, undefined);
    oBindingInfo = parse(sBinding2, oScope, false, true, false, false, oGlobalContext);
    assert.strictEqual(oBindingInfo.formatter, undefined);
    assert.deepEqual(oBindingInfo.functionsNotFound, ["Global.formatter"]);
    oBindingInfo = parse(sBinding3, oScope, false, false, false, false, oGlobalContext);
    assert.strictEqual(oBindingInfo.formatter.toString(), oGlobalContext.module1.formatter.bind(oGlobalContext.module1).toString());
    assert.deepEqual(oBindingInfo.functionsNotFound, undefined);
    oBindingInfo = parse(sBinding4, oScope, false, false, false, false, oGlobalContext);
    assert.strictEqual(oBindingInfo.formatter.toString(), oGlobalContext.module1.fn.bind(oGlobalContext.module1).toString());
    oBindingInfo.formatter();
    assert.strictEqual(enclosingContext, oGlobalContext.module1);
    assert.deepEqual(oBindingInfo.functionsNotFound, undefined);
    oBindingInfo = parse(sBinding5, oScope, false, false, false, false, oGlobalContext);
    assert.strictEqual(oBindingInfo.formatter.toString(), oGlobalContext.module1.ns.fn.bind(oGlobalContext.module1.ns.fn).toString());
    oBindingInfo.formatter();
    assert.strictEqual(enclosingContext, oGlobalContext.module1.ns);
    assert.deepEqual(oBindingInfo.functionsNotFound, undefined);
    oBindingInfo = parse(sBinding6, oScope, false, true, false, false, oGlobalContext);
    assert.strictEqual(oBindingInfo.formatter, undefined);
    assert.deepEqual(oBindingInfo.functionsNotFound, ["Global.ns.global"]);
    oBindingInfo = parse(sBinding6, oScope, false, false, false, false);
    assert.strictEqual(oBindingInfo.formatter.toString(), Global.ns.global.toString());
    oBindingInfo.formatter();
    assert.strictEqual(enclosingContext, oBindingInfo);
    assert.deepEqual(oBindingInfo.functionsNotFound, undefined);
    oBindingInfo = parse(sBinding7, oScope, false, false, false, false, oGlobalContext);
    assert.strictEqual(oBindingInfo.formatter, oGlobalContext.formatter, "function module");
    assert.deepEqual(oBindingInfo.functionsNotFound, undefined);
    enclosingContext = null;
});
QUnit.test("Expression binding with embedded composite binding", function (assert) {
    var sBinding = "{:= ${parts:['m2>/foo',{path:'/bar'}],formatter:'Global.joiningFormatter'} }", oModel = new JSONModel({ "bar": 1 }), oModel2 = new JSONModel({ "foo": 0 }), oInvisibleText = new InvisibleText({ models: { undefined: oModel, "m2": oModel2 } });
...
QUnit.test("Single expression binding, no mergeParts needed", function (assert) {
    var oModel = new JSONModel({ "foo": 0, "bar": 1 }), oInvisibleText = new InvisibleText({ models: oModel });
...
QUnit.test("BindingParser.parseExpression", function (assert) {
    var sInput = "[${namespace==='GWSAMPLE_BASIC']/entityType", sMessage = "no closing braces found in '" + sInput + "' after pos:2";
    this.oLogMock.expects("error").withExactArgs(sMessage, undefined, "sap.ui.base.ExpressionParser");
    assert.throws(function () {
