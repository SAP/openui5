...
import ShortcutHintsMixin from "sap/ui/core/ShortcutHintsMixin";
...
var sView = "<mvc:View xmlns:my=\"lib.my\" xmlns:mvc=\"sap.ui.core.mvc\" xmlns:core=\"sap.ui.core\" xmlns=\"sap.m\" controllerName=\"my.hints.ShortcutHints\" displayBlock=\"true\">" + "<App id=\"commands\">" + "<Page id=\"page\" title=\"Commands\">" + " <dependents>" + "  <core:CommandExecution id=\"CE_SAVE\" command=\"Save\" enabled=\"true\" execute=\".onSave\" />" + " </dependents>" + " <Panel id=\"myPanel\">" + "  <my:MyControl id=\"myControl\" myEvent=\"cmd:MyCommand\" />" + "  <my:MyControl id=\"myControl2\" myEvent=\"cmd:Save\" />" + "  <Button id=\"b1\" icon=\"sap-icon://save\" press=\"cmd:Save\" />" + "  <Button id=\"b2\" text=\"Save\" tooltip=\"User Tooltip\" press=\"cmd:Save\" />" + "  <Button id=\"b3\" icon=\"sap-icon://save\" tooltip=\"User Tooltip\" press=\"cmd:Save\" />" + " </Panel>" + "</Page>" + "</App>" + "</mvc:View>";
...
QUnit.test("add command then add config", function (assert) {
    return waitForViewReady().then(function (oView) {
        ...
        assert.ok(!ShortcutHintsMixin.isControlRegistered(oCtrl.getId()), "The control is not registered to show a shortcut");
        assert.ok(!ShortcutHintsMixin.isDOMIDRegistered(oCtrl.getId()), "The root DOM node is not registered to show a shortcut");
        ShortcutHintsMixin.addConfig(oCtrl, { event: "myEvent" }, oCtrl);
        assert.ok(ShortcutHintsMixin.isControlRegistered(oCtrl.getId()), "The control is registered to show a shortcut");
        assert.ok(ShortcutHintsMixin.isDOMIDRegistered(oCtrl.getId()), "The correct DOM node is registered to show a shortcut");
    });
});
...
QUnit.test("add config then add command", function (assert) {
    var oComponent;
    var oStub = sinon.stub(lib.my.MyControl.prototype, "init").callsFake(function () {
...
QUnit.test("add config then add command with an event that already has event handlers", function (assert) {
    ...
    assert.ok(ShortcutHintsMixin.isControlRegistered(oMyControl.getId()), "The control is registered to show a shortcut");
    oMyControl.destroy();
});
...
QUnit.test("DOM events when control is destroyed and re-created", function (assert) {
    return waitForViewReady().then(function (oView) {
        ...
        ShortcutHintsMixin.addConfig(oCtrl, { event: "myEvent" }, oCtrl);
        oCtrl.destroy();
        assert.notOk(ShortcutHintsMixin.isControlRegistered(oCtrl.getId()), "the control id is removed from the hints registry");
        assert.notOk(ShortcutHintsMixin.isDOMIDRegistered(oCtrl.getId()), "the dom node id is removed from the hints dom registry");
        Fragment.load({
            type: "XML",
...
QUnit.test("use id suffix", function (assert) {
    return waitForViewReady().then(function (oView) {
        ...
        assert.ok(ShortcutHintsMixin.isControlRegistered(oCtrl.getId()), "The control is registered to show a shortcut");
        assert.ok(ShortcutHintsMixin.isDOMIDRegistered(oCtrl.getId() + "-inner"), "The correct DOM node is registered to show a shortcut");
    });
});
QUnit.test("use different hint provider", function (assert) {
    return waitForViewReady().then(function (oView) {
        ...
        oView.placeAt("qunit-fixture");
        sap.ui.getCore().applyChanges();
        assert.ok(ShortcutHintsMixin.isDOMIDRegistered(oCtrl.getId()), "The control is registered to show a shortcut");
        assert.equal(oCtrl.getDomRef().getAttribute("aria-keyshortcuts"), "Ctrl+S", "The hint provider is OK");
    });
});
...
QUnit.test("sap.m.Button accessibility", function (assert) {
    return waitForViewReady().then(function (oView) {
        var oButton1 = oView.byId("b1");
        var oButton2 = oView.byId("b2");
        var oButton3 = oView.byId("b3");
        oView.placeAt("qunit-fixture");
        sap.ui.getCore().applyChanges();
        assert.ok(!oButton1.getDomRef().getAttribute("title"), "no native tooltip");
        assert.equal(oButton1.getDomRef().getAttribute("aria-keyshortcuts"), "Ctrl+S", "has hint accessibility");
        assert.ok(!oButton2.getDomRef().getAttribute("title"), "no native tooltip");
        assert.equal(oButton2.getDomRef().getAttribute("aria-keyshortcuts"), "Ctrl+S", "has hint accessibility");
        assert.ok(!oButton3.getDomRef().getAttribute("title"), "no native tooltip");
        assert.equal(oButton3.getDomRef().getAttribute("aria-keyshortcuts"), "Ctrl+S", "has hint accessibility");
    });
});