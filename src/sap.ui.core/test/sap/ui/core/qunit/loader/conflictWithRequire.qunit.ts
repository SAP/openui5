...
QUnit.module("", {
    before: function (assert) {
        return requireP(["sap/ui/thirdparty/require"]).then(function () {
...
QUnit.test("Complex test: load sap.viz", function (assert) {
    return requireP(["sap/ui/core/Core", "sap/ui/thirdparty/jquery"]).then(function (imports) {
        return Promise.resolve(imports[1].ajax({
            url: sap.ui.require.toUrl("sap/viz/library.js"),
            method: "HEAD"
        })).then(function () {
            sap.ui.getCore().boot();
            return sap.ui.getCore().loadLibrary("sap.viz", { async: true }).then(function () {
                return new Promise(function (resolve, reject) {
