...
QUnit.test("Should not call pattern matched only matched", function (assert) {
    var fnMatchedSpy = this.spy(), fnPatternMatchedSpy = this.spy();
    this.oRoute.attachMatched(fnMatchedSpy);
...
function fnRouteEventsTestCase(sTestName, sEventName) {
    QUnit.test(sTestName, function (assert) {
        var sName = "testRoute", aTargetNames = ["async1", "async2", "async3"], oRoute = new Route(this.oRouterStub, { name: sName, target: aTargetNames }), oListener = {}, oData = { some: "data" }, fnEventSpy = this.spy(function (oEvent, oActualData) {
...
function fnDetachRouteEventTestCase(sTestName, sEventName) {
    QUnit.test(sTestName, function (assert) {
        var sName = "testRoute", aTargetNames = ["async1", "async2", "async3"], fnRouteMatchedSpy = this.spy(), fnEventSpy = this.spy(), oListener = {};
        this.oRouterStub.fireRouteMatched = fnRouteMatchedSpy;
        this.oRouterStub.fireRoutePatternMatched = fnRouteMatchedSpy;
        var oRoute = new Route(this.oRouterStub, { name: sName, target: aTargetNames });
