...
QUnit.test("Calling 'navTo' method does not trigger any hash change when the router is stopped", function (assert) {
    ...
    assert.expect(4);
    var oSetHashSpy = this.spy(HashChanger.getInstance(), "setHash"), done = assert.async(), oApp = new App(), oRouter = fnCreateRouter([
        {
            name: "home",
            pattern: "",
...
QUnit.test("Should initialize the router instance", function (assert) {
    var parseSpy, router = fnCreateRouter();
...
QUnit.test("Should fire switched event on the last matched route when stopping the router instance", function (assert) {
    ...
    }), oRoute = router.getRoute("home"), oSwitchedSpy = this.spy();
...
QUnit.test("Should destroy the routers dependencies", function (assert) {
    ...
    var oTargets = oRouter.getTargets(), oViews = oRouter.getViews(), oRoute = oRouter.getRoute("myRoute");
    oRouter.destroy();
...
QUnit.test("Should log a warning if a router gets destroyed while the hash changes", function (assert) {
    var oWarningSpy = this.stub(Log, "warning"), oFirstRouter = fnCreateRouter({
        "matchingRoute": {
            pattern: "matches"
        }
...
QUnit.test("Should not match the undefined pattern", function (assert) {
    ...
    this.stub(router._oViews, "_getViewWithGlobalId").callsFake(function () {
        return createView(["<View xmlns=\"sap.ui.core.mvc\">", "</View>"]);
    });
...
QUnit.test("Should create a greedy route", function (assert) {
    var sPattern = "product", oRouter = fnCreateRouter([
        {
            name: "first",
            pattern: sPattern
        },
    ...
    var aRoutes = [oRouter.getRoute("first"), oRouter.getRoute("parent"), oRouter.getRoute("child")], aListenerSpies = [this.spy(), this.spy(), this.spy()], aRouteMatchedSpies = [];
...
QUnit.test("Should create a greedy route", function (assert) {
    ...
    var aRoutes = [oRouter.getRoute("first"), oRouter.getRoute("second"), oRouter.getRoute("last")], aListenerSpies = [this.spy(), this.spy(), this.spy()], aRouteMatchedSpies = [];
    aRoutes.forEach(function (oRoute, i) {
        oRoute.attachPatternMatched(aListenerSpies[i]);
        aRouteMatchedSpies.push(this.spy(oRoute, "_routeMatched"));
    }, this);
...
QUnit.test("Should attach to a route", function (assert) {
    var spy = this.spy(), router = fnCreateRouter([{
            name: "name",
            pattern: ""
        }]);
    var oRoute = router.getRoute("name"), oSpy = sinon.spy(oRoute, "_routeMatched");
...
QUnit.test("Should attach to a route using getRoute", function (assert) {
    var spy = this.spy(), oRouter = fnCreateRouter([{
            name: "name",
            pattern: ""
...
QUnit.test("Should go to a route", function (assert) {
    var callCount = 0, aArguments = [], matched = function (oEvent) {
...
QUnit.test("Should go to a route", function (assert) {
    var beforeCallCount = 0, callCount = 0, patternCallCount = 0, aArguments = [], bParentCallFirst = false, beforeMatched = function (oEvent) {
        beforeCallCount++;
...
QUnit.test("Should go to a route with target set to empty array", function (assert) {
    ...
    var oRouteMatchedEventSpy = sinon.spy(), oMatchedEventSpy = sinon.spy(), oRouteMatchedSpy = sinon.spy(oRoute, "_routeMatched");
...
QUnit.test("should be able to fire/attach/detach the created event", function (assert) {
    ...
    }), oFireReturnValue, oDetachReturnValue, oAttachReturnValue = this.oRouter.attachViewCreated(oData, fnEventSpy, oListener);
...
QUnit.test("Should fire the view created event if a view is created", function (assert) {
    var sViewType = ViewType.XML, sViewName = "foo", oParameters, fnEventSpy = this.spy(function (oEvent) {
...
QUnit.test("should be able to fire/attach/detach the titleChanged event", function (assert) {
    var oParameters = { foo: "bar" }, oListener = {}, oData = { some: "data" }, fnEventSpy = this.spy(function (oEvent, oActualData) {
        ...
        assert.strictEqual(oEvent.getParameters(), oParameters, "the parameters are correct");
        assert.strictEqual(this, oListener, "the this pointer is correct");
    }), oFireReturnValue, oDetachReturnValue, oAttachReturnValue;
    this.oRouterConfig = {
        routeName: {
...
QUnit.test("Should fire the titleChanged event if the matched route has a target with title defined", function (assert) {
    var oParameters, fnEventSpy = this.spy(function (oEvent) {
        oParameters = oEvent.getParameters();
    }), oRouteMatchedSpy;
    this.oRouterConfig = {
        routeName: {
...
QUnit.test("Should fire the titleChanged only on the active route", function (assert) {
    var oParameters, fnEventSpy = this.spy(function (oEvent) {
        oParameters = oEvent.getParameters();
    }), sTitle = "title", sTitle1 = "title1", sPattern = "pattern", sPattern1 = "pattern1", oTarget, oRouteMatchedSpy, oSequencePromise;
    this.oRouterConfig = {
        route1: {
            pattern: sPattern,
            target: "target"
...
QUnit.test("Should create a view with an component", function (assert) {
    var oUIComponent = new UIComponent({}), fnOwnerSpy = this.spy(oUIComponent, "runAsOwner"), oRouter = fnCreateRouter({}, {}, oUIComponent);
...
QUnit.test("Should attach and detach the bypassed event", function (assert) {
    var oListener = {}, oData = {}, fnBypassed = this.spy(function (oEvent, oDataInner) {
        var oParameters = oEvent.getParameters();
...
QUnit.test("Should create targets in the bypassed event", function (assert) {
    ...
    var done = assert.async(), fnBypassed = this.spy(function () {
...
QUnit.test("slash should be optional when it's between ')' and ':'", function (assert) {
    var callCount = 0, fnMatched = function (oEvent) {
...
QUnit.test("Correctly parse the hash with skipped optional params", function (assert) {
    ...
    var oRouteMatchedSpy = this.spy(oRoute, "_routeMatched");
    oRouter.initialize();
    oRouter.parse("test/1//3");
    oRouter.parse("test//2//4");
    oRouter.parse("test///3");
    oRouter.parse("test/1///4");
    oRouter.parse("test////4");
...
QUnit.test("fire events", function (assert) {
    var oParentRouteMatchedEvent, oParentRouteMatchedEventSpy = sinon.spy(function (oEvent) {
        oParentRouteMatchedEvent = jQuery.extend(true, {}, oEvent);
    }), oParentRoutePatternMatchedEventSpy = sinon.spy(), oChildRouteMatchedEvent, oChildRouteMatchedEventSpy = sinon.spy(function (oEvent) {
        oChildRouteMatchedEvent = jQuery.extend(true, {}, oEvent);
    }), oChildRoutePatternMatchedEventSpy = sinon.spy(), oParentRoute = this.oParentComponent.getRouter().getRoute("category"), oChildRoute = this.oChildComponent.getRouter().getRoute("product"), oParentRouteMatchedSpy = sinon.spy(oParentRoute, "_routeMatched"), oChildRouteMatchedSpy = sinon.spy(oChildRoute, "_routeMatched");
...
QUnit.test("nesting for multiple components", function (assert) {
    var that = this, iNumberOfComponents = 3, aComponents = [], aComponentInstances = [], aRoutes = [], aRouteMatchedSpies = [], aRouteMatchedEvents = [], aRouteMatchedEventSpies = [], aRoutePatternMatchedSpies = [], fnDeclareComponent = function (i, iDepth) {
...
function waitTillRouteMatched(oRouter, sRoute) {
    var oRoute = oRouter.getRoute(sRoute), fnRouteMatched;
    if (oRoute) {
        return new Promise(function (resolve, reject) {
...
QUnit.test("Should load and instantiate the nested component when the home route is matched", function (assert) {
    var that = this, oRouter = this.oParentComponent.getRouter();
    return new Promise(function (resolve, reject) {
        oRouter.getRoute("home").attachMatched(function () {
            var oContainer = that.oParentComponent.getRootControl(), oShell = oContainer.byId("shell");
...
QUnit.test("Should suspend a dynamic displayed target when router navigates away from it - routeRelevant: true", function (assert) {
    ...
    var that = this, oRouter = this.oParentComponent.getRouter(), sDynamicComponentName = "DynamicChildComponent" + count;
...
QUnit.test("Should suspend a dynamic displayed target and its parent when router navigates away from it - routeRelevant: true", function (assert) {
    ...
    var that = this, oRouter = this.oParentComponent.getRouter(), sDynamicComponentName = "DynamicChildComponent" + count, sParentTarget = sDynamicComponentName + "Parent";
    var oResetHashSpy = sinon.spy(oRouter.getHashChanger(), "resetHash");
    return waitTillRouteMatched(oRouter, "home").then(function (oParams) {
        var oContainer = that.oParentComponent.getRootControl(), oShell = oContainer.byId("shell");
        assert.equal(oShell.getContent().length, 1, "The nested component is loaded and placed into the aggregation");
...
QUnit.test("Call navTo with specific route and parameter for nested component", function (assert) {
    ...
    var that = this, oRouter = this.oParentComponent.getRouter(), iHomeRouteMatchCount = 0;
    var pHomeRouteMatched = new Promise(function (resolve, reject) {
        var fnMatched = function () {
            ...
            var oContainer = that.oParentComponent.getRootControl(), oShell = oContainer.byId("shell"), oControl = oShell.getContent()[0];
    ...
    return pHomeRouteMatched.then(function (oNestedComponent) {
        ...
        var oNestedRouter = oNestedComponent.getRouter(), sId = "productA";
        oRouter.navTo("home", {}, {
            home: {
                route: "product",
                parameters: {
...
QUnit.test("Call navTo with specific route and parameter for deep nested component", function (assert) {
    ...
    var pHomeRouteMatched = new Promise(function (resolve, reject) {
        oRouter.getRoute("home").attachMatched(function () {
            iHomeRouteMatchCount++;
            var oContainer = that.oParentComponent.getRootControl(), oShell = oContainer.byId("shell"), oNestedComponent = oShell.getContent()[0].getComponentInstance();
            resolve(oNestedComponent);
        });
    });
    return pHomeRouteMatched.then(function (oNestedComponent) {
        ...
        return new Promise(function (resolve, reject) {
            oNestedRouter.getRoute("product").attachMatched(function (oEvent) {
                assert.equal(iHomeRouteMatchCount, 1, "home route is still matched only once");
                var oParameters = oEvent.getParameter("arguments"), aViews = oEvent.getParameter("views");
                assert.equal(oParameters.id, sId, "correct route is matched with parameter");
                ...
                var oNestedComponent = aViews[0].getComponentInstance(), oRouter = oNestedComponent.getRouter();
    ...
    return pHomeRouteMatched.then(function (oNestedComponent) {
        ...
        return new Promise(function (resolve, reject) {
            oNestedRouter.getRoute("product").attachMatched(function (oEvent) {
                ...
                assert.equal(aViews.length, 1, "A target instance is created");
                assert.ok(aViews[0] instanceof sap.ui.core.ComponentContainer, "The target instance is an ComponentContainer");
                var oDeepNestedComponent = aViews[0].getComponentInstance(), oDeepNestedRouter = oDeepNestedComponent.getRouter();
...
QUnit.test("Order of routeMatched events in nested components", function (assert) {
    var done = assert.async(), oParentRouter = this.oParentComponent.getRouter();
...
QUnit.test("Propagate titleChange event from nested component", function (assert) {
    var done = assert.async(), that = this, oParentRouter = this.oParentComponent.getRouter();
    var oExpected1 = {
        "history": [],
        "nestedHistory": [{
                "history": [{
    ...
    oHomeRouteMatchedSpy.getCall(0).returnValue.then(function (oObject) {
        ...
        var oNestedComponent1 = oObject.view.getComponentInstance(), oNestedComponent1Router = oNestedComponent1.getRouter();
...
QUnit.test("NestedComponent has no routing enabled", function (assert) {
    var oParentRouter = this.oParentComponent.getRouter(), oRouterInitializeSpy = sinon.spy(Router.prototype, "initialize");
    var oHomeRoute = oParentRouter.getRoute("home");
    var oHomeRouteMatchedSpy = sinon.spy(oHomeRoute, "_routeMatched");
    oParentRouter.initialize();
...
QUnit.test("NestedComponent is using router another router", function (assert) {
    var oParentRouter = this.oParentComponent.getRouter(), oParentRouterInitializeSpy = sinon.spy(oParentRouter, "initialize"), oParentRouterStopSpy = sinon.spy(oParentRouter, "stop"), oGetRouterStub = sinon.stub(UIComponent.prototype, "getRouter").callsFake(function () {
                    ...
                    metadata: {
                        ...
                        routing: {
                            ...
                            targets: {
                                ...
                                nestedComponent: {
                                    name: "namespace.nested.ChildComponent",
                                    type: "Component",
                                    id: "componentNested",
...
QUnit.test("Configuration of title propagation - Scenario 1", function (assert) {
    var done = assert.async(), oLevel0TitleChangedSpy = sinon.spy(), ParentComponent = this._createParentComponent(4, undefined, undefined, true, 1, 2);
    ...
    oLevel0HomeRouteMatchedSpy.getCall(0).returnValue.then(function () {
        ...
        oLevel0SecondRouteMatchedSpy.getCall(0).returnValue.then(function (oObject) {
            ...
            var oLevel1Component = oObject.view.getComponentInstance(), oLevel1Router = oLevel1Component.getRouter();
...
QUnit.test("Configuration of title propagation - Scenario 2", function (assert) {
    var done = assert.async(), oLevel0TitleChangedSpy = sinon.spy(), ParentComponent = this._createParentComponent(2, undefined, true, undefined, 3);
    this._createChildWithGlobalTitlePropagation(3, false, true);
    var oLevel0Component = new ParentComponent("parent");
    var oLevel0Router = oLevel0Component.getRouter();
    var oLevel0HomeRouteMatchedSpy = sinon.spy(oLevel0Router.getRoute("home"), "_routeMatched");
    oLevel0Router.attachTitleChanged(oLevel0TitleChangedSpy);
...
QUnit.test("Configuration of title propagation - Scenario 3", function (assert) {
    var done = assert.async(), ParentComponent = this._createParentComponent(1, undefined, true, undefined, 5), oLevel0TitleChangedSpy = sinon.spy();
...
QUnit.test("Configuration of title propagation - Scenario 4: multiple targets with the same title", function (assert) {
    var done = assert.async(), oLevel0TitleChangedSpy = sinon.spy(), ParentComponent = this._createParentComponent(4, true, undefined, undefined, 6, 7, "MyTargetTitle", "MyTargetTitle");
...
QUnit.test("Configuration of title propagation - Scenario 5: parent target has no title but nested component target - attach to event after initialize of the router", function (assert) {
    var done = assert.async(), oLevel0TitleChangedSpy = sinon.spy(), ParentComponent = this._createParentComponent(5, true, undefined, undefined, 8, 9, null, "MyTargetTitle");
