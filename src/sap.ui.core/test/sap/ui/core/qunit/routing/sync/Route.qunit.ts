...
QUnit.test("Should fired beforeMatched before matched", function (assert) {
    ...
    var fnMatchedSpy = this.spy(), fnBeforeMatchedSpy = this.spy(function () {
...
QUnit.test("Should not call pattern matched only matched", function (assert) {
    var oRoute = new Route(oRouterStub, { name: "testRoute" }), fnMatchedSpy = this.spy(), fnPatternMatchedSpy = this.spy();
    oRoute.attachMatched(fnMatchedSpy);
...
function fnRouteEventsTestCase(sTestName, sEventName) {
    QUnit.test(sTestName, function (assert) {
        var sName = "testRoute", oListener = {}, oData = { some: "data" }, fnEventSpy = this.spy(function (oEvent, oActualData) {
        ...
        this.oRoute._routeMatched({}, true);
        this.clock.tick(0);
...
function fnDetachRouteEventTestCase(sTestName, sEventName) {
    QUnit.test(sTestName, function (assert) {
        var fnEventSpy = this.spy(), oListener = {};
