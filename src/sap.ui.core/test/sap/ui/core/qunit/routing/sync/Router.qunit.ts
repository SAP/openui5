...
QUnit.test("Should initialize the router instance", function (assert) {
    var parseSpy, router = new Router({}, {}, null, {});
...
QUnit.test("Should stop the router instance", function (assert) {
    var parseSpy, router = new Router();
    parseSpy = this.spy(router, "parse");
    hasher.setHash("");
    router.initialize();
    assert.strictEqual(parseSpy.callCount, 1, "did notify for initial hash");
...
QUnit.test("Should log a warning if a router gets destroyed while the hash changes", function (assert) {
    var oWarningSpy = this.stub(Log, "warning").callsFake(jQuery.noop), oFirstRouter = new Router({
        "matchingRoute": {
            pattern: "matches"
        }
...
QUnit.test("Should create a greedy route", function (assert) {
    var sPattern = "product", oRouter = new Router([
        {
            name: "first",
            pattern: sPattern
        },
...
QUnit.test("Should attach to a route", function (assert) {
    var spy = this.spy(), router = new Router([{
            name: "name",
            pattern: ""
        }]);
...
QUnit.test("Should attach to a route using getRoute", function (assert) {
    var spy = this.spy(), oRouter = new Router([{
            name: "name",
            pattern: ""
        }]);
...
QUnit.test("Should create a view", function (assert) {
    var that = this, fnStub = this.stub(View, "_create").callsFake(function (oViewOptions) {
...
QUnit.test("Should set a view to the cache", function (assert) {
    var that = this, oReturnValue, fnStub = this.stub(View, "_create").callsFake(function () {
        return that.oView;
    });
...
QUnit.test("Should fire the view created event if a view is created", function (assert) {
    var oView = createXmlView(), sViewType = "XML", sViewName = "foo", oParameters, fnEventSpy = this.spy(function (oEvent) {
...
QUnit.test("Should fire the titleChanged only on the active route", function (assert) {
    var oParameters, fnEventSpy = this.spy(function (oEvent) {
        oParameters = oEvent.getParameters();
    }), sTitle = "title", sTitle1 = "title1", sPattern = "pattern", sPattern1 = "pattern1", oTarget;
    this.oRouterConfig = {
        route1: {
            pattern: sPattern,
            target: "target"
...
QUnit.test("title history", function (assert) {
    var done = assert.async(), oParameters, sHomeTitle = "HOME", sProductTitle = "PRODUCT", sProductDetailTitle = "PRODUCT_DETAIL", fnEventSpy = this.spy(function (oEvent) {
        oParameters = oEvent.getParameters();
    });
    this.oRouterConfig = {
        home: {
...
QUnit.test("avoid title history redundancy", function (assert) {
    var oParameters, sHomeTitle = "HOME", sProductTitle = "PRODUCT", sProductDetailTitle = "PRODUCT_DETAIL", fnEventSpy = this.spy(function (oEvent) {
        oParameters = oEvent.getParameters();
    });
    this.oRouterConfig = {
        home: {
...
QUnit.test("titleChanged event is fired before next navigation shouldn't create new history entry", function (assert) {
    var oParameters, sHomeTitle = "homeTitle", sNewTitle = "newTitle", fnEventSpy = this.spy(function (oEvent) {
        oParameters = oEvent.getParameters();
...
QUnit.test("Back navigation from target w/o title should not remove history entry", function (assert) {
    var done = assert.async(), oParameters, sHomeTitle = "HOME", sProductTitle = "PRODUCT", fnEventSpy = this.spy(function (oEvent) {
        oParameters = oEvent.getParameters();
    });
    this.oRouterConfig = {
        home: {
...
QUnit.test("home route declaration", function (assert) {
    var sHomeTitle = "HOME", sProductTitle = "PRODUCT", done = assert.async(), oParameters, fnEventSpy = this.spy(function (oEvent) {
        oParameters = oEvent.getParameters();
    });
    this.oRouterConfig = {
        home: {
...
QUnit.test("App home indicator for later navigations", function (assert) {
    var sHomeTitle = "HOME", sProductTitle = "PRODUCT", done = assert.async();
    this.oRouterConfig = {
        home: {
            pattern: "",
            target: "home"
...
QUnit.test("Should create a view with an component", function (assert) {
    var oUIComponent = new UIComponent({}), fnOwnerSpy = this.spy(oUIComponent, "runAsOwner"), oView = createXmlView(), oRouter = new Router({}, {}, oUIComponent), fnViewStub = this.stub(View, "_create").callsFake(function () {
        return oView;
    });
...
QUnit.test("Should create targets in the bypassed event", function (assert) {
    ...
    var fnDisplayFooStub = this.stub(this.oRouter.getTarget("foo"), "display").callsFake(jQuery.noop), fnDisplayBarStub = this.stub(this.oRouter.getTarget("bar"), "display").callsFake(jQuery.noop);
    this.oRouter.initialize();
...
QUnit.test("Hash 'page12' shouldn't match pattern 'page1/:context:'", function (assert) {
    var oRouter = new Router([{
        ...
        }]), sMatchedRouteName, fnMatched = function (oEvent) {
