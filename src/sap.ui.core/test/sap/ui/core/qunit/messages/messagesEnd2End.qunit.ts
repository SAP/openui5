...
QUnit.test("MessageMixin: Ignore list bindings", function (assert) {
    var done = assert.async(), oModel = new ODataModel(this.sServiceUri);
    oModel.metadataLoaded().then(function () {
        var bCall1, bCall2, oInput = new Input({
        ...
        oInput.refreshDataState = function (sName, oDataState) {
            ...
            if (sName === "value") {
                assert.strictEqual(oInput.getValueStateText(), "Correct text", "refreshDataState for property 'value'");
                assert.strictEqual(oInput.getValueState(), "Warning");
                bCall1 = true;
            }
            else if (sName === "suggestionItems") {
                assert.strictEqual(oInput.getValueStateText(), "Correct text", "refreshDataState for property 'suggestionItems'");
                assert.strictEqual(oInput.getValueState(), "Warning");
                bCall2 = true;
            }
            if (bCall1 && bCall2) {
