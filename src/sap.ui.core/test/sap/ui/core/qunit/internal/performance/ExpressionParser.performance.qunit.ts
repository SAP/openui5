...
import ExpressionParser from "sap/ui/base/ExpressionParser";
import ManagedObject from "sap/ui/base/ManagedObject";
import JSONModel from "sap/ui/model/json/JSONModel";
var iCount = 100000, oModel = new JSONModel({ x: 2 }), sResult = "\tParse expression binding\tEvaluate expression binding\tFormatter" + "\tBind with expression\tBind with formatter\n", TestControl = ManagedObject.extend("TestControl", {
    metadata: {
        properties: {
            value: "int"
        }
...
[{
        ...
        name: "complex",
        expression: "{='foo'.charCodeAt(-17  + 2 * Math.floor(null || (${x} ===  2 ? 5 + 2 * ${x} : 42)))}"
    ...
    QUnit.test("Bind with formatter: " + oFixture.name, function (assert) {
        bindTest(assert, BindingParser.complexParser("{path: '/x', formatter: 'formatters." + oFixture.name + "'}"));
        sResult += "\n";
    });
});