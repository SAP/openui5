...
var mFixture = {
    ...
    "/fake/v2/QM_INSP_PLAN_SRVMetadataOnly/$metadata": { source: "v2/QM_INSP_PLAN_SRV/metadata_only.xml" },
    "/fake/v2/QM_INSP_PLAN_SRVAnnotationsOnly/": { source: "v2/QM_INSP_PLAN_SRV/annotations_only.xml" },
...
[
    ...
    { url: "v2MetadataOnly/$metadata", Converter: V2MetadataConverter, desc: "V4 loading V2 MetadataOnly" },
    { url: "v2AnnotationsOnly/", Converter: V2MetadataConverter, desc: "V4 loading V2 AnnotationsOnly" },
    { url: "v2/QM_INSP_PLAN_SRV/$metadata", Converter: V2MetadataConverter, desc: "V4 loading V2 QM_INSP_PLAN_SRV Metadata" },
    { url: "v2/QM_INSP_PLAN_SRVMetadataOnly/$metadata", Converter: V2MetadataConverter, desc: "V4 loading V2 QM_INSP_PLAN_SRV MetadataOnly" },
    { url: "v2/QM_INSP_PLAN_SRVAnnotationsOnly/", Converter: V2MetadataConverter, desc: "V4 loading V2 QM_INSP_PLAN_SRV Annotations_only" }
].forEach(function (oFixture) {
    QUnit.test(oFixture.desc, function (assert) {
        var sUrl = "/fake/" + oFixture.url;
        return repeatAsyncTest(10, function () {
            return new Promise(function (fnResolve) {
                var oRequest = new XMLHttpRequest(), iStart = Date.now();
...
["/fake/v2", "/fake/v2/QM_INSP_PLAN_SRV"].forEach(function (sBase) {
    QUnit.test("V2 AnnotationParser#parse: " + sBase, function (assert) {
        ...
        }, fnOriginalParse = AnnotationParser.parse, oTestResult = {};
        AnnotationParser.parse = function (oMetadata, oXMLDoc, sSourceUrl) {
            var iStart = Date.now(), oParseResult = fnOriginalParse(oMetadata, oXMLDoc, sSourceUrl);
            if (sSourceUrl) {
...
["/fake/v2", "/fake/v2/QM_INSP_PLAN_SRV"].forEach(function (sServiceUrl) {
    QUnit.test("v2.ODataMetaModel: " + sServiceUrl, function (assert) {
        var fnOriginalParse = AnnotationParser.parse, oTestResult = {};
        AnnotationParser.parse = function (oMetadata, oXMLDoc, sSourceUrl) {
            var iStart = Date.now(), oParseResult = fnOriginalParse(oMetadata, oXMLDoc, sSourceUrl);
            oTestResult.time = Date.now() - iStart;
            return oParseResult;
