...
sap.ui.define([
	...
	/**
	 * This async test access the same parameters twice.
	 * Once with multiple paramaters and once with a single parameter.
	 * Both async calls made must attach themselves to the Core's themeChanged event and
	 * invoke their callback separately.
	 */
			...
			var pParameterListCheck = new Promise(function(resolve) {
				var mParamsInitially = Parameters.get({
					...
					callback: function(mValues) {
						// we have requested the same parameter twice 'lib1_sample-variable':
						// once with and once without an optional prefix 'sap.ui.totallyOptional.prefix'
						...
						// > quoted
						assert.equal(
							mValues["lib1_paramWithRelativeUrlQuoted"],
							"url(\"" + sBaseUri + "/testdata/libraries/themeParameters/lib1variables/themes/sap_hcb/img/icons/relative_quoted.png\")",
							"[relative URL] quoted: CSS Variable containg a URL can be retrieved and is correctly resolved"
						);
						// > auto escaped
						assert.equal(
							mValues["lib1_paramWithRelativeUrlAutoEscaped"],
							"url(\"" + sBaseUri + "/testdata/libraries/themeParameters/lib1variables/themes/sap_hcb/img/icons/relative_auto-escaped.png\")",
							"[relative URL] auto-escaped: CSS Variable containg a URL can be retrieved and is correctly resolved"
						);

						// absolute urls
						// > quoted
						assert.equal(
							mValues["lib1_paramWithAbsoluteUrlQuoted"],
							"url(\"http://somewhere.foo/img/icons/absolute_quoted.png\")",
							"[absolute URL] quoted: CSS Variable with absolute URL is untouched.");
						// > auto-escaped
						assert.equal(
							mValues["lib1_paramWithAbsoluteUrlAutoEscaped"],
							"url(\"http://somewhere.foo/img/icons/absolute_auto-escaped.png\")",
							"[absolute URL] auto-escaped: CSS Variable with absolute URL is untouched.");

						// Async API variant must now return the parameter values synchronously:
						// This time the callback is not executed but all parameters are returned synchronously from cache.
						// ThemeChanged event from Core already happened.
						...
						// > quoted
						assert.equal(
							mSyncValues["lib1_paramWithRelativeUrlQuoted"],
							"url(\"" + sBaseUri + "/testdata/libraries/themeParameters/lib1variables/themes/sap_hcb/img/icons/relative_quoted.png\")",
							"[relative URL] quoted: CSS Variable containg a URL can be retrieved and is correctly resolved"
						);
						// > auto escaped
						assert.equal(
							mSyncValues["lib1_paramWithRelativeUrlAutoEscaped"],
							"url(\"" + sBaseUri + "/testdata/libraries/themeParameters/lib1variables/themes/sap_hcb/img/icons/relative_auto-escaped.png\")",
							"[relative URL] auto-escaped: CSS Variable containg a URL can be retrieved and is correctly resolved"
						);
						...
						// > quoted
						assert.equal(
							mSyncValues["lib1_paramWithAbsoluteUrlQuoted"],
							"url(\"http://somewhere.foo/img/icons/absolute_quoted.png\")",
							"[absolute URL] quoted: CSS Variable with absolute URL is untouched.");
						// > auto-escaped
						assert.equal(
							mSyncValues["lib1_paramWithAbsoluteUrlAutoEscaped"],
							"url(\"http://somewhere.foo/img/icons/absolute_auto-escaped.png\")",
							"[absolute URL] auto-escaped: CSS Variable with absolute URL is untouched.");
			...
			var pSingleParameterCheck = new Promise(function(resolve) {
				...
				var sParamInitially = Parameters.get({
					...
					callback: function(sValue) {
						...
						// Async API variant must now return the parameter value synchronously:
						// This time the callback is not executed but all parameters are returned synchronously from cache.
						// ThemeChanged event from Core already happened.
		...
		return sap.ui.getCore().loadLibrary("testlibs.themeParameters.lib5variables", { async: true }).then(function() {
			return new Promise(function (resolve) {
				var fnAssertThemeChanged = function () {
					...
					// we have requested the same parameter twice 'lib5_sample-variable':
					// once with and once without an optional prefix 'sap.ui.totallyOptional.prefix'
					assert.equal(Object.keys(mValues).length, 9, "Correct number of parameters returned");
					...
					// > quoted
					assert.equal(
						mValues["lib5_paramWithRelativeUrlQuoted"],
						"url(\"" + sBaseUri + "/testdata/libraries/themeParameters/lib5variables/themes/sap_hcb/img/icons/relative_quoted.png\")",
						"[relative URL] quoted: CSS Variable containg a URL can be retrieved and is correctly resolved"
					);
					// > auto escaped
					assert.equal(
						mValues["lib5_paramWithRelativeUrlAutoEscaped"],
						"url(\"" + sBaseUri + "/testdata/libraries/themeParameters/lib5variables/themes/sap_hcb/img/icons/relative_auto-escaped.png\")",
						"[relative URL] auto-escaped: CSS Variable containg a URL can be retrieved and is correctly resolved"
					);
					...
					// > quoted
					assert.equal(
						mValues["lib5_paramWithAbsoluteUrlQuoted"],
						"url(\"http://somewhere.foo/img/icons/absolute_quoted.png\")",
						"[absolute URL] quoted: CSS Variable with absolute URL is untouched.");
					// > auto-escaped
					assert.equal(
						mValues["lib5_paramWithAbsoluteUrlAutoEscaped"],
						"url(\"http://somewhere.foo/img/icons/absolute_auto-escaped.png\")",
						"[absolute URL] auto-escaped: CSS Variable with absolute URL is untouched.");
