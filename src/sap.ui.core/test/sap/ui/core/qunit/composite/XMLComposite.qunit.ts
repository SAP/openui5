...
import SortedList from "composites/SortedList";
import TextButton from "composites/TextButton";
import TextList from "composites/TextList";
import ForwardText from "composites/ForwardText";
import Field from "composites/Field";
import HiddenMetadata from "composites/HiddenMetadata";
import TemplateTest from "composites/TemplateTest";
import ChildOfAbstract from "composites/ChildOfAbstract";
import TextToggleButton from "composites/TextToggleButton";
import TextToggleButtonNested from "composites/TextToggleButtonNested";
import TextToggleButtonForwarded from "composites/TextToggleButtonForwarded";
import WrapperLayouter from "composites/WrapperLayouter";
import TranslatableText from "composites/TranslatableText";
import TranslatableTextLib from "composites/TranslatableTextLib";
import TranslatableTextBundle from "composites/TranslatableTextBundle";
...
QUnit.test("properties", function (assert) {
    assert.strictEqual(this.oXMLComposite.getProperty("_text"), "The hidden text", "Default Text is set");
    assert.strictEqual(HiddenMetadata.getMetadata().getManagedProperty("_text").defaultValue, "The hidden text", "Default Text is set");
    assert.strictEqual(this.oXMLComposite.setProperty("_text", "Hello"), this.oXMLComposite, "Instance returned");
...
QUnit.test("destroy aggregation control", function (assert) {
    ...
    assert.strictEqual(this.oText.getParent(), undefined, "Inner Control has no parent anymore");
...
QUnit.test("aggregation singular", function (assert) {
    ...
    assert.strictEqual(this.oXMLComposite.setText(this.oText), this.oXMLComposite, "The XMLComposite control was returned after setText");
...
QUnit.test("data binding", function (assert) {
    ...
    Core.setModel(null, "model");
    assert.strictEqual(this.oXMLComposite.getTextItems().length, 5, "Unbound global model");
    ...
    this.oXMLComposite.setModel(null, "model");
    assert.strictEqual(this.oXMLComposite.getTextItems().length, 5, "Unbound model");
...
QUnit.test("properties", function (assert) {
    ...
    assert.strictEqual(this.oXMLComposite.getText(), "Default Text", "Default value of inner control does not shine through to XMLComposite control");
    var oInnerText = this.oXMLComposite.getAggregation("_content").getItems()[0].getAggregation("_content");
...
QUnit.test("positive tests - not in root", function (assert) {
    var done = assert.async();
    var sFragmentContent = "<core:FragmentDefinition xmlns=\"sap.m\" xmlns:core=\"sap.ui.core\" xmlns:layout=\"sap.ui.layout\">" + "<layout:HorizontalLayout><Input id=\"innerInput\" placeholder=\"{$this>/placeholder}\" />" + "<Button id=\"myTranslatableTextControl\" text=\"{$this.i18n>buttonText}\" press=\"handleSearch\" /></layout:HorizontalLayout></core:FragmentDefinition>";
...
QUnit.test("positive tests - in root", function (assert) {
    var done = assert.async();
    var sFragmentContent = "<m:Panel id=\"myTranslatableTextControl\" headerText=\"{$this.i18n>panelText}\" xmlns:m=\"sap.m\"><m:Input id=\"innerInput\" placeholder=\"{$this>/placeholder}\" />" + "<m:Button text=\"{$this>/buttonText}\" press=\"handleSearch\" /></m:Panel>";
    var oXml = new DOMParser().parseFromString(sFragmentContent, "text/xml").documentElement;
...
QUnit.module("sap.ui.core.XMLComposite: create from given fragmentContent", {
    beforeEach: function () {
        this.sFragmentContent = "<core:FragmentDefinition xmlns=\"sap.m\" xmlns:core=\"sap.ui.core\" xmlns:layout=\"sap.ui.layout\">" + "<layout:HorizontalLayout><Input id=\"innerInput\" placeholder=\"{$this>/placeholder}\" />" + "<Button text=\"{$this>/buttonText}\" press=\"handleSearch\" /></layout:HorizontalLayout></core:FragmentDefinition>";
    },
    afterEach: function () {
    }
});
