...
QUnit.test("constructor: all parameters set", function (assert) {
    var aApplicationFilters = [new Filter("propertyPath", "GE", "foo")], oBinding, oContext = {}, oModel = {
    ...
    this.mock(oModel).expects("checkFilterOperation").withExactArgs(sinon.match.same(aApplicationFilters));
    oBinding = new ODataTreeBinding(oModel, "path", oContext, aApplicationFilters, mParameters, aSorters);
    ...
    assert.strictEqual(oBinding.bUseServersideApplicationFilters, "bUseServersideApplicationFilters");
...
QUnit.test("constructor: with single Filter object", function (assert) {
    var oApplicationFilter = new Filter("propertyPath", "GE", "foo"), oBinding, oContext = {}, oModel = {
        checkFilterOperation: function () { }
    }, oModelMock = this.mock(oModel), oNotAFilter = {};
...
QUnit.test("constructor: parameter defaulting, logging", function (assert) {
    ...
    oModelMock.expects("checkFilterOperation").withExactArgs(undefined);
    oBinding = new ODataTreeBinding(oModel, "path", oContext, undefined, { batchGroupId: "group" });
    assert.strictEqual(oBinding.sGroupId, "group");
    oModelMock.expects("checkFilterOperation").withExactArgs(undefined);
    this.oLogMock.expects("fatal").withExactArgs("To use an ODataTreeBinding at least " + "one CountMode must be supported by the service!");
    oBinding = new ODataTreeBinding(oModel, "path", oContext, undefined, { countMode: CountMode.None });
...
QUnit.test("setContext: context unchanged", function (assert) {
    ...
    this.mock(Context).expects("hasChanged").withExactArgs("oContext", sinon.match.same(oNewContext)).returns(false);
...
QUnit.test("setContext: context undefined, unchanged", function (assert) {
    var oBinding = {};
    this.mock(Context).expects("hasChanged").withExactArgs(undefined, undefined).returns(false);
...
[{
    ...
    QUnit.test("setContext: context changed, relative, resolved, " + i, function (assert) {
        ...
        }, oBindingMock = this.mock(oBinding), oExpectation, oNewContextMock = this.mock(oNewContext);
        ...
        this.mock(Context).expects("hasChanged").withExactArgs(oFixture.bSameContext ? oNewContext : "oContext", sinon.match.same(oNewContext)).returns(true);
        ...
        oBindingMock.expects("_fireChange").withExactArgs({ reason: ChangeReason.Context }).on(oBinding);
...
[undefined, "oAllKeys", "oKeys", "_aNodes"].forEach(function (sProperty, i) {
    QUnit.test("setContext: context changed, relative, unresolved, " + i, function (assert) {
        ...
        oBindingMock.expects("_fireChange").exactly(sProperty ? 1 : 0).withExactArgs({ reason: ChangeReason.Context });
        ODataTreeBinding.prototype.setContext.call(oBinding, oNewContext);
        assert.strictEqual(oBinding.oContext, oNewContext);
    });
});
...
[{ "~changedEntityKey": true }, { Foo: true }, {}, undefined].forEach(function (mChangedEntities, i) {
    QUnit.test("_refresh: test with given mChangedEntities: " + i, function (assert) {
        ...
        this.mock(oBinding).expects("_hasChangedEntity").withExactArgs(mChangedEntities).returns(bExpectChange ? true : false).exactly(mChangedEntities ? 1 : 0);
        this.mock(oBinding).expects("resetData").exactly(bExpectChange ? 1 : 0);
        this.mock(oBinding).expects("_fireRefresh").withExactArgs({ reason: ChangeReason.Refresh }).exactly(bExpectChange ? 1 : 0);
        ODataTreeBinding.prototype._refresh.call(oBinding, undefined, mChangedEntities, undefined);
...
QUnit.test("_refresh: uses getResolvedPath", function (assert) {
    ...
    this.mock(oBinding).expects("getResolvedPath").withExactArgs().returns(undefined);
    ODataTreeBinding.prototype._refresh.call(oBinding, undefined, undefined, "~mEntityTypes");
});
[{ "~changedEntityKey": true }, { Foo: true }, {}].forEach(function (mChangedEntities, i) {
    QUnit.test("_hasChangedEntity: " + i, function (assert) {
        ...
        }, bExpectChange = mChangedEntities["~changedEntityKey"], bResult;
...
["~resolvedPath", undefined].forEach(function (sResolvedPath, i) {
    QUnit.test("getDownloadUrl: getResolvedPath is called, " + i, function (assert) {
        ...
        this.mock(oBinding.oModel).expects("_createRequestUrl").withExactArgs(sResolvedPath, null, []).exactly(sResolvedPath ? 1 : 0).returns("~requestUrl");
        assert.strictEqual(ODataTreeBinding.prototype.getDownloadUrl.call(oBinding), sResolvedPath ? "~requestUrl" : undefined);
    });
});
...
QUnit.test("_loadSingleRootNodeByNavigationProperties: with resolved path", function (assert) {
    ...
    this.mock(oBinding.oModel).expects("read").withExactArgs("~resolvedPath", sinon.match.object.and(sinon.match.has("groupId")).and(sinon.match.has("success")).and(sinon.match.has("error"))).returns("~readHandler");
    ODataTreeBinding.prototype._loadSingleRootNodeByNavigationProperties.call(oBinding, undefined, "~requestKey");
...
QUnit.test("getRootContexts: initial", function (assert) {
    ...
    assert.deepEqual(ODataTreeBinding.prototype.getRootContexts.call(oBinding, undefined, undefined, undefined), []);
});
QUnit.test("getRootContexts: returns V2 Contexts with tree annotations", function (assert) {
    ...
    this.mock(oBinding).expects("_getContextsForNodeId").withExactArgs(null, "~iStartIndex", "~ilength", "~iThreshold").returns("~V2Context[]");
    assert.strictEqual(ODataTreeBinding.prototype.getRootContexts.call(oBinding, "~iStartIndex", "~ilength", "~iThreshold"), "~V2Context[]");
...
QUnit.test("getRootContexts: returns existing single V2 root context", function (assert) {
    ...
    assert.deepEqual(ODataTreeBinding.prototype.getRootContexts.call(oBinding, "~iStartIndex", "~ilength", "~iThreshold"), ["~V2RootContext"]);
});
QUnit.test("getRootContexts: missing root", function (assert) {
    ...
    this.mock(oBinding).expects("getContext").withExactArgs().returns("~oContext");
    this.mock(oModel).expects("isList").withExactArgs("~sPath", "~oContext").returns(false);
    assert.deepEqual(ODataTreeBinding.prototype.getRootContexts.call(oBinding, "~iStartIndex", "~ilength", "~iThreshold"), []);
});
QUnit.test("getRootContexts: calls _loadSingleRootNodeByNavigationProperties and returns []", function (assert) {
    var oModel = {
        isList: function () { }
    }, oBinding = {
    ...
    this.mock(oBinding).expects("getContext").withExactArgs().returns("~oContext");
    this.mock(oModel).expects("isList").withExactArgs("~sPath", "~oContext").returns(false);
    this.mock(oBinding).expects("_loadSingleRootNodeByNavigationProperties").withExactArgs("~resolvedPath", "null-~iStartIndex-~iPageSize-~iThreshold");
    assert.deepEqual(ODataTreeBinding.prototype.getRootContexts.call(oBinding, "~iStartIndex", "~ilength", "~iThreshold"), []);
});
QUnit.test("getRootContexts: returns V2 Contexts without tree annotations", function (assert) {
    ...
    this.mock(oBinding).expects("_getContextsForNodeId").withExactArgs("~resolvedPath/~navPath", "~iStartIndex", "~ilength", "~iThreshold", {
        navPath: "~navPath",
        numberOfExpandedLevels: "~iNumberOfExpandedLevels"
    }).returns("~V2Context[]");
    assert.strictEqual(ODataTreeBinding.prototype.getRootContexts.call(oBinding, "~iStartIndex", "~ilength", "~iThreshold"), "~V2Context[]");
});
QUnit.test("_loadSingleRootNodeByNavigationProperties: success handler sets V2 Context as root" + " context", function (assert) {
    var oModel = {
        _getKey: function () { },
    ...
    this.mock(oBinding).expects("_processODataObject").withExactArgs("~object", "~sNodeId", "~navPath");
...
QUnit.test("_loadSingleRootNodeByNavigationProperties: error handler sets _bRootMissing to" + " true", function (assert) {
    var oModel = {
        read: function () { }
    }, oBinding = {
        sGroupId: "~sGroupId",
        ...
        getResolvedPath: function () { }
    }, oError = { statusCode: 400 }, oExpectation;
    this.mock(oBinding).expects("getResolvedPath").withExactArgs().returns("~resolvedPath");
    oExpectation = this.mock(oModel).expects("read").withExactArgs("~resolvedPath", {
        error: sinon.match.func,
        groupId: "~sGroupId",
    ...
    assert.strictEqual(oBinding.bNeedsUpdate, true);
    assert.strictEqual(oBinding.oRootContext, "~oldRootContext");
    assert.notOk(oBinding.mRequestHandles.hasOwnProperty("~sRequestKey"));
});
QUnit.test("getNodeContexts: initial", function (assert) {
    ...
    this.mock(oBinding).expects("isInitial").withExactArgs().returns(true);
    assert.deepEqual(ODataTreeBinding.prototype.getNodeContexts.call(oBinding, undefined, undefined, undefined, undefined), []);
});
QUnit.test("getNodeContexts: with neither tree annotations nor navigation path", function (assert) {
    var oBinding = {
    ...
    assert.deepEqual(ODataTreeBinding.prototype.getNodeContexts.call(oBinding, oContext, undefined, undefined, undefined), []);
});
QUnit.test("getNodeContexts: with navigation path and without tree annotations", function (assert) {
    var oModel = {
        resolve: function () { }
    }, oBinding = {
        bHasTreeAnnotations: false,
    ...
    this.mock(oContext).expects("getPath").withExactArgs().returns("~getPath");
    this.mock(oBinding).expects("_getNavPath").withExactArgs("~getPath").returns("~navPath");
    this.mock(oModel).expects("resolve").withExactArgs("~navPath", sinon.match.same(oContext)).returns("~resolvedPath");
    this.mock(oBinding).expects("_getContextsForNodeId").withExactArgs("~resolvedPath", "~iStartIndex", "~ilength", "~iThreshold", {
        navPath: "~foo"
    }).returns("~V2Context[]");
    assert.strictEqual(ODataTreeBinding.prototype.getNodeContexts.call(oBinding, oContext, "~iStartIndex", "~ilength", "~iThreshold"), "~V2Context[]");
});
QUnit.test("_getContextsForNodeId: with operation mode 'Auto' and without" + " iTotalCollectionCount", function (assert) {
    var oBinding = {
        bCollectionCountRequested: true,
        sOperationMode: OperationMode.Auto,
        iTotalCollectionCount: null
    };
    assert.deepEqual(ODataTreeBinding.prototype._getContextsForNodeId.call(oBinding, "~sNodeId", "~iStartIndex", "~iLength", "~iThreshold", "~mRequestParameters"), []);
});
QUnit.test("_getContextsForNodeId: no keys for node ID available", function (assert) {
    ...
    this.mock(oModel).expects("getServiceMetadata").withExactArgs().returns({});
    assert.deepEqual(ODataTreeBinding.prototype._getContextsForNodeId.call(oBinding, "~sNodeId", 0, 20, 100, "~mRequestParameters"), []);
});
QUnit.test("_getContextsForNodeId: node ID available", function (assert) {
    ...
    assert.deepEqual(ODataTreeBinding.prototype._getContextsForNodeId.call(oBinding, "~sNodeId", 0, 20, 100, "~mRequestParameters"), ["~V2Context", undefined]);
});
QUnit.test("initialize", function (assert) {
    var oModel = {
        ...
        isResolved: function () { },
        oModel: oModel
    }, oBindingMock = this.mock(oBinding), oExpectation, oMetadataMock = this.mock(oModel.oMetadata);
...
QUnit.test("_initialize", function (assert) {
    ...
    this.mock(oBinding).expects("_hasTreeAnnotations").withExactArgs().returns("~bHasTreeAnnotations");
    this.mock(oBinding).expects("_getEntityType").withExactArgs().returns("~oEntityType");
    ...
    assert.strictEqual(ODataTreeBinding.prototype._initialize.call(oBinding, fnFireEvent), oBinding);
...
QUnit.test("_applyAdapter: without tree annotations", function () {
    var oAdapter = { apply: function () { } }, oBinding = {
        bHasTreeAnnotations: false,
        oNavigationPaths: {}
    }, oEventProvider = { fnFireEvent: function () { } }, oExpectation;
    oExpectation = this.mock(sap.ui).expects("require").withExactArgs(["sap/ui/model/odata/ODataTreeBindingAdapter"], sinon.match.func);
...
[{
    ...
    QUnit.test("_applyAdapter: with tree annotations, " + i, function () {
        var oAdapter = { apply: function () { } }, oMetadata = { _getEntityTypeByPath: function () { } }, oBinding = {
            getResolvedPath: function () { },
            bHasTreeAnnotations: true,
            oModel: {
                oMetadata: oMetadata
        ...
        }, oEntityType = { property: [] }, oEventProvider = { fnFireEvent: function () { } }, oExpectation;
        this.mock(oBinding).expects("getResolvedPath").withExactArgs().returns("~resolvedPath");
        this.mock(oMetadata).expects("_getEntityTypeByPath").withExactArgs("~resolvedPath").returns(oEntityType);
        oExpectation = this.mock(sap.ui).expects("require").withExactArgs([oFixture.sModuleName], sinon.match.func);
        this.mock(oAdapter).expects("apply").withExactArgs(sinon.match.same(oBinding));
        this.mock(oEventProvider).expects("fnFireEvent").withExactArgs();
        oExpectation.callsArgWith(1, oAdapter);
        ODataTreeBinding.prototype._applyAdapter.call(oBinding, oEventProvider.fnFireEvent);
...
QUnit.test("_applyAdapter: error case", function () {
    ...
    this.oLogMock.expects("error").withExactArgs("Neither hierarchy annotations, " + "nor navigation properties are specified to build the tree.", sinon.match.same(oBinding));
