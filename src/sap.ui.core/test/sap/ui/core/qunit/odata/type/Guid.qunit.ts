...
QUnit.test("default nullable is true", function (assert) {
    ...
    this.oLogMock.expects("warning").withExactArgs("Illegal nullable: foo", null, "sap.ui.model.odata.type.Guid");
...
QUnit.test("format success", function (assert) {
    ...
    assert.strictEqual(oType.formatValue("0050568d-393C-1ed4-9D97-E65F0F3FCC23", "string"), "0050568d-393C-1ed4-9D97-E65F0F3FCC23", "target type string");
    assert.strictEqual(oType.formatValue("0050568d-393C-1ed4-9D97-E65F0F3FCC23", "any"), "0050568d-393C-1ed4-9D97-E65F0F3FCC23", "target type any");
    this.mock(oType).expects("getPrimitiveType").withExactArgs("sap.ui.core.CSSSize").returns("string");
    assert.strictEqual(oType.formatValue("0050568d-393C-1ed4-9D97-E65F0F3FCC23", "sap.ui.core.CSSSize"), "0050568d-393C-1ed4-9D97-E65F0F3FCC23");
});
["int", "boolean", "float", "object"].forEach(function (sTargetType) {
    QUnit.test("format fail for target type " + sTargetType, function (assert) {
        ...
        try {
            ...
            assert.strictEqual(e.message, "Don't know how to format sap.ui.model.odata.type.Guid to " + sTargetType);
        }
    });
});
QUnit.test("parse success", function (assert) {
    ...
    assert.strictEqual(oType.parseValue("0050568d-393C-1ed4-9D97-E65F0F3FCC23", "string"), "0050568d-393C-1ed4-9D97-E65F0F3FCC23", "real GUID, mixed case");
    assert.strictEqual(oType.parseValue("0050568d393C1ed49D97E65F0F3FCC23", "string"), "0050568d-393C-1ed4-9D97-E65F0F3FCC23", "real GUID without '-'");
    assert.strictEqual(oType.parseValue("-005--05-68d3-93C1e-d49D97E65F0F3FCC23-", "string"), "0050568d-393C-1ed4-9D97-E65F0F3FCC23", "GUID with separators at wrong places");
    ...
    assert.strictEqual(oType.parseValue("0050568D-393c-1", "string"), "0050568D-393c-1", "parse invalid GUID");
    assert.strictEqual(oType.parseValue("005X568d-393C-1ed4-9D97-E65F0F3FCC23", "string"), "005X568d-393C-1ed4-9D97-E65F0F3FCC23", "invalid character X");
    this.mock(oType).expects("getPrimitiveType").withExactArgs("sap.ui.core.CSSSize").returns("string");
    assert.strictEqual(oType.parseValue("0050568d-393C-1ed4-9D97-E65F0F3FCC23", "sap.ui.core.CSSSize"), "0050568d-393C-1ed4-9D97-E65F0F3FCC23");
});
[[123, "int"], [true, "boolean"], [1.23, "float"], ["foo", "object"]].forEach(function (aFixture) {
    QUnit.test("parse fail for source type " + aFixture[1], function (assert) {
        ...
        try {
            ...
            assert.strictEqual(e.message, "Don't know how to parse sap.ui.model.odata.type.Guid from " + aFixture[1]);
        }
    });
});
