...
QUnit.test("w/ format options", function (assert) {
    var oFormatOptions = { parseKeepsEmptyString: true }, oResultFormatOptions, oType = new StringType(oFormatOptions, {});
...
[
    ...
    QUnit.test("constraints error #" + i, function (assert) {
        var oType;
        this.oLogMock.expects("warning").withExactArgs(oFixture.warning, null, "sap.ui.model.odata.type.String");
...
QUnit.test("parse", function (assert) {
    ...
    assert.strictEqual(oType.parseValue("", "string"), null, "empty string is converted to null");
...
QUnit.test("nullable", function (assert) {
    ...
    TestUtils.withNormalizedMessages(function () {
        ...
        that.oLogMock.expects("warning").withExactArgs("Illegal nullable: ", null, "sap.ui.model.odata.type.String");
...
["foo", 1, 0, null, "true", "false"].forEach(function (vParseKeepsEmptyString) {
    ...
    QUnit.test(sTitle, function (assert) {
        var oType;
        this.oLogMock.expects("warning").withExactArgs("Illegal parseKeepsEmptyString: " + vParseKeepsEmptyString, null, "sap.ui.model.odata.type.String");
        oType = new StringType({ parseKeepsEmptyString: vParseKeepsEmptyString });
        assert.strictEqual(oType.parseValue(null, "string"), null);
...
QUnit.test("isDigitSequence: constructor", function (assert) {
    ...
    [true, "true", false, "false"].forEach(function (vIsDigitSequence, i) {
        ...
        assert.deepEqual(oType.oConstraints, i < 2 ? { isDigitSequence: true } : undefined, "constructor test #" + i);
        oType = new StringType({}, { isDigitSequence: vIsDigitSequence, maxLength: 7 });
        assert.deepEqual(oType.oConstraints, i < 2 ? { isDigitSequence: true, maxLength: 7 } : { maxLength: 7 }, "constructor test with maxLength #" + i);
    });
    ["foo", 1, 0, null].forEach(function (vIsDigitSequence) {
        that.oLogMock.expects("warning").withExactArgs("Illegal isDigitSequence: " + vIsDigitSequence, null, "sap.ui.model.odata.type.String");
        oType = new StringType({}, { isDigitSequence: vIsDigitSequence });
        assert.deepEqual(oType.oConstraints, undefined, "Ignore: " + vIsDigitSequence);
    });
});
QUnit.test("isDigitSequence: format", function (assert) {
    var oType = new StringType({}, { isDigitSequence: true, maxLength: 7 }), oTypeNoDigit = new StringType({}, { isDigitSequence: false, maxLength: 7 }), oTypeNoLength = new StringType({}, { isDigitSequence: true }), sValue = "0012345";
    ...
    [
        ...
        assert.strictEqual(oType.formatValue(oFixture.v, "string"), oFixture.r, oFixture.v + " as string");
        assert.strictEqual(oTypeNoLength.formatValue(oFixture.v, "string"), oFixture.r, oFixture.v + " as string");
    });
    ...
    assert.strictEqual(oType.formatValue(sValue, "float"), 12345, "as float");
...
QUnit.test("isDigitSequence: parse", function (assert) {
    var sParsedValue, oType = new StringType({}, { isDigitSequence: true, maxLength: 7 }), sValue = "1234";
    assert.strictEqual(oType.parseValue(null, "string"), null, "null");
    assert.strictEqual(oType.parseValue(undefined, "string"), undefined, "undefined");
...
[undefined, false, true].forEach(function (bNullable) {
    QUnit.test("isDigitSequence: empty string, nullable=" + bNullable, function (assert) {
        ...
        oType = new StringType({}, { isDigitSequence: true, maxLength: 7, nullable: bNullable });
