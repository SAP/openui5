...
import ListItem from "sap/m/DisplayListItem";
...
var sView = "<mvc:View xmlns:core=\"sap.ui.core\" xmlns:mvc=\"sap.ui.core.mvc\" xmlns:form=\"sap.ui.layout.form\" xmlns=\"sap.m\" controllerName=\"my.Controller\">\t\t<List id=\"myList\" items=\"{path: '/Categories', suspended: true}\">\t\t\t<InputListItem>\t\t\t\t<content>\t\t\t\t\t<Input value=\"{path: 'CategoryName', suspended: true}\"/>\t\t\t\t</content>\t\t\t</InputListItem>\t\t</List>\t</mvc:View>";
var Filter = Filter;
...
QUnit.test("getMessagesByEntity", function (assert) {
    ...
    assert.strictEqual(oModel.getMessageScope(), MessageScope.RequestedObjects, "Initial message scope is: RequestedObjects");
...
QUnit.test("test invalidate", function (assert) {
    ...
    var oBinding = oModel.bindContext("/Products(3)", null, { expand: "Category" });
    oBinding.attachChange(function (oContext) {
        var oProduct = oModel.getProperty("/Products(3)"), oCategory = oModel.getProperty("/Categories(2)");
        assert.equal(spy.callCount, 1, "get request should be send");
...
QUnit.test("test getObject with multiple nested expands", function (assert) {
    ...
    sap.ui.getCore().setModel(oModel);
    var oBinding = oModel.bindContext("/Suppliers(7)", null, { select: "*,Products/*,Products/Supplier/*,Products/Category/CategoryID,Products/Category/CategoryName", expand: "Products,Products/Supplier,Products/Category" });
    oBinding.attachChange(function (oContext) {
...
QUnit.test("createBindingContext expand", function (assert) {
    var done = assert.async();
    var oModel = initModel(sURI, { json: false });
    oModel.createBindingContext("/Products(1)", null, { expand: "Category" }, function (oContext) {
...
QUnit.test("async test sequentializeRequests - single", function (assert) {
    ...
    var iCount = 0, iRequestCount = 0, iParallelCount = 0;
...
QUnit.test("Token request, automatic resubmit", function (assert) {
    var done = assert.async();
    var refreshSpy = sinon.spy(this.oModel, "refreshSecurityToken"), resetSpy = sinon.spy(this.oModel, "resetSecurityToken");
    this.oModel.create("/Categories(1)", {}, {
        success: function () {
            assert.ok(refreshSpy.calledOnce, "Token requested for POST request");
            fakeService.updateCsrfToken();
...
QUnit.test("Token request for different services on the same server", function (assert) {
    var done = assert.async(), sURI1 = "http://server/service1/", sURI2 = "http://server/service2/";
...
QUnit.test("Token request for different services on different servers", function (assert) {
    var done = assert.async(), sURI1 = "http://server1/service/", sURI2 = "http://server2/service/";
    fakeService.setBaseUrl(sURI1);
    this.oModel.destroy();
    this.oModel = initModel(sURI1, {
        json: true
...
QUnit.test("Metamodel binding refresh after metamodel load", function (assert) {
    var done = assert.async();
    var oPropertyBinding, oListBinding, oMetaBindingRelative, oMetaBindingAbsolute, propertySpy, listSpy, metaRelativeSpy, metaAbsoluteSpy;
        ...
        oHandle.contextCreated().then(function (oContextCallFunction) {
            that.oModel.submitChanges({
                success: function () {
                    assert.deepEqual("/Products(1000)", oContext.getPath());
                    assert.ok(oContext === that.oModel.getContext(oContext.getPath()), "Context must be the same");
                    assert.strictEqual(oContextCallFunction, that.oModel.getContext(oContextCallFunction.getPath()));
...
QUnit.test("submit:check success - no pending changes", function (assert) {
    ...
    this.oModel.metadataLoaded().then(function () {
        var fnSuccess = function (oData) {
            ...
            var oProduct = oContext.getObject();
            assert.ok(oProduct, "Product still exists");
            assert.ok(!that.oModel.oData[oContext.getPath().slice(1)].__metadata.created, "Product not flagged as created");
            done();
        };
        var fnError = function () {
            assert.ok(true, "error handler called");
        ...
        var oProduct = this.oModel.getProperty("", oContext);
        assert.ok(oProduct, "Product created");
        assert.ok(that.oModel.oData[oContext.getPath().slice(1)].__metadata.created, "Product flagged as created");
        var fnCompl = function (oInfo) {
            that.oModel.detachRequestCompleted(fnCompl);
            assert.ok(!oInfo.getParameter("success"), "request error");
            var oProduct = oContext.getObject();
            assert.ok(oProduct, "Product still exists");
            assert.ok(that.oModel.oData[oContext.getPath().slice(1)].__metadata.created, "Product still flagged as created");
        };
        this.oModel.attachRequestCompleted(fnCompl);
        this.oModel.submitChanges();
    }.bind(this));
...
QUnit.test("Check default: caching disabled", function (assert) {
    var done = assert.async();
    this.oModel.metadataLoaded().then(function (oMetadata) {
        assert.ok(!this.oModel.oMetadata.bUseCache, "caching disabled");
        done();
    }.bind(this));
    this.oModel.annotationsLoaded().then(function (oAnnotations) {
...
QUnit.test("Annotation error: no cache", function (assert) {
    ...
    this.oModel.metadataLoaded().then(function (oMetadata) {
        ...
        this.oModel2.metadataLoaded().then(function (oMetadata) {
            assert.ok(this.oModel2.oMetadata.sCacheKey, "caching enabled");
            oSpy.restore();
            done();
        }.bind(this));
        this.oModel2.annotationsLoaded().then(function (oAnnotations) {
