...
var sDefaultLanguage = sap.ui.getCore().getConfiguration().getLanguage(), oCircular = {};
...
function parseError(assert, oType, oValue, sReason) {
    TestUtils.withNormalizedMessages(function () {
        try {
            ...
            assert.strictEqual(e.message, "EnterTime " + oType.formatValue(createTime(23, 59, 58, 0), "string"), sReason + ": message");
        }
    });
}
...
QUnit.test("illegal value for nullable", function (assert) {
    ...
    this.oLogMock.expects("warning").withExactArgs("Illegal nullable: foo", null, "sap.ui.model.odata.type.Time");
...
QUnit.test("format success", function (assert) {
    var oTime = createTime(13, 53, 49, 567), oType = new Time();
...
["int", "boolean", "float", "object"].forEach(function (sTargetType) {
    QUnit.test("format failure for target type " + sTargetType, function (assert) {
        ...
        try {
            ...
            assert.strictEqual(e.message, "Don't know how to format sap.ui.model.odata.type.Time to " + sTargetType);
        }
    });
});
[
    ...
    QUnit.test("format failure for " + JSON.stringify(oTime), function (assert) {
        ...
        try {
            ...
            assert.ok(e instanceof FormatException);
            assert.strictEqual(e.message, "Illegal " + oType.getName() + " value: " + JSON.stringify(oTime));
        }
    });
});
QUnit.test("parse", function (assert) {
    ...
    assert.deepEqual(oType.parseValue("1:45:33 PM", "string"), createTime(13, 45, 33, 0), "valid time");
...
QUnit.test("parse, get primitive type", function (assert) {
    var oType = new Time();
    this.mock(oType).expects("getPrimitiveType").withExactArgs("sap.ui.core.CSSSize").returns("string");
    assert.deepEqual(oType.parseValue("1:45:33 PM", "sap.ui.core.CSSSize"), createTime(13, 45, 33, 0));
});
[[123, "int"], [true, "boolean"], [1.23, "float"], ["foo", "object"]].forEach(function (aFixture) {
    QUnit.test("parse failure for source type " + aFixture[1], function (assert) {
        ...
        try {
            ...
            assert.strictEqual(e.message, "Don't know how to parse sap.ui.model.odata.type.Time from " + aFixture[1]);
        }
    });
});
...
QUnit.test("getModelFormat()", function (assert) {
    var oModelTime = createTime(13, 54, 49, 567), oType = new Time(), oFormat = oType.getModelFormat(), oParsedTime = oFormat.parse(oModelTime);
...
[
    ...
    QUnit.test("validation failure for illegal model type #" + i, function (assert) {
        ...
        try {
            ...
            assert.strictEqual(e.message, "Illegal " + oType.getName() + " value: " + (oFixture.error || JSON.stringify(oFixture.value)));
        }
    });
});
QUnit.test("localization change", function (assert) {
    var oControl = new Control(), oType = new Time(), oValue = createTime(13, 53, 49, 0);
    oControl.bindProperty("tooltip", { path: "/unused", type: oType });
    oType.formatValue(oValue, "string");
    sap.ui.getCore().getConfiguration().setLanguage("de");
    assert.strictEqual(oType.formatValue(oValue, "string"), "13:53:49", "adjusted to changed language");
});
[
    ...
    { oFormatOptions: { style: "medium" }, oExpected: { UTC: true, strictParsing: true, style: "medium" } }
].forEach(function (oFixture) {
    QUnit.test("with oFormatOptions=" + JSON.stringify(oFixture.oFormatOptions), function (assert) {
        var oType = new Time(oFixture.oFormatOptions), oSpy = this.spy(DateFormat, "getTimeInstance");
