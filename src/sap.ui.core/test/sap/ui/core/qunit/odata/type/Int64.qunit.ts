...
var sDefaultLanguage = sap.ui.getCore().getConfiguration().getLanguage(), NUMBER_MIN_SAFE_INTEGER = -9007199254740991, NUMBER_MAX_SAFE_INTEGER = 9007199254740991;
...
[
    ...
    QUnit.test("setConstraints(" + JSON.stringify(oFixture.i) + ")", function (assert) {
        var oType;
        if (oFixture.warning) {
            this.oLogMock.expects("warning").withExactArgs(oFixture.warning, null, "sap.ui.model.odata.type.Int64");
        }
...
QUnit.test("format success", function (assert) {
    ...
    assert.strictEqual(oType.formatValue("-9223372036854775808", "string"), "-9,223,372,036,854,775,808", "min Int64, target type string");
    ...
    assert.strictEqual(oType.formatValue(String(NUMBER_MIN_SAFE_INTEGER), "int"), NUMBER_MIN_SAFE_INTEGER, "MIN_SAFE_INTEGER to int");
    assert.strictEqual(oType.formatValue(String(NUMBER_MAX_SAFE_INTEGER), "float"), NUMBER_MAX_SAFE_INTEGER, "MAX_SAFE_INTEGER to float");
    assert.strictEqual(oType.formatValue("+" + String(NUMBER_MAX_SAFE_INTEGER), "float"), NUMBER_MAX_SAFE_INTEGER, "MAX_SAFE_INTEGER to float");
...
[
    { test: "format to boolean", value: "12.34", targetType: "boolean", errorText: "Don't know how to format sap.ui.model.odata.type.Int64 to boolean" },
    { test: "format to int with overflow error", value: "9007199254740992", targetType: "int", errorText: "EnterNumberMax 9,007,199,254,740,991" },
    { test: "format to int with overflow error", value: "-9007199254740992", targetType: "int", errorText: "EnterNumberMin -9,007,199,254,740,991" }
].forEach(function (oFixture) {
...
QUnit.test("parse", function (assert) {
    ...
    assert.strictEqual(oType.parseValue("1,234,567", "string"), "1234567", "multiple grouping separators");
    ...
    assert.strictEqual(oType.parseValue(-123450, "float"), "-123450", "float in exp. notation");
    ...
    try {
        ...
        assert.strictEqual(e.message, "Don't know how to parse sap.ui.model.odata.type.Int64 from boolean");
    }
    this.mock(oType).expects("getPrimitiveType").withExactArgs("sap.ui.core.CSSSize").returns("string");
...
QUnit.test("parse other minus/plus symbols, unbreakable spaces", function (assert) {
    ...
    assert.strictEqual(oType.parseValue(">1 234 567 890 123 456", "string"), "1234567890123456", "plus sign, spaces");
...
QUnit.test("parse large numbers w/ format options", function (assert) {
    ...
    oType = new Int64();
    assert.strictEqual(oType.parseValue("1 234 567 890 123 456789", "string"), "1234567890123456789", "no format options -> full precision");
    oType = new Int64(oFormatOptions);
    assert.strictEqual(oType.parseValue("1 234 567 890 123 456789", "string"), "1234567890123456789", "only safe format options -> full precision");
    oFormatOptions.foo = "bar";
    oType = new Int64(oFormatOptions);
    assert.strictEqual(oType.parseValue("1 234 567 890 123 456 789", "string"), "1234567890123456789", "random format option");
...
QUnit.test("validate success", function (assert) {
    var oType = new Int64();
    ["+1", "-1", "+123", "-123", "0", null, "-9223372036854775808", "9223372036854775807", "+9223372036854775807"].forEach(function (sValue) {
...
[
    { test: "exceeds min by length", value: "-92233720368547758090", message: "EnterNumberMin -9,223,372,036,854,775,808" },
    { test: "exceeds max by length", value: "92233720368547758080", message: "EnterNumberMax 9,223,372,036,854,775,807" },
    { test: "exceeds min by 1", value: "-9223372036854775809", message: "EnterNumberMin -9,223,372,036,854,775,808" },
    { test: "exceeds max by 1", value: "9223372036854775808", message: "EnterNumberMax 9,223,372,036,854,775,807" }
].forEach(function (oFixture) {
...
QUnit.test("localization change", function (assert) {
    var oControl = new Control(), oType = new Int64();
...
[{
    ...
    QUnit.test("formatOptions: " + JSON.stringify(oFixture.set), function (assert) {
        var oSpy, oType = new Int64(oFixture.set);
