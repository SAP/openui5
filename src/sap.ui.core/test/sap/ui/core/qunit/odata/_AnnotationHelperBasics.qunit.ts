...
QUnit.test("toJSON, toErrorString", function (assert) {
    var oCircular = {}, fnTestFunction = function () { };
    [
        ...
        if (oFixture.hasOwnProperty("json")) {
            assert.strictEqual(Basics.toJSON(oFixture.value), oFixture.json, "toJSON:" + oFixture.json);
        }
        assert.strictEqual(Basics.toErrorString(oFixture.value), vJS, "toErrorString:" + vJS);
    });
    oCircular.circle = oCircular;
    assert.strictEqual(Basics.toErrorString(oCircular), "[object Object]", "toErrorString: circular references");
    assert.strictEqual(Basics.toErrorString(fnTestFunction), String(fnTestFunction), "toErrorString: function");
});
QUnit.test("error w/o component", function (assert) {
    var sErrorText = "Wrong! So wrong!", oPathValue = { path: "/path/to/foo", value: { foo: "bar" } }, sMessage = oPathValue.path + ": " + sErrorText;
    this.oLogMock.expects("error").withExactArgs(sMessage, Basics.toErrorString(oPathValue.value), "sap.ui.model.odata.AnnotationHelper");
...
QUnit.test("error w/ component", function (assert) {
    var sErrorText = "Wrong! So wrong!", oPathValue = { path: "/path/to/foo", value: { foo: "bar" } }, sMessage = oPathValue.path + ": " + sErrorText;
    this.oLogMock.expects("error").withExactArgs(sMessage, Basics.toErrorString(oPathValue.value), "custom.Component");
    assert.throws(function () {
        Basics.error(oPathValue, sErrorText, "custom.Component");
    }, new SyntaxError(sMessage));
});
QUnit.test("expectType", function (assert) {
    var aArray = [], oObject = {}, sString = "foo", aTests = [aArray, oObject, sString, undefined, null, true, 0, NaN, Function], oBasicsMock = this.mock(Basics);
    ...
    [
        ...
        aTests.forEach(function (vTest) {
            ...
            if (vTest !== oFixture.ok) {
                oBasicsMock.expects("error").withExactArgs(sinon.match.same(oPathValue), "Expected " + oFixture.type);
            }
...
[false, true].forEach(function (bTestProperty) {
    QUnit.test("descend, bTestProperty=" + bTestProperty, function (assert) {
        ...
        [
            ...
            oBasicsMock.expects("expectType").withExactArgs(sinon.match.same(oStart), oFixture.type);
            oBasicsMock.expects("expectType").exactly(bTestProperty ? 1 : 0).withExactArgs(oEnd, "string");
            oResult = bTestProperty ? Basics.descend(oStart, oFixture.property, "string") : Basics.descend(oStart, oFixture.property);
...
QUnit.test("descend 'as expression'", function (assert) {
    ...
    oResult = Basics.descend(oPathValue, "foo", true);
    assert.deepEqual(oResult, {
        asExpression: true,
        path: "/my/path/foo",
        value: "bar"
...
QUnit.test("property", function (assert) {
    var oPathValue = {};
    this.mock(Basics).expects("descend").withExactArgs(sinon.match.same(oPathValue), "p", "string").returns({ value: "foo" });
...
QUnit.test("resultToString: bindings", function (assert) {
    [{
        ...
        assert.strictEqual(Basics.resultToString(oFixture.value, false), oFixture.binding, oFixture.binding);
        assert.strictEqual(Basics.resultToString(oFixture.value, true), oFixture.expression, oFixture.expression);
    });
    assert.strictEqual(Basics.resultToString({ result: "composite", value: "{FirstName} {LastName}" }, false), "{FirstName} {LastName}", "composite to binding");
    assert.throws(function () {
        Basics.resultToString({ result: "composite", value: "{FirstName} {LastName}" }, true);
    }, /Trying to embed a composite binding into an expression binding/, "composite to expression");
});
[
    ...
    { type: "Edm.Double", value: -Infinity, expression: "-Infinity" },
    { type: "Edm.Guid", value: "12345678-ABCD-EFab-cdef-123456789012", expression: "'12345678-ABCD-EFab-cdef-123456789012'" },
    ...
    QUnit.test("resultToString: constant " + JSON.stringify(oFixture.value), function (assert) {
        ...
        assert.strictEqual(Basics.resultToString(oResult, true), oFixture.expression, oFixture.type + " -> expression");
        if (typeof oFixture.value !== "string") {
            this.mock(BindingParser.complexParser).expects("escape").never();
        }
        assert.strictEqual(Basics.resultToString(oResult, false), "binding" in oFixture ? oFixture.binding : String(oFixture.value), oFixture.type + " -> binding");
    });
});
QUnit.test("resultToString with type", function (assert) {
    [{
            ...
            binding: ",type:'sap.ui.model.odata.type.String'" + ",formatOptions:{'parseKeepsEmptyString':true}"
        }, {
        ...
        assert.strictEqual(Basics.resultToString(oFixture.value, false, true), "{path:'foo/\\'bar\\''" + oFixture.binding + "}", JSON.stringify(oFixture.value) + " (binding)");
        assert.strictEqual(Basics.resultToString(oFixture.value, true, true), "${path:'foo/\\'bar\\''" + oFixture.binding + "}", JSON.stringify(oFixture.value) + " (expression)");
    });
...
QUnit.test("followPath: Performance measurement points", function (assert) {
    var oAverageSpy = this.spy(Measurement, "average").withArgs("sap.ui.model.odata.AnnotationHelper/followPath", "", ["sap.ui.model.odata.AnnotationHelper"]), oEndSpy = this.spy(Measurement, "end").withArgs("sap.ui.model.odata.AnnotationHelper/followPath"), oMockedInterface = {
        getModel: function () {
            return {
