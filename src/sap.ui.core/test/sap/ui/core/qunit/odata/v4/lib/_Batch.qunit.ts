...
var oEntity = { "@odata.etag": "W/\"20180830000000.0000000\"" }, oEmployeesBody = {
...
function parseResponses(aResponses) {
    ...
    for (i = 0; i < aResponses.length; i += 1) {
        ...
        if (Array.isArray(oResponse)) {
            ...
            aResponses[i].responseText = JSON.parse(aResponses[i].responseText);
        }
    }
}
...
[{
        ...
        expectedBoundaryIDs: ["id-0123456789012-345", "id-9876543210987-654", "id-0123456789012-912"],
        requests: [
            {
                method: "GET",
                url: "Employees('1')",
        ...
        expectedBoundaryIDs: ["id-1450426018742-911", "id-1450426018742-912", "id-1450426018742-913"],
        requests: [
            [{
                    method: "POST",
                    url: "TEAMS",
    ...
    QUnit.test("serializeBatchRequest: " + oFixture.testTitle, function (assert) {
        var oBatchRequest, oHelperMock = this.mock(_Helper), aRequests = JSON.parse(JSON.stringify(oFixture.requests));
        ...
        assert.deepEqual(aRequests, oFixture.requests, "aRequests remained unchanged, apart from $ContentID");
...
[{
        ...
        errorMessage: "Invalid HTTP request method: GET. Change set must contain only POST, " + "PUT, PATCH, or DELETE requests."
    }].forEach(function (oFixture) {
    QUnit.test("validation serializeBatchRequest: " + oFixture.title, function (assert) {
        assert.throws(function () { _Batch.serializeBatchRequest(oFixture.requests); }, new Error(oFixture.errorMessage));
    });
});
[{
        ...
        contentType: " multipart/mixed; myboundary=\"invalid\"; " + "boundary=\"batch_id-0123456789012-345'()+_,-./:=?\"",
    ...
    QUnit.test("deserializeBatchResponse: " + oFixture.testTitle, function (assert) {
        var aResponses = _Batch.deserializeBatchResponse(oFixture.contentType, oFixture.body);
...
[{
    ...
    QUnit.test("Validation for deserializeBatchResponse: " + oFixture.title, function (assert) {
        assert.throws(function () {
            _Batch.deserializeBatchResponse("multipart/mixed; boundary=batch_id-0123456789012-345", oFixture.body);
...
if (TestUtils.isRealOData()) {
    [{ testTitle: "two get request for employees and departments", batchRequests: [{
                    method: "GET",
                    url: "EMPLOYEES",
                ...
                }] }, { testTitle: "get, delete and post request", batchRequests: [{
                    method: "GET",
                    url: "EMPLOYEES",
                    headers: { "Accept": "application/json" }
                }, {
                ...
                }] }, { testTitle: "GET not existing entity set", batchRequests: [{
                    method: "GET",
                    url: "Departments",
                    headers: { "Accept": "application/json" }
                }, {
                ...
                }] }, { testTitle: "POST to not existing entity within change set", batchRequests: [{
                    method: "GET",
                    url: "Departments",
                    headers: { "Accept": "application/json" }
                }, [
                ...
                }] }, { testTitle: "POST to not exist. entity within change set (odata.continue-on-error)", continueOnError: true, batchRequests: [{
                    method: "GET",
                    url: "Departments",
                    headers: { "Accept": "application/json" }
                }, [
                ...
                }] }, { testTitle: "changesets and individual requests", batchRequests: [{
                    method: "GET",
                    url: "EMPLOYEES",
                    headers: {
                        "Accept": "application/json"
                ...
                ]] }, { testTitle: "changeset with Content-ID reference", skip: true, batchRequests: [
                [{
                        method: "POST",
                        url: "TEAMS",
                        headers: {
        ...
        QUnit[oFixture.skip ? "skip" : "test"]("Multipart Integration Test: " + oFixture.testTitle, function (assert) {
            var oBatchRequestContent, done = assert.async();
            ...
            jQuery.ajax(sServiceUrl, {
                ...
                var sCsrfToken = jqXHR.getResponseHeader("X-CSRF-Token"), oBatchHeaders = {
                ...
                jQuery.ajax(sServiceUrl + "$batch", {
                    ...
                    aResponses = _Batch.deserializeBatchResponse(jqXHR.getResponseHeader("Content-Type"), oData);
    ...
    QUnit.test("Multipart Integration Test: response error handling", function (assert) {
        var oBatchRequestContent, done = assert.async(), sResolvedServiceUrl = sServiceUrl, aBatchRequests = [{
                method: "GET",
                url: "EMPLOYEES('9')",
                headers: { "Accept": "application/json" }
            }];
