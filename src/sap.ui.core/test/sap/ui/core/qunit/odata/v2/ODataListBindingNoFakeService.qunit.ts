...
[
    ...
    QUnit.test("loadData calls read w/ parameters refresh, headers, " + i, function (assert) {
        ...
        this.mock(oModel).expects("resolveDeep").withExactArgs("path", sinon.match.same(oContext)).returns("~deep");
...
[
    ...
    QUnit.test(sTitle, function (assert) {
        ...
        this.mock(oModel).expects("createCustomParams").withExactArgs(sinon.match.same(oFixture.parameters)).returns("~custom");
        ...
        oBinding = new ODataListBinding(oModel, "path", "context", undefined, undefined, oFixture.parameters);
...
["resolvedPath", undefined, null].forEach(function (sResolvedPath) {
    QUnit.test("_checkDataStateMessages: with deepPath: " + sResolvedPath, function (assert) {
        ...
        this.mock(oModel).expects("getMessagesByPath").withExactArgs("deepPath", true).exactly(sResolvedPath === "resolvedPath" ? 1 : 0).returns(aMessagesByPath);
        this.mock(oDataState).expects("setModelMessages").withExactArgs(aMessagesByPath).exactly(sResolvedPath === "resolvedPath" ? 1 : 0);
...
QUnit.test("_getFilterForPredicate: keys for predicate known", function (assert) {
    var oBinding = {}, oDataUtilsMock = this.mock(ODataUtils), oExpectedFilter = new Filter({
        and: true,
        filters: [
    ...
    assert.deepEqual(ODataListBinding.prototype._getFilterForPredicate.call(oBinding, sPredicate), oExpectedFilter);
});
QUnit.test("_getFilterForPredicate: key for predicate unknown", function (assert) {
    ...
    this.mock(oModel.oMetadata).expects("getKeyPropertyNamesByPath").withExactArgs("~deepPath~").returns(["SalesOrderID"]);
    this.mock(ODataUtils).expects("parseValue").withExactArgs("'42'").returns("~42~");
    assert.deepEqual(ODataListBinding.prototype._getFilterForPredicate.call(oBinding, "('42')"), new Filter("SalesOrderID", FilterOperator.EQ, "~42~"));
});
QUnit.test("_getFilterForPredicate: encoded key predicates; integrative", function (assert) {
    var oExpectedFilter = new Filter({
        ...
        aFilters: [
            ...
            new Filter("datetime", FilterOperator.EQ, new Date(Date.UTC(2021, 5, 18, 9, 50, 58))),
            new Filter("datetimems", FilterOperator.EQ, new Date(Date.UTC(2021, 5, 19, 9, 50, 58, 123))),
            new Filter("datetimeoffset", FilterOperator.EQ, new Date(Date.UTC(2021, 5, 20, 9, 50, 58))),
            new Filter("time", FilterOperator.EQ, { "__edmType": "Edm.Time", "ms": 34936000 }),
    ...
    }), sPredicate = "(" + "string='abc123''%20%21%22%c2%a7%24%25%26%2f%28%29%3d%3a%3b%2f%3f%2b'," + "datetime=datetime'2021-06-18T09%3a50%3a58'," + "datetimems=datetime'2021-06-19T09%3a50%3a58.123'," + "datetimeoffset=datetimeoffset'2021-06-20T09%3a50%3a58Z'," + "time=time'PT09H42M16S'," + "guid=guid'42010aef-0de5-1edb-aead-63ba217fb0e7'," + "null=null," + "decimal=-1.23m," + "double=-2.34d," + "float=-3.45f," + "int64=-123l," + "sbyte=-78," + "byte=255," + "true=true," + "false=false," + "binary=binary'0123456789abcdef'" + ")";
    assert.deepEqual(ODataListBinding.prototype._getFilterForPredicate.call({}, sPredicate), oExpectedFilter);
});
QUnit.test("requestFilterForMessages: unresolved", function (assert) {
    var oModel = { getMessagesByPath: function () { } }, oBinding = {
        oModel: oModel,
...
[true, false].forEach(function (bWithFilter) {
    [{
            ...
            aFilterForPredicate: bWithFilter ? ["(~keyPredicate0~)", "(~keyPredicate2~)"] : ["(~keyPredicate0~)", "(~keyPredicate1~)", "(~keyPredicate2~)", "(~keyPredicate3~)"],
            aMessages: [
        ...
        QUnit.test(sTitle, function (assert) {
            var oCallback = {
                fnFilter: function () { }
            }, oCallbackMock = this.mock(oCallback), aFilterForPredicate = oFixture.aFilterForPredicate, aFilters = [], aMessages = oFixture.aMessages, oModel = { getMessagesByPath: function () { } }, oBinding = {
                sDeepPath: "~deepPath~",
                oModel: oModel,
                _getFilterForPredicate: function () { },
                getResolvedPath: function () { }
            }, oBindingMock = this.mock(oBinding), oPromise;
            ...
            if (aMessages.length && bWithFilter) {
                aMessages.forEach(function (oMessage) {
                    oCallbackMock.expects("fnFilter").withExactArgs(sinon.match.same(oMessage)).returns(oMessage.message === "in");
                });
            }
            else {
            ...
            if (aFilterForPredicate.length) {
                aFilterForPredicate.forEach(function (sPredicate) {
                    ...
                    oBindingMock.expects("_getFilterForPredicate").withExactArgs(sPredicate).returns(oFilter);
            ...
            oPromise = ODataListBinding.prototype.requestFilterForMessages.call(oBinding, bWithFilter ? oCallback.fnFilter : undefined);
            assert.ok(oPromise instanceof Promise);
            return oPromise.then(function (oFilter) {
    ...
    [true, false].forEach(function (bBoundToList) {
        [true, false].forEach(function (bSuspended) {
            [true, false].forEach(function (bDataAvailable) {
                var sTitle = "initialize: initialize, #" + i + ", bound to a list: " + bBoundToList + ", suspended: " + bSuspended + ", data available: " + bDataAvailable;
                QUnit.test(sTitle, function (assert) {
                    var oBinding = {
                        oContext: oFixture.oContext,
                    ...
                    this.mock(oBinding).expects("getResolvedPath").withExactArgs().exactly(bBoundToList ? 0 : 1).returns(sResolvedPath);
                    this.oLogMock.expects("error").withExactArgs("List Binding is not bound against a list for ~resolvedPath").exactly(bBoundToList ? 0 : 1);
                    this.mock(oBinding).expects("_initSortersFilters").withExactArgs();
                    this.mock(oBinding).expects("_fireChange").withExactArgs({ reason: ChangeReason.Change }).exactly(!bSuspended && bDataAvailable ? 1 : 0);
                    this.mock(oBinding).expects("_fireRefresh").withExactArgs({ reason: ChangeReason.Refresh }).exactly(!bSuspended && !bDataAvailable ? 1 : 0);
...
[true, false].forEach(function (bV2Context) {
    var sTitle = "setContext: calls checkDataState if context changes; no created contexts;" + " use V2 context: " + bV2Context;
    QUnit.test(sTitle, function (assert) {
        var oModel = { resolveDeep: function () { } }, oBinding = {
            oContext: "~oContext",
            bInitial: false,
            oModel: oModel,
            sPath: "~sPath",
            ...
            getResolvedPath: function () { },
            isRelative: function () { }
        }, oBindingMock = this.mock(oBinding), oNewContext = {
        ...
        this.mock(Context).expects("hasChanged").withExactArgs("~oContext", sinon.match.same(oNewContext)).returns(true);
        ...
        this.mock(oModel).expects("resolveDeep").withExactArgs("~sPath", sinon.match.same(oNewContext)).returns("~resolvedDeepPath");
...
QUnit.test("setContext: context is transient, no created entities", function (assert) {
    ...
    this.mock(Context).expects("hasChanged").withExactArgs("~oContext", sinon.match.same(oNewV2Context)).returns(true);
    this.mock(oBinding).expects("_getCreatedContexts").withExactArgs().returns([]);
    this.mock(oBinding).expects("getResolvedPath").withExactArgs().returns("~resolvedPath");
    this.mock(oModel).expects("resolveDeep").withExactArgs("~sPath", sinon.match.same(oNewV2Context)).returns("~resolvedDeepPath");
...
QUnit.test("setContext: new context is transient and binding has persisted created entities", function (assert) {
    var oModel = { resolveDeep: function () { } }, oBinding = {
        aAllKeys: null,
        oContext: "~oContext",
        aKeys: [],
...
QUnit.test("setContext: set updated context while there are transient entities", function (assert) {
    var oContext = {
        isPreliminary: function () { },
        isRefreshForced: function () { },
        isTransient: function () { },
    ...
    this.mock(oBinding).expects("isRelative").withExactArgs().returns(true);
    this.mock(Context).expects("hasChanged").withExactArgs(sinon.match.same(oContext), sinon.match.same(oContext)).returns(true);
    this.mock(oBinding).expects("getResolvedPath").withExactArgs().returns("~resolvedPath");
    this.mock(oModel).expects("resolveDeep").withExactArgs("~sPath", sinon.match.same(oContext)).returns("~resolvedDeepPath");
    this.mock(oBinding).expects("_checkPathType").withExactArgs().returns(true);
    this.mock(oBinding).expects("checkDataState").withExactArgs();
    this.mock(oBinding).expects("_initSortersFilters").withExactArgs();
...
QUnit.test("checkExpandedList: getResolvedPath is called", function (assert) {
    var oModel = { _getObject: function () { } }, oBinding = {
        oContext: "~oContext",
        oModel: oModel,
        sPath: "~sPath",
        getResolvedPath: function () { }
    ...
    this.mock(oBinding).expects("getResolvedPath").withExactArgs().returns("~resolvedPath");
    this.mock(oModel).expects("_getObject").withExactArgs("~sPath", "~oContext").returns(undefined);
...
QUnit.test("getDownloadUrl: with resolved path", function (assert) {
    ...
    this.mock(oBinding).expects("getResolvedPath").withExactArgs().returns("~resolvedPath");
    this.mock(oBinding.oModel).expects("_createRequestUrl").withExactArgs("~resolvedPath", null, []).returns("~requestUrl");
...
QUnit.test("_getEntityType: with resolved path", function (assert) {
    ...
    this.mock(oBinding).expects("getResolvedPath").withExactArgs().returns("~resolvedPath");
    this.mock(oBinding.oModel.oMetadata).expects("_getEntityTypeByPath").withExactArgs("~resolvedPath").returns("~entityType");
...
QUnit.test("_getContexts: return V2 contexts, no created contexts", function (assert) {
    ...
    oModelMock.expects("getContext").withExactArgs("/~key(0)", "/~sDeepPath(0)").returns("~V2Context0");
    oModelMock.expects("getContext").withExactArgs("/~key(1)", "/~sDeepPath(1)").returns("~V2Context1");
    assert.deepEqual(ODataListBinding.prototype._getContexts.call(oBinding, 0, 2), ["~V2Context0", "~V2Context1"]);
});
[{
    ...
    QUnit.test("_getContexts: with createdContexts, #" + i, function (assert) {
        ...
        this.mock(oBinding).expects("_getCreatedContexts").withExactArgs().returns(oFixture.aCreatedContexts);
        oModelMock.expects("resolveDeep").withExactArgs("~path", "~context").returns("/~sDeepPath");
        this.mock(oBinding).expects("_getMaximumLength").withExactArgs().exactly(oFixture.iMaximumLength ? 1 : 0).returns(oFixture.iMaximumLength);
        oModelMock.expects("getContext").withExactArgs("/~key(0)", "/~sDeepPath(0)").exactly(oFixture.aResult.indexOf("~fromBackend0") >= 0 ? 1 : 0).returns("~fromBackend0");
        oModelMock.expects("getContext").withExactArgs("/~key(1)", "/~sDeepPath(1)").exactly(oFixture.aResult.indexOf("~fromBackend1") >= 0 ? 1 : 0).returns("~fromBackend1");
        assert.deepEqual(ODataListBinding.prototype._getContexts.call(oBinding, oFixture.iStart, oFixture.iLength), oFixture.aResult);
    });
});
[{
        bInitial: true
...
QUnit.test("getContexts: return V2 contexts returned by _getContexts", function (assert) {
    ...
    }, aContexts = ["~V2Context0", "~V2Context1"], aResultContexts;
...
[true, false].forEach(function (bLengthFinal) {
    QUnit.test("getContexts: use ODataUtils, bLengthFinal = " + bLengthFinal, function (assert) {
        var oModel = { getServiceMetadata: function () { } }, oBinding = {
            aKeys: [],
            iLength: 550,
            ...
            loadData: function () { },
            useClientMode: function () { }
        }, aContexts = [], oInterval = { start: 0, end: 110 }, aIntervals = [{ start: 0, end: 110 }];
        ...
        this.mock(oBinding).expects("_getSkipAndTop").withExactArgs(0, 10).returns({ skip: "~skip", top: "~top" });
        this.mock(ODataUtils).expects("_getReadIntervals").withExactArgs(oBinding.aKeys, "~skip", "~top", 100, bLengthFinal ? oBinding.iLength : undefined).returns(aIntervals);
        this.mock(ODataUtils).expects("_mergeIntervals").withExactArgs(sinon.match.same(aIntervals)).returns(oInterval);
        this.mock(oModel).expects("getServiceMetadata").withExactArgs().returns(true);
        this.mock(oBinding).expects("loadData").withExactArgs(sinon.match.same(oInterval.start), sinon.match.same(oInterval.end - oInterval.start));
        assert.deepEqual(ODataListBinding.prototype.getContexts.call(oBinding, 0, 10, 100), aContexts);
    });
});
QUnit.test("getContexts: use _getMaximumLength", function (assert) {
    ...
    this.mock(oBinding).expects("_getContexts").withExactArgs(0, "~iLength").returns("~aContexts");
    this.mock(oBinding).expects("useClientMode").withExactArgs().returns(true);
    assert.deepEqual(ODataListBinding.prototype.getContexts.call(oBinding, 0, undefined, 100), "~aContexts");
});
[
    ...
    var sTitle = "getContexts: use ODataUtils: pendingRequest = " + oFixture.pendingRequest + ", interval = " + oFixture.interval;
    QUnit.test(sTitle, function (assert) {
        var oModel = { getServiceMetadata: function () { } }, oBinding = {
            aKeys: [],
            iLength: 550,
            ...
            loadData: function () { },
            useClientMode: function () { }
        }, aContexts = [], aIntervals = [{ start: 0, end: 110 }];
        this.mock(oBinding).expects("_getContexts").withExactArgs(0, 10).returns(aContexts);
        this.mock(oBinding).expects("useClientMode").withExactArgs().returns(false);
        this.mock(oBinding).expects("_getSkipAndTop").withExactArgs(0, 10).returns({ skip: "~skip", top: "~top" });
        this.mock(ODataUtils).expects("_getReadIntervals").withExactArgs(oBinding.aKeys, "~skip", "~top", 100, oBinding.iLength).returns(aIntervals);
        this.mock(ODataUtils).expects("_mergeIntervals").withExactArgs(sinon.match.same(aIntervals)).returns(oFixture.interval);
        this.mock(oModel).expects("getServiceMetadata").withExactArgs().returns(true);
        if (oFixture.interval) {
            this.mock(oBinding).expects("loadData").withExactArgs(oFixture.interval.start, oFixture.interval.end - oFixture.interval.start).exactly(oFixture.expectLoad ? 1 : 0);
        }
        assert.deepEqual(ODataListBinding.prototype.getContexts.call(oBinding, 0, 10, 100), aContexts);
    });
});
...
[
    ...
    QUnit.test("getLength: bLengthFinal=true; #" + i, function (assert) {
        ...
        this.mock(oBinding).expects("_getCreatedContexts").withExactArgs().returns(oFixture.aCreatedContexts);
        assert.strictEqual(ODataListBinding.prototype.getLength.call(oBinding), oFixture.iExpectedValue);
    });
});
[{
        ...
        success: undefined
    }, undefined].forEach(function (mParameters, i) {
    QUnit.test("create: calls ODataModel#createEntry with parameters, #" + i, function (assert) {
        var oModel = {
            ...
            _fireChange: function () { },
            getResolvedPath: function () { }
        }, oCreatedContext = { created: function () { } }, oCreatedContextsCache = { addContext: function () { } }, mCreateParameters, oCreatePromise = { "catch": function () { } };
        this.mock(oModel.oMetadata).expects("isLoaded").withExactArgs().returns(true);
        this.mock(oBinding).expects("getResolvedPath").withExactArgs().returns("~resolvedPath");
        this.mock(oModel).expects("_getCreatedContextsCache").withExactArgs().returns(oCreatedContextsCache);
        this.mock(Object).expects("assign").withExactArgs(sinon.match(function (mParam0) {
        ...
        this.mock(oModel).expects("createEntry").withExactArgs("~resolvedPath", sinon.match(function (mParam) {
        ...
        this.mock(oCreatedContextsCache).expects("addContext").withExactArgs(sinon.match.same(oCreatedContext), "~resolvedPath", "~sCreatedEntitiesKey");
        this.mock(oBinding).expects("_fireChange").withExactArgs({ reason: ChangeReason.Add });
        assert.strictEqual(ODataListBinding.prototype.create.call(oBinding, "~oInitialData", "~bAtEnd", mParameters), oCreatedContext);
    });
});
QUnit.test("create: discard creation", function (assert) {
    var fnCatchHandler, oModel = {
        oMetadata: { isLoaded: function () { } },
        _getCreatedContextsCache: function () { },
        createEntry: function () { }
    }, oBinding = {
        ...
        _fireChange: function () { },
        getResolvedPath: function () { }
    }, oBindingMock = this.mock(oBinding), oCreatedContext = { created: function () { } }, oCreatedContextsCache = {
        addContext: function () { },
    ...
    this.mock(oBinding).expects("getResolvedPath").withExactArgs().returns("~resolvedPath");
    this.mock(oModel).expects("_getCreatedContextsCache").withExactArgs().returns(oCreatedContextsCache);
    this.mock(oModel).expects("createEntry").withExactArgs("~resolvedPath", {
        properties: "~oInitialData",
        refreshAfterChange: false
    }).returns(oCreatedContext);
    this.mock(oCreatedContext).expects("created").withExactArgs().returns(oCreatePromise);
    this.mock(oCreatePromise).expects("catch").withExactArgs(sinon.match(function (fnCatchHandler0) {
        fnCatchHandler = fnCatchHandler0;
        return true;
    }));
    this.mock(oCreatedContextsCache).expects("addContext").withExactArgs(sinon.match.same(oCreatedContext), "~resolvedPath", "~sCreatedEntitiesKey");
    oBindingMock.expects("_fireChange").withExactArgs({ reason: ChangeReason.Add });
    assert.strictEqual(ODataListBinding.prototype.create.call(oBinding, "~oInitialData", undefined, undefined), oCreatedContext);
    this.mock(oCreatedContextsCache).expects("removeContext").withExactArgs(sinon.match.same(oCreatedContext), "~resolvedPath", "~sCreatedEntitiesKey");
...
[
    ...
    QUnit.test("create: unsupported parameter: " + oFixture.sParameter, function (assert) {
        ...
        assert.throws(function () {
            ODataListBinding.prototype.create.call({}, undefined, undefined, mParameters);
        }, new Error(oFixture.sError));
    });
});
QUnit.test("create: bAtEnd set error", function (assert) {
    assert.throws(function () {
        ODataListBinding.prototype.create.call({}, undefined, true);
    }, new Error("Option 'bAtEnd' is not supported"));
});
QUnit.test("create: parent context isTransient error", function (assert) {
    var oContext = { isTransient: function () { } }, oBinding = { oContext: oContext };
...
QUnit.test("create: metadata not loaded error", function (assert) {
    var oModel = { oMetadata: { isLoaded: function () { } } }, oBinding = { oModel: oModel };
...
[{
        aCreatedContexts: Array(5),
        iLength: 23,
        bLengthFinal: false,
        iResult: 13
    }, {
        aCreatedContexts: Array(2),
        iLength: 23,
        bLengthFinal: true,
        iResult: 13
    }, {
        aCreatedContexts: Array(4),
        iLength: 8,
        bLengthFinal: true,
        iResult: 12
    }, {
        aCreatedContexts: Array(1),
        iLength: 10,
        bLengthFinal: true,
        iResult: 11
    }].forEach(function (oFixture, i) {
    QUnit.test("_getMaximumLength #" + i, function (assert) {
        ...
        this.mock(oBinding).expects("_getCreatedContexts").withExactArgs().exactly(oFixture.bLengthFinal ? 1 : 0).returns(oFixture.aCreatedContexts);
        assert.strictEqual(ODataListBinding.prototype._getMaximumLength.call(oBinding), oFixture.iResult);
    });
});
[{
        iCreated: 0,
        iLength: 10,
        iStartIndex: 3,
        oResult: { skip: 3, top: 10 }
    }, {
        iCreated: 4,
        iLength: 10,
        iStartIndex: 4,
        oResult: { skip: 0, top: 10 }
    }, {
        iCreated: 4,
        iLength: 10,
        iStartIndex: 6,
        oResult: { skip: 2, top: 10 }
    }, {
        iCreated: 6,
        iLength: 10,
        iStartIndex: 4,
        oResult: { skip: 0, top: 8 }
    }, {
        iCreated: 20,
        iLength: 5,
        iStartIndex: 4,
        oResult: { skip: 0, top: 0 }
    }].forEach(function (oFixture, i) {
    QUnit.test("_getSkipAndTop: #" + i, function (assert) {
        var oBinding = { _getCreatedContexts: function () { } };
        this.mock(oBinding).expects("_getCreatedContexts").withExactArgs().returns(Array(oFixture.iCreated));
        assert.deepEqual(ODataListBinding.prototype._getSkipAndTop.call(oBinding, oFixture.iStartIndex, oFixture.iLength), oFixture.oResult);
    });
});
QUnit.test("_getCreatedContexts:", function (assert) {
    var oModel = { _getCreatedContextsCache: function () { } }, oBinding = {
    ...
    this.mock(oBinding).expects("getResolvedPath").withExactArgs().returns("~Path");
    this.mock(oCache).expects("getContexts").withExactArgs("~Path", "~CreatedEntitiesKey").returns("~aContexts");
    assert.strictEqual(ODataListBinding.prototype._getCreatedContexts.call(oBinding), "~aContexts");
});