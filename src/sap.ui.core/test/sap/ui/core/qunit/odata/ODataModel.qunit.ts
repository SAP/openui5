...
QUnit.test("constructor: legacy arguments", function (assert) {
    var oModel = initModel(sURI, true, "user", "pa$$w0rd", { foo: "bar" }, false, true, true);
...
QUnit.test("test _isReloadNeeded() - __list check", function (assert) {
    var done = assert.async();
    var oModel = initModel(sURI, false), bReloadNeeded;
...
QUnit.test("test getKey decoded", function (assert) {
    var oModel = initModel(sURI, false, "Categories"), oObject = {
        __metadata: {
...
QUnit.test("Custom Header test", function (assert) {
    ...
    oModel.read("/Categories", null, null, false);
...
[false, true].forEach(function (bUseBatch) {
    ...
    QUnit.test(sTitle, function (assert) {
        ...
        this.oModel.oMetadata.loaded().then(function () {
            ...
            this.oModel._loadData("/Categories", {}, function () {
                var oRequest = bUseBatch ? oRequestSpy.args[1][0].data.__batchRequests[0] : oRequestSpy.args[1][0];
                assert.ok(oRequestSpy.calledTwice);
                assert.strictEqual(oRequest.method, "GET");
                assert.strictEqual(oRequest.requestUri, bUseBatch ? "Categories" : "/proxy/http/services.odata.org/V3/Northwind/Northwind.svc/Categories");
...
[false, true].forEach(function (bUseBatch) {
    var sTitle = "_loadData: x-csrf-token in GET requests if token handling is disabled, bUseBatch=" + bUseBatch;
    QUnit.test(sTitle, function (assert) {
        var done = assert.async();
        this.oModel.setUseBatch(bUseBatch);
        ...
        this.oModel.oMetadata.loaded().then(function () {
            var oRequestSpy = sinon.spy(OData, "request");
            this.oModel._loadData("/Categories", {}, function () {
                var oRequest = bUseBatch ? oRequestSpy.args[0][0].data.__batchRequests[0] : oRequestSpy.args[0][0];
                assert.ok(oRequestSpy.calledOnce);
                assert.strictEqual(oRequest.method, "GET");
                assert.strictEqual(oRequest.requestUri, bUseBatch ? "Categories" : "/proxy/http/services.odata.org/V3/Northwind/Northwind.svc/Categories");
...
[{
    ...
    QUnit.test("annotationsLoaded: not yet loaded, #" + i, function (assert) {
        ...
        this.mock(oModel.oMetadata).expects("isLoaded").withExactArgs().returns(oFixture.metadataLoaded);
        this.mock(oModel.oAnnotations).expects("isLoaded").withExactArgs().exactly(oFixture.metadataLoaded ? 1 : 0).returns(oFixture.annotationsLoaded);
...
[{ isLoaded: function () { } }, undefined].forEach(function (oAnnotations) {
    QUnit.test("annotationsLoaded: already loaded, with annotations: " + (!!oAnnotations), function (assert) {
        var oModel = {
