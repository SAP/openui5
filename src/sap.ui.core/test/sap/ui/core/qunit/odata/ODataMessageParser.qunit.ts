import fakeService from "test-resources/sap/ui/core/qunit/odata/data/ODataMessagesFakeService";
...
QUnit.test("ODataMessageParser reads headers case-insensitive", function (assert) {
    ...
    oMetadata.loaded().then(function () {
        var aNewMessages = [], oParser = new ODataMessageParser(sServiceURI, oMetadata), oRequest, oResponse;
...
QUnit.test("ODataMessageParser takes messages from specified header field", function (assert) {
    ...
    oMetadata.loaded().then(function () {
        ...
        oParser.parse(oResponse, oRequest);
        assert.equal(aNewMessages.length, 0, "No message parsed from 'invalid' header with invalid content");
