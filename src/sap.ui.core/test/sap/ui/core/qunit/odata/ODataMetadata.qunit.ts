...
import V1ODataModel from "sap/ui/model/odata/ODataModel";
...
var mInternalTests = {
    ...
    "_getFunctionImportMetadata method": function (oModel, assert, done) {
        assert.expect(this.functionImports.length * (6 + 3 * 3));
        var oMetadata = oModel.oMetadata;
...
var fnTestAnnotations = function (oModel, assert, done) {
    ...
    assert.equal(oServiceMetadata.dataServices.schema[0].annotations.length, 1, "exactly one annotation for the schema should exist");
    assert.equal(oMetadata._getAnnotation("/FlightCollection(1)/AirLineID/#@sap:label"), "Airline", "Airline label annotation correctly read");
    assert.equal(oMetadata._getAnnotation("/FlightCollection(1)/TotalBookingsSum/#@sap:unit"), "LocalCurrencyCode", "Total Bookings Sum unit annotation correctly read");
    assert.equal(oMetadata._getAnnotation("/FlightCollection(1)/FlightConnectionID/#@maxLength"), "4", "Flight Connection ID MaxLength attribute correctly read");
    assert.equal(oMetadata._getAnnotation("/FlightCollection(1)/FlightCarrier/CurrencyCode/#@sap:label"), "Airline Currency", "Label annotation correctly read via navigation property path");
    assert.equal(oMetadata._getAnnotation("/FlightCollection(1)/FlightCarrier/Unknown/#@sap:label"), null, "No annotation for unknown property name");
    assert.equal(oMetadata._getAnnotation("/#Flight/AirLineID/@sap:label"), "Airline", "Airline label annotation correctly read");
    assert.equal(oMetadata._getAnnotation("/#Flight/TotalBookingsSum/@sap:unit"), "LocalCurrencyCode", "Total Bookings Sum unit annotation correctly read");
    assert.equal(oMetadata._getAnnotation("/#Flight/FlightConnectionID/@maxLength"), "4", "Flight Connection ID MaxLength attribute correctly read");
    ...
    assert.deepEqual(mLabelAnnotation, { name: "label", value: "Airline", namespace: "http://www.sap.com/Protocols/SAPData" }, "Label Annotation correctly resolved");
    done();
};
...
QUnit.test("_getNavPropRefInfo", function (assert) {
    return this.oMetadata.loaded().then(function () {
        var oEntity = this.oMetadata._getEntityTypeByName("Product"), oNavPropRefInfo = {
            name: "ToSupplier",
...
QUnit.test("_getEntityAssociationEnd - metadata loaded", function (assert) {
    ...
    return oMetadata.loaded().then(function () {
        var oSalesOrderEntityType = oMetadata._getEntityTypeByName("GWSAMPLE_BASIC.SalesOrder"), oToBusinessPartnerAssociationEnd, oToLineItemsAssociationEnd;
        assert.strictEqual(oMetadata._mGetEntityAssociationEndCache, undefined, "Initial");
        oToLineItemsAssociationEnd = oMetadata._getEntityAssociationEnd(oSalesOrderEntityType, "ToLineItems");
        assert.strictEqual(oToLineItemsAssociationEnd.type, "GWSAMPLE_BASIC.SalesOrderLineItem", "ToLineItems");
        assert.strictEqual(oToLineItemsAssociationEnd.multiplicity, "*");
        assert.strictEqual(oToLineItemsAssociationEnd.role, "ToRole_Assoc_SalesOrder_SalesOrderLineItems");
        ...
        oToBusinessPartnerAssociationEnd = oMetadata._getEntityAssociationEnd(oSalesOrderEntityType, "ToBusinessPartner");
        assert.strictEqual(oToBusinessPartnerAssociationEnd.type, "GWSAMPLE_BASIC.BusinessPartner", "ToBusinessPartner");
        assert.strictEqual(oToBusinessPartnerAssociationEnd.multiplicity, "1");
        assert.strictEqual(oToBusinessPartnerAssociationEnd.role, "FromRole_Assoc_BusinessPartner_SalesOrders");
        assert.deepEqual(oMetadata._mGetEntityAssociationEndCache, {
            "GWSAMPLE_BASIC.SalesOrder/ToBusinessPartner": oToBusinessPartnerAssociationEnd,
            "GWSAMPLE_BASIC.SalesOrder/ToLineItems": oToLineItemsAssociationEnd
        });
        sinon.mock(_ODataMetaModelUtils).expects("findObject").withExactArgs(oSalesOrderEntityType.navigationProperty, "Foo");
        assert.strictEqual(oMetadata._getEntityAssociationEnd(oSalesOrderEntityType, "Foo"), null, "Foo");
        assert.deepEqual(oMetadata._mGetEntityAssociationEndCache, {
...
QUnit.test("_fillElementCaches - metadata loaded", function (assert) {
    var oMetadata = this.oMetadata;
    return oMetadata.loaded().then(function () {
        var oEntitySetMap, oSchema = oMetadata.oMetadata.dataServices.schema[0], aEntitySets = oSchema.entityContainer[0].entitySet, oSalesOrderType = oSchema.entityType[2], aSalesOrderNavigationProperties = oSalesOrderType.navigationProperty;
...
[{
        ...
        sPath: "/SalesOrderSet('42')/ToLineItems(SalesOrderID='42',ItemPosition='10')/ToHeader" + "/ToLineItems(SalesOrderID='42',ItemPosition='10')/ToHeader/Note",
        sReducedPath: "/SalesOrderSet('42')/Note"
    }, {
        iCacheItemReferencesCalls: 1,
        sPath: "/SalesOrderSet('42')/ToLineItems(SalesOrderID='42',ItemPosition='10')/ToProduct/Name",
        sReducedPath: "/SalesOrderSet('42')/ToLineItems(SalesOrderID='42',ItemPosition='10')/ToProduct/Name"
    }, {
        iCacheItemReferencesCalls: 1,
        ...
        iCacheItemReferencesCalls: 1,
        sPath: "/SalesOrderLineItemSet(SalesOrderID='42',ItemPosition='10')/ToHeader" + "/ToLineItems(SalesOrderID='42',ItemPosition='10')/Note",
        sReducedPath: "/SalesOrderLineItemSet(SalesOrderID='42',ItemPosition='10')/Note"
    }, {
        iCacheItemReferencesCalls: 1,
        sPath: "/SalesOrderLineItemSet(SalesOrderID='42',ItemPosition='10')/ToHeader" + "/ToLineItems(SalesOrderID='42',ItemPosition='20')/Note",
        sReducedPath: "/SalesOrderLineItemSet(SalesOrderID='42',ItemPosition='10')/ToHeader" + "/ToLineItems(SalesOrderID='42',ItemPosition='20')/Note"
    }, {
        iCacheItemReferencesCalls: 1,
        sPath: "/SalesOrderLineItemSet(SalesOrderID='42',ItemPosition='10')/ToHeader/ToLineItems",
        sReducedPath: "/SalesOrderLineItemSet(SalesOrderID='42',ItemPosition='10')/ToHeader" + "/ToLineItems"
    }, {
        iCacheItemReferencesCalls: 1,
        sPath: "/SalesOrder_Confirm(SalesOrderID='42')" + "/ToLineItems(SalesOrderID='42',ItemPosition='10')/ToHeader/Note",
        sReducedPath: "/SalesOrder_Confirm(SalesOrderID='42')/Note"
    }, {
        iCacheItemReferencesCalls: 2,
        sPath: "/SalesOrderSet('42')/ToBusinessPartner/ToContacts('aa-bb')/ToBusinessPartner" + "/ToSalesOrders('42')/Note",
        sReducedPath: "/SalesOrderSet('42')/Note"
    }, {
        iCacheItemReferencesCalls: 1,
    ...
    var sPath = oFixture.sPath, sReducedPath = oFixture.sReducedPath, sTitle = "_getReducedPath: " + sPath + " -> " + sReducedPath;
    QUnit.test(sTitle, function (assert) {
        var oMetadata = this.oMetadata;
        return oMetadata.loaded().then(function () {
            sinon.spy(oMetadata, "_fillElementCaches");
            assert.strictEqual(oMetadata._getReducedPath(sPath), sReducedPath);
            assert.strictEqual(oMetadata._fillElementCaches.callCount, oFixture.iCacheItemReferencesCalls);
...
[
    ...
    {
        sPath: "/SalesOrder_Confirm(SalesOrderID='42')" + "/ToLineItems(SalesOrderID='42',ItemPosition='10')/ToHeader",
        aKeyNames: ["SalesOrderID"]
    },
    {
        sPath: "/SalesOrder_Confirm(SalesOrderID='42')" + "/ToLineItems(SalesOrderID='42',ItemPosition='10')",
        aKeyNames: ["SalesOrderID", "ItemPosition"]
    },
    ...
    QUnit.test("getKeyPropertyNamesByPath: " + oFixture.sPath, function (assert) {
        var oMetadata = this.oMetadata;
        return oMetadata.loaded().then(function () {
            assert.deepEqual(oMetadata.getKeyPropertyNamesByPath(oFixture.sPath), oFixture.aKeyNames);
        });
    });
});