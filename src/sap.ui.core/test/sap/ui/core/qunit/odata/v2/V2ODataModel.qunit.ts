...
import ListItem from "sap/m/StandardListItem";
...
[true, false].forEach(function (bSuccess) {
    QUnit.test("Metadata promise " + (bSuccess ? "fulfilled" : "rejected"), function (assert) {
        ...
        return oModel.metadataLoaded(true).then(oTest.resolved, oTest.rejected).finally(function () {
            cleanSharedData();
            done();
        });
    });
...
QUnit.test("test oDataModel read sort test", function (assert) {
    ...
    oModel.read("/ProductSet", {
        ...
        filters: [new Filter("ProductID", "EQ", "HT-1000"), new Filter("ProductID", "EQ", "AD-1000"), new Filter("ProductID", "EQ", "HT-1041")],
        success: function (oData, oResponse) {
...
var initTable = function (mEntities) {
    ...
    for (var i = 0; i < mEntities.categories.properties.length; i++) {
        oTable.addColumn(new Column().setLabel(new Label({
            text: mEntities.categories.properties[i]
        })).setTemplate(new Input().bindProperty("value", mEntities.categories.properties[i])).setFilterProperty(mEntities.categories.properties[i]));
    }
    return oTable;
};
        ...
        success: function (oData, oResponse) {
            ...
            oModel.submitChanges({
                success: function (oData, oResponse) {
                    ...
                    assert.equal(oRequest.data.CurrencyCode, "EUR", "request payload currencyCode." + "Should be also here because price is a currency and has sap:unit = currency code");
                    var iCount = 0;
        ...
        success: function (oData, oResponse) {
            ...
            oModel.submitChanges({
                success: function (oData, oResponse) {
                    ...
                    assert.equal(oRequest.data.Name, "xy", "request payload name");
                    assert.equal(oRequest.data.Price, "4445.6", "request payload price");
                    assert.equal(oRequest.data.CurrencyCode, "USD", "request payload currencyCode." + "Should be also here because price is a currency and has sap:unit = currency code");
                    var iCount = 0;
                    each(oRequest.data, function (iIndex, oValue) {
                        iCount++;
                    });
...
QUnit.test("test eventing when using read with batch on", function (assert) {
    ...
    oModel.attachBatchRequestCompleted(this, function (oEvent) {
        ...
        if (iReqComp === 1 && iReqSent === 1 && bSuccess) {
            ...
            assert.equal(aRequests[0].response.statusText, "OK", "internal response, Param Check: statusText");
...
QUnit.test("test eventing when using read and internal error with batch on", function (assert) {
    ...
    oModel.attachBatchRequestCompleted(this, function (oEvent) {
        ...
        if (iReqComp === 1 && iReqSent === 1 && iReqFailed == 0) {
            ...
            assert.equal(aRequests[0].response.statusText, "Not Found", "internal response, Param Check: statusText");
            assert.ok(oEvent.getParameter("ID") !== null, "Check Request ID");
            assert.equal(oEvent.getParameter("ID"), sReqId, "Check Request ID");
            done();
        }
...
QUnit.test("test eventing when using multiple requests with batch on", function (assert) {
    ...
    oModel.attachBatchRequestCompleted(this, function (oEvent) {
        ...
        if (iReqComp === 1 && iReqSent === 1 && bSuccess) {
            ...
            assert.equal(aRequests[0].response.statusText, "No Content", "internal response, Param Check: statusText");
            assert.equal(aRequests[1].response.statusCode, "200", "internal response, Param Check: statusCode");
            assert.equal(aRequests[1].response.statusText, "OK", "internal response, Param Check: statusText");
            assert.equal(aRequests[2].response.statusCode, "200", "internal response, Param Check: statusCode");
            assert.equal(aRequests[2].response.statusText, "OK", "internal response, Param Check: statusText");
            assert.ok(oEvent.getParameter("ID") !== null, "Check Request ID");
            assert.equal(oEvent.getParameter("ID"), sReqId, "Check Request ID");
            done();
        }
...
QUnit.test("test eventing when using multiple requests with batch on: equal requests", function (assert) {
    ...
    oModel.attachBatchRequestCompleted(this, function (oEvent) {
        ...
        if (iReqComp === 1 && iReqSent === 1 && iSuccess === 5) {
            ...
            assert.equal(aRequests[1].response.statusText, "No Content", "internal response, Param Check: statusText");
            assert.equal(aRequests[2].response.statusCode, "200", "internal response, Param Check: statusCode");
            assert.equal(aRequests[2].response.statusText, "OK", "internal response, Param Check: statusText");
            assert.equal(aRequests[3].response.statusCode, "200", "internal response, Param Check: statusCode");
            assert.equal(aRequests[3].response.statusText, "OK", "internal response, Param Check: statusText");
            assert.ok(oEvent.getParameter("ID") !== null, "Check Request ID");
            assert.equal(oEvent.getParameter("ID"), sReqId, "Check Request ID");
            done();
        }
...
QUnit.test("test abort of batch request before Sending", function (assert) {
    ...
    oModel.read("/ProductSet('HT-1001')", {
        ...
        error: function (oError) {
            ...
            assert.strictEqual(oError.statusText, "abort", "status text 'abort'");
        },
    ...
    oModel.attachBatchRequestCompleted(this, function (oEvent) {
        ...
        assert.strictEqual(oEvent.getParameter("response").statusText, "abort", "status text 'abort'");
...
QUnit.test("test abort of all requests in batch request of one group", function (assert) {
    ...
    oModel.attachBatchRequestCompleted(this, function (oEvent) {
        ...
        if (iCounterBatch === 1) {
            ...
            assert.ok(oEvent.getParameter("response").statusText !== "abort", "status text not 'abort'");
            done();
        }
    });
    var iCounter = 0;
...
QUnit.test("XMLHttpRequest abort of batch requests with ODataModel internal", function (assert) {
    ...
    assert.expect(3);
    var iEventCounter = 0;
    var bSuccess1 = false, bSuccess2 = false, bSuccess3 = false;
...
QUnit.test("test oDataModel listbinding with table - event order change/dataReceived - data already loaded", function (assert) {
    var done = assert.async();
    var iCount = 0, iChange = 0, iReceived = 0;
    var mEntities = {
        categories: {
            collection: "/VH_CategorySet",
            properties: ["Category"]
