...
function parseError(assert, oType, oValue) {
    TestUtils.withNormalizedMessages(function () {
        try {
            ...
            assert.strictEqual(e.message, "EnterTime " + oType.formatValue("23:59:58", "string"));
        }
    });
}
...
QUnit.test("basics", function (assert) {
    var oFormatOptions = {}, oType = new TimeOfDay(oFormatOptions);
...
[false, true, undefined].forEach(function (bNullable) {
    QUnit.test("with nullable=" + bNullable, function (assert) {
        ...
        assert.deepEqual(oType.oConstraints, bNullable === false ? { nullable: false } : undefined);
    });
});
["false", "true", "foo"].forEach(function (sNullable) {
    QUnit.test("illegal nullable value " + sNullable, function (assert) {
        var oType;
        this.oLogMock.expects("warning").withExactArgs("Illegal nullable: " + sNullable, null, "sap.ui.model.odata.type.TimeOfDay");
...
[0, undefined, 12].forEach(function (vPrecision) {
    QUnit.test("with precision=" + vPrecision, function (assert) {
        ...
        assert.deepEqual(oType.oConstraints, !vPrecision ? undefined : { precision: vPrecision });
    });
});
[-1, "foo", "", 42, 8.7, "3"].forEach(function (vPrecision) {
    QUnit.test("illegal precision value " + vPrecision, function (assert) {
        var oType;
        this.oLogMock.expects("warning").withExactArgs("Illegal precision: " + vPrecision, null, "sap.ui.model.odata.type.TimeOfDay");
...
[
    ...
    QUnit.test("formatValue with oFormatOptions=" + JSON.stringify(oFixture.oFormatOptions), function (assert) {
        var oDateFormatMock = this.mock(DateFormat), oType = new TimeOfDay(oFixture.oFormatOptions);
        ...
        oDateFormatMock.expects("getTimeInstance").withExactArgs(oFixture.oExpected).callThrough();
        oType.formatValue("13:53:49", "string");
        oType.formatValue("13:53:49", "string");
    });
});
QUnit.test("formatValue success", function (assert) {
    var oDate = new Date(1970, 0, 1, 2, 53, 49), oDateWithMS = new Date(1970, 0, 1, 13, 53, 49), oType = new TimeOfDay(undefined, { precision: 7 }), sValue = "13:53:49.1234567";
...
["int", "boolean", "float"].forEach(function (sTargetType) {
    QUnit.test("formatValue failure for target type " + sTargetType, function (assert) {
        ...
        try {
            ...
            assert.strictEqual(e.message, "Don't know how to format sap.ui.model.odata.type.TimeOfDay to " + sTargetType);
        }
    });
});
...
QUnit.test("parse", function (assert) {
    var oDate = new Date(1970, 0, 1, 2, 53, 49), oType = new TimeOfDay(), oTypePrecision = new TimeOfDay({ pattern: "HH:mm:ss.SSS a" }, { precision: 5 });
...
[[123, "int"], [true, "boolean"], [1.23, "float"]].forEach(function (aFixture) {
    QUnit.test("parse failure for source type " + aFixture[1], function (assert) {
        var oType = new TimeOfDay();
        assert.throws(oType.parseValue.bind(oType, aFixture[0], aFixture[1]), new ParseException("Don't know how to parse sap.ui.model.odata.type.TimeOfDay from " + aFixture[1]));
    });
});
QUnit.test("validate success", function (assert) {
    var oType = new TimeOfDay(), oTypePrecision = new TimeOfDay(undefined, { precision: 5 });
    [null, "23:59", "23:59:59", "23:59:59.1", "23:59:59.123", "23:59:59.12345"].forEach(function (sValue) {
...
QUnit.test("validate failure", function (assert) {
    var oType = new TimeOfDay(), oTypePrecision = new TimeOfDay(undefined, { precision: 5 });
    ["23", "23:60", "23:59:60", "24:00:00", "23:59:59.123456"].forEach(function (sValue) {
        assert.throws(function () {
            oTypePrecision.validateValue(sValue);
        }, new ValidateException("Illegal sap.ui.model.odata.type.TimeOfDay value: " + sValue));
    });
    assert.throws(function () {
...
QUnit.test("validate: nullable", function (assert) {
    TestUtils.withNormalizedMessages(function () {
        ...
        assert.throws(oType.validateValue.bind(oType, null), new ValidateException("EnterTime 11:59:58 PM"));
    });
});
QUnit.test("format, parse, validate with target type object", function (assert) {
    var oType = new TimeOfDay({ pattern: "HH:mm:ss.SSS a" }), sFormattedDate = oType.formatValue("13:53:49", "object"), sResultingDate = oType.parseValue(sFormattedDate, "object");
...
QUnit.test("getModelFormat() uses Gregorian calendar type", function (assert) {
    var oFormat, sModelValue = "13:53:49.123", oType = new TimeOfDay(undefined, { precision: 3 }), oParsedTimeOfDay;
    ...
    assert.strictEqual(oParsedTimeOfDay.getTime(), Date.UTC(1970, 0, 1, 13, 53, 49, 123), "parse value");
...
QUnit.test("localization change", function (assert) {
    var oControl = new Control(), oType = new TimeOfDay(), sValue = "13:53:49";
    oControl.bindProperty("tooltip", { path: "/unused", type: oType });
    oType.formatValue(sValue, "string");
    sap.ui.getCore().getConfiguration().setLanguage("de");
    assert.strictEqual(oType.formatValue(sValue, "string"), sValue, "adjusted to changed language");
});
QUnit.test("_resetModelFormatter", function (assert) {
    var oType = new TimeOfDay(), oFormat = oType.getModelFormat();
