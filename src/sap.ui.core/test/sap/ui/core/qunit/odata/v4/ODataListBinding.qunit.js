...
sap.ui.require([
	...
	QUnit.test("getContexts() calls fetchValue() and slices", function (assert) {
		var aContexts,
			aData = createData(10, 0, true), // Note: oRange is ignored here!
	...
	QUnit.test("checkUpdate: relative binding with cache, parent binding data has changed",
		...
		oPrototype.expects("makeCache").returns(undefined); // from applyParameters
		oPrototype.expects("makeCache").returns(oCacheTeam1); // from setContext
