	...
	[false, true].forEach(function (bSuspended) {
		...
		QUnit.test(sTitle, function (assert) {
			...
			this.mock(oBinding).expects("reset").exactly(bSuspended ? 0 : 1)
				.withExactArgs(ChangeReason.Refresh, true)
				.callsFake(function () {
...
[
	...
	[ // [first successful call, second failing call]
		...
		QUnit.test("create: bAtEnd #" + i, function (assert) {
			...
			this.mock(oBinding).expects("getUpdateGroupId").twice()
...
[{
	...
	QUnit.test("moveEntityHere", function (assert) {
		...
		this.mock(_Helper).expects("getRelativePath")
			.withExactArgs("/EMPLOYEES('1')", "/EMPLOYEES").returns("('1')");
		this.mock(oBinding).expects("getRootBinding").twice()
			.withExactArgs().returns(oRootBinding);
		this.mock(oRootBinding).expects("isSuspended").withExactArgs().returns(true);
...
[undefined, "", "('1')/EMPLOYEE_2_TEAM"].forEach(function (sRelativePath) {
	QUnit.test("moveEntityHere: path mismatch: " + sRelativePath, function (assert) {
		...
		this.mock(_Helper).expects("getRelativePath")
			.withExactArgs("/TEAMS('1')", "/EMPLOYEES").returns(sRelativePath);
	...
	QUnit.test("moveEntityHere: $expand to collection in context binding", function (assert) {
		...
		assert.throws(function () {
			// code under test
			oBinding.moveEntityHere(oEntityContext);
		}, new Error("~entity~context~: could not move the entity. Probably a list binding"
	...
	QUnit.test("moveEntityHere: context binding not ready yet", function (assert) {
		...
		this.mock(oEntityBinding).expects("getInheritableQueryOptions").withExactArgs()
			.returns("~queryOptions~");
		// fetchValue can become async if the context binding's oCachePromise is still pending
		this.mock(oEntityContext).expects("fetchValue").withExactArgs(null, null, true)
			.returns(SyncPromise.resolve(Promise.resolve("~entity~")));
