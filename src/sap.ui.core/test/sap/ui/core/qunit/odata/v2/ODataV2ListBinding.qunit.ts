    ...
    oModel.metadataLoaded().then(function () {
        ...
        var filterChangeHandler = function () {
            ...
            assert.deepEqual(aPaths, [
                ...
                "/Categories(1)",
                "/Categories(8)"
            ], "After modification sort order is different");
...
QUnit.test("ListBinding sort after filter", function (assert) {
    ...
    oBinding.initialize();
    var oFilter = new Filter("CategoryName", FilterOperator.EQ, "Beverages"), oSorter = new Sorter("CategoryName");
    oBinding.attachRefresh(function () {
...
QUnit.test("Error Case: Service returns partial data, Binding should keep data, length final", function (assert) {
    ...
    var oBinding = oModel.bindList("/Orders", null, null, [new Filter("ShipCity", "EQ", "TEST_FAULT_TOLERANCE")], {
        faultTolerant: true,
...
var checkSingleBatchRequestContent = function (oList, assert, iCallIndex, aExpectedRequests, sMessage) {
    ...
    assert.equal(aRequests.length, aExpectedRequests.length, "multiple requests within one batch should be performed." + " Actual: " + JSON.stringify(aRequestUris) + " Expected: " + JSON.stringify(aExpectedRequests));
...
var checkMultipleBatchRequestsContent = function (oList, assert, iCallIndex, aExpectedRequests, sMessage) {
    ...
    aExpectedRequests.forEach(function (aExpectedRequest, iIndex) {
        ...
        assert.equal(oBatchRequest1.length, aExpectedRequest.length, "" + (iIndex + 1) + ". batch should contain " + aExpectedRequest.length + " request. " + "Actual: " + JSON.stringify(aRequestUris) + " Expected: " + JSON.stringify(aExpectedRequest));
