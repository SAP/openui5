...
function setup() {
    ...
    aTestData = "<teamMembers>" + "<member firstName=\"Andreas\" lastName=\"Klark\"></member>" + "<member firstName=\"Peter\" lastName=\"Miller\"></member>" + "<member firstName=\"Gina\" lastName=\"Rush\"></member>" + "<member firstName=\"Steave\" lastName=\"Ander\"></member>" + "<member firstName=\"Michael\" lastName=\"Spring\"></member>" + "<member firstName=\"Marc\" lastName=\"Green\"></member>" + "<member firstName=\"Frank\" lastName=\"Wallace\"></member>" + "</teamMembers>";
    aTestDataChild = "<pets>" + "<pet type=\"ape\" age=\"1\"></pet>" + "<pet type=\"bird\" age=\"2\"></pet>" + "<pet type=\"cat\" age=\"3\"></pet>" + "<pet type=\"fish\" age=\"4\"></pet>" + "<pet type=\"dog\" age=\"5\"></pet>" + "</pets>";
    oModel = new XMLModel();
...
QUnit.test("test model bindAggregation on Listbox", function (assert) {
    var oLB = new List("myLb", { displaySecondaryValues: true, height: "200px" }), oItemTemplate = new ListItem();
    oLB.placeAt("target2");
    ...
    listItems.forEach(function (item, i) {
        assert.equal(item.getTitle(), oModel.getProperty("/member/" + i + "/@firstName"), "firstname check");
        assert.equal(item.getDescription(), oModel.getProperty("/member/" + i + "/@lastName"), "lastname check");
    });
});
QUnit.test("test XMLModel XML constructor", function (assert) {
    var testModel = new XMLModel();
    testModel.setXML("<root>" + "<foo>The quick brown fox jumps over the lazy dog.</foo>" + "<bar>ABCDEFG</bar>" + "<baz>52</baz>" + "</root>");
...
QUnit.test("test XML setXML error", function (assert) {
    ...
    var sXML = "<?xml version=\"1.0\"><teamMembers>" + "<member firstName=\"Andreas\" lastName=\"Klark\"></member>" + "<member firstName=\"Peter\" lastName=\"Miller\"></member>" + "<member firstName=\"Gina\" lastName=\"Rush\"></member>" + "<member firstName=\"Steave\" lastName=\"Ander\"></member>" + "<member firstName=\"Michael\" lastName=\"Spring\"></member>" + "<member firstName=\"Marc\" lastName=\"Green\"></member>" + "<member firstName=\"Frank\" lastName=\"Wallace\"></member>" + "</teamMembers>";
    var error = false;
...
QUnit.test("test XML getXML", function (assert) {
    var oModel = new XMLModel();
    var sXML = "<root>" + "<foo>The quick brown fox jumps over the lazy dog.</foo>" + "<bar>ABCDEFG</bar>" + "<baz>52</baz>" + "</root>";
    oModel.setXML(sXML);
...
QUnit.test("test XML compatible syntax", function (assert) {
    var oModel = new XMLModel(), value, oContext;
    oModel.setLegacySyntax(true);
    oModel.setXML(aTestData);
...
QUnit.test("text XML getObject", function (assert) {
    var oModel = new XMLModel();
    oModel.setXML(aTestData);
    var oNode = oModel.getObject("/member/4/@lastName");
    assert.ok(oNode);
