...
sap.ui.define([
	...
	var bVersionInfoAlreadyLoaded = false;
	...
	QUnit.module("VersionInfo", {
		...
		checkVersionInfoRequest: function (assert, bError) {
			assert.strictEqual(this.oServer.requests.length, bVersionInfoAlreadyLoaded ? 0 : 1,
				"Server should have received one request (async).");

			if (!bVersionInfoAlreadyLoaded || bError) {
	...
	QUnit.test("VersionInfo.load - file not found", function(assert) {
		...
		return VersionInfo.load().then(function() {
			...
			this.checkVersionInfoRequest(assert, true);
		}.bind(this));
	});
