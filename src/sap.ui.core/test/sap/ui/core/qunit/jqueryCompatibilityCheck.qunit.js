...
sap.ui.define([
	"sap/m/MessageBox",
	"sap/m/FormattedText"
], function(MessageBox, FormattedText) {
	"use strict";

	QUnit.module("jQuery.htmlPrefilter self-closing-check");

	QUnit.test("Result of htmlPrefilter is same as input", function (assert) {
		var oConsoleErrorStub = sinon.stub(console, "error");
		var oMessageBoxStub = sinon.stub(MessageBox, "alert");
		var oAlertStub = sinon.stub(window, "alert");
		var oSapRequireStub = sinon.stub(sap.ui, "require");
		...
		oConsoleErrorStub.restore();
		oMessageBoxStub.restore();
		oAlertStub.restore();
		oSapRequireStub.restore();
	});
	...
	if (/(?:\?|&)sap-ui-xx-self-closing-check=(?:x|X|true)/.exec(window.location.search)) {
		...
		QUnit.test("MessageBox alert is shown", function (assert) {
			...
			var oMessageBoxStub = sinon.stub(MessageBox, "alert", function (oMsgText) {
				if (oMsgText && oMsgText instanceof FormattedText && /summary/.test(oMsgText.getHtmlText())) {
					finalAssertions();
				}
			});
			var oAlertStub = sinon.stub(window, "alert", function (sText) {
			...
			function finalAssertions() {
				...
				assert.ok(oMessageBoxAlertCallOne.args[0] instanceof FormattedText, "First finding - should be a sap.m.FormattedText control");
				...
				assert.ok(oMessageBoxCallTwo.args[0] instanceof FormattedText, "Second finding - should be a sap.m.FormattedText control");
		...
		QUnit.test("Fallback to alert when sap.ui.require is not available", function (assert) {
			...
			var oSapRequireStub = sinon.stub(sap.ui, "require").throws();
		...
		QUnit.test("Fallback to alert when sap.ui.require ErrorHandler is called", function (assert) {
			...
			var oSapRequireStub = sinon.stub(sap.ui, "require", function(aDependencies, fnCallback, fnErrBack) {
				fnErrBack(new Error());
				assert.ok(true, "sap.ui.require ErrorHandler was called.");
			});
			oSapRequireStub.withArgs(["sap/m/MessageBox", "sap/m/FormattedText", "sap/base/security/encodeXML"], sinon.match.any, sinon.match.any);
