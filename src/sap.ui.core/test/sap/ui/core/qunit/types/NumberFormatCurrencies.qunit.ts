...
QUnit.test("Currency format default formatting", function (assert) {
    ...
    assert.equal(oFormat.format(0.123, "EUR"), "0.12\u00A0EUR", "0.123");
    assert.equal(oFormat.format(123, "EUR"), "123.00\u00A0EUR", "123");
    assert.equal(oFormat.format(123.23, "EUR"), "123.23\u00A0EUR", "123.23");
    assert.equal(oFormat.format(1234, "EUR"), "1,234.00\u00A0EUR", "1234");
    assert.equal(oFormat.format(12345, "EUR"), "12,345.00\u00A0EUR", "12345");
    assert.equal(oFormat.format(12345.123, "EUR"), "12,345.12\u00A0EUR", "12345.123");
    assert.equal(oFormat.format(12345.12345, "EUR"), "12,345.12\u00A0EUR", "12345.12345");
    assert.equal(oFormat.format(1234567890, "EUR"), "1,234,567,890.00\u00A0EUR", "1234567890");
    assert.equal(oFormat.format(-123.23, "EUR"), "-123.23\u00A0EUR", "-123.23");
    assert.equal(oFormat.format("1.23e+9", "EUR"), "1,230,000,000.00\u00A0EUR", "1.23e+9");
    assert.equal(oFormat.format("1.23e-9", "EUR"), "0.00\u00A0EUR", "1.23e-9");
    assert.equal(oFormat.format("-1.23e+9", "EUR"), "-1,230,000,000.00\u00A0EUR", "-1.23e+9");
    assert.equal(oFormat.format("-1.23e-9", "EUR"), "-0.00\u00A0EUR", "-1.23e-9");
    assert.equal(oFormat.format("1.2345e+2", "EUR"), "123.45\u00A0EUR", "1.2345e+2");
    assert.equal(oFormat.format("12345e-2", "EUR"), "123.45\u00A0EUR", "12345e-2");
    assert.equal(oFormat.format("-1.2345e+2", "EUR"), "-123.45\u00A0EUR", "-1.2345e+2");
    assert.equal(oFormat.format("-12345e-2", "EUR"), "-123.45\u00A0EUR", "-12345e-2");
    assert.equal(oFormat.format("123.45e+2", "EUR"), "12,345.00\u00A0EUR", "123.45e+2");
    assert.equal(oFormat.format("12.345e-2", "EUR"), "0.12\u00A0EUR", "12.345e-2");
    assert.equal(oFormat.format("-123.45e+2", "EUR"), "-12,345.00\u00A0EUR", "-123.45e+2");
    assert.equal(oFormat.format("-12.345e-2", "EUR"), "-0.12\u00A0EUR", "-12.345e-2");
    assert.equal(oFormat.format("123456.789e+2", "EUR"), "12,345,678.90\u00A0EUR", "123456.789e+2");
    assert.equal(oFormat.format("123.456789e-2", "EUR"), "1.23\u00A0EUR", "123.456789e-2");
    assert.equal(oFormat.format("-123456.789e+2", "EUR"), "-12,345,678.90\u00A0EUR", "-123456.789e+2");
    assert.equal(oFormat.format("-123.456789e-2", "EUR"), "-1.23\u00A0EUR", "-123.456789e-2");
    assert.equal(oFormat.format("1000.00", "EUR"), "1,000.00\u00A0EUR", "1000.00");
    assert.equal(oFormat.format("1000.0000", "EUR"), "1,000.00\u00A0EUR", "1000.0000");
    assert.equal(oFormat.format(123456789.12345679, "EUR"), "123,456,789.12\u00A0EUR", "123456789.123456789 (number)");
    assert.equal(oFormat.format("123456789.123456789", "EUR"), "123,456,789.12\u00A0EUR", "123456789.123456789 (string)");
});
QUnit.test("Currency format default formatting preserveDecimals=true", function (assert) {
    ...
    assert.equal(oFormat.format(0.123, "EUR"), "0.123\u00A0EUR", "0.123");
    assert.equal(oFormat.format(123, "EUR"), "123.00\u00A0EUR", "123");
    assert.equal(oFormat.format(123.23, "EUR"), "123.23\u00A0EUR", "123.23");
    assert.equal(oFormat.format(1234, "EUR"), "1,234.00\u00A0EUR", "1234");
    assert.equal(oFormat.format(12345, "EUR"), "12,345.00\u00A0EUR", "12345");
    assert.equal(oFormat.format(12345.123, "EUR"), "12,345.123\u00A0EUR", "12345.123");
    assert.equal(oFormat.format(12345.12345, "EUR"), "12,345.12345\u00A0EUR", "12345.12345");
    assert.equal(oFormat.format(1234567890, "EUR"), "1,234,567,890.00\u00A0EUR", "1234567890");
    assert.equal(oFormat.format(-123.23, "EUR"), "-123.23\u00A0EUR", "-123.23");
    assert.equal(oFormat.format("1.23e+9", "EUR"), "1,230,000,000.00\u00A0EUR", "1.23e+9");
    assert.equal(oFormat.format("1.23e-9", "EUR"), "0.00000000123\u00A0EUR", "1.23e-9");
    assert.equal(oFormat.format("-1.23e+9", "EUR"), "-1,230,000,000.00\u00A0EUR", "-1.23e+9");
    assert.equal(oFormat.format("-1.23e-9", "EUR"), "-0.00000000123\u00A0EUR", "-1.23e-9");
    assert.equal(oFormat.format("1.2345e+2", "EUR"), "123.45\u00A0EUR", "1.2345e+2");
    assert.equal(oFormat.format("12345e-2", "EUR"), "123.45\u00A0EUR", "12345e-2");
    ...
    assert.equal(oFormat.format("-12345e-2", "EUR"), "-123.45\u00A0EUR", "-12345e-2");
    assert.equal(oFormat.format("123.45e+2", "EUR"), "12,345.00\u00A0EUR", "123.45e+2");
    assert.equal(oFormat.format("12.345e-2", "EUR"), "0.12345\u00A0EUR", "12.345e-2");
    assert.equal(oFormat.format("-123.45e+2", "EUR"), "-12,345.00\u00A0EUR", "-123.45e+2");
    assert.equal(oFormat.format("-12.345e-2", "EUR"), "-0.12345\u00A0EUR", "-12.345e-2");
    assert.equal(oFormat.format("123456.789e+2", "EUR"), "12,345,678.90\u00A0EUR", "123456.789e+2");
    assert.equal(oFormat.format("123.456789e-2", "EUR"), "1.23456789\u00A0EUR", "123.456789e-2");
    assert.equal(oFormat.format("-123456.789e+2", "EUR"), "-12,345,678.90\u00A0EUR", "-123456.789e+2");
    assert.equal(oFormat.format("-123.456789e-2", "EUR"), "-1.23456789\u00A0EUR", "-123.456789e-2");
    assert.equal(oFormat.format("1.20300", "EUR"), "1.203\u00A0EUR", "1.20300");
    assert.equal(oFormat.format("1000.00", "EUR"), "1,000.00\u00A0EUR", "1000.00");
    assert.equal(oFormat.format("1000.0000", "EUR"), "1,000.00\u00A0EUR", "1000.0000");
    assert.equal(oFormat.format("1000.00000000", "EUR"), "1,000.00\u00A0EUR", "1000.00000000");
    assert.equal(oFormat.format(123456789.12345679, "EUR"), "123,456,789.12345679\u00A0EUR", "123456789.123456789 (number)");
    assert.equal(oFormat.format("123456789.123456789", "EUR"), "123,456,789.123456789\u00A0EUR", "123456789.123456789 (string)");
});
...
QUnit.test("Currency parse with showNumber=false", function (assert) {
    ...
    assert.equal(oFormat.parse("1.00\nEUR"), null, "1.00 EUR");
    assert.equal(oFormat.parse("1.23\nXXX"), null, "1.23 XXX");
...
QUnit.test("Currency format with sMeasure and style", function (assert) {
    ...
    assert.equal(oFormat.format(123456.789, "EUR").toString(), "EUR" + "\u00A0" + "123K", "123456.789 EUR");
    oFormat = getCurrencyInstance({ style: "short" }, oLocale);
    assert.equal(oFormat.format(123456.789, "EUR").toString(), "EUR" + "\u00A0" + "123K", "123456.789 EUR");
    oFormat = getCurrencyInstance({ style: "standard" }, oLocale);
    assert.equal(oFormat.format(123456.789, "EUR").toString(), "EUR" + "\u00A0" + "123,456.79", "123456.789 EUR");
...
QUnit.test("Currency Format with fraction as decimals", function (assert) {
    ...
    assert.equal(oFormat.format(2, "EUR"), "EUR" + "\u00A0" + "2.000000", "fractions should set the decimals if not specified");
});
...
QUnit.test("Format using currency instance", function (assert) {
    ...
    }), sFormatted = oFormat.format(123456.789, "EUR");
    ...
    }), sFormatted2 = oFormat.format(123456.789, "EUR");
...
QUnit.test("Missing decimals information in defined custom currencies", function (assert) {
    ...
    assert.equal(sFormatted, "BTC" + "\u00A0" + "123,456.79", "Default decimals are 2");
});
QUnit.test("Custom Currencies defined via currency instance options", function (assert) {
    ...
    assert.equal(sFormatted, "BTC" + "\u00A0" + "123,456.789", "BTC 123,456.789");
...
QUnit.test("Format with currencies from global config", function (assert) {
    ...
    assert.equal(oFormat.format(123456.789, "BTC"), "BTC" + "\u00A0" + "123,456.79", "\u0243\u00A0123,456.79 - symbol lookup in global configuration.");
    assert.deepEqual(oFormat.parse(oFormat.format(123456.789, "BTC")), [123456.79, "BTC"], "[123456.79, 'BTC']");
    assert.equal(oFormat.format(123456.789, "Bitcoin"), "Bitcoin" + "\u00A0" + "123,456.789", "Bitcoin\u00A0123,456.789 - No symbol found");
    assert.deepEqual(oFormat.parse(oFormat.format(123456.789, "Bitcoin")), [123456.789, "Bitcoin"], "[123456.789, 'Bitcoin']");
    assert.equal(oFormat.format(123456.789, "DOLLAR"), "DOLLAR" + "\u00A0" + "123,456.7890", "DOLLAR 123,456.7890.");
...
QUnit.test("Custom Currencies instance overwrites global configuration", function (assert) {
    ...
    assert.equal(oFormat1.format(12345.6789, "DOLLAR"), "DOLLAR" + "\u00A0" + "12,345.679", "DOLLAR 12,345.679");
...
QUnit.test("Currencies with numbers in their names Edge Cases", function (assert) {
    ...
    assert.deepEqual(oFormatEN.format("2", "EUR"), "2.00" + "\u00A0" + "EUR", "2.00 EUR");
    assert.deepEqual(oFormatEN.format("2", "1"), "2.0" + "\u00A0" + "1", "2.0 1");
...
QUnit.test("Currencies with numbers in their names", function (assert) {
    ...
    assert.equal(oFormatEN.format(1234.5678, "4DOL"), "4DOL" + "\u00A0" + "1,234.57", "format in English locale - number at the start");
    ...
    assert.equal(oFormatEN.format(1234.5678, "DO"), "DO" + "\u00A0" + "1,234.5678", "format in English locale - number at the start");
    ...
    assert.equal(oFormatEN.format(1234.5678, "D4OL"), "D4OL" + "\u00A0" + "1,234.6", "format in English locale - number in the middle");
    ...
    assert.equal(oFormatEN.format(1234.5678, "DOL4"), "DOL4" + "\u00A0" + "1,234.568", "format in English locale - number at the end");
...
QUnit.test("Multiple custom currencies with same currency symbol", function (assert) {
    ...
    assert.equal(oFormat.format(12345.6789, "MON"), "MON" + "\u00A0" + "12,345.68", "MON 12,345.68");
    assert.equal(oFormat.format(12345.6789, "MONERO"), "MONERO" + "\u00A0" + "12,345.67890", "MONERO 12,345.6789");
...
QUnit.test("Currency format with sMeaure and the precision option should be ignored", function (assert) {
    ...
    assert.equal(oFormat.format(123456, "EUR"), "EUR" + "\u00A0" + "123,456.00", "123456 EUR");
    assert.equal(oFormat.format([123456.7, "EUR"]), "EUR" + "\u00A0" + "123,456.70", "123456.7 EUR");
...
QUnit.test("Currency format with sMeaure and style short. The default precision option shouldn't be ignored", function (assert) {
    ...
    assert.equal(oFormat.format(123456, "EUR"), "EUR" + "\u00A0" + "123K", "123456 EUR");
    assert.equal(oFormat.format([1234567.8, "EUR"]), "EUR" + "\u00A0" + "1.2M", "123456.7 EUR");
    assert.equal(oFormat.format(12345678.9, "EUR"), "EUR" + "\u00A0" + "12M", "-123456.78 EUR");
});
...
QUnit.test("format/parse indian lakhs/crores", function (assert) {
    ...
    var oFormat = getCurrencyInstance({}, oLocale);
    assert.equal(oFormat.format(100000, "INR"), "INR\u00A01,00,000.00", "INR is formatted with correct grouping");
    assert.equal(oFormat.format(10000000, "INR"), "INR\u00A01,00,00,000.00", "INR is formatted with correct grouping");
    assert.equal(oFormat.format(10000000000, "INR"), "INR\u00A01,000,00,00,000.00", "INR is formatted with correct grouping");
    assert.equal(oFormat.format(1000000000000, "INR"), "INR\u00A01,00,000,00,00,000.00", "INR is formatted with correct grouping");
    assert.equal(oFormat.format(100000000000000, "INR"), "INR\u00A01,00,00,000,00,00,000.00", "INR is formatted with correct grouping");
    oFormat = getCurrencyInstance({ style: "short" }, oLocale);
    assert.equal(oFormat.format(100000, "INR"), "INR\u00A01 Lk", "INR is formatted as Lk/Cr");
    assert.equal(oFormat.format(10000000, "INR"), "INR\u00A01 Cr", "INR is formatted as Lk/Cr");
    assert.equal(oFormat.format(10000000000, "INR"), "INR\u00A01,000 Cr", "INR is formatted as Lk/Cr");
    assert.equal(oFormat.format(1000000000000, "INR"), "INR\u00A01 Lk Cr", "INR is formatted as Lk/Cr");
    assert.equal(oFormat.format(100000000000000, "INR"), "INR\u00A01 Cr Cr", "INR is formatted as Lk/Cr");
    assert.equal(oFormat.format(100000, "USD"), "USD\u00A0100K", "USD is formatted as M/B/T");
    assert.equal(oFormat.format(1000000, "USD"), "USD\u00A01M", "USD is formatted as M/B/T");
    assert.equal(oFormat.format(1000000000, "USD"), "USD\u00A01B", "USD is formatted as M/B/T");
