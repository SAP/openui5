...
var oDateTime = new Date("Tue Sep 11 08:46:13 2001"), oTZDateTime = new Date("Tue Sep 11 03:46:13 2001 GMT+0530"), oDefaultDate = DateFormat.getInstance(), oDefaultDateTime = DateFormat.getDateTimeInstance(), oDefaultTime = DateFormat.getTimeInstance();
...
QUnit.test("format custom date", function (assert) {
    var oDate = new Date("Wed Jul 4 12:08:56 2001"), sCustomPattern, oCustomDate, oCustomDatePatterns = {
...
QUnit.test("format custom date UTC", function (assert) {
    var oDate = new Date("Wed Jul 4 12:08:56 2001 UTC"), oCustomDate;
...
QUnit.test("format custom date timezone UTC+0 (GMT)", function (assert) {
    var oDate = new Date("Tue Oct 9 13:37:56 2018"), oCustomDateFormat, oFormatted;
    oDate.getTimezoneOffset = function () { return 0; };
    oDate.setMilliseconds(235);
    [{
...
function getExpectedRelativeDate(iDiff, iTarget, oFormatOptions, sLocale) {
    ...
    var oFormat = DateFormat.getDateInstance(oFormatOptions, new Locale(sLocale)), sTargetDate = oFormat.format(new Date(iTarget)) + "", iDays = Math.abs(iDiff), aRange = oFormatOptions.relativeRange || [-6, 6], oLocaleData = LocaleData.getInstance(new Locale(sLocale));
...
function doTestRelative(assert, bFormat, oFormatOptions, sLocale, sTestInfo) {
    [undefined, "wide", "short", "narrow"].forEach(function (sStyle) {
        ...
        var oFormat1 = DateFormat.getDateInstance(jQuery.extend({ relative: bFormat }, oFormatOptions), new Locale(sLocale)), oFormat2 = DateFormat.getDateInstance(oFormatOptions, new Locale(sLocale)), oToday = new Date(), iToday = oToday.getTime(), iTarget, aExpected;
        for (var i = -10; i <= 10; i++) {
...
QUnit.module("DateFormat#parse (anno 1978)", {
    beforeEach: function () {
        this.clock = sinon.useFakeTimers(270909420000);
    },
    afterEach: function () {
        this.clock.restore();
    }
...
QUnit.test("parse date two digit year", function (assert) {
    ...
    var twoDigitMinus70 = "08";
    var twoDigitMinus71 = "07";
...
QUnit.module("DateFormat#parse (anno 2018)", {
    beforeEach: function () {
        this.clock = sinon.useFakeTimers(1533209820000);
    },
    afterEach: function () {
        this.clock.restore();
    }
...
QUnit.test("parse date two digit year", function (assert) {
    var oFormat = DateFormat.getDateInstance({ pattern: "yyyy-MM-dd" }), oDate;
    var twoDigitPlus30 = "48";
    var twoDigitPlus29 = "47";
...
QUnit.test("parse date two digit year UTC", function (assert) {
    var oFormat = DateFormat.getDateInstance({ pattern: "yyyy-MM-dd", UTC: true }), oDate;
    oDate = oFormat.parse("2014-03-12");
    assert.equal(oDate.getFullYear(), 2014, "Year 2014");
    oDate = oFormat.parse("0014-03-12");
    assert.equal(oDate.getFullYear(), 14, "Year 14");
...
QUnit.test("parse with strict check", function (assert) {
    var oFormat = DateFormat.getDateInstance({ pattern: "yyyy-MM-dd", strictParsing: true }), oDate;
...
QUnit.test("parse short format without delimiter", function (assert) {
    var oDate = new Date("Mon Apr 7 00:00:00 2014"), oFormat = DateFormat.getDateInstance(), sDateIn = "040714", oDateOut = oFormat.parse(sDateIn);
...
QUnit.test("parse and format two digit years", function (assert) {
    var oFormat = DateFormat.getDateInstance({ pattern: "M/d/y" }), sDate, oDate;
...
QUnit.test("parse and format quarters", function (assert) {
    var oDate = new Date("Wed Jul 4 2001"), oQuarter1 = DateFormat.getDateInstance({ pattern: "qq" }), oQuarter2 = DateFormat.getDateInstance({ pattern: "qqq" }), oQuarter3 = DateFormat.getDateInstance({ pattern: "qqqq" }), oQuarter4 = DateFormat.getDateInstance({ pattern: "qqqqq" }), oQuarter5 = DateFormat.getDateInstance({ pattern: "QQQQQ" }), oQuarterCombined = DateFormat.getDateInstance({ pattern: "EEE, MMM d yyyy, QQQ" });
...
QUnit.test("parse with fallback patterns", function (assert) {
    var oLocaleEN = new Locale("en_US"), oLocaleDE = new Locale("de_DE"), oFormat, iCompare = new Date(1975, 3, 16).getTime(), iFallbackOptionsLength = DateFormat.oDateInfo.aFallbackFormatOptions.length;
...
QUnit.test("parse and format fractional seconds", function (assert) {
    var oDate1 = new Date(1234), oDate2 = new Date(14), oMS1 = DateFormat.getDateInstance({ pattern: "s.S" }), oMS2 = DateFormat.getDateInstance({ pattern: "s.SSS" }), oMS3 = DateFormat.getDateInstance({ pattern: "s.SSSSSS" });
...
QUnit.test("format and parse time with am/pm in locale pt_PT", function (assert) {
    var oFormat = DateFormat.getTimeInstance({
        pattern: "hh:mm a"
    }, new Locale("pt_PT")), oDate = new Date(), sFormattedTime = oFormat.format(oDate), oParsedDate = oFormat.parse(sFormattedTime);
...
QUnit.test("Parse precedence: day (d) over dayName (E)", function (assert) {
    var oDate = new Date(1985, 9, 9);
    var sPattern = "yyyy-MM-dd EEEE";
    var oDateFormat = DateFormat.getDateInstance({
        pattern: sPattern
    }, new Locale("en_US"));
    var sFormattedWrongDayName = "1985-10-09 Friday";
...
QUnit.test("Parse precedence: year/month/day (yMd) over week (w)", function (assert) {
    ...
    var sFormattedWrongWeek = "1985-10-09 (CW 42)";
...
QUnit.test("Parse and format different formats", function (assert) {
    var oDate = new Date(1985, 9, 9), oDateFormat, sFormattedDate, oParsedDate;
...
function date(scale, diff) {
    var oNow = new Date(), oResult = new Date(Date.UTC(oNow.getFullYear(), oNow.getMonth(), oNow.getDate(), oNow.getHours(), oNow.getMinutes(), oNow.getSeconds(), oNow.getMilliseconds()));
    switch (scale) {
        case "second":
            oResult.setUTCSeconds(oResult.getUTCSeconds() + diff);
            break;
        case "minute":
            oResult.setUTCMinutes(oResult.getUTCMinutes() + diff);
            break;
        case "hour":
            oResult.setUTCHours(oResult.getUTCHours() + diff);
            break;
        case "day":
            oResult.setUTCDate(oResult.getUTCDate() + diff);
            break;
        case "month":
            oResult.setUTCMonth(oResult.getUTCMonth() + diff);
            break;
        case "year":
            oResult.setUTCFullYear(oResult.getUTCFullYear() + diff);
            break;
...
QUnit.test("Time relative: format and parse", function (assert) {
    var aStyles = [undefined, "wide", "short", "narrow"], aTestData = [{
            scale: "auto",
            data: [
...
QUnit.test("Date relative: format and parse", function (assert) {
    var that = this, aStyles = [undefined, "wide", "short", "narrow"], aTestData = [{
            scale: "auto",
            data: [
                ...
                { unit: "month", diff: 13, results: ["in 1 year", "in 1 year", "in 1 yr.", "in 1 yr."], description: "Today + 13 Months --> in 1 year", parseDiff: (31 * 24 + (that.dst ? 1 : 0)) * 60 * 60 * 1000 },
                { unit: "month", diff: 26, results: ["in 2 years", "in 2 years", "in 2 yr.", "in 2 yr."], description: "Today + 26 Months --> in 2 years", parseDiff: (61 * 24 + (that.dst ? 1 : 0)) * 60 * 60 * 1000 },
...
QUnit.test("DateTime relative: format and parse", function (assert) {
    var aStyles = [undefined, "wide", "short", "narrow"], that = this, aTestData = [{
            scale: "auto",
            data: [
                { unit: "second", diff: 0, results: ["now", "now", "now", "now"], description: "now" },
                { unit: "second", diff: 1, results: ["in 1 second", "in 1 second", "in 1 sec.", "in 1 sec."], description: "Now + 1 Second --> in 1 second" },
...
QUnit.test("format/parse date with Gannen instead of Ichinen", function (assert) {
    var oDate = new Date("May 1 2019"), sDate = "\u4EE4\u548C\u5143\u5E745\u67081\u65E5", oDateFormat = DateFormat.getDateInstance({
        calendarType: CalendarType.Japanese
    }, new Locale("ja_JP")), sFormatted = oDateFormat.format(oDate), oParsed = oDateFormat.parse(sDate);
...
QUnit.test("Check if end date is before start date", function (assert) {
    var aParsedInterval, oIntervalFormat = DateFormat.getDateInstance({
        interval: true,
        strictParsing: true
    });
...
QUnit.test("Allow single date", function (assert) {
    var oDate1 = new Date(2019, 0, 24), oDate2 = new Date(2019, 0, 31);
