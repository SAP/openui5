...
Object.keys(PRIMITIVE_TYPES).forEach(function (type) {
    var oTypeSetup = PRIMITIVE_TYPES[type], valid = nonEmptyArray(oTypeSetup.valid), invalid = nonEmptyArray(oTypeSetup.invalid);
    QUnit.test("'" + type + "[]'", function (assert) {
        var typeObject = DataType.getType(type);
        ...
        assert.ok(arrayTypeObject.getBaseType() instanceof DataType && arrayTypeObject.getBaseType().getName() === "array", "array types should have 'array' as base type");
...
QUnit.test("multiple levels of derivation", function (assert) {
    ...
    DataType.createType("myWorldSuffixType", {
        isValid: function (oValue) {
...
QUnit.test("AbsoluteCSSSize", function (assert) {
    ...
    assert.equal(type.isValid("calc(10 3rem)"), false, "an operand is mandatory between values");
    assert.equal(type.isValid("calc(10 + 3rem)"), true, "a valid calc() expression must be valid");
