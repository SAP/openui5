...
sap.ui.define([
	...
	"sap/ui/core/format/TimezoneUtil"
		...
		QUnit.test("convert CLDR to ABAP", function (assert) {
			...
			var sABAP_ID,
				oDateTimeFormatMock = this.mock(Intl.DateTimeFormat.prototype);

			Object.keys(TimezoneUtil.mCLDR2ABAPTimezones).forEach(function (sCLDR_ID) {
				TimezoneUtil._clearLocalTimezoneCache();
				oDateTimeFormatMock.expects("resolvedOptions").returns({timeZone: sCLDR_ID});

				sABAP_ID = TimezoneUtil.mCLDR2ABAPTimezones[sCLDR_ID];
...
[
	...
	QUnit.test("calculateOffset: Daylight Saving Time (DST) switch far away", function (assert) {
		var oFirstGuessInTimezone,
			oDate = {getTime: function () {}},
			oDateInTimezone = {getTime: function () {}},
			sTimezone = "~sTimezone",
		...
		oFirstGuessInTimezone = {getTime: function () {}};
	...
	QUnit.test("calculateOffset: DST switch, second guess matches", function (assert) {
		var oFirstGuessInTimezone, oSecondGuessInTimezone,
			oDate = {getTime: function () {}},
			oDateInTimezone = {getTime: function () {}},
			sTimezone = "~sTimezone",
			oTimezoneUtilMock = this.mock(TimezoneUtil);
		...
		oSecondGuessInTimezone = {getTime: function () {}};
	...
	//*********************************************************************************************
[true, false].forEach(function (bUseFirstOffset){
	var sTitle = "calculateOffset: DST switch, second guess doesn't match, use "
