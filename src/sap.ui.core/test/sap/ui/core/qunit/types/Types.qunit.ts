...
import DateIntervalType from "sap/ui/model/type/DateInterval";
import DateTimeType from "sap/ui/model/type/DateTime";
import DateTimeIntervalType from "sap/ui/model/type/DateTimeInterval";
import FileSizeType from "sap/ui/model/type/FileSize";
...
import TimeType from "sap/ui/model/type/Time";
import TimeIntervalType from "sap/ui/model/type/TimeInterval";
...
QUnit.test("currency formatValue", function (assert) {
    ...
    assert.equal(currencyType.formatValue([22, "EUR"], "string"), "22.00" + "\u00A0" + "EUR", "format test");
    assert.equal(currencyType.formatValue([22, "JPY"], "string"), "22" + "\u00A0" + "JPY", "format test");
    assert.equal(currencyType.formatValue([-6622.333, "EUR"], "string"), "-6,622.333" + "\u00A0" + "EUR", "format test");
    assert.equal(currencyType.formatValue([1, "EUR"], "string"), "1.00" + "\u00A0" + "EUR", "format test");
    assert.equal(currencyType.formatValue([1, "EUR"], "string"), "1.00" + "\u00A0" + "EUR", "format test");
    ...
    assert.throws(function () { currencyType.formatValue(22, "int"); }, FormatException, "format test");
    assert.throws(function () { currencyType.formatValue(22, "float"); }, FormatException, "format test");
    assert.throws(function () { currencyType.formatValue(22, "untype"); }, FormatException, "format test");
});
[
    ...
    [null, undefined].forEach(function (vInputValue) {
        var sTitle = "formatValue: showNumber=false skips invalid number check; " + i + ", " + vInputValue;
        QUnit.test(sTitle, function (assert) {
            var bSkipFormat = oFixture.result === null, oType = new CurrencyType(oFixture.formatOptions), aValues = [vInputValue, "EUR"];
            this.mock(oType).expects("getPrimitiveType").withExactArgs("string").exactly(bSkipFormat ? 0 : 1).returns("string");
            this.mock(oType.oOutputFormat).expects("format").withExactArgs(sinon.match.same(aValues)).exactly(bSkipFormat ? 0 : 1).returns("~formatted");
...
QUnit.test("currency parseValue", function (assert) {
    ...
    assert.deepEqual(currencyType.parseValue("USD" + "\u00A0" + "3333.555", "string"), [3333.555, "USD"], "parse test");
...
QUnit.test("parseValue: format option 'showNumber'; only enter the currency", function (assert) {
    ...
    assert.deepEqual(new CurrencyType({ showNumber: false }).parseValue("USD", "string"), [undefined, "USD"]);
});
QUnit.test("currency validateValue", function (assert) {
    var currencyType, oLogMock = this.mock(Log);
    oLogMock.expects("warning").never();
    currencyType = new CurrencyType(null, { minimum: 3, maximum: 10 });
    try {
        assert.equal(currencyType.validateValue([3, "EUR"]), undefined, "validate test");
    ...
    oLogMock.expects("warning").withExactArgs("Unknown constraint 'unknown': Value is not validated.", null, "sap.ui.model.type.Currency");
    currencyType.validateValue([3, "EUR"]);
});
QUnit.test("currency formatOptions", function (assert) {
    ...
    assert.equal(currencyType.formatValue([1, "USD"], "string"), "1.00", "format test");
    assert.equal(currencyType.formatValue([1, "JPY"], "string"), "1", "format test");
...
QUnit.test("currency formatOptions.source", function (assert) {
    ...
    assert.equal(currencyType.parseValue("EUR3333", "string"), "3333.00" + "\u00A0" + "EUR", "parse test");
    assert.equal(currencyType.parseValue("USD3333.555", "string"), "3333.56" + "\u00A0" + "USD", "parse test");
    assert.equal(currencyType.parseValue("$3.555", "string"), "3.56" + "\u00A0" + "USD", "parse test");
    assert.equal(currencyType.parseValue("JPY-3.555", "string"), "-4" + "\u00A0" + "JPY", "parse test");
    assert.equal(currencyType.formatValue("EUR22", "string"), "22.00" + "\u00A0" + "EUR", "format test");
    assert.equal(currencyType.formatValue("USD-6622.333", "string"), "-6,622.333" + "\u00A0" + "USD", "format test");
    assert.equal(currencyType.formatValue("JPY-6622.339", "string"), "-6,622.339" + "\u00A0" + "JPY", "format test");
});
...
QUnit.test("Parse/Format emptyString values (parseAsString)", function (assert) {
    var oCurrencyType = new CurrencyType({ parseAsString: true });
    assert.throws(function () {
        oCurrencyType.parseValue("", "string");
    }, new ParseException("Enter a valid currency amount"));
...
[{
    ...
    QUnit.test("Currency: getParseException #" + i, function (assert) {
        var oResult, oType = new CurrencyType(oFixture.oFormatOptions);
        TestUtils.withNormalizedMessages(function () {
...
QUnit.test("date validateValue", function (assert) {
    ...
    var dateType = new DateType(null, {
        minimum: dateValueMin,
        maximum: dateValueMax
    });
...
QUnit.test("filesize validateValue", function (assert) {
    ...
    try {
        assert.equal(filesizeType.validateValue(1000), undefined, "validate test: 1000.0-floatcompare");
    ...
    assert.throws(function () { filesizeType2.validateValue(1000); }, Error, "validate test: 1000.0-stringcompare");
    ...
    try {
        assert.equal(filesizeType.validateValue(1000), undefined, "validate test: 1000.0-floatcompare-inputformat");
    ...
    try {
        assert.equal(filesizeType2.validateValue(1000), undefined, "validate test: 1000.0-stringcompare-inputformat");
...
QUnit.test("float formatValue", function (assert) {
    ...
    assert.equal(floatType.formatValue(1, "string"), "1", "format test");
    assert.equal(floatType.formatValue(1, "string"), "1", "format test");
    ...
    assert.equal(floatType.formatValue(0, "int"), 0, "format test");
    assert.equal(floatType.formatValue(34.64, "int"), 34, "format test");
    assert.equal(floatType.formatValue(30, "int"), 30, "format test");
    ...
    assert.equal(floatType.formatValue(-344456.5667, "float"), -344456.5667, "format test");
    assert.equal(floatType.formatValue(134, "float"), 134, "format test");
    assert.equal(floatType.formatValue(134, "float"), 134, "format test");
    assert.throws(function () { floatType.formatValue(22, "untype"); }, FormatException, "format test");
});
...
QUnit.test("float validateValue", function (assert) {
    ...
    try {
        assert.equal(floatType.validateValue(3), undefined, "validate test");
    ...
    assert.throws(function () { floatType.validateValue(3); }, /~formatted3.001~/);
...
QUnit.test("float formatOptions", function (assert) {
    ...
    assert.equal(floatType.formatValue(1, "string"), "1.00", "format test");
    assert.equal(floatType.formatValue(1, "string"), "1.00", "format test");
...
QUnit.test("integer parseValue", function (assert) {
    ...
    TestUtils.withNormalizedMessages(function () {
        assert.throws(function () { intType.parseValue("3333.555", "string"); }, new ParseException("EnterInt"), "parse test");
        assert.throws(function () { intType.parseValue("true", "float"); }, new ParseException("EnterInt"), "parse test");
    });
...
QUnit.test("integer validateValue", function (assert) {
    var oFormatMock, intType = new IntegerType(null, {
        minimum: 3,
        maximum: 9999
    });
    try {
...
QUnit.test("Single constraint", function (assert) {
    var oBundle = sap.ui.getCore().getLibraryResourceBundle(), sExpectedMessage = oBundle.getText("Integer.Minimum").replace("{0}", "1"), oType = new IntegerType(null, {
        minimum: 1
    });
    try {
...
QUnit.test("Multiple constraints", function (assert) {
    var oBundle = sap.ui.getCore().getLibraryResourceBundle(), sMessage1 = oBundle.getText("Integer.Minimum").replace("{0}", "2"), sMessage2 = oBundle.getText("Integer.Maximum").replace("{0}", "0"), sExpectedMessage = sMessage1 + ". " + sMessage2 + ".";
...
QUnit.test("string formatValue", function (assert) {
    ...
    assert.throws(function () { stringType.formatValue("notfalse", "boolean"); }, FormatException);
    assert.throws(function () { stringType.formatValue("NaN", "int"); }, FormatException);
    assert.throws(function () { stringType.formatValue("d3f.442fs", "float"); }, FormatException);
});
...
QUnit.test("string validateValue", function (assert) {
    var oLogMock = this.mock(Log), stringType = new StringType(null, {
        minLength: 3,
        maxLength: 10
    });
    ...
    assert.throws(function () { stringType.validateValue("ddggggggggggg"); }, checkValidateException);
    ...
    assert.throws(function () { stringType.validateValue("abdccsbaab"); }, checkValidateException);
    stringType = new StringType(null, {
        equals: "ab"
    });
    ...
    assert.throws(function () { stringType.validateValue("adccsbba"); }, checkValidateException);
    stringType = new StringType(null, {
        foo: "ab"
    });
    oLogMock.expects("warning").withExactArgs("Ignoring unknown constraint: 'foo'", null, "sap.ui.model.type.String");
...
QUnit.test("unit formatValue", function (assert) {
    ...
    assert.equal(unitType.formatValue([1, "duration-hour"], "string"), "1 hr", "format test");
    assert.equal(unitType.formatValue([1, "duration-hour"], "string"), "1 hr", "format test");
    assert.equal(unitType.formatValue([1, "electric-ohm"], "string"), "1 \u03A9", "format test");
    ...
    assert.throws(function () { unitType.formatValue(22, "int"); }, FormatException, "format test");
    assert.throws(function () { unitType.formatValue(22, "float"); }, FormatException, "format test");
    assert.throws(function () { unitType.formatValue(22, "untype"); }, FormatException, "format test");
});
[
    { formatOptions: {}, result: null },
    { formatOptions: { showNumber: true }, result: null },
    ...
    [null, undefined].forEach(function (vInputValue) {
        var sTitle = "formatValue: showNumber=false skips invalid number check; " + i + ", " + vInputValue;
        QUnit.test(sTitle, function (assert) {
            var oOutputFormat = { format: function () { } }, bSkipFormat = oFixture.result === null, oType = new UnitType(oFixture.formatOptions), aValues = [vInputValue, "duration-hour"];
            this.mock(oType).expects("getPrimitiveType").withExactArgs("string").exactly(bSkipFormat ? 0 : 1).returns("string");
            this.mock(oType).expects("extractArguments").withExactArgs(sinon.match.same(aValues)).exactly(bSkipFormat ? 0 : 1).returns("~aDynamicValues");
            this.mock(oType).expects("_getInstance").withExactArgs("~aDynamicValues", "duration-hour").exactly(bSkipFormat ? 0 : 1).returns(oOutputFormat);
            this.mock(oOutputFormat).expects("format").withExactArgs(sinon.match.same(aValues)).exactly(bSkipFormat ? 0 : 1).returns("~formatted");
            assert.strictEqual(oType.formatValue(aValues, "string"), oFixture.result);
        });
    });
});
...
QUnit.test("parseValue: format option 'showNumber'; only enter the unit", function (assert) {
    ...
    assert.deepEqual(new UnitType({ showNumber: false }).parseValue("mph", "string"), [undefined, "speed-mile-per-hour"]);
});
...
QUnit.test("unit format and parse - custom units (local)", function (assert) {
    ...
    }, ParseException, "ParseException is thrown for wrong unit");
...
QUnit.test("unit validateValue - minimum and maximum value constraints", function (assert) {
    ...
    try {
        assert.equal(unitType.validateValue([3, "duration-hour"]), undefined, "validate test");
...
QUnit.test("unit type - formatValue with showMeasure false", function (assert) {
    ...
    assert.equal(unitType.formatValue([1, "electric-ohm"], "string"), "1", "format test");
    assert.equal(unitType.formatValue([1, "speed-mile-per-hour"], "string"), "1", "format test");
...
QUnit.test("unit type - formatValue with maxFractionDigits 2", function (assert) {
    ...
    assert.equal(unitType.formatValue([1, "electric-ohm"], "string"), "1 \u03A9", "format test");
    assert.equal(unitType.formatValue([1, "speed-mile-per-hour"], "string"), "1 mph", "format test");
...
QUnit.test("unit dynamic values", function (assert) {
    var MeterType = UnitType.extend("sap.ui.core.test.MeterType", {
        ...
        validateValue: function (vValue) {
            ...
            var oBundle = sap.ui.getCore().getLibraryResourceBundle(), aValues = vValue, iValue;
...
QUnit.test("Multiple Unit-Instances with bound custom units and other distinct format options", function (assert) {
    ...
    var oCustomUnitType = new CustomUnitType();
...
QUnit.test("Parse/Format emptyString values (parseAsString)", function (assert) {
    var oUnitType = new UnitType({ parseAsString: true });
    assert.throws(function () {
        oUnitType.parseValue("", "string");
    }, new ParseException("Enter a valid unit amount"));
...
[{
    ...
    QUnit.test("Unit: getParseException #" + i, function (assert) {
        var oResult, oType = new UnitType(oFixture.oFormatOptions);
        TestUtils.withNormalizedMessages(function () {
            oResult = oType.getParseException();
        });
        assert.ok(oResult instanceof ParseException);
