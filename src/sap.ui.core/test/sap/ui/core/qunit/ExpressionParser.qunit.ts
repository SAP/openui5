...
function check(assert, sExpression, vResult, oScope) {
    ...
    assert.strictEqual(oInvisibleText.getText(), oInvisibleText.validateProperty("text", vResult));
    oInvisibleText.destroy();
}
function checkFixtures(sTitle, aFixtures, fnInit) {
    aFixtures.forEach(function (oFixture) {
        QUnit.test(sTitle + " : " + oFixture.expression + " --> " + oFixture.result, function (assert) {
            if (fnInit) {
                fnInit.call(this);
            }
...
QUnit.module("sap.ui.base.ExpressionParser", {
    ...
    checkError: function (assert, sExpression, sMessage, iAt) {
        this.oLogMock.expects("error").withExactArgs(sMessage + (iAt !== undefined ? " at position " + iAt : ""), sExpression, "sap.ui.base.ExpressionParser");
        try {
...
[
    ...
    QUnit.test("Valid String literal " + oFixture.binding, function (assert) {
        var oExpression;
        oExpression = ExpressionParser.parse(function () { assert.ok(false, "unexpected call to fnResolveBinding"); }, oFixture.binding, 2);
...
[
    ...
    QUnit.test("Valid embedded binding " + oFixture.binding, function (assert) {
        ...
        oExpression = ExpressionParser.parse(resolveBinding, oFixture.binding, 2);
        assert.strictEqual(oExpression.result.formatter("tel"), "tel");
        assert.deepEqual(oExpression.result.parts, [oBinding.result], "parts array holds binding(s)");
...
checkFixtures("Operators ===, !==", [
    ...
    { expression: "${/tel} === ${/mail}", result: "false" },
    { expression: "true || false === false", result: "true" },
...
checkFixtures("Boolean literals, &&, ||, !", [
    ...
    { expression: "null && null.foo", result: "null" },
    { expression: "true || null.foo", result: "true" }
]);
...
checkFixtures("Member access and function call", [
    ...
    { expression: "odata.fillUriTemplate('http://www.foo.com')", result: "http://www.foo.com" },
    { expression: "odata.fillUriTemplate('http://www.foo.com', '/foo')", result: "http://www.foo.com/foo" },
    { expression: "'foo'.concat('bar')", result: "foobar" },
    { expression: "encodeURIComponent('foo bar')", result: "foo%20bar" },
    { expression: "odata.fillUriTemplate('http://foo/{t},{m}', {t: ${/tel}, 'm': ${/mail}})", result: "http://foo/tel,mail" },
    { expression: "odata.fillUriTemplate('http://foo/{t},{m}', {t: ${/mail}, 'm': ${/tel}})", result: "http://foo/mail,tel" }
], function () {
    var mGlobals = {
        odata: {
    ...
    this.mock(ExpressionParser).expects("parse").callsFake(function (fnResolveBinding, sInput, iStart) {
...
QUnit.test("Locals: function call, undefined, shadowing", function (assert) {
    var sInput = "{= MODEL.someFunction(undefined, ${A}) + Infinity }", mLocals = {
        Infinity: 42,
        MODEL: {
...
QUnit.test("Locals: constant", function (assert) {
    var sInput = "{= MODEL.someFunction() }", mLocals = {
        MODEL: {
    ...
    this.mock(ManagedObject).expects("bindingParser").withExactArgs(sInput, sinon.match.same(mLocals), true).callsFake(function (sString, oContext, bUnescape) {
...
QUnit.test("parse: no globals - no undefined!", function (assert) {
    this.oLogMock.expects("warning").withExactArgs("Unsupported global identifier 'undefined' in expression parser input 'undefined'", undefined, "sap.ui.base.ExpressionParser");
...
[
    ...
    { binding: "{={foo: 'bar', }}", message: "Expected IDENTIFIER but instead saw }", at: 16 },
    { binding: "{={foo: 'bar',", message: "Expected IDENTIFIER but instead saw end of input" },
    { binding: "{={true: 'bar'}}", message: "Expected IDENTIFIER but instead saw true", token: "true" },
    ...
    { binding: "{=odata.'foo'}", message: "Expected IDENTIFIER but instead saw 'foo'", token: "'foo'" },
    ...
    { binding: "{=[1,]}", message: "Unexpected ]", token: "]" },
    { binding: "{= '}", message: "Bad string", at: 6 },
    { binding: "{= 0 '}", message: "Bad string", at: 8 }
].forEach(function (oFixture) {
    QUnit.test("Error handling " + oFixture.binding + " --> " + oFixture.message, function (assert) {
        this.checkError(assert, oFixture.binding, oFixture.message, oFixture.at || oFixture.token && oFixture.binding.lastIndexOf(oFixture.token) + 1);
    });
});
[
    ...
    QUnit.test("Error handling: excess tokens: " + oFixture.binding, function (assert) {
        ...
        }, new SyntaxError("Expected '}' and instead saw '" + oFixture.binding[oFixture.at] + "' in expression binding " + oFixture.binding + " at position " + oFixture.at));
    });
});
[
    { binding: "{={}}", result: {} },
    ...
    QUnit.test("Object literal " + oFixture.binding, function (assert) {
        var oBindingInfo = ExpressionParser.parse(undefined, oFixture.binding, 2);
...
QUnit.test("Warning for undefined global identifier", function (assert) {
    this.oLogMock.expects("warning").withExactArgs("Unsupported global identifier 'foo' in expression parser input '{=42 === foo}'", undefined, "sap.ui.base.ExpressionParser");
    check(assert, "{=42 === foo}", "false");
    this.oLogMock.expects("warning").withExactArgs("Unsupported global identifier 'typeof42' in expression parser input '{=typeof42}'", undefined, "sap.ui.base.ExpressionParser");
    check(assert, "{=typeof42}", "undefined");
});
...
[true, false].forEach(function (bPreloaded) {
    QUnit.test("odata.compare, bPreloaded=" + bPreloaded, function (assert) {
        ...
        oSapUiMock.expects("require").withExactArgs("sap/ui/model/odata/v4/ODataUtils").returns(bPreloaded ? oDataUtils : undefined);
        oSapUiMock.expects("requireSync").withExactArgs("sap/ui/model/odata/v4/ODataUtils").exactly(bPreloaded ? 0 : 1).returns(oDataUtils);
...
[true, false].forEach(function (bPreloaded) {
    QUnit.test("odata.uriEncode, bPreloaded=" + bPreloaded, function (assert) {
        ...
        oSapUiMock.expects("require").withExactArgs("sap/ui/model/odata/ODataUtils").returns(bPreloaded ? oDataUtils : undefined);
        oSapUiMock.expects("requireSync").withExactArgs("sap/ui/model/odata/ODataUtils").exactly(bPreloaded ? 0 : 1).returns(oDataUtils);
...
[true, false].forEach(function (bPreloaded) {
    QUnit.test("odata.fillUriTemplate, bPreloaded=" + bPreloaded, function (assert) {
        var oOriginalExpand = URI.expand, oSapUiMock = this.mock(sap.ui), oURI = {
            expand: function () { }
        };
        oSapUiMock.expects("requireSync").withExactArgs("sap/ui/thirdparty/URITemplate").exactly(bPreloaded ? 0 : 1).callsFake(function () {
            URI.expand = oURI.expand;
            return "~URITemplate";
        });
        this.mock(oURI).expects("expand").withExactArgs("http://foo/{t},{m}", { m: "tel", t: "mail" }).returns("http://foo/mail,tel");
        URI.expand = bPreloaded ? oURI.expand : undefined;
        oSapUiMock.expects("require").withExactArgs("sap/ui/core/ResizeHandler").atLeast(0).callThrough();
        check(assert, "odata.fillUriTemplate('http://foo/{t},{m}', {'t': ${/mail}, 'm': ${/tel}})", "http://foo/mail,tel");
...
QUnit.test("errors during evaluation", function (assert) {
    ...
    this.oLogMock.expects("warning").withExactArgs(sinon.match(/TypeError:.*null/i), sExpression, "sap.ui.base.ExpressionParser");
    check(assert, sExpression, String(undefined));
});
QUnit.test("Internal incident 1570800230", function (assert) {
    var oIcon, oModel = new JSONModel({});
    oIcon = new Icon({
    ...
    assert.strictEqual(oIcon.validateProperty("decorative", undefined), true, "default value used!");
    oIcon.destroy();
});
QUnit.test("multiple references to the same binding", function (assert) {
    function checkParts(sExpression, sExpectedResult, iExpectedParts) {
        var sBinding = "{=" + sExpression + "}", oBindingInfo = BindingParser.complexParser(sBinding, oScope, true);
    ...
    checkParts("${path: '/five', type: 'sap.ui.model.type.String'} ? " + "${path: '/five', type: 'sap.ui.model.type.String'} : '7'", "5", 1);
    checkParts("${/five} ? ${path: '/five', formatter: '.myFormatter'} : '7'", "~5~", 2);
    checkParts("${path: '/five', formatter: '.myFormatter'} ? " + "${path: '/five', formatter: '.myFormatter'} : '7'", "~5~", 1);
...
[false, 42, "bar", undefined].forEach(function (vPrimitiveValue) {
    QUnit.test("saveBindingAsPart: primitive value " + vPrimitiveValue, function (assert) {
        ...
        this.mock(JSTokenizer).expects("parseJS").once().withExactArgs(sBinding, 5).returns({
            at: sBinding.indexOf("}"),
            result: {
                foo: vPrimitiveValue
            }
...
QUnit.test("parse: Performance measurement points", function (assert) {
    var oAverageSpy = this.spy(Measurement, "average").withArgs("sap.ui.base.ExpressionParser#parse", "", ["sap.ui.base.ExpressionParser"]), oEndSpy = this.spy(Measurement, "end").withArgs("sap.ui.base.ExpressionParser#parse");
...
QUnit.test("${} does not set targetType : 'any'", function (assert) {
    var sBinding = "{:= ${/five} === 5 }", oBindingInfo = BindingParser.complexParser(sBinding);
    assert.notOk("targetType" in oBindingInfo.parts[0]);
    check(assert, sBinding, true);
    check(assert, "{:= ${path : '/five', type : 'sap.ui.model.type.Integer'} === 5 }", false);
});
QUnit.test("${} does not set targetType : 'any' recursively", function (assert) {
    var sBinding = "{:= ${formatter:'.join', parts:[{path:'/five'},{path:'/thirteen'}]} }", oBindingInfo = BindingParser.complexParser(sBinding, oScope);
    assert.notOk("targetType" in oBindingInfo.parts[0]);
...
QUnit.test("%{} sets targetType : 'any'", function (assert) {
    var sBinding = "{:= %{path:'/five', type:'sap.ui.model.type.Integer'} === 5 }", oBindingInfo = BindingParser.complexParser(sBinding);
    assert.strictEqual(oBindingInfo.parts[0].targetType, "any");
    check(assert, sBinding, true);
});
QUnit.test("%{} sets targetType : 'any' recursively", function (assert) {
    var sBinding = "{:= %{formatter:'.join', parts:[" + "{path:'/five', type:'sap.ui.model.type.Integer'}," + "{path:'/thirteen', type:'sap.ui.model.type.Integer'}]} }", oBindingInfo = BindingParser.complexParser(sBinding, oScope);
    assert.strictEqual(oBindingInfo.parts[0].targetType, "any");
    assert.strictEqual(oBindingInfo.parts[1].targetType, "any");
    check(assert, sBinding, "5,13", oScope);
});
QUnit.test("%{} does not override targetType", function (assert) {
    var sBinding = "{:= %{path:'/five', targetType:'int', type:'sap.ui.model.type.Integer'} === 5 }", oBindingInfo = BindingParser.complexParser(sBinding);
...
QUnit.test("%{} does not override targetType recursively", function (assert) {
    var sBinding = "{:= %{formatter:'.join', parts:[" + "{path:'/five', targetType:'int', type:'sap.ui.model.type.Integer'}," + "{path:'/thirteen', targetType:'float', type:'sap.ui.model.type.Integer'}]} }", oBindingInfo = BindingParser.complexParser(sBinding, oScope);
    assert.strictEqual(oBindingInfo.parts[0].targetType, "int");
    assert.strictEqual(oBindingInfo.parts[1].targetType, "float");
    check(assert, sBinding, "5,13", oScope);
});
QUnit.test("%{} vs. ${}", function (assert) {
    var sBinding = "{:= %{path:'/five', type:'sap.ui.model.type.Integer'}" + " !== ${path:'/five', type:'sap.ui.model.type.Integer'} }", oBindingInfo = BindingParser.complexParser(sBinding);
    assert.strictEqual(oBindingInfo.parts.length, 2);
    check(assert, sBinding, true);
});
QUnit.test("JSTokenizer throws SyntaxError object", function (assert) {
    var iAt = 42, sInput = "{= 'foo' }", sMessage = "message", oError = {
        name: "SyntaxError",
        message: sMessage,
        at: iAt,
        text: sInput
    };
    this.mock(JSTokenizer.prototype).expects("white").withExactArgs().throws(oError);
    this.checkError(assert, sInput, sMessage, iAt);
});
QUnit.test("JSTokenizer throws Error", function (assert) {
    var sExpression = "{= 'foo' }", oError = new Error("Must not set index 0 before previous index 1");
    this.mock(JSTokenizer.prototype).expects("setIndex").throws(oError);
    assert.throws(function () {
        BindingParser.complexParser(sExpression);
    }, oError);
});
QUnit.test("Internal incident 1680322832", function (assert) {
    var oInvisibleText, oModel = new JSONModel({ ID: "T 1000" });
    oInvisibleText = new InvisibleText({
        text: "'{= encodeURIComponent(${/ID}) }'",
        models: oModel
    });
    assert.strictEqual(oInvisibleText.getText(), oInvisibleText.validateProperty("text", "'T%201000'"));
    oInvisibleText.destroy();
});
QUnit.test("Internal incident 1870185604", function (assert) {
    var mGlobals = {
        ...
        that: {}
    }, oMethodExpectation = this.mock(mGlobals.My).expects("method"), oResult;
    oMethodExpectation.twice().withExactArgs("mail").returns(42);
    oResult = BindingParser.parseExpression("{= My.method.call(that, ${/mail}) }", 2, null, mGlobals);
