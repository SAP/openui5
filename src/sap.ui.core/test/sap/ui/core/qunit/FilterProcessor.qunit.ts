...
QUnit.test("Contains w/ caseSensitive = undefined", function (assert) {
    ...
    var oNormalizeFilterValueSpy = sinon.spy(FilterProcessor, "normalizeFilterValue"), oToUpperCaseSpy = sinon.spy(String.prototype, "toUpperCase");
    var a = ["Wakeboarding", "Skateboarding", "Tennis", "Marathon", "Cycling", "Snowboarding", "Surfing"];
    var aFiltered = FilterProcessor.apply(a, oFilter, function (s) {
        return s;
    });
...
QUnit.test("apply: values contain 'toString' value", function (assert) {
    var aData = ["foo", "toString", "bar", "foo bar"], oFilter = new Filter({ path: "name", operator: FilterOperator.Contains, value1: "foo" });
    assert.deepEqual(FilterProcessor.apply(aData, oFilter, function (sValue) { return sValue; }), ["foo", "foo bar"]);
});
if (String.prototype.normalize) {
    QUnit.test("Caching of normalized values", function (assert) {
        var aData = ["Wakeboarding", "Skateboarding", "Tennis", "Marathon", "Cycling", "Snowboarding", "Surfing"], oFilter = new Filter({
            filters: [
            ...
            and: false
        }), aFiltered, oSpy = sinon.spy(String.prototype, "normalize");
        aFiltered = FilterProcessor.apply(aData, oFilter, function (s) { return s; }, {});
        assert.strictEqual(aFiltered.length, 2, "Two results found");
        assert.strictEqual(oSpy.callCount, 9, "Normalize is only called once per unique data or filter value");
        oSpy.restore();
    });
    ["foo", "toString"].forEach(function (sValue) {
        QUnit.test("normalizeFilterValue: case sensitive, value: " + sValue, function (assert) {
            ...
            this.mock(String.prototype).expects("normalize").on(sValue).withExactArgs("NFC").returns("~normalized");
            assert.deepEqual(FilterProcessor.normalizeFilterValue(sValue, true), "~normalized");
            assert.strictEqual(FilterProcessor._normalizeCache["true"][sValue], "~normalized", "noramalized value for '" + sValue + "' has been cached");
            assert.deepEqual(FilterProcessor.normalizeFilterValue(sValue, true), "~normalized");
        });
    });
}