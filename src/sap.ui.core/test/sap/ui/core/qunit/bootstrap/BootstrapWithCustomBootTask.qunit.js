...
sap.ui.define([
	...
	QUnit.test("BasicClassesAvailable", function(assert) {
		...
		var checkBootstrap = function () {
			...
			assert.equal(testresults.oLinksBefore && testresults.oLinksBefore.length, 1, "there should be 1 link tags for theme styles when the hook is called");
			assert.equal(testresults.oLinksAfter && testresults.oLinksAfter.length, 1, "there should be 1 link tags for theme styles after the hook has been called");
			assert.ok(document.querySelectorAll('head > link[id^="sap-ui-theme-"]').length === 1, "there should be same amount of link tags for theme styles after the hook has been called");
			assert.ok(document.querySelectorAll('head > link[id^="sap-ui-theme-"]')[0].href.includes("SapSampleTheme2"), "The link tag should be updated");
