...
sap.ui.define([
	...
	QUnit.test("_getContextsOrNodes: iStartIndex is undefined, no watermark", function (assert) {
		...
		oBindingMock.expects("_buildTree").withExactArgs(undefined, "~iSizeLimit");
		oBindingMock.expects("_retrieveNodeSection")
			.withExactArgs(sinon.match.same(oBinding._oRootNode), undefined, "~iSizeLimit")
			.returns(aNodes);
		oBindingMock.expects("_updateRowIndexMap")
			.withExactArgs(sinon.match.same(aNodes), undefined);
