...
import AnalyticalVersionInfo from "sap/ui/model/analytics/AnalyticalVersionInfo";
...
QUnit.module("sap.ui.model.analytics.odata4analytics", {
    ...
    beforeEach: function () {
        var oParameterizationRequest, oQueryResult, that = this;
        ...
        return this.oODataModel.getMetaModel().loaded().then(function () {
            that.oModel = new odata4analytics.Model(new odata4analytics.Model.ReferenceByModel(that.oODataModel));
            oQueryResult = that.oModel.findQueryResultByName("ActualPlannedCostsResults");
            oParameterizationRequest = new odata4analytics.ParameterizationRequest(oQueryResult.getParameterization());
...
QUnit.test("Get names of all query results offered by the service", function (assert) {
    ...
    assert.strictEqual(aQueryResultNames[1], "TypeWithHierarchiesResults", "Second Query Result Name is 'TypeWithHierarchiesResults'.");
    assert.strictEqual(aQueryResultNames[2], "CONTRACTPERFResults", "Third Query Result Name is 'CONTRACTPERFResults'.");
    assert.strictEqual(aQueryResultNames.length, 3, "Number of interpreted Query Results is correct.");
});
...
QUnit.test("Find query result by name", function (assert) {
    var mQueryResults = this.oModel.getAllQueryResults(), oQueryResult = this.oModel.findQueryResultByName("ActualPlannedCostsResults");
...
QUnit.test("Get all parameter names", function (assert) {
    var oQueryResult = this.oModel.findQueryResultByName("ActualPlannedCostsResults"), oParameterization = oQueryResult.getParameterization(), aParameterNames = oParameterization.getAllParameterNames();
...
QUnit.test("Try to fetch parameters from the parameterization", function (assert) {
    var oQueryResult = this.oModel.findQueryResultByName("ActualPlannedCostsResults"), oParameterization = oQueryResult.getParameterization(), oParameter = oParameterization.findParameterByName("P_CostCenter");
...
QUnit.test("Fetch peer boundary of an interval boundary parameter", function (assert) {
    var oQueryResult = this.oModel.findQueryResultByName("ActualPlannedCostsResults"), oParameter = oQueryResult.getParameterization().findParameterByName("P_CostCenter"), oPeerBoundaryIntervalParameter;
...
QUnit.test("Get all dimension names", function (assert) {
    var oQueryResult = this.oModel.findQueryResultByName("ActualPlannedCostsResults"), aDimensionNames = oQueryResult.getAllDimensionNames();
...
QUnit.test("Try to fetch specific dimensions from the query result", function (assert) {
    var oQueryResult = this.oModel.findQueryResultByName("ActualPlannedCostsResults"), oDimensionByName = oQueryResult.findDimensionByName("Currency"), oDimensionByAccess = oQueryResult.getAllDimensions()["Currency"];
...
QUnit.test("Getting attribute names from Dimensions", function (assert) {
    var oQueryResult = this.oModel.findQueryResultByName("ActualPlannedCostsResults"), oDimensionByName = oQueryResult.findDimensionByName("ControllingArea"), aDimensionAttributeNames = oDimensionByName.getAllAttributeNames();
    assert.ok(Array.isArray(aDimensionAttributeNames), "Attribute names Array could be retrieved.");
...
QUnit.test("Get all measure names", function (assert) {
    var oQueryResult = this.oModel.findQueryResultByName("ActualPlannedCostsResults"), aMeasureNames = oQueryResult.getAllMeasureNames();
...
QUnit.test("Try to fetch specific measures from the query result", function (assert) {
    var oQueryResult = this.oModel.findQueryResultByName("ActualPlannedCostsResults"), oMeasureByName = oQueryResult.findMeasureByName("ActualCosts"), oMeasureByAccess = oQueryResult.getAllMeasures()["ActualCosts"];
...
QUnit.test("Get all attribute names", function (assert) {
    var oQueryResult = this.oModel.findQueryResultByName("ActualPlannedCostsResults"), oDimension = oQueryResult.findDimensionByName("ControllingArea"), aAttributeNames = oDimension.getAllAttributeNames();
...
QUnit.test("Try to fetch specific attributes", function (assert) {
    var oQueryResult = this.oModel.findQueryResultByName("ActualPlannedCostsResults"), oDimension = oQueryResult.findDimensionByName("ControllingArea"), oAttributeByName = oDimension.findAttributeByName("ControllingAreaText"), oAttributeByAccess = oDimension.getAllAttributes()["ControllingAreaText"];
...
QUnit.test("Determine which properties of a query result are updatable", function (assert) {
    var oQueryResult = this.oModel.findQueryResultByName("ActualPlannedCostsResults"), oUpdatablePropertyNames = oQueryResult.getEntitySet().getUpdatablePropertyNameSet(), oUpdatable, iPropertyCount = 0, iDimensionCount = 0, iMeasureCount = 0, iUpdatablePropertyCount = 0, iCount;
...
QUnit.test("Access query with hierarchy from entity type", function (assert) {
    ...
    assert.ok(Array.isArray(aHierPropNames) && typeof aHierPropNames[0] === "string", "Hierarchy property names correctly fetched.");
...
QUnit.test("Access hierarchy from dimension", function (assert) {
    var oQueryResult = this.oModel.findQueryResultByName("ActualPlannedCostsResults"), oHierarchy, sHierarchyDimensionName = "CostCenter", sOtherDimensionName = "ControllingArea", oDimension = oQueryResult.findDimensionByName(sHierarchyDimensionName);
    assert.ok(!!oDimension, "Dimension correctly fetched.");
    oHierarchy = oDimension.getHierarchy("CostCenter");
    assert.ok(!!oHierarchy, "Hierarchy correctly fetched.");
    oDimension = oQueryResult.findDimensionByName(sOtherDimensionName);
    assert.ok(!!oDimension, "Dimension correctly fetched.");
    oHierarchy = oDimension.getHierarchy("ControllingArea");
    assert.ok(!!oHierarchy, "Hierarchy correctly fetched.");
});
QUnit.test("Create request for query parameterization", function (assert) {
    var oQueryResult = this.oModel.findQueryResultByName("ActualPlannedCostsResults"), oParamRequest = new odata4analytics.ParameterizationRequest(oQueryResult.getParameterization()), sURIToParamEntitySetRelative = oParamRequest.getURIToParameterizationEntitySet(), sURIToParamEntitySetAbsolute = oParamRequest.getURIToParameterizationEntitySet(sServiceURL);
    ...
    assert.strictEqual(sURIToParamEntitySetAbsolute, sServiceURL + "/ActualPlannedCosts", "Absolute URI to Parameterization Entity Set is correct.");
...
QUnit.test("Create request with parameters, no aggregation level and no measueres selected", function (assert) {
    var aDimNames, aMeasNames, oQueryResultRequest = this.oQueryResultRequest, sRefURI = "/ActualPlannedCosts(P_ControllingArea=%27US01%27,P_CostCenter=%27%27," + "P_CostCenterTo=%27%27)/Results", sSetURI = oQueryResultRequest.getURIToQueryResultEntries();
...
QUnit.test("Create request for query results", function (assert) {
    var oQueryResultRequest = this.oQueryResultRequest, sRefSetURI = "/ActualPlannedCosts(P_ControllingArea=%27US01%27,P_CostCenter=%27%27," + "P_CostCenterTo=%27%27)/Results", sSetURI, sRefEntriesURI, sEntriesURI, aRefDimNames, aDimNames, aRefMeasNames, aMeasNames;
    ...
    sRefEntriesURI = sRefSetURI + "?$select=ControllingArea,CostCenter,CostCenterText,ActualPlannedCostsDifference,id";
...
QUnit.test("Create filter expressions on the query result", function (assert) {
    var oQueryResultRequest = this.oQueryResultRequest, oFilterExpression = oQueryResultRequest.getFilterExpression(), sRefFilterOptionString, sFilterOptionString, sRefSetURI, sSetURI, sRefEntriesURI, sEntriesURI;
    ...
    sRefFilterOptionString = "(CostCenter eq %27100-1000%27 or (CostCenter ge %27100-1000%27" + " and CostCenter le %27200-3000%27)) and (Currency eq %27EUR%27 or Currency eq" + " %27USD%27 or Currency eq %27GBP%27)";
    ...
    sRefSetURI = "/ActualPlannedCosts(P_ControllingArea=%27US01%27,P_CostCenter=%27%27," + "P_CostCenterTo=%27%27)/Results";
    sSetURI = oQueryResultRequest.getURIToQueryResultEntitySet();
    assert.strictEqual(sSetURI, sRefSetURI, "QueryResultEntitySet URI correctly constructed.");
    sRefEntriesURI = sRefSetURI + "?$filter=(CostCenter eq %27100-1000%27 or (CostCenter ge %27100-1000%27 and" + " CostCenter le %27200-3000%27)) and (Currency eq %27EUR%27 or Currency eq" + " %27USD%27 or Currency eq %27GBP%27)";
    sEntriesURI = oQueryResultRequest.getURIToQueryResultEntries();
    assert.strictEqual(sEntriesURI, sRefEntriesURI, "QueryResultEntries URI correctly constructed.");
});
QUnit.test("Fetch value help for some parameters with filter", function (assert) {
    var oQueryResult = this.oModel.findQueryResultByName("ActualPlannedCostsResults"), oParameter = oQueryResult.getParameterization().findParameterByName("P_CostCenter"), oParamValueSetRequest, sRefParameterValueSetURI, sParameterValueSetURI, oBaseParameter, oFilterExpression, sRefFilterOptionString, sFilterOptionString, sURIToPVS, sRefURIToPVS;
    ...
    oBaseParameter = oQueryResult.getParameterization().findParameterByName("P_ControllingArea");
    ...
    oFilterExpression.addCondition(oBaseParameter.getName(), FilterOperator.EQ, "US01");
    sRefFilterOptionString = "(P_ControllingArea eq %27US01%27)";
    sFilterOptionString = oFilterExpression.getURIFilterOptionValue();
    assert.strictEqual(sFilterOptionString, sRefFilterOptionString, "FilterExpression URI correctly constructed.");
    sURIToPVS = oParamValueSetRequest.getURIToParameterValueSetEntries();
    sRefURIToPVS = "/ActualPlannedCosts?$select=P_CostCenter&$filter=(P_ControllingArea eq" + " %27US01%27)";
...
QUnit.test("Fetch value help for some query result dimension", function (assert) {
    var oQueryResult = this.oModel.findQueryResultByName("ActualPlannedCostsResults"), oParamRequest = new odata4analytics.ParameterizationRequest(oQueryResult.getParameterization()), oDimension, oDimMemberSetRequest, sRefURI, sURI, oBaseDim, oFilterExpression;
    ...
    oDimMemberSetRequest = new odata4analytics.DimensionMemberSetRequest(oDimension, oParamRequest, false);
    oDimMemberSetRequest.includeDimensionTextAttributes(true);
    sRefURI = "/ActualPlannedCosts(P_ControllingArea=%27US01%27,P_CostCenter=%27%27," + "P_CostCenterTo=%27%27)/Results?$select=CostCenter,CostCenterText";
...
QUnit.test("Create sorting expressions on the query result", function (assert) {
    var oQueryResultRequest = this.oQueryResultRequest, oFilterExpression = oQueryResultRequest.getFilterExpression(), oSortExpression, sRefBaseURI, sRefFilterOptionString, sFilterOptionString, sSorterString, sEntriesURI, sSetURI;
    ...
    oFilterExpression.addCondition("CostCenter", FilterOperator.BT, "100-1000", "200-3000");
    sRefBaseURI = "/ActualPlannedCosts(P_ControllingArea=%27US01%27,P_CostCenter=%27%27," + "P_CostCenterTo=%27%27)/Results?$select=ControllingArea,CostCenter,ActualCosts," + "PlannedCosts,ActualPlannedCostsDifference,ActualPlannedCostsPercentage&$filter=";
    sRefFilterOptionString = "((CostCenter ge %27100-1000%27 and CostCenter le %27200-3000%27))" + " and (Currency eq %27EUR%27 or Currency eq %27USD%27 or Currency eq %27GBP%27)";
    sFilterOptionString = oFilterExpression.getURIFilterOptionValue();
    ...
    sSorterString = "&$orderby=CostCenter asc";
    sSetURI = oQueryResultRequest.getURIToQueryResultEntries();
    assert.strictEqual(sSetURI, sRefBaseURI + sSorterString, "Ignore dimension which is not selected.");
    ...
    assert.strictEqual(oQueryResultRequest.getURIToQueryResultEntries(), "/ActualPlannedCosts(P_ControllingArea=%27US01%27,P_CostCenter=%27%27," + "P_CostCenterTo=%27%27)/Results?$select=ControllingArea,CostCenter," + "ActualCosts&$filter=" + sRefFilterOptionString + "&$orderby=CostCenter asc,PlannedCosts desc", "Allow measure which is not selected.");
});
QUnit.test("Fetch value help for some parameter with sorting", function (assert) {
    var oQueryResult = this.oModel.findQueryResultByName("ActualPlannedCostsResults"), oParamRequest = new odata4analytics.ParameterizationRequest(oQueryResult.getParameterization()), oQueryResultRequest = new odata4analytics.QueryResultRequest(oQueryResult), oParameter, oParamValueSetRequest, sRefParameterValueSetURI, sParameterValueSetURI, oBaseParameter;
...
QUnit.test("Add paging and a count to the query result", function (assert) {
    var oQueryResultRequest = this.oQueryResultRequest, oFilterExpression = oQueryResultRequest.getFilterExpression(), oSortExpression = oQueryResultRequest.getSortExpression(), sRefBaseURI, sRefFilterOptionString, sFilterOptionString, sRefCountURI, sSetURI;
    oQueryResultRequest.setAggregationLevel([
        "ControllingArea",
        "CostCenter"
    ]);
    ...
    assert.strictEqual(sFilterOptionString, sRefFilterOptionString, "FilterOptionString correctly constructed.");
    sRefBaseURI += sRefFilterOptionString;
    sRefCountURI = "&$orderby=CostCenter asc,ControllingArea desc&" + "$top=10&$inlinecount=allpages";
...
QUnit.test("Combining with some simple condition", function (assert) {
    var oQueryResultRequest = this.oQueryResultRequest, oFilterExpression = oQueryResultRequest.getFilterExpression(), oFilter1 = new Filter("Currency", FilterOperator.EQ, "GBP"), oFilter2 = new Filter("Currency", FilterOperator.EQ, "USD"), oFilter3 = new Filter("Currency", FilterOperator.EQ, "EUR"), oMultiFilter = new Filter([oFilter1, oFilter2, oFilter3], false), aUI5FilterArray, sRefFilterOptionString, sFilterOptionString;
    ...
    assert.ok((aUI5FilterArray instanceof Array && aUI5FilterArray[0] instanceof Filter), "UI5 filter array correctly constructed.");
    sRefFilterOptionString = "(CostCenter eq %27100-1000%27 or CostCenter eq %27100-1100%27)" + " and (((Currency eq %27GBP%27) or (Currency eq %27USD%27) or" + " (Currency eq %27EUR%27)))";
    sFilterOptionString = oFilterExpression.getURIFilterOptionValue();
    assert.strictEqual(sFilterOptionString, sRefFilterOptionString, "FilterOptionString correctly constructed.");
});
QUnit.test("Combining with other UI5 filters on same sPath", function (assert) {
    var oQueryResultRequest = this.oQueryResultRequest, oFilterExpression = oQueryResultRequest.getFilterExpression(), oFilter1 = new Filter("Currency", FilterOperator.EQ, "GBP"), oFilter2 = new Filter("Currency", FilterOperator.EQ, "USD"), oFilter3 = new Filter("Currency", FilterOperator.EQ, "EUR"), oMultiFilter = new Filter([oFilter1, oFilter2, oFilter3], false), aUI5FilterArray, sRefFilterOptionString, sFilterOptionString, sRefSetURI, sSetURI;
    oFilterExpression.addUI5FilterConditions([
        oMultiFilter
    ]);
    oFilterExpression.addUI5FilterConditions([
...
QUnit.test("Combining with other UI5 filters on different sPaths", function (assert) {
    ...
    aUI5FilterArray = oFilterExpression.getExpressionAsUI5FilterArray();
    assert.ok((aUI5FilterArray instanceof Array && aUI5FilterArray[0] instanceof Filter), "UI5 filter array correctly constructed.");
    sRefFilterOptionString = "(ControllingArea eq %27US01%27) and" + " (CostCenter eq %27100-1000%27) and (((Currency eq %27GBP%27) or" + " (Currency eq %27USD%27) or (Currency eq %27EUR%27)))";
    sFilterOptionString = oFilterExpression.getURIFilterOptionValue();
    assert.strictEqual(sFilterOptionString, sRefFilterOptionString, "FilterOptionString correctly constructed.");
...
QUnit.test("Tests 1-3 together", function (assert) {
    ...
    aUI5FilterArray = oFilterExpression.getExpressionAsUI5FilterArray();
    assert.ok((aUI5FilterArray instanceof Array && aUI5FilterArray[0] instanceof Filter), "UI5 filter array correctly constructed.");
    sRefFilterOptionString = "(ControllingArea eq %27US01%27) and (CostCenter eq %27100-1000%27" + " or CostCenter eq %27100-1100%27) and (((Currency eq %27GBP%27)" + " or (Currency eq %27USD%27) or (Currency eq %27EUR%27)))";
    sFilterOptionString = oFilterExpression.getURIFilterOptionValue();
    assert.strictEqual(sFilterOptionString, sRefFilterOptionString, "FilterOptionString correctly constructed.");
...
QUnit.test("Multiple UI5 filter arrays are combined with AND", function (assert) {
    ...
    aUI5FilterArray = oFilterExpression.getExpressionAsUI5FilterArray();
    assert.ok((aUI5FilterArray instanceof Array && aUI5FilterArray[0] instanceof Filter), "UI5 filter array correctly constructed.");
    sRefFilterOptionString = "(ControllingArea eq %27US01%27 or ControllingArea eq %27US02%27)" + " and (CostCenter eq %27100-1100%27 or CostCenter eq %27100-1000%27)" + " and (((Currency eq %27GBP%27) or (Currency eq %27USD%27)" + " or (Currency eq %27EUR%27)))";
    sFilterOptionString = oFilterExpression.getURIFilterOptionValue();
    assert.strictEqual(sFilterOptionString, sRefFilterOptionString, "FilterOptionString correctly constructed.");
});
QUnit.test("This is how it all begins", function (assert) {
    var oModelReference = new odata4analytics.Model.ReferenceByModel(this.oODataModel), oOData4SAPAnalyticsModel = new odata4analytics.Model(oModelReference);
...
QUnit.test("sap.chart.Chart", function (assert) {
    var bIsAnalytical, oModelReference = new odata4analytics.Model.ReferenceByModel(this.oODataModel), oOData4SAPAnalyticsModel = new odata4analytics.Model(oModelReference);
    ODataModelAdapter.apply(this.oODataModel);
    this.oODataModel.setAnalyticalExtensions(oOData4SAPAnalyticsModel);
    bIsAnalytical = oOData4SAPAnalyticsModel.findQueryResultByName("ActualPlannedCostsResults") !== undefined;
    assert.strictEqual(bIsAnalytical, true);
});
QUnit.test("From model to query results", function (assert) {
    var oActualPlannedCostsResults, oCONTRACTPERFResults, oOData4SAPAnalyticsModel = this.oODataModel.getAnalyticalExtensions(), oTypeWithHierarchiesResults;
    assert.deepEqual(oOData4SAPAnalyticsModel.getAllQueryResultNames(), ["ActualPlannedCostsResults", "TypeWithHierarchiesResults", "CONTRACTPERFResults"]);
    oActualPlannedCostsResults = oOData4SAPAnalyticsModel.findQueryResultByName("ActualPlannedCostsResults");
    assert.ok(oActualPlannedCostsResults instanceof odata4analytics.QueryResult);
    oTypeWithHierarchiesResults = oOData4SAPAnalyticsModel.findQueryResultByName("TypeWithHierarchiesResults");
    assert.ok(oTypeWithHierarchiesResults instanceof odata4analytics.QueryResult);
    oCONTRACTPERFResults = oOData4SAPAnalyticsModel.findQueryResultByName("CONTRACTPERFResults");
...
QUnit.test("Parameterization", function (assert) {
    var oQueryResult = this.oODataModel.getAnalyticalExtensions().findQueryResultByName("ActualPlannedCostsResults"), oQueryResultRequest, oParameterization, oParameterizationRequest;
    ...
    assert.strictEqual(oQueryResultRequest.getParameterizationRequest(), oParameterizationRequest);
    assert.strictEqual(oQueryResultRequest.getURIToQueryResultEntitySet(), "/ActualPlannedCosts(P_ControllingArea=%27US01%27,P_CostCenter=%27from%27" + ",P_CostCenterTo=%27to%27)/Results");
});
QUnit.test("sap.ovp.cards.AnnotationHelper", function (assert) {
    var oModelReference = new odata4analytics.Model.ReferenceByModel(this.oODataModel), oOData4SAPAnalyticsModel = new odata4analytics.Model(oModelReference), oQueryResult = oOData4SAPAnalyticsModel.findQueryResultByName("ActualPlannedCostsResults"), oQueryResultRequest = new odata4analytics.QueryResultRequest(oQueryResult), oParameterization = oQueryResult.getParameterization(), oParameterizationRequest = new odata4analytics.ParameterizationRequest(oParameterization);
    oQueryResultRequest.setParameterizationRequest(oParameterizationRequest);
    oParameterizationRequest.setParameterValue("P_ControllingArea", "US01");
    oParameterizationRequest.setParameterValue("P_CostCenter", "from", "to");
    assert.strictEqual(oQueryResultRequest.getURIToQueryResultEntitySet(), "/ActualPlannedCosts(P_ControllingArea=%27US01%27,P_CostCenter=%27from%27" + ",P_CostCenterTo=%27to%27)/Results", "result of sap.ovp.cards.AnnotationHelper.resolveParameterizedEntitySet");
    assert.strictEqual(oQueryResult.getEntitySet().getQName(), "ActualPlannedCostsResults", "catch block in case getURIToQueryResultEntitySet() fails");
});
QUnit.test("EntityType - static members, constructor and getters", function (assert) {
    var sNamespace = "http://www.sap.com/Protocols/SAPData", oFilterRestrictions, oHierarchy, oKey0 = {
        extensions: [
        ...
        extensions: [
            ...
            { namespace: sNamespace, name: "hierarchy-node-external-key-for", value: "property0" },
        ...
        extensions: [
            { namespace: sNamespace, name: "hierarchy-drill-state-for", value: "property2" }
        ],
        name: "property4"
    }, oDataJSEntityType = {
        key: {
        ...
        name: "EntityType",
        property: [oKey0, oKey1, oProperty0, oProperty1, oProperty2, oProperty3, oProperty4]
    }, oModel = {}, oProperties, oSchema = { namespace: "Schema.Namespace" }, oEntityType = new odata4analytics.EntityType(oModel, oSchema, oDataJSEntityType);
    ...
    assert.deepEqual(oEntityType.getFilterablePropertyNames(), ["key0", "property0", "property1", "property2", "property3", "property4"], "filterable properties");
    assert.deepEqual(oEntityType.getSortablePropertyNames(), ["key1", "property0", "property1", "property2", "property3", "property4"], "sortable properties");
    assert.deepEqual(oEntityType.getRequiredFilterPropertyNames(), ["key0", "property0"], "required-in-filter properties");
    ...
    assert.strictEqual(oEntityType.getAllHierarchyPropertyNames(), oEntityType._aHierarchyPropertyNames, "array of hierarchy properties stored");
    ...
    assert.strictEqual(oHierarchy.getNodeExternalKeyProperty(), undefined, "Hierarchy - property2");
    ...
    assert.strictEqual(odata4analytics.EntityType.prototype._oEntityType, null, "prototype members");
    ...
    assert.strictEqual(odata4analytics.EntityType.prototype._aRequiredFilterPropertyNames, null);
    assert.strictEqual(odata4analytics.EntityType.prototype._oPropertyFilterRestrictionSet, null);
...
QUnit.test("RecursiveHierarchy - constructor and getters", function (assert) {
    var oEntityType = {}, oNodeIDProperty = {}, oParentNodeIDProperty = {}, oNodeLevelProperty = {}, oNodeValueProperty = {}, oNodeExternalKeyProperty = {}, oRecursiveHierarchy = new odata4analytics.RecursiveHierarchy(oEntityType, oNodeIDProperty, oParentNodeIDProperty, oNodeLevelProperty, oNodeValueProperty, oNodeExternalKeyProperty);
    ...
    assert.strictEqual(oRecursiveHierarchy._oEntityType, oEntityType, "provided by constructor");
    ...
    assert.strictEqual(oRecursiveHierarchy.getNodeExternalKeyProperty(), oNodeExternalKeyProperty);
    assert.strictEqual(odata4analytics.RecursiveHierarchy.prototype._oEntityType, null, "prototype members");
    assert.strictEqual(odata4analytics.RecursiveHierarchy.prototype._oNodeIDProperty, null);
    assert.strictEqual(odata4analytics.RecursiveHierarchy.prototype._oParentNodeIDProperty, null);
    ...
    assert.strictEqual(odata4analytics.RecursiveHierarchy.prototype._oNodeExternalKeyProperty, null);
});
QUnit.test("QueryResultRequest - constructor and getters/setters", function (assert) {
    var oAnyValue = {}, oParameterizationRequest = {}, oQueryResult = {}, oQueryResultRequest = new odata4analytics.QueryResultRequest(oQueryResult, oParameterizationRequest);
    assert.strictEqual(oQueryResultRequest.getQueryResult(), oQueryResult);
    assert.strictEqual(oQueryResultRequest.getParameterizationRequest(), oParameterizationRequest);
    ...
    assert.strictEqual(oQueryResultRequest.getParameterizationRequest(), oAnyValue, "simple Getter/Setter");
    assert.strictEqual(odata4analytics.QueryResultRequest.prototype._oQueryResult, null, "prototype members");
    assert.strictEqual(odata4analytics.QueryResultRequest.prototype._oParameterizationRequest, null);
    ...
    assert.strictEqual(odata4analytics.QueryResultRequest.prototype._oDimensionHierarchies, null);
...
QUnit.test("QueryResultRequest#addRecursiveHierarchy", function (assert) {
    ...
    }, oDimensionMock = sinon.mock(oDimension), oHierarchy = {}, bIncludeExternalKey = {}, bIncludeText = {}, sName = "HierarchyDimensionName", oQueryResult = {
        findDimensionByName: function () { }
    }, oQueryResultMock = sinon.mock(oQueryResult), oQueryResultRequest = new odata4analytics.QueryResultRequest(oQueryResult);
    ...
    oQueryResultMock.expects("findDimensionByName").withExactArgs(sName).returns(oDimension);
    ...
    assert.strictEqual(oQueryResultRequest._oSelectedPropertyNames, null, "compiled list of selected properties reset");
    assert.deepEqual(oQueryResultRequest._oDimensionHierarchies[sName], {
        id: true,
        externalKey: bIncludeExternalKey,
        text: bIncludeText
    });
    assert.strictEqual(oQueryResultRequest._oDimensionHierarchies[sName].externalKey, bIncludeExternalKey);
...
QUnit.test("QueryResultRequest#getURIQueryOptionValue: $select - aggregation level", function (assert) {
    var mDimensions, oQueryResult = {
        findDimensionByName: function (sName) {
            return mDimensions[sName];
        }
    }, oDimension0 = new odata4analytics.Dimension(oQueryResult, { name: "dimension0" }), oDimension1 = new odata4analytics.Dimension(oQueryResult, { name: "dimension1" }), oDimension2 = new odata4analytics.Dimension(oQueryResult, { name: "dimension2" }), oDimension0Mock = sinon.mock(oDimension0), oDimension2Mock = sinon.mock(oDimension2), oQueryResultRequest = new odata4analytics.QueryResultRequest(oQueryResult);
    mDimensions = {
    ...
    oDimension2Mock.expects("getTextProperty").withExactArgs().returns({ name: "dimension2Text" });
    oDimension2Mock.expects("findAttributeByName").withExactArgs("dimension1").returns({ getName: function () { return "dimension1"; } });
    oDimension2Mock.expects("findAttributeByName").withExactArgs("attribute1").returns({ getName: function () { return "attribute1"; } });
    assert.strictEqual(oQueryResultRequest.getURIQueryOptionValue("$select"), "dimension1,dimension2,dimension2Text,attribute1");
...
QUnit.test("QueryResultRequest#getURIQueryOptionValue: $select - measures", function (assert) {
    var mMeasures, oQueryResult = {
        findMeasureByName: function (sName) {
            return mMeasures[sName];
        }
    }, oMeasure0 = new odata4analytics.Measure(oQueryResult, { name: "measure0" }), oMeasure1 = new odata4analytics.Measure(oQueryResult, { name: "measure1" }), oMeasure2 = new odata4analytics.Measure(oQueryResult, { name: "measure2" }), oMeasure0Mock = sinon.mock(oMeasure0), oMeasure1Mock = sinon.mock(oMeasure1), oMeasure2Mock = sinon.mock(oMeasure2), oQueryResultRequest = new odata4analytics.QueryResultRequest(oQueryResult);
    mMeasures = {
        measure0: oMeasure0,
        measure1: oMeasure1,
        measure2: oMeasure2
    ...
    oMeasure1Mock.expects("getRawValueProperty").withExactArgs().returns({ name: "measure1" });
    oMeasure2Mock.expects("getRawValueProperty").withExactArgs().returns({ name: "measure2" });
    oMeasure0Mock.expects("getFormattedValueProperty").withExactArgs().returns(null);
    oMeasure2Mock.expects("getFormattedValueProperty").withExactArgs().returns({ name: "measure2Text" });
    oMeasure0Mock.expects("getUnitProperty").withExactArgs().returns(null);
    oMeasure2Mock.expects("getUnitProperty").withExactArgs().returns({ name: "measure2Unit" });
    assert.strictEqual(oQueryResultRequest.getURIQueryOptionValue("$select"), "measure1,measure2,measure2Text,measure2Unit");
    assert.deepEqual(oQueryResultRequest._oSelectedPropertyNames, {
        measure1: true,
        measure2: true,
        measure2Text: true,
...
QUnit.test("QueryResultRequest#getURIQueryOptionValue: $select - keys", function (assert) {
    var oKeyProperty0 = { name: "key0" }, oKeyProperty1 = { name: "key1" }, oQueryResult = {
...
QUnit.test("QueryResultRequest#getURIQueryOptionValue: $select - dimension hierarchies", function (assert) {
    var oEntityType = { getTextPropertyOfProperty: function () { } }, oEntityTypeMock = sinon.mock(oEntityType), mHierarchy, oHierarchy0 = new odata4analytics.RecursiveHierarchy({}, {
        name: "nodeId0"
    }, {}, {}, {}, {
        name: "externalKey0"
    ...
    oQueryResultRequest._oDimensionHierarchies = {
        dimension0: { id: true, externalKey: true, text: true },
        dimension1: { id: true, text: true },
        dimension2: {}
    };
    oEntityTypeMock.expects("getTextPropertyOfProperty").withExactArgs("nodeId0").returns(null);
    oEntityTypeMock.expects("getTextPropertyOfProperty").withExactArgs("nodeId1").returns({ name: "nodeIdText1" });
    assert.strictEqual(oQueryResultRequest.getURIQueryOptionValue("$select"), "nodeId0,externalKey0,nodeId1,nodeIdText1");
    assert.deepEqual(oQueryResultRequest._oSelectedPropertyNames, {
        externalKey0: true,
        nodeId0: true,
        nodeId1: true,
...
QUnit.test("QueryResultRequest#getURIQueryOptionValue: $select - mixed", function (assert) {
    var mDimensions, oKeyProperty = { name: "key" }, mMeasures, oEntityType = {
    ...
    }, oDimension = new odata4analytics.Dimension(oQueryResult, { name: "dimension" }), oDimensionMock = sinon.mock(oDimension), oEntityTypeMock = sinon.mock(oEntityType), oHierarchy = new odata4analytics.RecursiveHierarchy({}, {
        name: "nodeId"
    }, {}, {}, {}, {
        name: "externalKey"
    }), oHierarchyDimension = new odata4analytics.Dimension(oQueryResult, { name: "hierarchyDimension" }), oHierarchyDimensionMock = sinon.mock(oHierarchyDimension), oMeasure = new odata4analytics.Measure(oQueryResult, { name: "measure" }), oMeasureMock = sinon.mock(oMeasure), oQueryResultRequest = new odata4analytics.QueryResultRequest(oQueryResult);
    mDimensions = {
    ...
    oDimensionMock.expects("findAttributeByName").withExactArgs("attribute").returns({ getName: function () { return "attribute"; } });
    oHierarchyDimensionMock.expects("getHierarchy").withExactArgs().returns(oHierarchy);
    oMeasureMock.expects("getRawValueProperty").withExactArgs().returns({ name: "measure" });
    oMeasureMock.expects("getFormattedValueProperty").withExactArgs().returns({ name: "measureText" });
    oMeasureMock.expects("getUnitProperty").withExactArgs().returns({ name: "dimension" });
    oEntityTypeMock.expects("getTextPropertyOfProperty").withExactArgs("nodeId").returns({ name: "nodeIdText" });
    assert.strictEqual(oQueryResultRequest.getURIQueryOptionValue("$select"), "dimension,dimensionText,attribute,measure,measureText,nodeId,externalKey,nodeIdText," + "key");
    assert.deepEqual(oQueryResultRequest._oSelectedPropertyNames, {
        attribute: true,
        dimension: true,
        dimensionText: true,
...
QUnit.test("deepEqual", function (assert) {
    ...
    function test(iExpectedResult, sProperty) {
        var oNewColumn = {}, aNewColumns = [{}, oNewColumn], oOldColumn = {}, aOldColumns = [{}, oOldColumn];
        oNewColumn[sProperty] = "new";
        oOldColumn[sProperty] = "old";
        if (iExpectedResult === 2) {
            aNewColumns.unshift({ formatter: "new" });
            aOldColumns.unshift({ formatter: "old" });
        }
        assert.strictEqual(odata4analytics.helper.deepEqual(aOldColumns, aNewColumns), iExpectedResult);
    }
    assert.strictEqual(odata4analytics.helper.deepEqual(undefined, []), 2, "_aLastChangedAnalyticalInfo is initially undefined");
...
QUnit.test("deepEqual: fnFormatterChanged", function (assert) {
    ...
    assert.strictEqual(odata4analytics.helper.deepEqual(aOldColumns, aNewColumns, o.formatterChanged), 1);
});
[{
    ...
    [true, false].forEach(function (bPreloaded) {
        var sTitle = "Model#_init: requires " + oFixture.sModel + " instance, already loaded: " + bPreloaded;
        QUnit.test(sTitle, function (assert) {
            var oModel = {}, oModelReference = new odata4analytics.Model.ReferenceByURI("~sServiceURI"), oODataModel = {
                getServiceMetadata: function () { },
                attachMetadataLoaded: function () { }
            }, oODataModelClassMock = this.mock(), oODataModelMock = this.mock(oODataModel), mParameter = { modelVersion: oFixture.iVersion }, oSapUiMock = this.mock(sap.ui);
            oSapUiMock.expects("require").withExactArgs(oFixture.sModel).returns(bPreloaded ? oODataModelClassMock : undefined);
            oSapUiMock.expects("requireSync").withExactArgs(oFixture.sModel).exactly(bPreloaded ? 0 : 1).returns(oODataModelClassMock);
            oODataModelClassMock.withExactArgs("~sServiceURI").returns(oODataModel);
            oODataModelMock.expects("getServiceMetadata").withExactArgs().exactly(2).returns(undefined);
