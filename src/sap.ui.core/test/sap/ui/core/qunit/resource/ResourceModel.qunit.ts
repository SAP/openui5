...
var oModel, oLabel, oLabel2, oDolly, sCustomMessagesProperties = "test-resources/sap/ui/core/qunit/testdata/messages_custom.properties", sMessagesProperties = "test-resources/sap/ui/core/qunit/testdata/messages.properties", sOtherMessagesProperties = "test-resources/sap/ui/core/qunit/testdata/messages_other.properties";
...
QUnit.module("sap.ui.model.resource.ResourceModel: " + "Resources bundle loaded via name / set Model with alias", {
    beforeEach: function () {
        oModel = new ResourceModel({ bundleName: "testdata.messages" });
...
QUnit.test("test model origin info", function (assert) {
    assert.expect(3);
    var value = oModel.getProperty("TEST_TEXT"), info = value.originInfo;
...
QUnit.module("sap.ui.model.resource.ResourceModel: Resources bundle passed as parameter -" + "Language change on enhanced model", {
    prepare: function (opts) {
        sap.ui.getCore().getConfiguration().setLanguage("en");
...
QUnit.module("sap.ui.model.resource.ResourceModel Async: Resources bundle passed as parameter -" + " Language change on enhanced model", {
    prepare: function (opts) {
        sap.ui.getCore().getConfiguration().setLanguage("en");
        var p;
        if (opts && typeof opts.bundle === "function") {
...
QUnit.test("Model enhancement with bundleUrl and enhanceWith with configurations", function (assert) {
    var oModel = new ResourceModel({
        bundleUrl: sMessagesProperties,
        bundleLocale: "en",
        enhanceWith: [{
    ...
    assert.equal(oModel.getProperty("TEST_TEXT"), "A text en", "text TEST_TEXT of enhanced model is correct");
    assert.equal(oModel.getProperty("TEST_TEXT_CUSTOM"), "An overridden modified text", "text TEST_TEXT_CUSTOM of enhanced model is correctly overridden");
    assert.equal(oModel.getProperty("TEST_TEXT_OTHER"), "Another text", "text TEST_TEXT_OTHER of enhanced model is correct");
    oModel.destroy();
});
QUnit.test("Model enhancement with bundleUrl and enhanceWith with configurations async", function (assert) {
    var oModel = new ResourceModel({
        bundleUrl: sMessagesProperties,
        bundleLocale: "en",
        enhanceWith: [
...
QUnit.test("Model enhancement with bundleUrl and enhanceWith with ResourceBundles", function (assert) {
    var oModel = new ResourceModel({
        bundleUrl: sMessagesProperties,
        bundleLocale: "en",
        enhanceWith: [
...
QUnit.test("Model enhancement with bundleUrl and enhanceWith with ResourceBundles async", function (assert) {
    var oModel = new ResourceModel({
        bundleUrl: sMessagesProperties,
        bundleLocale: "en",
        enhanceWith: [
...
QUnit.test("invalid parameter terminologies in enhanceWith with parameter bundle", function (assert) {
    ...
    }, Error("'terminologies' parameter is not supported for enhancement"), "'terminologies' parameter is not supported for enhancement");
});
QUnit.test("invalid parameter terminologies with enhanceWith containing bundles", function (assert) {
    ...
    }, Error("'terminologies' parameter and 'activeTerminologies' parameter are not supported in configuration when enhanceWith contains ResourceBundles"), "correct error message");
});
...
QUnit.test("supportedLocales parameter", function () {
    var aSupportedLocales = ["de", "fr", "de_CH"], oResourceModel;
...
QUnit.test("fallbackLocale parameter", function (assert) {
    var sFallbackLocale = "de_CH", oResourceModel;
    this.mock(ResourceBundle).expects("create").withExactArgs({
        async: undefined,
        bundleLocale: "de",
        bundleName: "testdata.messages",
...
QUnit.test("terminologies parameter", function (assert) {
    var mTerminologies = {}, oResourceModel;
    this.mock(ResourceBundle).expects("create").withExactArgs({
        async: undefined,
        bundleLocale: "de",
        bundleName: "testdata.messages",
...
QUnit.test("loadResourceBundle", function () {
    var mTerminologies = {}, aActiveTerminologies = [];
    this.mock(ResourceBundle).expects("create").withExactArgs({
...
QUnit.test("_sanitizeBundleName", function (assert) {
    ...
    oLogMock.expects("error").withExactArgs("Incorrect resource bundle name " + "\".testdata.messages\"", "Leading slashes or dots in resource bundle names are ignored, since such names are " + "invalid UI5 module names. Please check whether the resource bundle " + "\".testdata.messages\" is actually needed by your application.", "sap.base.i18n.ResourceBundle");
    assert.equal(ResourceModel._sanitizeBundleName(".testdata.messages"), "testdata.messages");
    oLogMock.expects("error").withExactArgs("Incorrect resource bundle name " + "\"/testdata.messages\"", "Leading slashes or dots in resource bundle names are ignored, since such names are " + "invalid UI5 module names. Please check whether the resource bundle " + "\"/testdata.messages\" is actually needed by your application.", "sap.base.i18n.ResourceBundle");
    assert.equal(ResourceModel._sanitizeBundleName("/testdata.messages"), "testdata.messages");
    oLogMock.expects("error").never();
    assert.equal(ResourceModel._sanitizeBundleName("i18n.testdata.messages"), "i18n.testdata.messages", "sanitized bundle name for input which does not need to be sanitized");
});
