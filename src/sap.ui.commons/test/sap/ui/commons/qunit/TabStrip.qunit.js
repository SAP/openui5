...
sap.ui.define([
	...
	QUnit.test("size changing", function(assert) {

		assert.ok(oTabStrip.$().hasClass("sapUiTabStripScrollable") == false, "TabStrip is not scrollable");

		oTabStrip.setWidth('300px');

		// rerendering must be finished
		sap.ui.getCore().applyChanges();
		this.clock.tick(1000);
	...
	QUnit.test("mouse click", function(assert) {
		var $leftArrow = oTabStrip.$().find('.sapUiTabStripLeftScrollIcon');
		qutils.triggerMouseEvent($leftArrow, 'click');
		...
		var $rightArrow = oTabStrip.$().find('.sapUiTabStripRightScrollIcon');
		qutils.triggerMouseEvent($rightArrow, 'click');
