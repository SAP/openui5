...
function createEditorTag(id, changes, mode, language, designtime) {
	language = language || "";
	var json = {
	...
	if (designtime && designtime !== "") {
		manifest["temp"].configuration.editor = "withPreview/" + designtime;
		json.manifest = manifest;
	}
	return '<ui-integration-editor section="temp" id="' + id +
		'" mode="' + mode +
		'" language="' + language +
...
function loadCurrentValues(id) {
	...
	div.innerHTML = createEditorTag(id, [settings], dom.getAttribute("mode"), sLanguage, dom.getAttribute("designtime") || "");
...
function updateAllLayerEditor() {
	...
	target.innerHTML = createEditorTag("editorAll", settings, "all", this._sLanguage || "");
}
...
function updateAdminContentLayerEditor() {
	...
	target.innerHTML = createEditorTag("editorAdminContent", settings, "content", "");
}
function updateAdminContentTranslationLayerEditor() {
	...
	target.innerHTML = createEditorTag("editorAdminContentTranslation", settings, "translation", sLanguage);
}
