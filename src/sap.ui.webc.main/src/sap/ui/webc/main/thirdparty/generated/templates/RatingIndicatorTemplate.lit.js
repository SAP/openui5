sap.ui.define(['sap/ui/webc/common/thirdparty/base/renderer/LitRenderer'], function (litRender) { 'use strict';
	...
	const block5 = (item, index, context, tags, suffix) => litRender.html`${ context.readonly ? block6(item) : block7(item) }`;
	const block6 = (item, index, context, tags, suffix) => litRender.html`<div class="ui5-rating-indicator-icon ui5-rating-indicator-readonly-unselected-icon" data-value="${litRender.ifDefined(item.index)}">&#9733;</div>`;
	const block7 = (item, index, context, tags, suffix) => litRender.html`<div class="ui5-rating-indicator-icon" data-value="${litRender.ifDefined(item.index)}">&#9734;</div>`;
