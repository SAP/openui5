...
sap.ui.require([
	...
	function initForm(bOneContainer) {
		oGridLayout = new sap.ui.layout.form.GridLayout("GL1");
		...
		if (!bOneContainer) {
			...
			oTitle2 = new sap.ui.core.Title("T2", {text: "Test"});
			oFormContainer2 = new sap.ui.layout.form.FormContainer("FC2",{
				title: oTitle2,
				tooltip: "Test",
				expandable: true,
				formElements: [ oFormElement3, oFormElement4 ],
				layoutData: new sap.ui.layout.form.GridContainerData("GCD2", {halfGrid: true})
			});
			aFormContainers.push(oFormContainer2);
			oFormContainer1.setLayoutData(new sap.ui.layout.form.GridContainerData("GCD1", {halfGrid: true}));
		}
		var oTitle1 = new sap.ui.core.Title("T1",{text: "Title", tooltip: "Title tooltip"});
	...
	QUnit.test("Form", function(assert) {
		...
		assert.ok(jQuery.sap.domById("GL1"), "Grid is rendered");
		assert.ok(jQuery("#GL1").is("table"), "Grid is rendered as table");
		assert.ok(jQuery.sap.domById("T1"), "Title is rendered");
	...
	QUnit.test("BackgroundDesign", function(assert) {
		...
		oGridLayout.setBackgroundDesign(sap.ui.layout.BackgroundDesign.Solid);
		sap.ui.getCore().applyChanges();
	...
	QUnit.test("Form Title", function(assert) {
		...
		oForm.destroyTitle();
		sap.ui.getCore().applyChanges();
		assert.notOk(jQuery.sap.byId("F1").find("th")[0], "no title rendered");

		var oTitle1 = new sap.ui.core.Title("T1",{text: "Form Title", level: sap.ui.core.TitleLevel.H1});
	...
	QUnit.test("Form Toolbar", function(assert) {
		...
		assert.notOk(jQuery.sap.domById("T1"), "Title is not rendered");
		assert.ok(jQuery.sap.domById("TB1"), "Toolbar is rendered");
		assert.ok(jQuery("#TB1").parent().is("th"), "Toolbar is in <TH>");
	});

	QUnit.test("FormContainer Title", function(assert) {
		assert.notOk(jQuery.sap.byId("F1").find("h5")[0], "no H5 rendered");

		var oTitle2 = new sap.ui.core.Title("T2",{text: "Title"});
		oFormContainer1.setTitle(oTitle2);
		sap.ui.getCore().applyChanges();

		assert.ok(jQuery.sap.domById("T2"), "Title is rendered");
	...
	QUnit.test("FormElement", function(assert) {
		assert.ok(jQuery.sap.domById("FE1"), "FormElement is rendered");
	...
	QUnit.test("Labels and Fields", function(assert) {
		assert.ok(jQuery.sap.domById("L1"), "Label1 is rendered");
		assert.equal(jQuery("#L1").parent().attr("colspan"), "3", "Label1 rendered using 3 grid cells");
		assert.ok(jQuery.sap.domById("I1"), "Field1 is rendered");
		...
		assert.ok(!jQuery("#I1").parent().attr("rowspan") || (sap.ui.Device.browser.internet_explorer && jQuery("#I1").parent().attr("rowspan") == "1"), "Field1 no rowspan");
		assert.ok(jQuery.sap.domById("L2"), "Label2 is rendered");
		...
		assert.ok(!jQuery("#I2").parent().attr("rowspan") || (sap.ui.Device.browser.internet_explorer && jQuery("#I2").parent().attr("rowspan") == "1"), "Field2 no rowspan");
		...
		assert.ok(!jQuery("#I3").parent().attr("rowspan") || (sap.ui.Device.browser.internet_explorer && jQuery("#I3").parent().attr("rowspan") == "1"), "Field3 no rowspan");
	});
	...
	QUnit.test("FormElement visibility", function(assert) {
		...
		sap.ui.getCore().applyChanges();
		assert.notOk(jQuery.sap.domById("FE1"), "invisible FormElement is not rendered");
		assert.notOk(jQuery.sap.domById("L1"), "Label1 is not rendered");
		assert.notOk(jQuery.sap.domById("I1"), "Field1 is not rendered");
	...
	QUnit.test("too much fields", function(assert) {
		...
		oField6 = new sap.m.Input("I6");
		oField7 = new sap.m.Input("I7");
		var oField8 = new sap.m.Input("I8");
		var oField9 = new sap.m.Input("I9");
		var oField10 = new sap.m.Input("I10");
		var oField11 = new sap.m.Input("I11");
		var oField12 = new sap.m.Input("I12");
		var oField13 = new sap.m.Input("I13");
		var oField14 = new sap.m.Input("I14");
		var oField15 = new sap.m.Input("I15");
		var oField16 = new sap.m.Input("I16");
	...
	QUnit.test("GridElementData hCells", function(assert) {
		var oGED1 = new sap.ui.layout.form.GridElementData("GE1", {hCells: "3"});
		...
		oGED1.setHCells("2");
		var oGED2 = new sap.ui.layout.form.GridElementData("GE2", {hCells: "2"});
		...
		assert.notOk(jQuery.sap.domById("I3"), "Field3 is not rendered");
		assert.ok(findLogEntry('Element "FE2" - Too much fields for one row!',1),"Error entry in log found for Element2");

		var oGED3 = new sap.ui.layout.form.GridElementData("GE3", {hCells: "full"});
		...
		var oGED4 = new sap.ui.layout.form.GridElementData("GE4", {hCells: "5"});
	...
	QUnit.test("GridElementData vCells", function(assert) {
		var oGED1 = new sap.ui.layout.form.GridElementData("GE1", {vCells: 2});
		...
		// complex example
		oFormElement3 = new sap.ui.layout.form.FormElement("FE3",{
			label: new sap.m.Label("L3", {text:"Label"}),
			fields: [new sap.m.TextArea("TA1",{height: "10rem", layoutData: new sap.ui.layout.form.GridElementData({vCells: 3})}),
			         new sap.m.TextArea("TA2",{height: "6.5rem", layoutData: new sap.ui.layout.form.GridElementData({hCells: "3",vCells: 2})}),
			         new sap.m.TextArea("TA3",{height: "3rem", layoutData: new sap.ui.layout.form.GridElementData({vCells: 1})})
								]
		});
		oFormElement4 = new sap.ui.layout.form.FormElement("FE4",{
			fields: [new sap.m.TextArea("TA4",{height: "3rem", layoutData: new sap.ui.layout.form.GridElementData({vCells: 1})}),
			         new sap.m.TextArea("TA5",{height: "6.5rem", layoutData: new sap.ui.layout.form.GridElementData({vCells: 2})})
								]
		});
		var oFormElement5 = new sap.ui.layout.form.FormElement("FE5",{
			label: new sap.m.Label("L4", {text:"Label"}),
			fields: [new sap.m.TextArea("TA6",{height: "3rem", layoutData: new sap.ui.layout.form.GridElementData({vCells: 1})})
								]
		});
		var oFormElement6 = new sap.ui.layout.form.FormElement("FE6",{
			label: new sap.m.Label("L5", {text:"Label"}),
			fields: [new sap.m.TextArea("TA7",{height: "6.5rem", layoutData: new sap.ui.layout.form.GridElementData({hCells:"13",vCells: 2})})
								]
		});
		...
		assert.ok(!jQuery("#TA3").parent().attr("rowspan") || (sap.ui.Device.browser.internet_explorer && jQuery("#TA3").parent().attr("rowspan") == "1"), "Element3: Field3 (with vCells=1) rendered without rowspan");
		...
		assert.ok(!jQuery("#TA4").parent().attr("rowspan") || (sap.ui.Device.browser.internet_explorer && jQuery("#TA4").parent().attr("rowspan") == "1"), "Element4: Field1 (with vCells=1) rendered without rowspan");
		...
		assert.ok(!jQuery("#TA6").parent().attr("rowspan") || (sap.ui.Device.browser.internet_explorer && jQuery("#TA6").parent().attr("rowspan") == "1"), "Element5: Field1 (with vCells=1) rendered without rowspan");
		...
		sap.ui.getCore().applyChanges();
		assert.notOk(jQuery.sap.domById("TA6"), "Field is not rendered");
	...
	QUnit.test("FormContainer Title", function(assert) {
		assert.ok(jQuery.sap.domById("T2"), "Title is rendered");
		assert.ok(jQuery("#T2").is("h5"), "Title is rendered as H5");
		assert.ok(jQuery.sap.domById("FC2--Exp"), "Expander is rendered");
		assert.notOk(jQuery("#T2").parent().hasClass("sapUiFormContainerToolbar"), "Toolbar class not rendered");
		assert.ok(jQuery("#T2").parent().hasClass("sapUiFormContainerTitle"), "Title class rendered");
	...
	QUnit.test("FormContainer Toolbar", function(assert) {
		...
		assert.notOk(jQuery.sap.domById("T2"), "Title is not rendered");
		assert.ok(jQuery.sap.domById("TB1"), "Toolbar is rendered");
		assert.ok(jQuery("#TB1").parent().is("td"), "Toolbar is in <TD>");
	...
	QUnit.test("FormElement", function(assert) {
		assert.notOk(jQuery.sap.domById("FE1"), "FormElement is not rendered as own DOM");
	});
	...
	QUnit.test("FormContainer visibility", function(assert) {
		...
		sap.ui.getCore().applyChanges();
		assert.notOk(jQuery.sap.domById("T2"), "Title is not rendered");
		assert.notOk(jQuery.sap.domById("I4"), "Field4 is not rendered");

		oFormContainer2.setVisible(true);
		sap.ui.getCore().applyChanges();
		assert.ok(jQuery.sap.domById("T2"), "Title is rendered");
		assert.ok(jQuery.sap.domById("I4"), "Field4 is rendered");
	});
	...
	QUnit.test("GridElementData hCells", function(assert) {
		var oGED1 = new sap.ui.layout.form.GridElementData("GE1", {hCells: "full"});
		oField1.setLayoutData(oGED1);
		sap.ui.getCore().applyChanges();
	...
	QUnit.test("GridElementData vCells", function(assert) {
		var oFormElement5 = new sap.ui.layout.form.FormElement("FE5",{
			label: new sap.m.Label("L3", {text:"Label"}),
			fields: [new sap.m.Input("I8")]
		});
		var oFormElement6 = new sap.ui.layout.form.FormElement("FE6",{
			label: new sap.m.Label("L4", {text:"Label"}),
			fields: [new sap.m.Input("I9")]
		});
		var oFormElement7 = new sap.ui.layout.form.FormElement("FE7",{
			label: new sap.m.Label("L5", {text:"Label"}),
			fields: [new sap.m.Input("I10")]
		});
		var oFormElement8 = new sap.ui.layout.form.FormElement("FE8",{
			label: new sap.m.Label("L6", {text:"Label"}),
			fields: [new sap.m.TextArea("TA1",{height: "6.5rem", layoutData: new sap.ui.layout.form.GridElementData({hCells: "auto", vCells: 2})})]
		});
		var oFormElement9 = new sap.ui.layout.form.FormElement("FE9",{
			label: new sap.m.Label("L7", {text:"Label"}),
			fields: [new sap.m.TextArea("TA2",{height: "6.5rem", layoutData: new sap.ui.layout.form.GridElementData({hCells: "2", vCells: 2})}),
			         new sap.m.Input("I11")]
		});
		var oFormElement10 = new sap.ui.layout.form.FormElement("FE10");
		var oFormElement11 = new sap.ui.layout.form.FormElement("FE11",{
			label: new sap.m.Label("L8", {text:"Label"}),
			fields: [new sap.m.Input("I12",{layoutData: new sap.ui.layout.form.GridElementData({hCells: "1"})})]
		});
		oFormContainer1.addFormElement(oFormElement5);
		oFormContainer1.addFormElement(oFormElement6);
		...
		var oFormElement12 = new sap.ui.layout.form.FormElement("FE12",{
			label: new sap.m.Label("L9", {text:"Label"}),
			fields: [new sap.m.TextArea("TA3",{height: "6.5rem", layoutData: new sap.ui.layout.form.GridElementData({hCells: "full", vCells: 2})})]
		});
		var oFormElement13 = new sap.ui.layout.form.FormElement("FE13",{
			label: new sap.m.Label("L10", {text:"Label"}),
			fields: [new sap.m.Input("I13")]
		});
		var oFormElement14 = new sap.ui.layout.form.FormElement("FE14");
	...
	QUnit.test("stand alone Half size containe", function(assert) {
		var oFormContainer3 = new sap.ui.layout.form.FormContainer("FC3",{
			formElements: [
							new sap.ui.layout.form.FormElement("FE5",{
								label: new sap.m.Label("L3",{text:"Label"}),
								fields: [new sap.m.TextArea("TA1",{height: "10rem", layoutData: new sap.ui.layout.form.GridElementData({hCells: "2", vCells: 3})})]
							})
			],
	...
	QUnit.test("Keyboard Navigation", function(assert) {
		...
		jQuery.sap.byId("I2-inner").focus();
		assert.equal(jQuery.sap.byId("I2-inner").is(":focus"), true, "Mouseclick: Container1, Field2 - Selected");
		qutils.triggerKeyboardEvent("I2-inner", "F6");
		assert.equal(jQuery.sap.byId("I4-inner").is(":focus"), true, "F6: Container2, Field1 - Selected");
		qutils.triggerKeyboardEvent("I4-inner", "F6", true, false, false);
		assert.equal(jQuery.sap.byId("I1-inner").is(":focus"), true, "Shift + F6: Container1, Field2 - Selected");
		qutils.triggerKeyboardEvent("I4-inner", "F6");
		assert.equal(jQuery.sap.byId("I1-inner").is(":focus"), false, "F6: Container1, Field1 - NOT Selected (No cycling!)");
		jQuery.sap.byId("I1-inner").focus();
		qutils.triggerKeyboardEvent("I1-inner", "F6");
		assert.equal(jQuery.sap.byId("I4-inner").is(":focus"), true, "F6: Container2, Field1 - Selected");
		jQuery.sap.byId("I1-inner").focus();
		qutils.triggerKeyboardEvent("I1-inner", jQuery.sap.KeyCodes.TAB);
		assert.equal(jQuery.sap.byId("I2-inner").is(":focus"), true, "Tab: Container1, Field2 - Selected");
		qutils.triggerKeyboardEvent("I2-inner", jQuery.sap.KeyCodes.TAB);
		assert.equal(jQuery.sap.byId("I3-inner").is(":focus"), true, "Tab: Container1, Field3 - Selected");
		qutils.triggerKeyboardEvent("I3-inner", jQuery.sap.KeyCodes.TAB, true, false, false);
		assert.equal(jQuery.sap.byId("I2-inner").is(":focus"), true, "Shift+Tab: Container1, Field2 - Selected");
		qutils.triggerKeyboardEvent("I2-inner", jQuery.sap.KeyCodes.TAB, true, false, false);
		assert.equal(jQuery.sap.byId("I1-inner").is(":focus"), true, "Shift+Tab: Container1, Field1 - Selected");
		jQuery.sap.byId("I3-inner").focus();
		qutils.triggerKeyboardEvent("I3-inner", jQuery.sap.KeyCodes.TAB);
		assert.equal(jQuery.sap.byId("FC2--Exp").is(":focus"), true, "Tab: Container2 Expander - Selected");
		qutils.triggerKeyboardEvent("FC2--Exp", jQuery.sap.KeyCodes.TAB);
		assert.equal(jQuery.sap.byId("I4-inner").is(":focus"), true, "Tab: Container2, Field1 - Selected");
		qutils.triggerKeyboardEvent("I4-inner", jQuery.sap.KeyCodes.TAB, true, false, false);
		assert.equal(jQuery.sap.byId("FC2--Exp").is(":focus"), true, "Shift+Tab: Container2 Expander - Selected");
		qutils.triggerKeyboardEvent("FC2--Exp", jQuery.sap.KeyCodes.TAB, true, false, false);
		assert.equal(jQuery.sap.byId("I3-inner").is(":focus"), true, "Shift+Tab: Container1, Field 3 - Selected");
	});
	...
	QUnit.test("Container expander", function(assert) {
		...
		sap.ui.getCore().applyChanges();
		assert.notOk(jQuery.sap.domById("I4"), "Container2 content area is not visible if not expanded");
		...
		sap.ui.getCore().applyChanges();
		assert.ok(jQuery.sap.domById("I4"), "Container2 content area is visible if expanded");
