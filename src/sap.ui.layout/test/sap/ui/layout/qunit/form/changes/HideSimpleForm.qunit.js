	...
	QUnit.module("using sap.ui.layout.changeHandler.HideSimpleForm with a new change format", {
		beforeEach: function () {
			...
			this.oMockedComponent = {
				...
				createId: function (sString) {return this.getId() + "---" + sString;},
				getLocalId: function(sId) {
					var sPrefix = this.getId() + "---";
					return (sId && sId.indexOf(sPrefix) === 0) ? sId.slice(sPrefix.length) : null;
				}
			};
			sinon.stub(sap.ui.fl.Utils, "getAppComponentForControl").returns(this.oMockedComponent);
		...
		afterEach: function () {
			this.oSimpleForm.destroy();
			sap.ui.fl.Utils.getAppComponentForControl.restore();
		}
	});
