...
(function () {
	...
	QUnit.test("applyChange shall raise an exception if the control does not have the required methods", function (assert) {
		...
		//Call CUT
		try {
			this.oChangeHandler.applyChange(this.oLegacyChangeWrapper, oControl, {modifier: JsControlTreeModifier});
		} catch (ex) {
			exception = ex;
		}
	...
	QUnit.test('when calling completeChangeContent', function (assert) {
		...
		new sap.m.Label("component---newId");
		var oChangeWrapper = new sap.ui.fl.Change(oChange);
		var oSpecificChangeInfo = {index: 5, newControlId: "component---newId", groupLabel: "New Control"};
		...
		assert.ok(oChange.content.group.selector.idIsLocal, "the id is flagged as local");
